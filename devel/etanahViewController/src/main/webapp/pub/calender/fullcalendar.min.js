...
(function(o,K){
    ...
    function jb(a,b,f,c,g,n,k,y,w,t,H){
        ...
        for(G=0;G<fa;G++){
            ...
            if(k===false)c.remove();
                ...
                D.element=c;
                if(Q._id===H)t(Q,c,D);else c[0]._fci=G;f.reportEventElement(Q,c)
                }
                }
                rb(w,a,t);for(G=0;G<fa;G++){
                ...
                for(G=0;G<fa;G++){
                D=a[G];if(c=D.element)c[0].style.width=D.outerWidth-D.hsides+"px"
                }
                for(G=0;G<fa;G++){
                D=a[G];if(c=D.element){
                t=ca[w=D.key];D.outerHeight=c[0].offsetHeight+(t===K?(ca[w]=tb(c[0])):t)
                }
                }
                for(X=G=0;X<b;X++){
                ...
                t=Math.max(t,D.outerHeight||0);D.top=ca;G++
                }
                ua[X]=n(X).find("td:first div.fc-day-content > div").height(ca+t)
                }
                for(X=0;X<b;X++)O[X]=ua[X][0].offsetTop;for(G=0;G<fa;G++){
                D=a[G];if(c=
                    D.element){
                c[0].style.top=O[D.row]+D.top+"px";Q=D.event;f.trigger("eventAfterRender",Q,Q,c)
                }
                }
                }
                function ub(a,b,f,c){
                function g(d,i){
                m=d;na=b.theme?"ui":"fc";La=b.weekends?0:1;vb=b.firstDay;if(Ta=b.isRTL){
                Y=-1;ta=m-1
                }
                else{
        Y=1;
    ...
    var ya=o.fullCalendar={},Oa=ya.views={},Va={
        defaultView:"month",
        aspectRatio:1.35,
        header:{
            left:"title",
