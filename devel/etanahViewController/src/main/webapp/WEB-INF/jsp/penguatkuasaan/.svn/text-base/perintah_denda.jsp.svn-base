...
<script type="text/javascript">
    ...
    <c:if test="${!statusPembayaranAnsuranRemedi}">
    ...
    <c:if test="${statusPembayaranAnsuranRemedi}">
            var a = 0.00;
            var bil = ${fn:length(actionBean.senaraiPermohonanTuntutanBayar)};
            for (var i = 0; i < bil; i++){
                var amountRemedi = document.getElementById('amountRemedi'+i).value;
                if(amountRemedi !=""){
                    a += parseFloat(amountRemedi);
...
<s:form beanclass="etanah.view.penguatkuasaan.MaklumatPerintahDendaActionBean" name="form1">
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            <c:if test="${edit}">
                ...
                <div class="subtitle">
                    <fieldset class="aras1">
                        ...
                        <c:if test="${actionBean.stageId eq 'kpsn_pemantauan1'}">
                        ...
                        <c:if test="${actionBean.stageId eq 'sedia_denda_tambahan' || actionBean.stageId eq 'sedia_perintah_denda_tamb'}">
                            <%-- stage id for MLK : sedia_denda_tambahan
                                 stage id for NS : kpsn_pemantauan1--%>
                            <p>
                                <label><em>*</em>Jumlah Denda Tambahan (RM) :</label>
                                <s:text id="amaun" name="amaun" size="15" class="number" formatPattern="0.00" maxlength="10" onkeyup="validateNumber(this,this.value);" onblur="returnCurrency2(this.value);"/>&nbsp;
                            </p>

                        </c:if>

                        <c:if test="${actionBean.stageId eq 'kpsn_enkuiri_b7e' || actionBean.stageId eq 'sedia_perintah_denda'}">
                            <%-- stage id for MLK : sedia_perintah_denda
                                stage id for NS : perakuan_srt_denda--%>
                            <%-- stage id for MLK : sedia_perintah_denda
                              stage id for NS : kpsn_enkuiri_b7e  updated (22/5/12)--%>
                            <p>
            ...
            <c:if test="${status}">
                <legend>
                    <c:if test="${actionBean.statusBayaranRemediFlag eq true}">
                        Maklumat Bayaran Kos Remedi
                    </c:if>  
                    <c:if test="${actionBean.statusBayaranRemediFlag ne true}">
                        <c:choose>
                            <c:when test="${actionBean.stageId eq 'maklum_bayaran_denda_tamb' || actionBean.stageId eq 'arah_pemantauan2'}">
                ...
                <div class="content" align="center">
                    <c:if test="${actionBean.statusBayaranRemediFlag eq true}">
                        <display:table class="tablecloth" name="${actionBean.senaraiKompaun}" cellpadding="0" cellspacing="0" id="line" >
                            ...
                            <display:column title="Jumlah Kos Remedi(RM)" property="amaun" format="{0,number, 0.00}"></display:column>
                            <display:column title="Status Bayaran">
                                <c:forEach items="${actionBean.senaraiPermohonanTuntutanBayar}" var="i">
                                    <c:if test="${i.dokumenKewangan.idDokumenKewangan eq line.resit.idDokumenKewangan}">
                            ...
                            <display:column title="Tarikh Bayar">
                                <c:forEach items="${actionBean.senaraiPermohonanTuntutanBayar}" var="i">
                                    <c:if test="${i.dokumenKewangan.idDokumenKewangan eq line.resit.idDokumenKewangan}">
                                        <fmt:formatDate pattern="dd/MM/yyyy hh:mm:ss aaa" value="${i.tarikhBayar}"/>
                                    </c:if>
                                </c:forEach>
                    ...
                    <c:if test="${actionBean.statusBayaranRemediFlag eq false}">
                        <display:table class="tablecloth" name="${actionBean.senaraiKompaun}" cellpadding="0" cellspacing="0" id="line" >
                            <display:column title="Bil" sortable="true">${line_rowNum}</display:column>
                            <display:column title="Id Hakmilik">${line.noRujukan}</display:column>
                            <display:column title="No.Resit" property="resit.idDokumenKewangan"></display:column>
                            <c:if test="${actionBean.stageId eq 'maklum_byrn_denda_tmbhn' || actionBean.stageId eq 'maklum_bayaran_denda_tamb' || actionBean.stageId eq 'maklum_laporan3' || actionBean.stageId eq 'maklum_bayaran_denda_tamb' || actionBean.stageId eq 'arah_pemantauan2'}">
                            ...
                            <c:if test="${actionBean.stageId ne 'maklum_byrn_denda_tmbhn' && actionBean.stageId ne 'maklum_bayaran_denda_tamb' && actionBean.stageId ne 'maklum_laporan3' || actionBean.stageId eq 'maklum_bayaran_denda_tamb' || actionBean.stageId eq 'arah_pemantauan2'}">
            ...
            <c:if test="${statusPembayaranDenda}">
                <legend>
                    Maklumat Perintah Denda
                </legend>
                <div class="content" align="center">
                    <display:table class="tablecloth" name="${actionBean.senaraiDenda}" cellpadding="0" cellspacing="0" id="line" >
                        <display:column title="Bil" sortable="true">${line_rowNum}</display:column>
                        <display:column title="Id Hakmilik">${line.noRujukan}</display:column>
                        <display:column title="No.Resit" property="resit.idDokumenKewangan"></display:column>
                        <display:column title="Nilai Denda Harian(RM)" property="amaun" format="{0,number, 0.00}"></display:column>
                        ...
                        <display:column title="Tarikh Bayar">
                            <c:if test="${actionBean.pembayaranDenda.permohonanTuntutanKos.idKos eq line.permohonanTuntutanKos.idKos}">
                                <fmt:formatDate pattern="dd/MM/yyyy hh:mm:ss aaa" value="${actionBean.pembayaranDenda.tarikhBayar}"/>
                            </c:if>
                        </display:column>
                    </display:table>
                </div>
                ...
                <div class="content" align="center">
                    <display:table class="tablecloth" name="${actionBean.senaraiDendaTambahan}" cellpadding="0" cellspacing="0" id="line" >
                        <display:column title="Bil" sortable="true">${line_rowNum}</display:column>
                        <display:column title="Id Hakmilik">${line.noRujukan}</display:column>
                        <display:column title="No.Resit" property="resit.idDokumenKewangan"></display:column>
                        <display:column title="Jumlah Denda(RM)">
                            <c:if test="${actionBean.pembayaranDendaTambahan.permohonanTuntutanKos.idKos eq line.permohonanTuntutanKos.idKos}">
                                <fmt:formatNumber pattern="#,##0.00" value="${actionBean.pembayaranDendaTambahan.amaun}"/>
                            </c:if>
                        </display:column>
                        <display:column title="Status Bayaran">
                            <c:if test="${line.resit.idDokumenKewangan eq null}"><font color="red">Belum Dibayar</font></c:if>
                        ...
                        <display:column title="Tarikh Bayar">
                            <c:if test="${actionBean.pembayaranDendaTambahan.permohonanTuntutanKos.idKos eq line.permohonanTuntutanKos.idKos}">
                                <fmt:formatDate pattern="dd/MM/yyyy hh:mm:ss aaa" value="${actionBean.pembayaranDendaTambahan.tarikhBayar}"/>
                            </c:if>
                        </display:column>
            ...
            <c:if test="${statusPembayaranAnsuranRemedi}">
                ...
                <div class="content" align="center">
                    <display:table class="tablecloth" name="${actionBean.senaraiKompaun}" cellpadding="0" cellspacing="0" id="line" >
                        <display:column title="Bil" sortable="true">${line_rowNum}</display:column>
                        <display:column title="Id Hakmilik">${line.noRujukan}</display:column>
                        <display:column title="Jumlah Bayaran Remedi (RM)" property="amaun" format="{0,number, 0.00}"></display:column>
                    </display:table>
                    ...
                    <display:table class="tablecloth" name="${actionBean.senaraiPermohonanTuntutanBayar}" cellpadding="0" cellspacing="0" id="line" >
                        <display:column title="Bil" sortable="true">${line_rowNum}</display:column>
                        <display:column title="No.Resit" property="dokumenKewangan.idDokumenKewangan"></display:column>
                        <display:column title="Tarikh Bayar">
                            <fmt:formatDate pattern="dd/MM/yyyy hh:mm:ss aaa" value="${line.tarikhBayar}"/>
                        </display:column>
                        <display:column title="Caj Bulanan (RM)">
                            <fmt:formatNumber pattern="#,##0.00" value="${line.amaun}"/>
                            <input type="hidden" value="${line.amaun}" id="amountRemedi${line_rowNum-1}">
                        </display:column>
                        <display:footer>
                            <tr>
                                <td colspan="3" align="right">Jumlah (RM):</td>
