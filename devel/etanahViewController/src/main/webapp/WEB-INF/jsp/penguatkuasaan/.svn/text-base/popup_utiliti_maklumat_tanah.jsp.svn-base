<%-- 
    Document   : popup_utiliti_maklumat_tanah
    ...
    <%--   $('#diusaha').val("");
       ...
       $('#usahaTanam').val("");--%>
           ...
           function carianHakmilik(idHakmilik,section){
               ...
               if(idHakmilik != ""){
                   $.get('${pageContext.request.contextPath}/penguatkuasaan/utiliti_laporan_tanah?findHakmilikLTNH&hakmilikCarian='+idHakmilik,
                   function(data){
                       //                       alert(data);
           ...
           function closeAndRefresh(){
               ...
               self.opener.refreshFindInfo(id,idMohon);
               self.close();
           }
...
<s:form  name="form2" beanclass="etanah.view.penguatkuasaan.UtilitiLaporanTanahActionBean">
    <input type="hidden" name="idPermohonan" value="${actionBean.idPermohonan}" id="idPermohonan"/>
    <div id ="msgInfoDiv">
        <s:messages/>
        <s:errors/>
    </div>
