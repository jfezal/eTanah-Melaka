...
<script type="text/javascript">
    function addRecord(){
        ...
        window.open("${pageContext.request.contextPath}/penguatkuasaan/surat_panggilan_saksi?addRecord", "eTanah",
    ...
    function refreshPageSuratSaksi(){
        var url = '${pageContext.request.contextPath}/penguatkuasaan/surat_panggilan_saksi?refreshpage';
    ...
    function editRecord(id){
        var left = (screen.width/2)-(1000/2);
        var top = (screen.height/4)-(150/4);
        window.open("${pageContext.request.contextPath}/penguatkuasaan/surat_panggilan_saksi?editRecord&idKertas="+id, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=1000,height=700,scrollbars=yes,left=" + left + ",top=" + top);
    }
    ...
    function viewSurat(id){
        var left = (screen.width/2)-(1000/2);
        var top = (screen.height/4)-(150/4);
        var url = '${pageContext.request.contextPath}/penguatkuasaan/surat_panggilan_saksi?viewRecord&idKertas='+id;
    ...
    function deleteRecord(id){
        if(confirm('Adakah anda pasti untuk hapus?')) {
            var url = '${pageContext.request.contextPath}/penguatkuasaan/surat_panggilan_saksi?deleteRecord&idKertas='+id;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<s:form name="form1" id="form1"  beanclass="etanah.view.penguatkuasaan.SuratPanggilanSaksiActionBean">
    <s:errors/>
    <s:messages/>
    <div class="subtitle">
        <fieldset class="aras1">
            ...
            <c:if test="${edit}">
                <div class="instr-fieldset"><br>
                    Klik butang tambah untuk tambah surat panggilan saksi
                </div>
                <div class="content" align="center">
                    <display:table class="tablecloth" name="${actionBean.senaraiSuratSaksi}" cellpadding="0" cellspacing="0" id="line" >
                        ...
                        <display:column title="No.Saman">
                            <u><a class="popup" onclick="viewSurat(${line.idKertas})">${line.nomborRujukan}</a></u>
                        </display:column>
