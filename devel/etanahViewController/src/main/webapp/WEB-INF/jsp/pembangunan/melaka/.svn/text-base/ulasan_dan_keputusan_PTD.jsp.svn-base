<%--
    Document   : ulasan_dan_keputusan_PTD
...
<style type="text/css">
    ...
    function deleteRow()
    {
        ...
        var c1 = $('#count').val();
            ...
            alert("c3:"+c1);--%>
        if(rc1 == c1){
            var url = '${pageContext.request.contextPath}/pembangunan/melaka/ulasanDanKeputusan?deleteSingle&idKandungan='+idKandungan;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<s:form beanclass="etanah.view.stripes.pembangunan.UlasanDanKeputusanPTDActionBean">
    <s:messages/>
    <s:errors/>
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            <legend>ULASAN DAN KEPUTUSAN PENTADBIR TANAH ${actionBean.permohonan.cawangan.name}</legend>
            <div class="content" align="center">
               ...
               <%--<c:if test="${actionBean.stageId ne 'sediaminitbebas'}">
                    <tr>
                        <td><b>Syor Memo Perakuan</b></td>
                    </tr>
                    <c:if test="${edit}">
                    <tr>
                        <td><s:textarea name="syorMemoPerakuan" id="syorMemoPerakuan"  rows="5" cols="150" class="normal_text"></s:textarea></td>
                    </tr>
                    </c:if>
                    <c:if test="${!edit}">
                    <tr>
                        <td>${actionBean.syorMemoPerakuan}</td>
                    </tr>
                    </c:if>
               </c:if>--%>
