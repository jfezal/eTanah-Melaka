...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#tukarkawasanpekan').hide();
        $('#tukarkawasanbandar').hide();
        <c:if test="${actionBean.kodT eq '01'}">
            $('#tukarkawasanbandar').show();
            $('#kodKwsnPekan').attr("name","sssss");
            $('#kodKwsnBandar').attr("name","kodKwsnPekan");
        </c:if>
        <c:if test="${actionBean.kodT eq '02'}">
            $('#tukarkawasanpekan').show();
            $('#kodKwsnBandar').attr("name","sssss");
            $('#kodKwsnPekan').attr("name","kodKwsnPekan");
        </c:if>
        <c:if test="${actionBean.kodT ne '02' and actionBean.kodT ne '01' }">
            $('#tukarkawasanpekan').hide();            
            $('#tukarkawasanbandar').hide();
            $('#kodKwsnBandar').attr("name","sssss");
            $('#kodKwsnPekan').attr("name","sss");
        </c:if>
        ...
        function showjenistanahlain(value){
            ...
            showtukartanah(value);
        }
        
        function showtukartanah(value){
            if(value == '01'){
                $('#tukarkawasanbandar').show();
                $('#tukarkawasanpekan').hide();
                $('#kodKwsnPekan').attr("name","sssss");
                ...
                $('#tukarkawasanbandar').hide();
                $('#kodKwsnPekan').attr("name","kodKwsnPekan");
                $('#kodKwsnBandar').attr("name","ssssss");
            }else{                
                $('#tukarkawasanpekan').hide();
                $('#tukarkawasanbandar').hide();
                $('#kodKwsnPekan').attr("name","sss");
        ...
        function uploadForm(pandanganImej) {
            ...
            var idHakmilik = $('#idHakmilik').val();
            //            alert(idLapor);
            window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?uploadDoc&pandanganImej="+pandanganImej+"&idLapor="+idLapor+"&idHakmilik="+idHakmilik, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
        }
        ...
        function tambahPermohonanTerdahulu(){
            //            alert('test');
            NoPrompt();
            window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?permohonanTerdahuluPopup", "eTanah",
        ...
        function carianHakmilikPopUp(){
            ...
            NoPrompt();
            var idHakmilik = $('#idHakmilik').val();
            window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?carianHakmilikPopup&idHakmilik="+idHakmilik, "eTanah",
...
//        alert("hi");
        ...
        $.post('${pageContext.request.contextPath}/pembangunan/laporanTanahv2?senaraiKodDunByKodPar&kodPar=' + kodPar,
                function(data) {
                  if (data != '') {
      ...
      function deleteMohonTerdahulu(idMohonManual,f,tName)
        {   
            ...
            if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
                var q = $(f).formSerialize();
                $.post('${pageContext.request.contextPath}/pembangunan/laporanTanahv2?deleteRow&idKandungan='+idMohonManual+'&tName='+tName,q,
                function(data){
                    $('#page_div').html(data);
                ...
                <div id="perihaltanah">
                    ...
                    <div class="content" align="center">
                        ...
                        <display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" pagesize="20" cellpadding="0" cellspacing="0" id="line"
                            ...
                            <c:if test="${line.hakmilikPermohonan.hakmilik ne null}">
                                ...
                                <display:column title="Bandar/Pekan/Mukim">
                                    ...
                                    <%--</c:if>--%>
                                    <%--<c:if test="${line.hakmilikPermohonan.noLot ne null}">
                                        ${line.hakmilikPermohonan.bandarPekanMukimBaru.nama}00
                                    </c:if>--%>
                                </display:column>
                        ...
                        <table class="tablecloth" border="0">
                            ...
                            <tr id="tukarkawasanbandar">
                                <td>
                                    Kawasan Bandar :
                                </td>
                                <td>
                                    <s:select name="kodKwsnPekan" style="width:150px" id="kodKwsnBandar">
                                        <s:option value="0">Sila Pilih</s:option>
                                        <s:options-collection collection="${actionBean.senaraiKodTanahBandar}" label="nama" value="kod"/>
                                    </s:select>
                                </td>
                            </tr>
                            <tr id="tukarkawasanpekan">
                                <td>
                                    Kawasan Pekan :
                                </td>
                                <td>
                                    <s:select name="kodKwsnPekan" style="width:150px" id="kodKwsnPekan">
                                        <s:option value="0">Sila Pilih</s:option>
                                        <s:options-collection collection="${actionBean.senaraiKodTanahPekan}" label="nama" value="kod"/>
                                    </s:select>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td>
                                    <font color="red" size="4">*</font>Kedudukan Tanah / Nama Tempat :
                                </td>
                                <td>
