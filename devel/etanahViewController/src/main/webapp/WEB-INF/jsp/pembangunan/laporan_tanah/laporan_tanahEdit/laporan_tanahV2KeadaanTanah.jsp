...
<script type="text/javascript">
    ...
    <c:if test="${actionBean.permohonanLaporanUsaha.diUsahaOleh eq null}">
        ...
        function tambahBangunan(kategori){
            NoPrompt();
            window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?tambahBangunanPopup&kategori="+kategori+"&idLaporTanah="+${actionBean.laporanTanah.idLaporan}, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=910,height=800");
        }
        
        function tambahLaporUsaha(kategori){
            NoPrompt();
            window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?tambahNamaPengusahaPopup&kategori="+kategori+"&idLaporTanah="+${actionBean.laporanTanah.idLaporan}, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=910,height=800");
        }
    ...
    <c:choose>
        ...
        <c:otherwise>
                ...
                function editLaporanBangunan(idLaporBangunan,kategori){
                    NoPrompt();
                    var idHakmilik = $('#idHakmilik').val();
                    window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?editBangunanPopup&kategori="+kategori+"&idHakmilik="+idHakmilik+"&idLaporBangunan="+idLaporBangunan+"&idLaporTanah="+${actionBean.laporanTanah.idLaporan}, "eTanah",
                    "status=0,toolbar=0,location=0,menubar=0,width=910,height=800");
                }
        
                function removeLaporanBangunan(val,f)
                {   
                    var idHakmilik = $('#idHakmilik').val();
                    var idmohon = $('#idPermohonan').val();
                    var idLapor = $('#idLapor').val();
                    
                    NoPrompt();
                    if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
                        var q = $(f).formSerialize();
                        $.post('${pageContext.request.contextPath}/pembangunan/laporanTanahv2?deleteLaporanBangunan&idLaporBangunan='+val,q,
                        function(data){
                            $('#page_div').html(data);
                    
                        }, 'html');
                        self.refreshpage2(val,idHakmilik,idmohon,idLapor) ;
                    }
                }
        
                function refreshpage2(val,idhakmilik,idmohon,idLapor){
                    var url = '${pageContext.request.contextPath}/pembangunan/laporanTanahv2?reloadBangunan&idLaporBangunan='+val+'&idHakmilik='+idhakmilik+'&idPermohonan='+idmohon+'&idLapor='+idLapor;
                ...
                <div id="perihaltanah">
                    ...
                    <div class="content" align="center">
                        <table class="tablecloth" border="0">
                            ...
                            <tr>
                                <td style="text-align: right"> 
                                    Talian Telefon / Talian Telefon Bawah Tanah :
                                </td>
                                <td>
                                ...
                                <td colspan="2">
                                    <display:table class="tablecloth" name="${actionBean.permohonanLaporanBangunanList}" pagesize="5" cellpadding="0" cellspacing="0"
                                        ...
                                        <display:column title="Hapus">
                                            <div align="center">
                                                <img alt='Hapus' border='0' src='${pageContext.request.contextPath}/pub/images/not_ok.gif' class='rem'
                                                     id='rem_${line_rowNum}' onmouseover="this.style.cursor='pointer';" onclick="removeLaporanBangunan('${line.idLaporBangunan}', this.form);"/>
                                            </div>  <%--alert('edit'+${line.idLaporBangunan});--%>
                                        </display:column>
