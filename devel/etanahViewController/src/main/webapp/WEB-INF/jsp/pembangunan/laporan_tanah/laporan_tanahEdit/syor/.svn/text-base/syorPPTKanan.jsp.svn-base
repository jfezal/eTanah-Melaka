...
<script type="text/javascript">
    ...
    {   
        NoPrompt();
        var url = '${pageContext.request.contextPath}/pembangunan/laporanTanahv2?tambahRow&index='+index;
        window.open(url, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
    }
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pembangunan/laporanTanahv2?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=PPTKanan',q,
            function(data){
                $('#page_div').html(data);
