<%-- 
    Document   : dev_kertasMMK_carianSy
    Created on : May 26, 2014, 12:20:46 PM
...
<div class="subtitle">
    <s:form beanclass="etanah.view.stripes.pembangunan.KertasMMKActionBean">
        <fieldset class="aras1">
            <c:if test = "${actionBean.forEdit eq 'NY'}">
                ...
                <p>
                    <label>&nbsp;</label>
                    <s:submit name="cariKodSyaratNyataKertasMMK" value="Cari" class="btn" />
            ...
            <c:if test = "${actionBean.forEdit eq 'SK'}">
                ...
                <div class="subtitle">
                    ...
                    <p>
                        <label>&nbsp;</label>
                        <s:submit name="cariKodSekatanKertasMMK" value="Cari" class="btn"/>
                        <s:button name="tutup" value="Tutup" class="btn" onclick="self.close()"/>
                    </p>
                </div>
            </c:if>
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            ...
            <c:if test = "${actionBean.forEdit eq 'NY'}">           
                <p>
                    <display:table style="width:100%" class="tablecloth" name="${actionBean.listKodSyaratNyata}" pagesize="10" cellpadding="0" cellspacing="0" requestURI="/pembangunan/kertas_mmk" id="line">
            ...
            <c:if test = "${actionBean.forEdit eq 'SK'}">   
                <p>
                    <display:table style="width:100%" class="tablecloth" name="${actionBean.listKodSekatan}" pagesize="10" cellpadding="0" cellspacing="0" requestURI="/pembangunan/kertas_mmk" id="line">
            ...
            <div align="center">
                <c:if test="${fn:length(actionBean.listKodSekatan) > 0 || fn:length(actionBean.listKodSyaratNyata) > 0}">
                    <s:submit name="simpanKodSyaratNyataKertasMMK" value="Simpan" id="simpanKodSyaratNyata" class="btn"/>
                </c:if>
                <s:button name="tutup" value="Tutup" class="btn" onclick="self.close()"/>
            </div>
        </fieldset>
