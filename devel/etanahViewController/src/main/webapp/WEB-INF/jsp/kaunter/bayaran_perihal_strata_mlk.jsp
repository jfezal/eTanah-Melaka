<%--
    Document   : bayaran_perihal_strata_mlk
    Created on : July 10, 2013, 11:35:34 AM
    Author     : murali
--%>
...
<s:messages />
<s:errors />
<s:form beanclass="etanah.view.kaunter.BayaranPerihalMLKStrataActionBean">
    <s:hidden name="selectedItem" id="selectedItem" />
    <fieldset class="aras1">
        <legend>Terima Bayaran</legend></br>
        <p class=instr><em>*</em>DDSPastikan maklumat di bawah adalah betul dan masukkan butir-butir pembayaran. PERINGATAN: Tidak dibenarkan menggunakan cara pembayaran yang lain
    ...
    <p align="center">      
        ...
        <c:if test="${actionBean.stageId eq 'terimabayaran'}">
            <c:if test="${actionBean.isBayaranVisible eq 'true'}">
                <s:submit name="save" value="Bayar3" class="btn" onclick="" />
            </c:if>
            <c:if test="${actionBean.isBayaranVisible ne 'true'}">
        ...
        <c:if test="${actionBean.stageId eq 'terimabayaran2'}">            
            <c:if test="${actionBean.bayaran2Visible eq 'true'}">
                <s:submit name="save" value="Bayar2" class="btn" onclick="" />
            </c:if>
