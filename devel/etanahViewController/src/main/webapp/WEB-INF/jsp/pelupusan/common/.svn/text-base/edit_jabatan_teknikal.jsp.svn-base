<%-- 
    ...
    Created on : Jun 7, 2011, 3:02:35 PM
    Author     : afham
--%>
...
<script type="text/javascript">
    ...
    <%--  alert(kodAsal) ;--%>
            // alert("search:"+index);
            var url = '${pageContext.request.contextPath}/pelupusan/sedia_jabatan?kodAgensiPopup2&index='+index+'&kodAsal='+kodAsal + '&idRujukan=' + idRujukan;
    ...
    <%--  alert(kodAsal) ;--%>
            // alert("search:"+index);
            var url = '${pageContext.request.contextPath}/pelupusan/sedia_jabatan?kodAgensiPopup3&index='+index+'&kodAsal='+kodAsal + '&idRujukan=' + idRujukan;
            window.open(url, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=910,height=800,scrollbars=yes");
        }
        ...
        function removePermohonanRujSalinan(idRujukan,idSalinan){
            ...
            if(confirm('Adakah anda pasti?')) {
                var url = '${pageContext.request.contextPath}/pelupusan/sedia_jabatan?deleteRujukanSalinan&idRujukan='+idRujukan+'&idSalinan='+idSalinan;
                $.get(url,
                function(data){
                    $('#page_div').html(data);
        ...
        function viewPermohonanRujDokumen(v) {
        ...
        function removePermohonanRujDokumen(idRujukan,idDokumen){
            //                alert(idRujukan + " and row = " + idSalinan) ;
            if(confirm('Adakah anda pasti?')) {
                var url = '${pageContext.request.contextPath}/pelupusan/sedia_jabatan?deleteRujukanDokumen&idRujukan='+idRujukan+'&idDokumen='+idDokumen;
                $.get(url,
                function(data){
                    $('#page_div').html(data);
                    alert("Rekod dihapuskan") ;
...
<s:errors/>
<s:messages/>
<s:form beanclass="etanah.view.stripes.pelupusan.JabatanTeknikalActionBean" name="edit">
        ...
        <div class="content" align="center">
            ...
            <table>
                ...
                <tr><td>Catatan <br/><br/><br/><br/><br/></td>
                    ...
                    <td><s:textarea name="catatan"  cols="50" rows="5" value="${actionBean.pprl.catatan}"/></td></tr>
                    <%--<c:if test="${line.tarikhDisampai eq null}">--%>
                <tr><td>Tarikh Hantar </td>
                    <td> : </td>
                    <td><s:text name="tarikhHantar" class="datepicker" id="date1" formatPattern="dd/MM/yyyy" size="15" onchange="javascript:setNewDate()" value="${actionBean.pprl.tarikhDisampai}"/></td></tr>

                <tr><td>Tempoh hari </td>
                    <td> : </td>
                    <td><s:text name="jangkaMasa_hari" id="hari" value="${actionBean.pprl.tempohHari}"/></td></tr>
                <tr><td>Tarikh Jangka Terima </td>
                    <td> : </td>
                    <td><s:text name="tarikhJangkaTerima1" formatPattern="dd/MM/yyyy" size="15" readonly="true" value="${actionBean.pprl.tarikhJangkaTerima}"/></td></tr>
                <tr><td>Mandatori </td>
                    <td> : </td>
        ...
        <div class="content" align="center">
            ...
            <display:table class="tablecloth" name="${actionBean.prsd}" cellpadding="0" cellspacing="0" id="line"
                ...
                <display:column title ="Papar">
                    <img alt='Klik Untuk Hapus' border='0' src='${pageContext.request.contextPath}/pub/images/icons/edit_pelupusan_small.png' class='rem'
                         id='rem_${line_rowNum}' onclick="viewPermohonanRujDokumen(${line.dokumen.idDokumen})"  onmouseover="this.style.cursor='pointer';"/>
                </display:column>
                <display:column title ="Hapus">
                    <img alt='Klik Untuk Hapus' border='0' src='${pageContext.request.contextPath}/pub/images/not_ok.gif' class='rem'
