<%-- 
    Document   : borangV2Add
    Created on : Jun 17, 2013, 11:48:56 AM
    Author     : afham
--%>
...
<script type="text/javascript">
    ...
    function selectCheckBox()
    {
        ...
        if(confirm("Adakah anda pasti?")) {
            var url = '${pageContext.request.contextPath}/pelupusan/borangV2?simpanKodTuntut&item='+item+'&harga='+harga ;
            $.get(url,
            function(data){
                $('#page_div').html(data);
    ...
    function isChecked(val,c){
        ...
        if(c.checked){
            if(kod == 'DISFT'){
                ...
                if($('#jenisHakmilik').val() == 'GRN' || $('#jenisHakmilik').val() == 'PN'){
                    $('#harga'+val).val(40);
                }else{
                    $('#harga'+val).val(20);
                }
    ...
    function openFrame(type){
        NoPrompt();
        var url = '${pageContext.request.contextPath}/pelupusan/borangV2?openFrame&type='+type;
...
<body>
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.BorangPelupusanV2ActionBean" name="frm">
        <s:useActionBean beanclass="etanah.view.ListUtil" id="listUtil" />
        <s:errors/>
        <s:messages/>
        <div class="subtitle" id="main">
            <s:hidden name="${actionBean.sizeKod}" id="sizeKod" value="${actionBean.sizeKod}"/>
            <s:hidden name="jenisHakmilik" id="jenisHakmilik" value="${actionBean.hakmilikPermohonan.kodHakmilik.kod}"/>
            <fieldset class="aras1">
                <div id="maklumatborang" align="center">
                    <legend>Maklumat Borang</legend>
                    <c:set var="i" value="0" />
                    <display:table style="width:100%" class="tablecloth" name="${actionBean.listKodTuntut}" pagesize="100" cellpadding="0" cellspacing="0" requestURI="/pelupusan/borangV2" id="line">
                        <display:column> <s:checkbox name="checkmate" id="checkmate" value="${line.kod}" onclick="isChecked(${i},this);"/>
                            <s:hidden name="kod${i}" id="kod${i}" value="${line.kod}"/>
                        </display:column>
