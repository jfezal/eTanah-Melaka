...
<script type="text/javascript">
    ...
    function menyimpan(index,i,f)
    {
        ...
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_jkbb?simpanKandunganNS&index='+index+'&kandungan='+kand,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    ...
    {
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_jkbb?tambahRowNS&index='+index,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
    function deleteRow(idKandungan,f)
    {
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
             var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/draf_jkbb?deleteRowNS&idKandungan='+idKandungan,q,
            function(data){
                $('#page_div').html(data);
            }, 'html');
        }
          ...
          <tr>
            <td colspan="4">
                    ...
                    <fieldset class="aras1">
                        <legend> 
                        <c:if test="${actionBean.urusanStatus eq 'PBMMK'}">
                            <c:if test="${actionBean.stageId eq 'semakan_draf_mmk'}">
                                 No. Rujukan : <s:text name="no_ruj" id="no_ruj" size="20"/><br><br>
                            </c:if>
                            <c:if test="${actionBean.stageId eq 'semakan_syor_draf' or actionBean.stageId eq 'semakhurai_draf' or actionBean.stageId eq 'perakuan_draf' or actionBean.stageId eq 'cetak_kertas_mmk'}">
                                No. Rujukan : ${actionBean.no_ruj}<br><br>
                            </c:if>
                            DRAF RENCANA MAJLIS MESYUARAT KERAJAAN NEGERI
                        </c:if>
                        <c:if test="${actionBean.urusanStatus eq 'PPTR'}">
                            <c:if test="${actionBean.stageId eq '13Trm_Draf_MMK'}">
                                 No. Rujukan : <s:text name="no_ruj" id="no_ruj" size="20"/><br><br>
                            </c:if>
                            <c:if test="${actionBean.stageId eq '14Smk_Huraian' or actionBean.stageId eq '15Smk_Huraian' or actionBean.stageId eq '16Smk_Huraian' or actionBean.stageId eq '17Perakuan' or actionBean.stageId eq '18Ctk_Krts_MMK'}">
                                No. Rujukan : ${actionBean.no_ruj}<br><br>
                            </c:if>
                            DRAF RENCANA MAJLIS MESYUARAT KERAJAAN NEGERI
                        </c:if>
...
<!--          <tr>
          ...
          <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PBPTD'}">
          <tr>
            <td width="1%">3.</td>
          ...
          <tr>  
              <td colspan="4">
                  <center>
                      ...
                      <c:if test="${actionBean.stageId eq 'semakan_draf_mmk'}">
                          <s:button name="SimpandrafJKBBNS" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div')"/>
                      </c:if>
                  </center>
            </td>
          </tr>
                    ...
                    --%>
                     ...
                     <tr>
                      ...
                      <tr>
                          ...
                          <td colspan="2"><div align="left">${actionBean.kuantiti}${actionBean.noktahbertindih}&nbsp<s:text name="syaratBahanBatu.jumlahIsipadu" id="kuantitiSyarat" onkeyup="calculateSyarat()"/> ${actionBean.syaratBahanBatu.jumlahIsipaduUom.nama}</div></td>
                      </tr>
                      <tr>
                          <td>&nbsp;</td>
