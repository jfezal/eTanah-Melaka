...
<s:form beanclass="etanah.view.stripes.pelupusan.KertasPindaanMMKV2ActionBean">
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            <legend>
                 3. HURAIAN <c:choose><c:when test="${actionBean.permohonan.cawangan.kod eq '08'}">TIMBALAN PENTADBIR TANAH GEMAS</c:when><c:otherwise>PENTADBIR TANAH ${actionBean.permohonan.cawangan.daerah.nama}</c:otherwise></c:choose>
                <span style="float:right">
            ...
            <table class="tablecloth" align="center">
                <c:if test="${!empty actionBean.senaraiKandunganHuraianPtd}">
                    ...
                    <c:forEach items="${actionBean.senaraiKandunganHuraianPtd}" var="line">
                        <tr>
                            <td style="text-align: right">3.${a}</td>
                            <td><s:textarea value="${line.kandungan}" cols="80" readonly="true" name="syorUlasan" rows="5" class="normal_text"/></td>
                        </tr>
                        <c:set value="${a+1}" var="a"/>
                    </c:forEach>
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            <legend>
                4. SYOR <c:choose><c:when test="${actionBean.permohonan.cawangan.kod eq '08'}">TIMBALAN PENTADBIR TANAH GEMAS</c:when><c:otherwise>PENTADBIR TANAH ${actionBean.permohonan.cawangan.daerah.nama}</c:otherwise></c:choose>
                <span style="float:right">
                    <c:if test="${actionBean.disKertasMMKController.kPerakuanPTD}">
                        <%-- <a onclick="viewSyor();" onmouseover="this.style.cursor='pointer';" ><img alt="Papar" height="25px" width="25px" src='${pageContext.request.contextPath}/pub/images/icons/view_pelupusan_small.png'/></a> | --%>
                        <a onclick="openFrame('kSyorPtd');" onmouseover="this.style.cursor='pointer';"><img alt="Kemaskini"  height="25px" width="25x" src='${pageContext.request.contextPath}/pub/images/icons/edit_pelupusan_small.png'/> </a>
                        </c:if>
                </span>
            </legend>
            <table class="tablecloth" align="center">
                <c:if test="${!empty actionBean.senaraiKandunganSyorPtd}">
                    <c:set value="1" var="i"/>
    ...
    <c:if test="${actionBean.disKertasMMKController.vPTG}">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>
                    5. HURAIAN PENGARAH TANAH DAN GALIAN
                    <span style="float:right">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>
                    6. SYOR PENGARAH TANAH DAN GALIAN
                    <span style="float:right">
                        <c:if test="${actionBean.disKertasMMKController.kPerakuanPTG}">
                            <%-- <a onclick="viewSyor();" onmouseover="this.style.cursor='pointer';" ><img alt="Papar" height="25px" width="25px" src='${pageContext.request.contextPath}/pub/images/icons/view_pelupusan_small.png'/></a> | --%>
