<%--
    ...
    Created on : Jun 14, 2010, 1:31:32 PM
...
<script type="text/javascript">
    $(document).ready( function() {   
        if('${actionBean.permohonan.kodUrusan.kod}' == "PLPS" || '${actionBean.permohonan.kodUrusan.kod}' == "PLPTD" || '${actionBean.permohonan.kodUrusan.kod}' == "RLPS" || '${actionBean.permohonan.kodUrusan.kod}' == "RLPSK" ){
...
<s:form name="form2" id="form2" beanclass="etanah.view.stripes.pelupusan.MaklumatTanahActionBean">
    ...
    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'MLCRG' or actionBean.permohonan.kodUrusan.kod eq 'MPJLB'}">         
    ...
    <c:if test="${hakmilik eq false && edit eq true}">
        <div class="subtitle">
            <fieldset class="aras1">
                ...
                <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'MLCRG' and actionBean.permohonan.kodUrusan.kod ne 'MPJLB'}">
                    <p>
                        <label>Daerah :</label>
                    ...
                    <c:if test="${actionBean.forBPM}">
                        <p>
                            ...
                            <s:select name="bandarPekanMukimBaru.kod" value="${actionBean.hakmilikPermohonan.bandarPekanMukimBaru.kod}" id="mpb" onchange="doSomething2(this.form);">
                                <s:option value="">Sila Pilih</s:option>
                                <s:options-collection collection="${actionBean.senaraiKodBPMMLCRGMPJLB}" label="nama" value="kod" />
                            </s:select>
                        </p>
                    </c:if>
                    <s:hidden name="test" value="${actionBean.hakmilikPermohonan.kodSeksyen.kod}"style="width:300px;" id="_kodSeksyen2"/>
                ...
                <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PPBB' && actionBean.permohonan.kodUrusan.kod ne 'PBPTG' && actionBean.permohonan.kodUrusan.kod ne 'PBPTD' && actionBean.permohonan.kodUrusan.kod ne 'MLCRG' && actionBean.permohonan.kodUrusan.kod ne 'PLPTD'}">
                    <p>
                ...
                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PPBB' || actionBean.permohonan.kodUrusan.kod eq 'PBPTG' || actionBean.permohonan.kodUrusan.kod eq 'PBPTD' || actionBean.permohonan.kodUrusan.kod eq 'MLCRG' || actionBean.permohonan.kodUrusan.kod eq 'PLPTD'}">
                    <p>
                        <label><font color="red"></font>Tempat/Lokasi Tanah Dipohon :</label>
                        <s:textarea name="hakmilikPermohonan.lokasi" rows="5" cols="70" id="tempat" class="normal_text"/>
                    </p>
                </c:if>
                <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'MLCRG'}">
                    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PPRU' and actionBean.permohonan.kodUrusan.kod ne 'PPTR'}">
                        <p>
                            <label>Luas :</label>
                    ...
                    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PPTR' or 'MMPJLB'}">
                ...
                <c:if test="${actionBean.negeri eq '04'}">
                    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'MLCRG'}">
                        <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PPTR'}">
                            <p>
                                <label>Nombor Lot  :</label>
                                <s:hidden name="kodLot.kod" id="kodlot" value="1"/>
                                <c:if test="${actionBean.hakmilikPermohonan.noLot ne null}">
                                    LOT &nbsp;
                                </c:if>
                                <s:text name="hakmilikPermohonan.noLot" id="noLot" onkeyup="validateNumber(this,this.value);"/>&nbsp;
                            </p> 
                        </c:if>
                    ...
                    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'MLCRG'}">
                        <p>
                            <label><font color="red">*</font>Nombor Lot  :</label>
                            <s:hidden name="kodLot.kod" id="kodlot" value="1"/>
                            <c:if test="${actionBean.hakmilikPermohonan.noLot ne null}">
                                LOT &nbsp;
                            </c:if>
                ...
                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PRMP'}">
                    <p>
                        ...
                        <%--<s:text name="kodGunaTanah" id="catatan"/>--%>
                    </p>
                    <p>
                        <label>Luas Dipohon :</label>
            ...
            <fieldset class="aras1">
                ...
                <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PPTR'}">
                    <p>
                        ...
                        <c:if test="${actionBean.hakmilikPermohonan.noLot ne null}">${actionBean.hakmilikPermohonan.noLot}</c:if>
                    </p>
                </c:if>    
                ...
                <c:if test="${actionBean.negeri eq '04'}">
                    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PPTR'}">
                        ...
                        <p id="namaJarak">
                            ...
                            <c:if test="${actionBean.jarakDariNama ne null}">
                                ${actionBean.jarakDariNama} 
                            </c:if>
                            <c:if test="${actionBean.jarakDariNama eq null}">
                                &nbsp;-
                            </c:if>
