...
$(document).ready(function() {
    ...
    {   
        ...
        var url = '${pageContext.request.contextPath}/pelupusan/kertas_JKTDV2?tambahRow&index='+index+'&kodksn='+kodksn+'&ksn='+kpsn;
        window.open(url, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
    }
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_JKTDV2?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kPerakuanPtd',q,
            function(data){
                $('#page_div').html(data);
    ...
    function cariPopup(val){
            var idMh = $('#idMh').val();
            NoPrompt();
            window.open("${pageContext.request.contextPath}/pelupusan/kertas_JKTDV2?searchSyarat&jenisSyarat="+val+"&idMh="+idMh, "eTanah",
        ...
        function kiraCukai(f) {
            ...
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_JKTDV2?kiraCukai&kodTanah=' + kodTanah , q,
            function(data) {
                if (data == '-1') {
                } else {
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.KertasJKTDV2ActionBean" name="form">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>Perakuan <c:choose><c:when test="${actionBean.permohonan.cawangan.kod eq '08'}">Timbalan Pentadbir Tanah Gemas</c:when><c:otherwise>Pentadbir Tanah</c:otherwise></c:choose></legend>
