<%-- 
    Document   : kertasMMKPerihalPermohonan
    Created on : Apr 16, 2013, 12:39:44 PM
    Author     : afham
--%>
...
<script type="text/javascript">
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_JKTDPBGSA?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kPermohonan',q,
            function(data){
                $('#page_div').html(data);

            }, 'html');
            self.refreshpage2('kPermohonan') ;
        }
    }
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.KertasJKTDPBGSAActionBean" name="form">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>Perihal Permohonan</legend>
                <table class="tablecloth" border="0" align="center">
                    <c:set var="i" value="1" />
                    <c:set var="num" value="1"/>
                    <c:forEach items="${actionBean.senaraiKandunganPerihalPermohonan}" var="line">
                        <tr>
                            <td style="text-align: right"><c:out value="2.1.${num}"/></td>
                            <td>
                                <s:textarea  id="kandungan5${i}" name="senaraiKandunganPerihalPermohonan[${i-1}].kandungan" cols="80"  rows="5" onclick="checkKandungan(${i})" onblur="setKandungan(${i},${line.idKandungan})" class="normal_text"/>
                    ...
                    <tr>
                        <td style="text-align:center;" colspan="3">      
                            <s:button value="Tambah" class="btn" name="simpan1"  onclick="addRow('2',this.form)"/>
