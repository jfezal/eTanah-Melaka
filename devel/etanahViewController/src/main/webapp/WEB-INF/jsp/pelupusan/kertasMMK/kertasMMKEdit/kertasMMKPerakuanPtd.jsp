...
<script type="text/javascript">
    ...
    <c:choose>
        ...
        function addRow(index, f)
        {
            ...
            var url = '${pageContext.request.contextPath}/pelupusan/kertas_MMKV2?tambahRow&index=' + index + '&kodksn=' + kpsn + '&ksn=' + kpsn;
            window.open(url, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
        }
        ...
        function setKandunganSyarat(i, idKandungan) {
            var index = i;
            var kandungan = $('#kandungan14' + index).val();
            $('#' + idKandungan + 'kandunganKertas').val(kandungan);
        }
        ...
        function cariPopup(val) {
            ...
            //            alert(idMh);
            NoPrompt();
            window.open("${pageContext.request.contextPath}/pelupusan/kertas_MMKV2?searchSyarat&jenisSyarat=" + val + "&idMh=" + idMh, "eTanah",
        ...
        function kiraCukai(f) {
            ...
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_MMKV2?kiraCukai&kodTanah=' + kodTanah, q,
            function(data) {
                if (data == '-1') {
                } else {
        ...
        function checkKandunganSyarat(i) {
            var index = i;
            var kandungan = $('#kandungan14' + index).val();
            if (kandungan == 'Sila Tambah Ayat Di Sini') {
                $('#kandungan5' + index).val("");
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.KertasMMKV2ActionBean" name="form">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend> <c:choose><c:when test="${actionBean.permohonan.cawangan.kod eq '08'}">Huraian Timbalan Pentadbir Tanah Gemas</c:when><c:otherwise>Perakuan Pentadbir Tanah</c:otherwise></c:choose></legend>
                <table class="tablecloth" border="0" align="center">
                    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'BMRE' and actionBean.permohonan.kodUrusan.kod ne 'WMRE' and actionBean.permohonan.kodUrusan.kod ne 'MMRE' and actionBean.permohonan.kodUrusan.kod ne 'JMRE'}">
                        <tr>
                            <td>
                                Syor:
                            </td>
                        ...
                        <td colspan="3">
                            <c:choose>
                                ...
                                <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PLPS'}">
                                    <c:import url="syarat/syaratPLPS.jsp"></c:import>
                                </c:when>
                    ...
                    <tr>
                        <td colspan="3">Syarat Tambahan : </td>
                    </tr>
                    <c:set var="i" value="1" />
                    <c:set var="num" value="1"/>
                    <c:forEach items="${actionBean.senaraiSyarat}" var="line">
                        <tr>
                            <td style="text-align: right"><c:out value="${num}"/></td>
                            <td>
                                <s:textarea  id="kandungan14${i}" name="senaraiSyarat[${i-1}].kandungan" cols="80"  rows="5" onclick="checkKandunganSyarat(${i})" onblur="setKandunganSyarat(${i},${line.idKandungan})" class="normal_text"/>
                                <s:hidden id="${line.idKandungan}kandunganKertas" name="${line.idKandungan}kandunganKertas"/>
                                <s:hidden name="idKandungan1" id="idKandungan1" value="${line.idKandungan}"/>
                            </td>
                            <td style="vertical-align: middle;">
                    ...
                    <tr>
                        <td style="text-align:center;" colspan="3">
                            <c:choose>
                                <c:when test="${actionBean.permohonan.kodUrusan.kod ne 'BMRE' and actionBean.permohonan.kodUrusan.kod ne 'WMRE' and actionBean.permohonan.kodUrusan.kod ne 'MMRE'}">
                                    <s:button value="Tambah Huraian" class="btn" name="simpan1"  onclick="addRow('5',this.form)"/>
                                    <s:button value="Tambah Syarat" class="btn" name="simpan1"  onclick="addRowSyarat('14',this.form)"/>
                                ...
                                <c:otherwise>
                                    <s:button value="Tambah Ulasan" class="btn" name="simpan1"  onclick="addRow1('5',this.form)"/>
                                    <s:hidden name="indexKertas" id="indexKertas" value="5"/>
                                    <s:submit name="simpanKandungan" value="Simpan" class="btn" onclick="NoPrompt();"/>
                                    <%--<s:button name="simpanSyor" value="Simpan" class="btn" onclick="menyimpan('5',${i-1},this.form)"/>--%>
                                    <s:button name="tutup" value="Tutup" class="btn" onclick="refreshpage()"/>
