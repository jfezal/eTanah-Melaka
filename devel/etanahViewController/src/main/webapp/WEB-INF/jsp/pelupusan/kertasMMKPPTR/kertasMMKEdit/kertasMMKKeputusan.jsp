<%-- 
    Document   : kertasMMKKeputusan
    Created on : Apr 16, 2013, 12:43:17 PM
    Author     : afham
--%>
...
<title>Keputusan Pengarah Tanah dan Galian</title>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        <c:if test="${actionBean.fasaPermohonan.keputusan ne null}">
            <c:choose>
            <c:when test="${actionBean.fasaPermohonanptg.keputusan.kod eq 'SL'}">
                                    var ksn1 = "SL";
                    doFunc1(ksn);
            </c:when>
            <c:when test="${actionBean.fasaPermohonanptg.keputusan.kod eq 'ST'}">
                                    var ksn1 = "ST";
                                    doFunc(ksn1);
            </c:when>
            <c:otherwise>
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_MMKPPTR?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kKeputusan',q,
            function(data){
                $('#page_div').html(data);

            }, 'html');
            self.refreshpage2('kKeputusan') ;
        }
    }
...
<body>
    <script>
        ...
        function deleteRowPBPTG(idKandungan,f,tName)
        {   
            NoPrompt();
            if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
                var q = $(f).formSerialize();
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.KertasMMKPPTRActionBean" name="form">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>Keputusan Pengarah Tanah dan Galian</legend>
                <table class="tablecloth" border="0" align="center">
                    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PBPTG'}">
                        <c:set var="i" value="1" />
                        ...
                        <c:forEach items="${actionBean.senaraiKandunganKeputusan}" var="line">
                            <tr>
                                ...
                                <td>
                                    <s:textarea  id="kandungan5${i}" name="senaraiKandunganKeputusan[${i-1}].kandungan" cols="80"  rows="5" onclick="checkKandungan(${i})" onblur="setKandungan(${i},${line.idKandungan})" class="normal_text"/>
                        ...
                        <tr>
                            <td style="text-align:center;" colspan="3">      
                                <s:button value="Tambah" class="btn" name="simpan1"  onclick="addRow('7',this.form)"/>
                                <s:submit name="simpanKandungan" value="Simpan" class="btn" onclick="NoPrompt();"/>
                                <s:hidden name="indexKertas" id="indexKertas" value="7"/>
                    ...
                    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PBPTG'}">
                        ...
                        <c:forEach items="${actionBean.senaraiKandunganKeputusan}" var="line">
                            <tr>
                                ...
                                <td style="vertical-align: middle;">
                                    <s:button value="Hapus" class="btn" name="tambah" onclick="deleteRowPBPTG(${line.idKandungan},this.form,'mohonKertasKandungan')"></s:button> 
                                    </td>
                                </tr>
                        ...
                        <tr>
                            <td style="text-align:center;" colspan="3">      
                                <s:button value="Tambah" class="btn" name="simpan1"  onclick="addRowPBPTG('7',this.form)"/>
                                <s:submit name="simpanKandungan" value="Simpan" class="btn" onclick="NoPrompt();"/>
                                <s:hidden name="indexKertas" id="indexKertas" value="7"/>
                                <%--<s:button name="simpanSyor" value="Simpan" class="btn" onclick="menyimpan('5',${i-1},this.form)"/>--%>
                                <s:button name="tutup" value="Tutup" class="btn" onclick="refreshpage()"/>
