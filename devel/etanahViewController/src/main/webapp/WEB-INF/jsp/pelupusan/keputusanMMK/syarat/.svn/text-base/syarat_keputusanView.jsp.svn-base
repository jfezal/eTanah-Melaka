<%-- 
    Document   : syarat_keputusanView
    Created on : Mar 27, 2013, 4:38:51 PM
    Author     : afham
--%>
                ...
                <fieldset class="aras1">
                    ...
                    <div class="content" align="center">
                        ...
                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PBMT'}">
                            <c:import url="syaratView/syaratPBMT.jsp"></c:import>
                        </c:when>
                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PPTR'}">
                            <c:import url="syaratView/syaratPPTR.jsp"></c:import>
                        </c:when>
                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PPRU'}">
                            <c:import url="syaratView/syaratPPRU.jsp"></c:import>
                        </c:when>
                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PPRU'}">
                            <c:import url="syaratView/syaratPLPS.jsp"></c:import>
                        </c:when>
                         <c:otherwise> <c:import url="syaratView/syaratPLPS.jsp"></c:import></c:otherwise>
                        </c:choose>
                    </div>
                </fieldset>
            </div>
