<%-- 
    Document   : draf_pertimbangan_ptd
    Created on : Apr 21, 2010, 4:49:25 PM
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        for (var i=0; i<=len; i++){
            $('.hakmilik'+i).click( function() {
                window.open("${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?popupHakmilikDetail&idHakmilik="+$(this).text(), "eTanah",
    ...
    function menyimpan(index,i,f){
        ...
        $.post('${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?simpanKandungan&index='+index+'&kandungan='+kand,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    ...
    {
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?tambahRow&index='+index,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
    function deleteRow(idKandungan,f)
    {
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?deleteRow&idKandungan='+idKandungan,q,
            function(data){
                $('#page_div').html(data);
            }, 'html');
        }
    ...
    function cariPopup(){
        var url = '${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?search';
        window.open(url, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=700 scrollbars=yes");
    }
    function cariPopup2(){
        var url = '${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?showFormCariKodSekatan';
        window.open(url, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=700 scrollbars=yes");
    }
    function semakSyor(f,v){
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_pertimbangan_ptd?showsyortolaklulus&syortolaklulus='+v,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
</script>
<s:form beanclass="etanah.view.stripes.pelupusan.DrafPertimbanganPTDActionBean">
    <s:messages/>
    <s:errors/>
    ...
    <table width="90%" border="0" >
        ...
        <tr>
            ...
            <td><display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" pagesize="20" cellpadding="0" cellspacing="0" id="line"
                           requestURI="/pelupusan/draf_pertimbangan_ptd">
        ...
        <c:if test="${actionBean.openPTD}">
            <c:if test="${actionBean.editPTD}">
                <tr>
                    <td width="1%"><b><c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PHLA'}"> 5.</c:if>
                                  </b>
                    </td>
                ...
                <c:forEach items="${actionBean.senaraiLaporanKandunganPTD}" var="line">
                    <tr>
                        <td>&nbsp;</td>
                       <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PHLA'}">
                                <td valign="top"><c:out value="5.${i}"/></td>
                            </c:if>
                          <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PHLA'}">
                              <td valign="top"><c:out value="5.${i}"/></td>
                          </c:if>
                        <td colspan="2">
                ...
                <tr>
                    <td width="1%"><b>
                                      <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PHLA'}">5</c:if>
                                  </b>
                    </td>
                    <td colspan="3"><div align="left"><b><font style="text-transform: capitalize">PERAKUAN PENTADBIR TANAH MELAKA TENGAH</font></b></div></td>
                </tr>
