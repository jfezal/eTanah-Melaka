<%-- 
    Document   : draf_jkm_mlk_mpjlb
    Created on : May 29, 2012, 10:49AM
    Author     : Aizuddin Orogenic Group
    Purpose    : This is jsp for DrafMMK_MPJLB, also can be use for MLCRG
--%>
...
<script type="text/javascript">
    ...
    function menyimpan(index,i,f){
        ...
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmk_mpjlb?simpanKandungan&index='+index+'&kandungan='+kand+'&viewMMKN='+viewMMKN,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
    function deleteRow(idKandungan,f){
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var viewMMKN = $('#viewMMKN').val();
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/draf_mmk_mpjlb?deleteRow&idKandungan='+idKandungan+'&viewMMKN='+viewMMKN,q,
            function(data){
                $('#page_div').html(data);
            }, 'html');
        }}
    ...
    function addRow(index){
        var viewMMKN = $('#viewMMKN').val();
        var url = '${pageContext.request.contextPath}/pelupusan/draf_mmk_mpjlb?tambahRow&index='+index+'&viewMMKN='+viewMMKN;
...
<s:form beanclass="etanah.view.stripes.pelupusan.DrafMMK_MPJLBActionBean" name="form" id="form">
            ...
            <td colspan="4">
                <div class="subtitle" style="text-transform: capitalize">
                    <fieldset class="aras1">
                        <legend>${actionBean.topTopic}</legend>
        ...
        <tr>
            ...
            <td></td><td>
                <display:table class="tablecloth" name="${actionBean.pihakPengarahList}" cellpadding="0" cellspacing="0" id="line"
                               requestURI="${pageContext.request.contextPath}/pelupusan/draf_mmk_mpjlb">
...
<!--            <td><b>7.</b></td>
        ...
        <c:if test="${!actionBean.viewMMKN}">
            <tr>
                <td><b>
                        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'MLCRG' or actionBean.permohonan.kodUrusan.kod eq 'MPJLB'}">8.
                        </c:if>
                        <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'MLCRG' and actionBean.permohonan.kodUrusan.kod ne 'MPJLB'}">
                            9.
                        </c:if>    
