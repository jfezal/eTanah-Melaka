     ...
     function save(event, f){
       ...
       function masuk(){
          // alert('t1');
           ...
           //  alert('t2');
             self.close();
             opener.document.getElementById('syaratNyata1').focus();
...
<s:form beanclass="etanah.view.stripes.pelupusan.SekatanNyataKepentinganActionBean">
    <s:hidden name="syarat" />
        ...
        <fieldset class="aras1">
                    <legend></legend>
                    <p>
                        <display:table style="width:100%" class="tablecloth" name="${actionBean.listKodSekatan}" pagesize="10" cellpadding="0" cellspacing="0" requestURI="/pelupusan/sekatanNyataKepentingan" id="line">
                    ...
                    <c:if test="${fn:length(actionBean.listKodSekatan) > 0}" >
                        <p><label>&nbsp;</label>
                            <s:button name="simpanKodSekatan" value="Simpan" id="simpanKodSekatan" class="btn" onclick="javascript:masuk();"/>
                        </p>
                    </c:if>
                </fieldset>   
</s:form>