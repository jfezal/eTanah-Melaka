...
<script type="text/javascript">
    function openFrame(type){
        ...
        //        alert(idHakmilik);
        window.open("${pageContext.request.contextPath}/pelupusan/rekod_keputusanV2?openFrame&idHakmilik="
    ...
    function refreshRekodKeputusanV2(type){
        var url = '${pageContext.request.contextPath}/pelupusan/rekod_keputusanV2?refreshpage&type='+type;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function reload (val) {
        doBlockUI();
        var url = '${pageContext.request.contextPath}/pelupusan/rekod_keputusanV2?reload&idHakmilik=' + val;
        $.ajax({
            type:"GET",
            url : url,
            dataType : 'html',
...
<s:form beanclass="etanah.view.stripes.pelupusan.RekodKeputusanV2ActionBean">
    <s:errors/>
    <s:messages/>
                ...
                <fieldset class="aras1">
                    ...
                    <c:choose>
                        ...
                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PBMT'}">
                            <c:import url="keputusanView/syarat/syaratPBMT.jsp"></c:import>
                        </c:when>
                        <c:otherwise>
