<%-- 
    Document   : muat_naik_imej
    Created on : Jan 30, 2012, 3:46:50 PM
    Author     : Srinivas
--%>
...
<script type="text/javascript">
    ...
    return false;--%>
        ...
        function muatNaikForm(idPermohonan,lokasi) {
            ...
            var url = '${pageContext.request.contextPath}/pelupusan/muat_naik_imej?uploadDoc&idPermohonan=' + idPermohonan+'&lokasi=' + lokasi;
            window.open(url, 'etanah', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=200, left=" + left + ",top=" + top);
        }

        function refreshPage(){
            var url = '${pageContext.request.contextPath}/pelupusan/muat_naik_imej?reload';
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
//            },'html');
            ...
            var left = (screen.width/2)-(1000/2);
            var top = (screen.height/2)-(150/2);
            var url = '${pageContext.request.contextPath}/pelupusan/muat_naik_imej?uploadDoc';
        ...
        function hapusimej(idDokumen){
            if((confirm("Anda pasti ingin menghapuskan imej ini?"))){
                var url = '${pageContext.request.contextPath}/pelupusan/muat_naik_imej?hapusImej&idDokumen='+idDokumen;
                $.get(url,
                function(data){
                    $('#page_div').html(data);
                },'html');
        ...
        function popImej(idDok){
            ...
            var url = '${pageContext.request.contextPath}/pelupusan/muat_naik_imej?viewImejPopup&idPermohonan='+id+'&idDokumen='+idDok;
...
<s:form id="testForm" name="myform" beanclass="etanah.view.stripes.pelupusan.MuatNaikImejActionBean">
                            ...
                            <c:forEach items="${actionBean.senaraiImejLaporan}" var="item" varStatus="loop">
                                ...
                                <tr>
                                    ...
                                    <td valign="top">
                                        ...
                                        <ul class="thumb">
                                            <li>
                                                <img id="img${item.dokumen.idDokumen}" alt="${item.dokumen.namaFizikal}" align="center" src="${pageContext.request.contextPath}/dokumen/view/${item.dokumen.idDokumen}" height="100" width="100" data-plusimage="${pageContext.request.contextPath}/dokumen/view/${item.dokumen.idDokumen}" data-plussize="400,300" onclick="popImej('${item.dokumen.idDokumen}')" /><%-- onclick="doSubmit(this.form);" --%>
                                            </li>
                                        </ul>
                                        <br>
                                    </td>
...
<div id="muatnaik" style="display: none">
    <s:form name="myform1" beanclass="etanah.view.stripes.pelupusan.MuatNaikImejActionBean">
        <fieldset class="aras1">
            <div id="error"/>
