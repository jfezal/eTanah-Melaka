...
<script type="text/javascript">
    ...
    function deleteRow(idKandungan, f, tName)
    {
        ...
        if (confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/laporan_tanahV2?deleteRow&idKandungan=' + idKandungan + '&tName=' + tName + '&typeName=PPTKanan', q,
                    function(data) {
                        $('#page_div').html(data);
                                ...
                                <legend>
                                    <c:choose>
                                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PBMT'}">
                                            <c:if test="${actionBean.permohonan.cawangan.kod eq '08'}">
                                                Ulasan Penolong Pentadbir Tanah    
                                            </c:if>
                                            <c:if test="${actionBean.permohonan.cawangan.kod ne '08'}">
                                                Ulasan Penolong Pegawai Tanah (Kanan)     
                                            </c:if>                       
                                        </c:when>
                                        <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'PPBB'}">
