<%--
    Document   : maklumatTanahListPHPP
            ...
            <div class="subtitle">
                <fieldset class="aras1">
                    ...
                    <p>
                        ...
                        <c:if test="${actionBean.hakmilik.idHakmilik ne null}"> ${actionBean.hakmilik.idHakmilik}
                        </c:if>
                    ...
                    <c:if test="${actionBean.kodNegeri1 eq '05'}">
                        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'HT'}">
                            <c:if test="${actionBean.permohonan.permohonanSebelum.kodUrusan.kod ne 'PHPP' && actionBean.permohonan.permohonanSebelum.kodUrusan.kod ne 'PHPC'}">
                                <p>
                                    <label>Keluasan  :</label>
                                    <c:if test="${actionBean.hakmilik.luas ne null}">  <s:format formatPattern="#,##0.0000" value="${actionBean.hakmilik.luas}"/>&nbsp; ${actionBean.hakmilik.kodUnitLuas.nama}&nbsp; </c:if>
                                    <c:if test="${actionBean.hakmilik.luas eq null}"> Tiada Data </c:if>
                    ...
                    <c:if test="${actionBean.kodNegeri1 eq '05'}">
                        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'HT'}">
                            <c:if test="${actionBean.permohonan.permohonanSebelum.kodUrusan.kod eq 'PHPP' || actionBean.permohonan.permohonanSebelum.kodUrusan.kod eq 'PHPC'}">
                                <p>
                                    <label>Nombor Skim :</label>
                                    <c:if test="${actionBean.hakmilik.noSkim ne null}">${actionBean.hakmilik.noSkim}</c:if>
                                    <c:if test="${actionBean.hakmilik.noSkim eq null}"> Tiada Data </c:if>
