<%--
    Document   : maklumat_jadualpetak_PBS
    Created on : AUG 22, 2012, 10:42:01 AM
    Author     : Murali
--%>
...
<script type="text/javascript">
    ...
    function popup(b,p,idT){
        ...
        window.open("${pageContext.request.contextPath}/strata/urusanPBS?showForm2&namaBangunan="+b+"&tingkat="+p+"&idTingkat="+idT, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=800,height=250");
    }

    function popupPetakAksesori(b,t,p){

        window.open("${pageContext.request.contextPath}/strata/urusanPBS?showForm3&idBangunan="+b+"&idTingkat="+t+"&idPetak="+p, "eTanah",
    ...
    function removePtkAksr(val){
        ...
        if(confirm('Adakah anda pasti?')) {
            var url = '${pageContext.request.contextPath}/strata/urusanPBS?deletePtkAksr&idAksesori='+val;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
    ...
    function selectedBangunan(idBangunanVal){
        ...
        var url = '${pageContext.request.contextPath}/strata/urusanPBS?selectedBangunan&idBangunan='+idBangunanVal;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function selectedTingkat(idBangunanVal,idTingkatVal){
        ...
        var url = '${pageContext.request.contextPath}/strata/urusanPBS?selectedTingkat&idBangunan='+idBangunanVal+'&idTingkat='+idTingkatVal;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function selectedPetak(idBangunanVal,idTingkatVal,idPetakVal){
        ...
        var url = '${pageContext.request.contextPath}/strata/urusanPBS?selectedPitak&idBangunan='+idBangunanVal+'&idTingkat='+idTingkatVal+"&idPetak="+idPetakVal;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
...
<table><tr><td>
            <s:form beanclass="etanah.view.strata.MaintainBangunanActionBeanPBSN9">
                <s:messages/>
                <s:errors/>  
                <c:if test="${actionBean.kodurusan eq 'PBS' || actionBean.kodurusan eq 'PBBSS' 
                              || actionBean.kodurusan eq 'PSBS' || actionBean.kodurusan eq 'RMHS5'
                              || actionBean.kodurusan eq 'PBTS'}">
                            ...
                            <div class="subtitle">
                                <fieldset class="aras1">
                                    ...
                                    <table class="tablecloth" width="100%">
                                        ...
                                        <c:set var="i" value="1" />
                                        <c:forEach items="${actionBean.bptk}" var="petak" varStatus="statusB">

                                            <tr>
                                                <td><center>L${petak.nama}</center></td>
                                                <%--<td><center><s:text name="area" readonly="true">${bgn.untukKegunaanLain}</s:text></center></td>--%>
                                                <td><center><s:text name="syor" readonly="true">${petak.syer}</s:text></center></td>
                                            </tr>
