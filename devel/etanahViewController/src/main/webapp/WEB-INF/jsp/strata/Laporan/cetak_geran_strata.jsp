...
//        var idHakmilikStrata = $('#idHakmilikStrata').val();
        ...
        var url = '${pageContext.request.contextPath}/strata/cetakGeranstrata?cari&idPermohonan='+ idPermohonan;
        frm.action = url;
        frm.submit();
    }
    ...
    function printSelectedDocuments1 () {
        ...
        for (var i=0; i<len; i++) {
            ...
            if (c.checked) {
                ...
                var url = '${pageContext.request.contextPath}/strata/cetakGeranstrata?cetakSemula&sbb_cetakan_semula=' + sbb + '&id_dokumen=' + c.value + '&idHakmilik=' + x.value;
                $.ajax({
                    type:"GET",
                    url : url,
    ...
    function doSaveCapaian(v,b){
        ...
        var url = '${pageContext.request.contextPath}/strata/cetakGeranstrata?cetakSemula&sbb_cetakan_semula=' + sbb + '&id_dokumen=' + v + '&idHakmilik=' + idHakmilik;
        $.ajax({
            type: "GET",
            url: url,
            success: function(data) {
    ...
    function doPrintViaApplet(docId) {
        ...
        function sejarahCetakan(id){
            window.open("${pageContext.request.contextPath}/strata/cetakGeranstrata?viewSejarahCetakan&idDokumen="+id, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=900,height=700");
        }
        function sejarahPaparan(id){
            window.open("${pageContext.request.contextPath}/strata/cetakGeranstrata?viewSejarahPaparan&idDokumen="+id, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=900,height=700");
        }
...
<div id="laporan">
    <s:messages/>
    <s:errors/>
    <s:form beanclass="etanah.view.strata.laporan.CetakGeranStrataActionBean" id="cetak_hakmilik">
        <div class="subtitle">
                ...
                <div align="center">
                    <table border=0 class="tablecloth"><tr>
                            ...
                            <td align=left>
                                ...
                                <s:text name="idPermohonan" id="idPermohonan"  size="37"
        ...
        <fieldset class="aras1">
            ...
            <c:if test="${fn:length(actionBean.senaraiDokumenProvKK) >0}">
                <c:set var="rowNum" value="0"/>
                <display:table style="width:90%" class="tablecloth" name="${actionBean.senaraiDokumenProvKK}" cellpadding="0" cellspacing="0" id="line1">
                        <c:if test = "${line1.hakmilik eq line.hakmilikStrata}">
                            <display:column title="Tajuk">
                                ${line1.tajuk}
                            </display:column>
