...
<script type="text/javascript">
    ...
    <c:if test='${actionBean.checked}'>
        ...
        function muatturun2()
        {
            ...
            if(hantar == '1') {
                ...
                if (r==true)
                {
                    ...
                    var url = '${pageContext.request.contextPath}/strata/gis/mlk?updateVal&hantarfile='+val;
                    $.get(url, q,
                    function(data){
                        $('#page_div').html(data);
                    },'html');
                ...
                if (r==true)
                {
                    ...
                    var dokVal = document.getElementById("form1");
                    var q = $(dokVal).formSerialize();
                    var url = '${pageContext.request.contextPath}/strata/gis/mlk?hantarKeKomms&hantarfile='+val;
                    $.get(url, q,
                    function(data){
                        $('#page_div').html(data);
                        $.unblockUI();
        ...
        function simpandoc(){
            ...
            var dokVal = document.getElementById("form1");
            var q = $(dokVal).formSerialize();
            var url = '${pageContext.request.contextPath}/strata/gis/mlk?simpanDoc';
            $.get(url, q,
            function(data){
                $('#page_div').html(data);
                $.unblockUI();
        ...
        function tutuphantar(){
            ...
            var url = '${pageContext.request.contextPath}/strata/gis/mlk?tutupHantar&hantarfile='+val;
            $.get(url,
            function(data){
                $('#page_div').html(data);
                $.unblockUI();
...
<s:form id="form1" name="form1" beanclass="etanah.view.strata.GisMLKStrataActionBean">
    <s:messages/>
    <s:errors/>
    <div>
