<html>
  ...
  <body>
    <script type="text/javascript">
       ...
       function doSubmit(e, f) {
        var n = $('#namaPenerima').val();  
        $('#namaPemberi').val(""); 
        var q = $(f).formSerialize();
        f.action = f.action + '?' + e+ '&namaPenerima=' + n;
        f.submit();
      }
      
      function doSubmit2(e, f) {
        var n = $('#namaPemberi').val();  
        $('#namaPenerima').val(""); 
        var q = $(f).formSerialize();
        f.action = f.action + '?' + e+ '&namaPemberi=' + n;
        f.submit();
      }
      
      function clickclearcaripenerima() {
       $('#namaPenerima').val(""); 
      }
      
      function clickclearcaripemberi() {
       $('#namaPemberi').val(""); 
      }
    ...
    <s:form action="/daftar/pertanyaanSBSWSA" >
        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SW'}">
            <s:hidden name="permohonanSW" value="${actionBean.permohonan.idPermohonan}"/>
        </c:if>
      ...
      <fieldset class="aras1">
       ...
       <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'SA'}">
        ...
        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SW'}">
          <legend></legend>
        <span class=instr>Carian Terus Pemberi.<em>* Bagi senarai yang banyak pemberi</em></span><br/><br/>
        <br>
        <p><label>Nama Pemberi :</label>
            <s:text name="namaPemberi" id="namaPemberi" maxlength="50" size="31" onkeyup="this.value=this.value.toUpperCase();" />
        </p>
        <p>
            <label>&nbsp;</label>  
            <s:button name="caripemberi" value="Cari" class="btn" onclick="doSubmit2('caripemberi',this.form)" />
            <s:button  name="reset" value="Isi Semula" class="btn" onclick="clickclearcaripemberi();"/>
        </p>
         <c:if test="${fn:length(actionBean.carianListWakilKuasaPemberi) > 0}">
          <legend>Maklumat Pemberi</legend>
          <div class="content" align="center">
            <display:table class="tablecloth" name="${actionBean.carianListWakilKuasaPemberi}" pagesize="10" cellpadding="0" cellspacing="0"
                           id="line">
              <display:column title="Bil" sortable="true">${line_rowNum}
              </display:column>
              <display:column property="wakilKuasa.permohonan.idPermohonan" title="ID Perserahan"  class="nama"/>
        ...
        <br>
        <legend></legend>
        <span class=instr>Carian Terus Penerima.<em>* Bagi senarai yang banyak penerima</em></span><br/><br/>
        <br>
        <p><label>Nama Penerima :</label>
            <s:text name="namaPenerima" id="namaPenerima" maxlength="50" size="31" onkeyup="this.value=this.value.toUpperCase();" />
        </p>
        <p>
            <label>&nbsp;</label>  
            <s:button name="caripenerima" value="Cari" class="btn" onclick="doSubmit('caripenerima',this.form)" />
            <s:button  name="reset" value="Isi Semula" class="btn" onclick="clickclearcaripenerima();"/>
        </p>
        <c:if test="${fn:length(actionBean.carianListWakilKuasaPenerima) > 0}">
          <legend>Maklumat Penerima</legend>  
          <div class="content" align="center">
            <display:table class="tablecloth" name="${actionBean.carianListWakilKuasaPenerima}" pagesize="10" cellpadding="0" cellspacing="0"
                           requestURI="${pageContext.request.contextPath}/daftar/pertanyaanSBSWSA" id="line">
              <display:column title="Bil" sortable="true">${line_rowNum}
              </display:column>
              <display:column property="wakilKuasa.permohonan.idPermohonan" title="ID Perserahan"  class="nama"/>
