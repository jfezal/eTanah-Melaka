...
//    $.post('${pageContext.request.contextPath}/daftar/utiliti/kutipanData?senaraiKodBPMByDaerah&kodDaerah=' + kodDaerah + '&popup=true',
    $.post('${pageContext.request.contextPath}/daftar/utiliti/kutipanData?cariBPM&kodDaerah=' + kodDaerah + '&popup=true&bpm=' + $('#bpm').val(),
            function(data) {
              if (data != '') {
...
<script>
  ...
  <c:choose>
    ...
    <c:otherwise>
        ...
        var url = '${pageContext.request.contextPath}/daftar/utiliti/kutipanData?' + v;
        frm.action = url;
        frm.submit();
      }
</script>
...
<div class="kutipdata">
  ...
  <s:form beanclass="etanah.view.daftar.utiliti.KutipanDataActionBean" name="form1">    
    <fieldset class="aras1">
      ...
      <legend>Utiliti Kutipan Data </legend>
      <br>
      <c:choose>
        <c:when test="${actionBean.kelompok eq 'true'}"> 
          <p>
      ...
      <p>
        <label>Negeri :</label>
        <s:text name="kodNegeri" id="kodNegeri" readonly="true" size="4"/>&nbsp;-&nbsp; 
        <s:text name="namaNegeri" id="namaNegeri" readonly="true" style="text-transform: uppercase;" size="29"/>
      </p>
      <p>
        <label>Daerah :</label>
        <s:text name="kodDaerah" size="4" id="kodDaerah" readonly="true" />&nbsp;-&nbsp;
        <s:select name="namaDaerah" id="namaDaerah" onchange="filterKodBPM(this.value);" style="width:200">
          <c:choose>
            <c:when test="${actionBean.pguna.kodCawangan.kod eq '00'}">    
      ...
      <s:hidden name="bpm2" id="bpm" value="${actionBean.bpm}"/> 
      ...
      <p>
        ...
        <s:select name="jenisHakmilik" id="namaJenisHakmilik" style="width:200">
          ...
          <c:choose>
            ...
            <c:when test="${actionBean.pguna.kodCawangan.kod eq '00' && actionBean.kodNegeri eq '05'}">                  
              <s:options-collection collection="${actionBean.senaraiKodHakmilikPTG}" label="nama" value="kod" />
            </c:when>   
            ...
            <c:when test="${actionBean.pguna.kodCawangan.kod eq '00' && actionBean.kodNegeri eq '04'}">                  
              <%--<s:options-collection collection="${actionBean.senaraiKodHakmilikPTDmlk}" label="nama" value="kod" />--%>
              <s:options-collection collection="${actionBean.senaraiKodHakmilikPTG}" label="nama" value="kod" />
            </c:when> 
          </c:choose>
      ...
      <c:choose>
        <c:when test="${actionBean.kelompok eq 'true'}"> 
          <p>
            ...
            Mula -&nbsp;<s:text name="noHakmilikSiriDari[0]" id="noHakmilikSiriDari0" 
                    onblur="checkNoHakmilik(this.value);this.value=this.value.toUpperCase();" size="10" 
                    onchange="appendAuto(this.value, '0');"/>
            &nbsp;Hingga -
            <s:text name="noHakmilikSiriHingga[0]" id="noHakmilikSiriHingga0" 
                    onblur="checkNoHakmilik1(this.value);this.value=this.value.toUpperCase();" size="10"
                    onchange="appendAuto(this.value, '0');"/> 
          </p>
        </c:when>
        <c:otherwise>
