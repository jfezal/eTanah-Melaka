    ...
    <script type="text/javascript">
        ...
        function removeMultipleUrusan() {
            if (confirm('Adakah anda pasti untuk hapus Urusan ini')) {
                ...
                var url = '${pageContext.request.contextPath}/pendaftaran/kemasukan_perincian_hakmilik_utiliti?deleteMultipleUrusan' + param + '&idHakmilik=' + id;
                $.get(url,
                        function (data) {
                            $('#page_div').html(data);
                        }, 'html');
        ...
        function saveMultipleUrusan() {
         ...
         function saveMultipleHakmilikPihakTertinggal() {
                if (confirm('Adakah anda pasti untuk tambah Pihak ini')) {
                    ...
                    var url = '${pageContext.request.contextPath}/pendaftaran/kemasukan_perincian_hakmilik_utiliti?saveMultipleHakmilikPihakTertinggal' + param + '&idHakmilik=' + id;
                    $.get(url,
                            function(data) {
                                $('#page_div').html(data);
                            }, 'html');
...
//            var kodSeksyen = '${actionBean.hakmilik.seksyen.seksyen}'
        ...
        function removeHakmilikUrusan(val) {
            ...
            var answer = confirm("adakah anda pasti untuk Hapus?");
            if (answer) {
                var url = '${pageContext.request.contextPath}/pendaftaran/kemasukan_perincian_hakmilik_utiliti?deleteHakmilikUrusan&idUrusan=' + val + '&idHakmilik=' + id;
                $.blockUI({
                    message: $('#displayBox'),
                    css: {
                        top: ($(window).height() - 50) / 2 + 'px',
    ...
    <s:form beanclass="etanah.view.stripes.KemasukanPerincianHakmilikUtilitiActionBean" id="kemasukanPerincianHakmilik">
        <s:messages/>
        <s:errors/>
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                ...
                <c:if test="${actionBean.p.kodUrusan.kod ne 'HT'}">
                    ...
                    <c:if test="${actionBean.p.kodUrusan.kod eq 'HKABS' || actionBean.p.kodUrusan.kod eq 'HKSB' || actionBean.p.kodUrusan.kod eq 'HSSB' || actionBean.p.kodUrusan.kod eq 'HKABT'}">
                        ...
                        <%--<s:select  style="text-transform:uppercase" name="kodUOM" onchange="kiraCukai(this.form,'${actionBean.hakmilik.idHakmilik}');cariKadarCukai(this.form,'${actionBean.hakmilik.idHakmilik}');cariKodCukai(this.form,'${actionBean.hakmilik.idHakmilik}');">
                               <s:option value="">Sila Pilih</s:option>
            ...
            <div class="subtitle">
                ...
                <p>
                    <label>&nbsp;</label>
                    <s:hidden name="idPermohonan" id="${actionBean.p.idPermohonan}"/>
                    <s:hidden name="hakmilik.seksyen.kod" id="kodSeksyen"/>
                    <s:submit name="simpan" id="simpan" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div')"/>&nbsp;
                    <%--<s:submit name="simpanKelompok" id="simpanKelompok" value="Simpan Berkelompok" class="longbtn" onclick="doSubmit(this.form, this.name, 'page_div',kodSeksyen)"/>--%>
                </p>
                <br>
            </fieldset>
        </div>
              ...
              <div class="subtitle">
                  <fieldset class="aras1">
                      ...
                      <p>
                          <label>&nbsp;</label>
                          <c:if test="${fn:length(actionBean.senaraiHakmilikUrusan) > 0}">
<!--                              <s:button class="btn" value="Hapus1" name="" onclick="removeMultipleUrusan();"/>&nbsp;-->
                          </c:if>
...
<!--                              <s:submit name="saveMultipleHakmilikPihakTertinggal" id="simpan" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div')"/>&nbsp;-->
                          </c:if>
                      </div>
                      <br>
                  </fieldset>
        ...
        <%-- <div class="subtitle">
             <fieldset class="aras1">
                 ...
                 <div class="content" align="center" id="listpihak">
                     <s:button name="btnpopupPihak"  id="btnpopupPihak"  value="Tambah" class="btn" onclick="popupPihak('${actionBean.hakmilik.idHakmilik}');" />
                     <display:table class="tablecloth" style="width:90%;" name="${actionBean.pihakKepentinganList}" cellpadding="0" cellspacing="0" id="linepihak">
