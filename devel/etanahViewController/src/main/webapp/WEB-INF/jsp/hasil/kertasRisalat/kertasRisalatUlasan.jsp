<%-- 
    Document   : kertasRisalatUlasan
    Created on : Jun 26, 2013, 1:41:04 AM
    Author     : khadijah
--%>
...
<script type="text/javascript">
    ...
    function deleteRow(idKandungan, f, tName)
    {
        ...
        if (confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/hasil/kertas_risalat?deleteRow&idKandungan=' + idKandungan + '&tName=' + tName + '&typeName=kUlasan', q,
                    function(data) {
                        $('#page_div').html(data);

                    }, 'html');
            self.refreshpage2('kUlasan');
        }
    }
    ...
    <s:form beanclass="etanah.view.stripes.hasil.KertasRisalatMMKNActionBean" name="form">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>Ulasan Jabatan Kewangan dan Perbendaharaan Negeri Melaka</legend>
                <table class="tablecloth" border="0" align="center">
                    ...
                    <c:forEach items="${actionBean.senaraiUlasanJab}" var="line">
                        <tr>
                            ...
                            <td>
                                <s:textarea  id="kandungan5${i}" name="senaraiUlasanJab[${i-1}].kandungan" cols="80"  rows="5" onclick="checkKandungan(${i})" onblur="setKandungan(${i},${line.idKandungan})" class="normal_text"/>
                    ...
                    <tr>
                        <td style="text-align:center;" colspan="3">
                            <s:button value="Tambah" class="btn" name="simpan1"  onclick="addRow('10',this.form)"/>
                            <s:submit name="simpanKandungan" value="Simpan" class="btn" onclick="NoPrompt();"/>
                            <s:hidden name="indexKertas" id="indexKertas" value="10"/>
