...
<script type="text/javascript">
    function cetakKertasMMK(){
        window.open("${pageContext.request.contextPath}/hasil/ulasan_mmk_mlk?cetakMMK", "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=500");
    }
</script>

<s:form beanclass="etanah.view.stripes.hasil.UlasanMMKMlkActionBean">
    <s:messages/>
    <s:errors/>
        ...
        <fieldset class="aras1">
            ...
            <div class="content" align="center">
                <table border="0" width="96%">
                    ...
                    <tr><td><b><font style="text-transform:uppercase">PERMOHONAN DARIPADA <c:forEach items="${actionBean.senaraiPemohon}" var="senaraiPemohon">${senaraiPemohon.pihak.nama}, </c:forEach> UNTUK ${actionBean.permohonan.kodUrusan.nama} BAGI HAKMILIK ${actionBean.hakmilik.kodHakmilik.kod}&nbsp;${actionBean.hakmilik.noHakmilik}&nbsp;${actionBean.hakmilik.lot.nama}&nbsp;
                    ...
                    <tr><td width="96%">Kertas ini bertujuan untuk mendapatkan pertimbangan Majlis Mesyuarat Kerajaan, Negeri Melaka di atas permohonan <c:forEach items="${actionBean.senaraiPemohon}" var="senaraiPemohon">${senaraiPemohon.pihak.nama}, No. Kad Pengenalan ${senaraiPemohon.pihak.noPengenalan}, </c:forEach>
                            untuk <font style="text-transform:capitalize;">${actionBean.permohonan.kodUrusan.nama}</font> bagi hakmilik ${actionBean.hakmilik.kodHakmilik.kod}&nbsp;${actionBean.hakmilik.noHakmilik}&nbsp;${actionBean.hakmilik.lot.nama}&nbsp; ${actionBean.hakmilik.noLot},&nbsp;${actionBean.hakmilik.bandarPekanMukim.nama}&nbsp;,
                    ...
                    <c:if test="${edit}">
                        <tr><td><s:textarea name="latarBelakang" value="${actionBean.latarBelakang}" rows="5" cols="120"/></td></tr>
                    </c:if>
                    <c:if test="${!edit}">
                            ...
                            <table border="0" width="100%">
                                ...
                                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PPDL'}">
                                    ...
                                    <tr>
                                        ...
                                        <td id="tdDisplay"><fmt:formatNumber pattern="#,##0.00" value="${actionBean.hakmilik.akaunCukai.baki}"/>&nbsp;</td>
                                    </tr>
                                </c:if>
                            </table>
                        </td></tr>
                    ...
                    <c:if test="${edit}">
                        ...
                        <c:if test="${editPPT}"><tr><td><s:textarea name="huraianPentadbir" value="${actionBean.huraianPentadbir}" rows="5" cols="120"/></td></tr></c:if>
                        <c:if test="${!editPPT}"><tr><td>${actionBean.huraianPentadbir}&nbsp;<s:hidden name="huraianPentadbir"/></td></tr></c:if>
                        <tr><td><b>4. <font style="text-transform:uppercase">SYOR PENTADBIR TANAH  ${actionBean.hakmilik.daerah.nama}&nbsp;</font></b></td></tr>
                        <c:if test="${editPPT}"><tr><td><s:textarea name="syorPentadbir" value="${actionBean.syorPentadbir}" rows="5" cols="120"/></td></tr></c:if>
                        <c:if test="${!editPPT}"><tr><td>${actionBean.syorPentadbir}&nbsp;<s:hidden name="syorPentadbir"/></td></tr></c:if>
                        <tr><td><b>5. HURAIAN PENGARAH TANAH DAN GALIAN NEGERI MELAKA</b></td></tr>
                        <c:if test="${editTPTG}"><tr><td><s:textarea name="huraianPtg" value="${actionBean.huraianPtg}" rows="5" cols="120"/></td></tr></c:if>
                        <c:if test="${!editTPTG}"><tr><td>
                        ...
                        <c:if test="${editPTG}"><tr><td><s:textarea name="syorPtg" value="${actionBean.syorPtg}" rows="5" cols="120"/></td></tr></c:if>
                        <c:if test="${!editPTG}"><tr><td>
            ...
            <c:if test="${edit}">
                ...
                <p>
                    ...
                    <%--<s:button onclick="cetakKertasMMK();" name="" value="Cetak Kertas MMK" class="longbtn" disabled="${actionBean.cetak}"/>&nbsp;--%>
