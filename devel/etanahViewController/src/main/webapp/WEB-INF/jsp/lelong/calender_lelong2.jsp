...
<script type="text/javascript">
    $(document).ready(function() {
        alert('${actionBean.tarikhEnkuiri}');
    ...
    <c:forEach items="${actionBean.listCalender2}" var="x"  varStatus="n">
        ...
        <c:forEach items="${x.nama}" var="z">
            ...
            name2 = name2 + "Penggadai : <br>";
            </c:if>
            <c:if test="${z.jenis.kod eq 'PG'}" >
                    name2 = name2 + "${z.jenis.nama} : <br>";
            </c:if>
        </c:forEach>
    ...
    <c:forEach items="${actionBean.listCalender2}" var="k" varStatus="l" >
            ...
            $('#calendar').fullCalendar({
                ...
                dayClick: function(date, allDay, jsEvent, view) {
                    ...
                    if (bilL < 2) {
                        ...
                        var dE = '${actionBean.tarikhLelongTerdahulu}';
                        var dateEn = dateValidation3(dE);
                        if (dateEn > date) {
                            var sdate = append(dateEn.getUTCDate()) + "/" + append((dateEn.getUTCMonth() + 1)) + "/" + dateEn.getFullYear();
                            alert("Tarikh Dimasukkan Salah. Sila Pilih Tarikh Selepas Daripada " + sdate);
                        } else {
        ...
        function dateValidation(value) {
            ...
            var fulldate = vsplit[1] + '/' + vsplit[0] + '/' + vsplit[2];
            var date = new Date(fulldate);
            var date2 = (date.getUTCMonth() + 5);
            var date1 = (date.getUTCMonth() + 5) + '/' + (date.getUTCDate()) + '/' + (date.getFullYear());//+1900);
            var date3 = date2 - 12;
            if (date2 > 12) {
                date1 = (date3) + '/' + (date.getUTCDate()) + '/' + (date.getFullYear() + 1);
            }
//        var date1 = (date.getUTCMonth() + 5) + '/' + (date.getUTCDate()) + '/' + (date.getFullYear());//+1900);
            var dateNew = new Date(date1);
            var sdate = append(dateNew.getUTCDate() - 1) + "/" + append((dateNew.getUTCMonth() + 1)) + "/" + dateNew.getFullYear();

            return sdate;
        }
        //30 hari selepas tarikh enkuiri
        ...
        //lelongan terdahulu
        function dateValidation3(value) {
            var vsplit = value.split('/');
            var fulldate = vsplit[1] + '/' + vsplit[0] + '/' + vsplit[2];
            var date = new Date(fulldate);
            var date1 = (date.getUTCMonth() + 2) + '/' + (date.getUTCDate() + 2) + '/' + (date.getFullYear());//+1900);
            var dateNew = new Date(date1);
