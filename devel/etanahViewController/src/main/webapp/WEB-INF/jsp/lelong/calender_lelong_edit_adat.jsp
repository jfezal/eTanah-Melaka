<%-- 
    Document   : calender_lelong_edit
    Created on : Jun 10, 2011, 11:15:09 AM
...
<script type="text/javascript">
    ...
    <c:forEach items="${actionBean.listCalender}" var="i" varStatus="j" >
            ...
            titleArray[no-1] = {title: '\n ${i.idMohon}  \n\n SIASATAN DI ${i.tempat} \n\n',
                start: startDate,
                description:'IDPermohonan : ${i.idMohon} <br> IDHakmilik : ${i.hakmilik} <br> SIASATAN DI ${i.tempat} <br>'+pihakle[${j.index}],
                allDay: false};
    </c:forEach>
    ...
    <c:forEach items="${actionBean.listCalender2}" var="k" varStatus="l" >
            ...
            titleArray[no-1] = {title: '\n ${k.idMohon}  \n\n LELONGAN DI ${k.tempat} \n\n',
                start: startDate,
                description:'IDPermohonan : ${k.idMohon} <br> IDHakmilik : ${k.hakmilik} <br> LELONGAN DI ${k.tempat} <br>'+pihakle2[${l.index}],
                allDay: false};
    </c:forEach>
        ...
        function dateValidation2(value){
            ...
            var fulldate = vsplit[1]+'/'+vsplit[0]+'/'+vsplit[2];
            var date = new Date(fulldate);
            var date1 = (date.getUTCMonth()+1)+'/'+(date.getUTCDate()+181)+'/'+(date.getFullYear());//+1900);
            var dateNew = new Date(date1);
