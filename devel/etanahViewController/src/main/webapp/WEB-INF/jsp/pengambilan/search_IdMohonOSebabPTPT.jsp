<%--
    Document   : search_IdMohonOSebabPTPT
    Created on : Apr 14, 2011, 5:44:54 PM
    Author     : massita
    Modified By: Murali 10-01-2012
--%>
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            <%-- <c:if test="${fn:length(actionBean.listIdMohon) == 1}">--%>
            <c:forEach items="${actionBean.listIdMohon}" var="line" >
                <c:set var="j" value="1"/>
                <c:forEach items="${line.senaraiHakmilik}" var="line2">                     
                    <c:forEach items="${actionBean.curPermohonan.senaraiHakmilik}" var="line3">
                ...
                <c:set var="j" value="1"/>
                    <c:forEach items="${line.senaraiHakmilik}" var="line2">
                           <c:forEach items="${actionBean.curPermohonan.senaraiHakmilik}" var="line3">
                                        if(document.getElementById("hak_"+('${line.idPermohonan}')+'${j}') != null){                                          
                                                <c:if test="${line2.hakmilik.idHakmilik eq line3.hakmilik.idHakmilik}">
                                                    document.getElementById("hak_"+('${line.idPermohonan}')+'${j}').disabled = false;
                                                </c:if>
    ...
    <body>
        ...
        <s:messages/>
        <div class="subtitle">
            <s:form beanclass="etanah.view.stripes.pengambilan.carian_permohonanPendudukanSementaraActionBean">
            ...
            <div class="subtitle">
                <c:if test="${fn:length(actionBean.listIdMohon) > 0}" >
                    <fieldset class="aras1">
                        ...
                        <p>
                            <display:table style="width:97%" class="tablecloth" name="${actionBean.listIdMohon}" pagesize="3" cellpadding="1" cellspacing="1" requestURI="/pengambilan/pemulihanTanahCarianPendudukanSementara" id="line">
