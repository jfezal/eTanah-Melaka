<%--
    Document   :  syorMMMCL(Editable).jsp
    Created on :  April 10, 2012, 02:52:00 PM
    Author     :  Shazwan
--%>
...
<title>SYOR PENOLONG PEGAWAI TANAH (KANAN)</title>
...
<script type="text/javascript">
    ...
    <c:if test="${actionBean.ksn ne null}">
        ...
        function showmcmclsokong(val) {
            $('#mmmclsokong').show();
            $('#mmmcltidaksokong').hide();
            setKpsn2(val);
        }
    
        function showmcmcltidaksokong(val) {
            $('#mmmcltidaksokong').show();
            $('#mmmclsokong').hide();
            setKpsn2(val);
        }
        ...
        {   
            ...
            var url = '${pageContext.request.contextPath}/pengambilan/laporan_tanahV2?tambahRow&index='+index+'&kodksn='+kodksn;
            window.open(url, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
        }
        ...
        {   
            ...
            if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
                var q = $(f).formSerialize();
                $.post('${pageContext.request.contextPath}/pengambilan/laporan_tanahV2?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=PPT',q,
                function(data){
                    $('#page_div').html(data);
            ...
            <fieldset class="aras1">
                <div id="perihaltanah">
                    <legend>
                        <c:if test="${actionBean.kodNegeri eq '04'}">
                            Syor Penolong Pegawai Tanah (Kanan)
                        </c:if>
                    ...
                    <c:when test="${actionBean.kodNegeri eq '04'}">
                        <table class="tablecloth" align="center">
                            <tr>
                                ...
                                <td>
                                    <s:radio name="ksn" id="ksn" value="SL" onclick="showmmmclsokong(this.value);" />&nbsp;Sokong
                                    <s:radio name="ksn" id="ksn" value="ST" onclick="showmmmcltidaksokong(this.value);" />&nbsp;Tidak Sokong
                ...
                <table class="tablecloth" border="0" align="center">
                   <%-- <c:if test="${!empty actionBean.senaraiLaporanKandungan1}">
                    ...
                    <%--<c:if test="${actionBean.pguna.perananUtama eq '24'}">--%>
                        <tr>
                            <td><s:textarea cols="80" name="syorUlasanPPTK" rows="5"/></td>
                        </tr>
                    <%--</c:if>--%>
                    <%--<c:if test="${actionBean.pguna.perananUtama eq '32'}">--%>
                       <%-- <tr>
                            <td><s:textarea value="TIADA ULASAN KETUA PENOLONG PEGAWAI TANAH TELAH DIBUAT." cols="80" name="syorUlasanPPTK" rows="5"/></td>
                        </tr>--%>
                    <%--</c:if>--%>
                    <tr>
                        <%--readonly="true"--%>
                        <td style="text-align:center;" colspan="3">
                            <%--onclick="addRow('5',this.form)"--%>
                            <%--<s:button value="Tambah Ulasan" class="btn" name="simpan1"  onclick="addRow('5',this.form)"/>--%>
                            <%--<s:submit name="simpanUlasan" value="Simpan Submit" class="btn"/>
                            <s:button name="simpanUlasan" id="save" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div')"/>--%>
                            <s:submit name="simpanUlasanPPTK" value="Simpan" class="btn" onclick="NoPrompt();"/>
