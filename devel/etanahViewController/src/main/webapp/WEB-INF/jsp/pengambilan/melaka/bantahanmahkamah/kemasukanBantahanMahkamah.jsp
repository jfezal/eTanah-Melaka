<%-- 
    Document   : kemasukanBantahanMahkamah
...
<script type="text/javascript">
    ...
    function search(){
        ...
        var url = '${pageContext.request.contextPath}/pengambilan/carianIdHakmilikBantahanMahkamah?HakmilikPopup&idHakmilik='+idH;
    ...
    function searchHakmilik(){
        ...
        if(idH != null){
            var url = '${pageContext.request.contextPath}/pengambilan/carianIdHakmilikBantahanMahkamah?searchHakmilik&idHakmilik='+idH;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<s:form name="form1" id="form1" beanclass="etanah.view.stripes.pengambilan.KemasukanBantahanMahkamahActionBean">
    <div  id="hakmilik_details">
                                    ...
                                    <display:column title="IdHakmilik">
                                        <c:if test="${line1.hakmilik.idHakmilik eq actionBean.idHakmilik}">
                                            <s:link beanclass="etanah.view.stripes.pengambilan.KemasukanBantahanMahkamahActionBean"
                                                    event="selectHakmilik" onclick="return ajaxLink(this, '#hakmilik_details');" >
        ...
        <c:if test="${fn:length(actionBean.senaraiHPihak) > 0}" >
            <div class="subtitle">
                <fieldset class="aras1">
                    <legend>Senarai Hakmilik</legend><br />
                    <div  align="center">
                        <display:table style="width:30%" class="tablecloth" name="${actionBean.senaraiHPihak}" pagesize="5" cellpadding="1" cellspacing="1" requestURI="/pengambilan/kemasukanBantahanMahkamah" id="line">
                            <%--<display:column> <s:radio name="radio_" id="${line.id}" value="${line.id}" style="width:15px" onclick="javascript:selectMH(this)"/></display:column>--%>
                            <%--<display:column  property="idPermohonan" title="Id Permohonan" style="vertical-align:top;"/>--%>
                            <%--<display:column title="Nama Projek" property="sebab" style="text-transform:uppercase;vertical-align:top;" />--%>
                            <display:column title="Hakmilik" >
                                <%--<display:table name="${line.senaraiHakmilik}" id="line1">--%>
                                <c:if test="${line.idHakmilik eq actionBean.idHakmilik}">
                                    <s:link beanclass="etanah.view.stripes.pengambilan.KemasukanBantahanMahkamahActionBean"
                                            event="selectHakmilik" onclick="return ajaxLink(this, '#hakmilik_details');" >
                                        <%--<s:param name="selectedIdSblm" value="${line.idPermohonan}"/>--%>
                                        <s:param name="selectedIdHakmilik" value="${line.idHakmilik}"/>
                                        <s:param name="idHakmilik" value="${actionBean.idHakmilik}"/>
                                        ${line.idHakmilik}
                                    </s:link>
                                </c:if>
