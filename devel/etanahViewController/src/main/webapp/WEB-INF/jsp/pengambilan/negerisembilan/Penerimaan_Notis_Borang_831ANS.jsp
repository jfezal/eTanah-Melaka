<%-- 
    Document   : Penerimaan_Notis_Borang_831ANS
    Created on : 19-Oct-2011, 11:31:51
    Author     : nordiyana
--%>
...
<script type="text/javascript">
    ...
    <%--function validate(){
        ...
        function validateDate(i,date){
            ...
            var date3 = $('#tarikhTerima'+i).val();
            var vsplit = date2.split('/');
            var date2 = vsplit[1]+'/'+vsplit[0]+'/'+vsplit[2];
            var vsplit3 = date3.split('/');
            var date3 = vsplit3[1]+'/'+vsplit3[0]+'/'+vsplit3[2];
            var hantar = new Date(date2);
            var terima = new Date(date3);
    ...
    <%-- if(tarikh.value == "" ){
         ...
         function muatNaikForm(idPihak) {
             ...
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupUpload&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&showHP='+showHP;
         ...
         function muatNaikFormPemohon(idPihak) {
             ...
             var left = (screen.width/2)-(1000/2);
             var top = (screen.height/2)-(150/2);
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupUpload&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&showHP='+showHP+'&isPemohonPihak=true';
             window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=200, left=" + left + ",top=" + top);
         }
         ...
         function muatNaikFormMahkamah() {
             var left = (screen.width/2)-(1000/2);
             var top = (screen.height/2)-(150/2);
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupUpload&isMahkamah=true';
             window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=200, left=" + left + ",top=" + top);
         }
         function scanMahkamah() {
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupScanMahkamah&isMahkamah=true';
         ...
         function scanPemohon(idPihak) {
             ...
             var showPP = $('#showPP').val();
             var showHP = $('#showHP').val();
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupScanPemohon&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&isPemohonPihak=true';
             window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=800,height=700");
         }

         function scanNJT(idNotis) {
             var left = (screen.width / 2) - (1000 / 2);
             var top = (screen.height / 2) - (150 / 2);
             var url = '${pageContext.request.contextPath}/dokumen/scan/' + idNotis ;
         ...
         function addPenghantarNotis(){
             ...
             var showPP = $('#showPP').val();
             var showHP = $('#showHP').val();
             window.open("${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupPenghantarNotis&idHakmilik="+idHakmilik+'&showPP='+showPP+'&showHP='+showHP, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=400");
         }

         function addPenghantarNotisM(){
             window.open("${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupPenghantarNotis&isMahkamah=true", "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=400");
         }
         ...
         function scan(idPihak) {
             ...
             var showPP = $('#showPP').val();
             var showHP = $('#showHP').val();
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?popupScan&idPihak='+idPihak+'&idHakmilik='+idHakmilik;
             window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=800,height=700");
         }


         function refreshPagePenerimaanBorang(){
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?refreshpage';
             $.get(url,
             function(data){
                 $('#page_div').html(data);
             },'html');
         ...
         function notis(h){
             var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?notisPopup&idPihak='+h;
         ...
         function refreshNotis(idHakmilik,showPP,showHP,isMahkamah){
             if(showHP == 'true'){
                 if(idHakmilik !=''){
                     var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?hakmilikDetails&idHakmilik='+idHakmilik+"&showPP="+showPP;
                     $.get(url,
                     function(data){
                         $('#page_div').html(data);
                     },'html');
                     ...
                     if(showPP == 'true'){
                         var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?showFormPP';
                         $.get(url,
                         function(data){
                             $('#page_div').html(data);
                         },'html');
                 ...
                 var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?showFormPemohon';
                 $.get(url,
                 function(data){
                     $('#page_div').html(data);
                 },'html');
             }else if(isMahkamah == 'true'){
                 var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_ns831a?showFormMahkamah';
                 $.get(url,
                 function(data){
                     $('#page_div').html(data);
                 },'html');
...
<s:form beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorang831ANSActionBean">
    <div  id="hakmilik_details">
        <s:messages/>
        <s:errors/>
        <c:if test="${actionBean.kodNegeri eq '05'}">
            <s:hidden name="showJpph" value="${actionBean.showJpph}" id="showJpph"/>
        </c:if>
        ...
        <fieldset class="aras1"><br/>
            ...
            <c:if test="${actionBean.showHP eq 'true'}">
                <p align="center">
                    <display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" pagesize="5" cellpadding="0" cellspacing="0"
                                   requestURI="/pengambilan/penerimaan_notis_borang_ns831a" id="line">
                        <display:column title="No" sortable="true">${line_rowNum}</display:column>
                        <display:column title="ID Hakmilik">
                            <s:link beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorang831ANSActionBean"
            ...
            <%--|| (fn:length(actionbBean.NotisNPJSenarai) > 0)--%>
            <c:if test="${actionBean.showJpph eq true}">
                <div align="center" id="pemohonList">
                    <display:table class="tablecloth" name="${actionBean.notisNPJ}"
                                   cellpadding="0" cellspacing="0" requestURI="/pengambilan/penerimaan_notis_borang_ns831a" id="line">
                        ...
                        <display:column title="Tarikh" >
                            ...
                            <display:column title="Tindakan">
                                <c:forEach items="${actionBean.notisNPJ}" var="senarai">
                                <p align="center">
                                    <c:if test="${actionBean.idDokumenListP[line_rowNum-1] eq ''}">
                                        <img src="${pageContext.request.contextPath}/pub/images/icons/upload.png" height="30" width="30" alt="Muat Naik"
                                             onmouseover="this.style.cursor='pointer';" onclick="muatNaikFormPemohon('${senarai.pihak.idPihak}');return false;" title="Muat Naik Dokumen"/>
                                        /
                                        <img src="${pageContext.request.contextPath}/pub/images/icons/scanner.png" height="30" width="30" alt="Imbas"
                                             onmouseover="this.style.cursor='pointer';" onclick="scanNJT('${senarai.idNotis}');return false;" title="Imbas Dokumen"/>
                                    </c:if>
                                    <c:if test="${actionBean.idDokumenListP[line_rowNum-1] ne ''}">
                                        ...
                                        <img src="${pageContext.request.contextPath}/pub/images/icons/_active__document.png" height="30" width="30" alt="papar"
                                             onclick="doViewReport('${senarai.buktiPenerimaan.idDokumen}');" onmouseover="this.style.cursor='pointer';" title="Papar Dokumen"/>
                                    </c:if>
                                        <%--${actionBean.idDokumenListP[line_rowNum-1]}--%>
                                </p>
                            </c:forEach>
                        </display:column>
                    </display:table><br /><br />
            ...
            <br/>
            <br>
            <c:if test="${actionBean.showPP eq true && ((fn:length(actionBean.listNotisPemohonASB) > 0) ||(fn:length(actionBean.listNotisPemohonPB) > 0)||(fn:length(actionBean.listNotisPemohonASBG) > 0))}">
                <div align="center" id="pemohonList">
                        ...
                        <div align="center">
                            ...
                            <display:table class="tablecloth" name="${actionBean.senaraiPermohonanPihak}"
                                ...
                                <display:column title="Tarikh" class="${line_rowNum}">
                                    <p>H : <s:text class="datepicker" onchange="validateHantar12('${line_rowNum-1}');" id="tarikhHantar${line_rowNum-1}" name="tarikhHantar[${line_rowNum-1}]" formatPattern="dd/MM/yyyy" style="width:100px;"/>
                                        <br>
                                        &nbsp;<font size="1" color="red"> (cth : hh / bb / tttt)</font>
                                    </p>
                                    <p>T : <s:text class="datepicker" onchange="validateDate('${line_rowNum-1}',this.value);" id="tarikhTerima${line_rowNum-1}" name="tarikhTerima[${line_rowNum-1}]" formatPattern="dd/MM/yyyy" style="width:100px;"/>
                                        <br>
                                        &nbsp;<font size="1" color="red"> (cth : hh / bb / tttt)</font>
                                    </p>
                                </display:column>
            ...
            <c:if test="${(actionBean.hakmilik ne null && actionBean.show eq true ) || (actionBean.showPP eq true && actionBean.show eq true ) || (actionBean.showJpph eq true)}">
                <div align="center">
                    <s:hidden name="idHakmilik" value="${actionBean.hakmilik.idHakmilik}" id="idHakmilik"/>
                    <s:button class="btn"  name="simpan" value="Simpan" onclick="if(validate())doSubmit(this.form, this.name,'page_div')"/>
                </div>
