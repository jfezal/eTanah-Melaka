...
<body>
    <script>
        ...
        function doSubmit(e, f, g) {
            ...
            alert("idFailUpload>>>>"+idFailUpload);
           // if (confirm('Adakah anda pasti?')) {
                var url = '${pageContext.request.contextPath}/jomPay/uploadCsvFail?paparListDelete&idUploadFile='
                        + idFailUpload;
                $.get(url,
                        function(data) {
                            $('#page_div').html(data);
            ...
            <p>
                <display:table style="width:100%" class="tablecloth" name="${actionBean.listJompayDetails}"  cellpadding="0" cellspacing="0" id="line">
                    ...
                    <display:column title="Kemaskini">
                        <s:hidden name="idUploadFile" id="idUploadFile" value="${line.idJomPayUpload}"/>
                        <s:submit name="paparListDelete" value="Hapus" class="btn"/>
                        <%--<s:button name="" id="btnHapus2" value="Hapus" class="btn" onclick="deleteFail('${line.idJomPayUpload}')"/>--%>
