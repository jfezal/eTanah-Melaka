 ...
 * @author wan.fairul
 */
@UrlBinding("/strata/kertasPHPP")
public class KertasPertimbanganPHPPActionBean extends BasicTabActionBean {
    ...
    String syorTP;
    private static final Logger LOG = Logger.getLogger(KertasPertimbanganPHPPActionBean.class);
...
//    private String stageId = "";
    ...
    public void rehydrate() {
        ...
        permohonanStrata = strService.findIDByidMH(idPermohonan, idMh1);
//        LOG.info("--permohonanStrata--::" + permohonanStrata.getIdStrata());


        //added by murali 13/08/2012 to get data from hakmilik :START
        ...
        if (hp != null) {
            if (kodNegeri.equals("04")) {
                ...
                if (hp.getHakmilik() != null) {
                    int NoHakmilik = Integer.parseInt(hp.getHakmilik().getNoHakmilik());
                    int NoLot = Integer.parseInt(hp.getHakmilik().getNoLot());
                    // butirhakmilikTemp.append(hp.getHakmilik().getKodHakmilik().getNama() + " " + hp.getHakmilik().getNoHakmilik() + ", " + " Lot " + hp.getHakmilik().getNoLot()
                    //       + ", " + hp.getHakmilik().getBandarPekanMukim().getNama() + ", " + hp.getHakmilik().getDaerah().getNama());
        ...
        if (permohonanStrata != null) {
            // Reading for HartaBersama to avoid commas when data is empty
            harta = "";
            ...
            if (permohonanStrata.getLaporanKemudahan12() != null) {
                butirhakmilikTemp1.append("Lain-lain: " + permohonanStrata.getLaporanKemudahan12() + ", ");
            }
        ...
        if (checkUrusanRayuan(permohonan)) {
            ...
            for (PermohonanBangunan bngn : pBangunanL) {
                bil_tgkt = bngn.getSenaraiTingkat().size();
            }
        ...
        if (kodNegeri.equals("05")) {
            ...
            PermohonanKertas pkerts2 = strService.findKertas(idPermohonan);
            if (pkerts != null) {
                String pkertas = String.valueOf(pkerts2.getIdKertas());
    ...
    public Resolution showForm3() {
        ...
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new JSP("strata/pbbm/kertas_pertimbanganPHPP_NS.jsp").addParameter("tab", "true");
    }

    //Add by ida on 241013 for n9
    ...
    public Resolution simpan() {
        ...
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new RedirectResolution(KertasPertimbanganPHPPActionBean.class, "showForm").addParameter("error", error).addParameter("message", msg);
    }
    ...
    public Resolution simpanSemakKertasptg() {
        ...
        msg = "Maklumat telah berjaya disimpan.";
        getContext().getRequest().setAttribute("display", Boolean.TRUE);
        return new RedirectResolution(KertasPertimbanganPHPPActionBean.class, "showForm2").addParameter("error", error).addParameter("message", msg);
    }
    ...
    public Resolution deleteSingle() {
        ...
        return new RedirectResolution(KertasPertimbanganPHPPActionBean.class, "locate");
    }
    ...
    public Resolution deleteSingleSemakKertasptg() {
        ...
        getContext().getRequest().setAttribute("display", Boolean.TRUE);
        addSimpleMessage("Maklumat telah berjaya dihapus.");
        return new RedirectResolution(KertasPertimbanganPHPPActionBean.class, "showForm2");
    }
    ...
    public Resolution simpan1() {
        ...
        addSimpleMessage(msg);
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new RedirectResolution(KertasPertimbanganPHPPActionBean.class, "showForm3").addParameter("error", error).addParameter("message", msg);
    }
    ...
    public Resolution hakdetails() {
        ...
        LOG.info("---------------idMh1:--------" + idMh1);
        rehydrate();
        String norujTemp = getContext().getRequest().getParameter("noruj");
        String ulasanJabatanUkurdanPemetaanTemp = getContext().getRequest().getParameter("ulasanJabatan");
        LOG.info("----------norujTemp-----------:" + norujTemp);
        LOG.info("----------ulasanJabatanUkurdanPemetaanTemp-----------:" + ulasanJabatanUkurdanPemetaanTemp);
        if (norujTemp != null) {
            noruj = norujTemp;
        }
        if (ulasanJabatanUkurdanPemetaanTemp != null) {
            ulasanJabatanUkurdanPemetaan = ulasanJabatanUkurdanPemetaanTemp;
        }
        
        HakmilikPermohonan hp = strService.findHKByIdMh(permohonan.getIdPermohonan(), idMh1) ;
        
        hakmilik = strService.findInfoByIdHakmilik(hp.getHakmilik().getIdHakmilik());

        String asaspertimbanganTemp = getContext().getRequest().getParameter("asaspertimbangan");
        String syorTPTGTemp = getContext().getRequest().getParameter("syorTPTG");
        LOG.info("----------asaspertimbanganTemp-----------:" + asaspertimbanganTemp);
        LOG.info("----------syorTPTGTemp-----------:" + syorTPTGTemp);

        if (asaspertimbanganTemp != null) {
            asaspertimbangan = asaspertimbanganTemp;
        }

        if (syorTPTGTemp != null) {
            syorTPTG = syorTPTGTemp;
        }
