 ...
 */
@UrlBinding("/strata/rtpsmaklumat_Pemohon")
public class RTPSMaklumatPemohonActionBean extends BasicTabActionBean {
    ...
    public Resolution showForm() {
        ...
        /* code for IdHakmilik Search -- Start -- */
        if(getContext().getRequest().getSession().getAttribute("idHakmilikStrata") != null ){
            idHakmilikStrata = (String) getContext().getRequest().getSession().getAttribute("idHakmilikStrata");
            HakmilikPihakBerkepentingan hakmilikPihak = strService.findPihakByIdHakmilik(idHakmilikStrata);
        ...
        return new JSP("strata/Lanjutan_Tempo_Mohon/rtpsmaklumat_pemohon.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution terus() {
        ...
        getContext().getRequest().getSession().removeAttribute("idHakmilikStrata");

        if (idPermohonanTerdahulu != null) {
            getContext().getRequest().getSession().removeAttribute("error");
            getContext().getRequest().getSession().setAttribute("idPermohonanTerdahulu", idPermohonanTerdahulu);
            /* code for IdHakmilik Search -- Start -- */
             if(idHakmilikStrata != null){
                 getContext().getRequest().getSession().setAttribute("idHakmilikStrata", idHakmilikStrata);
             }
             /* code for IdHakmilik Search -- End -- */
            permohonan = permohonanDAO.findById(idPermohonanTerdahulu);
            ...
            if (permohonan != null && fp == null) {
                ...
                getContext().getRequest().getSession().removeAttribute(pageId);
                getContext().getRequest().setAttribute("edit2", Boolean.TRUE);
                getContext().getRequest().setAttribute("edit1", Boolean.TRUE);
                return new JSP("strata/Lanjutan_Tempo_Mohon/rtpsmaklumat_pemohon.jsp").addParameter("tab", "true");
            }
         /* code for IdHakmilik Search -- Start -- */
        }else if(idHakmilikStrata != null){
            System.out.println("-------idHakmilikStrata-------:"+idHakmilikStrata);
                getContext().getRequest().getSession().removeAttribute("error");
...
//           if(permohonan != null){
//               getContext().getRequest().getSession().setAttribute("idHakmilikPermohonan", permohonan.getIdPermohonan());
//               getContext().getRequest().getSession().setAttribute("idHakmilikStrata", idHakmilikStrata);
//
//            FasaPermohonan fp = devService.findFasaPermohonanByIdAliran(permohonan.getIdPermohonan(), stageId);
//            if (fp != null) {
//                addSimpleError("IDPermohonan ini telah diproses");
//                getContext().getRequest().setAttribute("edit2", Boolean.TRUE);
//                getContext().getRequest().setAttribute("edit1", Boolean.TRUE);
...
//            }
//          }else{
//                addSimpleError(" Id Hakmilik ini tidak di dalam pangkalan data");
//                getContext().getRequest().getSession().removeAttribute(pageId);
//                getContext().getRequest().setAttribute("edit2", Boolean.TRUE);
//                getContext().getRequest().setAttribute("edit1", Boolean.TRUE);
//                return new JSP("strata/Lanjutan_Tempo_Mohon/rthsmaklumat_pemohon.jsp").addParameter("tab", "true");
//          }
          /* code for IdHakmilik Search -- End -- */
            ...
            getContext().getRequest().getSession().setAttribute("error", "error");
            addSimpleError(" Sila masukkan ID Permohonan terdahulu.");
...
strService.savePermohonan(permohonanBaru);
