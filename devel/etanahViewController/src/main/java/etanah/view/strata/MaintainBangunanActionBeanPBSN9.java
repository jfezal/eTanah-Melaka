 ...
 * @author Murali
 */
@UrlBinding("/strata/urusanPBS")
public class MaintainBangunanActionBeanPBSN9 extends BasicTabActionBean {
    ...
    private List<BangunanPetak> bptk;
    private BangunanPetak bpetak;
    private String bpnama;
    private BigDecimal bpluas;
    private Integer bpsyer;
    ...
    private static final Logger LOG = Logger.getLogger(MaintainBangunanActionBeanPBSN9.class);
...
//        getContext().getRequest().setAttribute("ekstrak", Boolean.TRUE);
        return new JSP("strata/bngn_sementara/jadual_petak_PBS.jsp").addParameter("tab", "true");
    }
...
//                return new JSP("strata/pecahPetak/tambahBangunan.jsp").addParameter("tab", "true");
    ...
    public Resolution showForm2() {
        ...
        return new JSP("strata/bngn_sementara/tambah_maklumatpetak.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution showForm3() {
        ...
        return new JSP("strata/bngn_sementara/tambah_petakaksesori.jsp").addParameter("popup", "true");
    }
...
//            }
           ...
           if(pBangunanLandParcel.size() != 0){
            BangunanTingkat bt = strService.findByIDBangunan2(pBangunanLandParcel.get(0).getIdBangunan());

            bptk = strService.findByPetakByIDTingkat(bt.getIdTingkat());
            //bptk = new ArrayList<BangunanTingkat>();
          
            for( BangunanPetak bpetak : bptk){
                
                bpnama = bpetak.getNama();
                bpluas = bpetak.getLuas();
                bpsyer = bpetak.getSyer();
            }
            
           
            LOG.info("----Bangun Petak -----" + bptk.get(0).getNama());
           }
...
//        stageId = "g_semakkemasukan";
    ...
    public Resolution tambahBangunan() {
        ...
        return new RedirectResolution(MaintainBangunanActionBeanPBSN9.class, "showForm1").addParameter("error", error).addParameter("message", msg);
    ...
    public Resolution tambahPetak() {
        ...
        return new RedirectResolution(MaintainBangunanActionBeanPBSN9.class, "showForm1");
    }
...
//                        extrakTgkt(n3, n4, bgn.getIdBangunan());
                ...
                if (n8.getLength() > 0) {
                    KodKategoriBangunan kodKatBangunan2 = new KodKategoriBangunan();
                    PermohonanBangunan bgn2 = new PermohonanBangunan();
                    bgn2.setPermohonan(permohonan);
                    bgn2.setInfoAudit(infoAudit);
                    bgn2.setCawangan(peng.getKodCawangan());
                    kodKatBangunan2 = kodKategoriBangunanDAO.findById("L");
                    bgn2.setPermohonanSkim(mohonSkim);
                    bgn2.setNama("L1");
                    bgn2.setBilanganTingkat(1);
                    bgn2.setKodKategoriBangunan(kodKatBangunan2);
                    int bilPetakL = 0;
                    ...
                    for (LP = 0; LP < n8.getLength(); LP++) {
                        bilPetakL = bilPetakL + 1;
                        bd = new BigDecimal(n8.item(LP).getAttributes().getNamedItem("a_area").getNodeValue());
                        totalLuas = totalLuas.add(bd);
                        jumlahSyer = jumlahSyer + Integer.parseInt(n8.item(LP).getAttributes().getNamedItem("unitsyer").getNodeValue());
                    }
                    LOG.info("---bilangan_petak-from schema for landparcel to save--" + bilPetakL);
                    bgn2.setBilanganPetak(bilPetakL);

                    bgn2.setUntukKegunaanLain(totalLuas.toString());
                    bgn2.setSyerBlok(jumlahSyer);
                    strService.simpanBangunan(bgn2);

                    LOG.info("------------------------- END Saving in mohon_bngn --------------------------");
                    /*saving LandParcels in Mohon_BNG */

                    /*saving LandParcels in bngn_tingkat */
                    LOG.info("---saving LandParcels in BNGN_TGKT---");
                    PermohonanBangunan pb5 = strService.findBangunanByKatgBngn(permohonan.getIdPermohonan(), "L");
                    BangunanTingkat bgnTkt = new BangunanTingkat();
                    bgnTkt.setBangunan(pb5);
                    bgnTkt.setBilanganPetak(pb5.getBilanganPetak());
                    bgnTkt.setBilanganPetakAksesori(bilanganPetakAksesori);
                    bgnTkt.setInfoAudit(infoAudit);
                    bgnTkt.setNama("1");
                    bgnTkt.setTingkat(1);
                    // bgnTkt.setSenaraiPetak(senaraiPetak);
                    bgnTkt.setBilanganPetak(pb5.getBilanganPetak());

                    strService.simpanBangunanTingkat(bgnTkt);

                    LOG.info("------------------------- END Saving in bngn_tgkt --------------------------");

                    /*saving LandParcels in bngn_petak */
                    LOG.info("---saving LandParcels in BNGN_PETAK---");

                    BangunanTingkat bngnTgkt = strService.findByIDBangunan2(pb5.getIdBangunan());
                    ...
                    for (LP = 0; LP < n8.getLength(); LP++) {
                        ...
                        BangunanPetak bgnPetak = new BangunanPetak();

                        // String petaktype = n8.item(LP).getAttributes().getNamedItem("petaktype").getNodeValue();
...
//                        if (kodguna1 != null) {
//                            bgnPetak.setKegunaan(kodguna1);
//                        }
//                    }
                        //temporary - untuk landed nk dpt dri mana kod guna petak?
                        // KodKegunaanPetak kodguna1 = kodKegunaanPetakDAO.findById("03");
                        // bgnPetak.setKegunaan(kodguna1);

                        bgnPetak.setInfoAudit(infoAudit);

                        String nama = getIntegerValue(n8.item(LP).getAttributes().getNamedItem("landparcelno").getNodeValue());
                        bgnPetak.setNama(nama);

                        BigDecimal luasPetak = new BigDecimal(n8.item(LP).getAttributes().getNamedItem("a_area").getNodeValue());
                        bgnPetak.setLuas(luasPetak);

                        String unitluas = n8.item(LP).getAttributes().getNamedItem("a_unit").getNodeValue();
                        KodUOM kodUOM = kodUOMDAO.findById(unitluas);
                        if (kodUOM != null) {
                            bgnPetak.setLuasUom(kodUOM);
                        }

                        String noPAB = getIntegerValue(n8.item(LP).getAttributes().getNamedItem("pab").getNodeValue());
                        bgnPetak.setPabPetak(noPAB);

                        //bgnPetak.setSenaraiPetakAksesori(senaraipetakAksesori);
                        int syerPetak = Integer.parseInt(n8.item(LP).getAttributes().getNamedItem("unitsyer").getNodeValue());
                        bgnPetak.setSyer(syerPetak);

                        bgnPetak.setTingkat(bngnTgkt);

                        strService.simpanPetak(bgnPetak);
                    }
                    LOG.info("------------------------- END Saving in bngn_petak --------------------------");
...
//                    bgn.setNama("P" + getIntegerValue(n9.item(PB).getAttributes().getNamedItem("bangunanno").getNodeValue()));
    ...
    public String getIntegerValue(String s) {
        ...
        if (i != -1) {
//            result = s.substring(0, i);
            result = s.substring(remove.length());
            s = result;
        }
        return s;
    }
...
//        BangunanTingkat bgnTkt = new BangunanTingkat();
//        BangunanPetak ptk = new BangunanPetak();
//        bgnTkt = bangunanTingkatDAO.findById(idTingkat);
//        LOG.debug("--bgnTkt--::" + bgnTkt);
//        List<BangunanPetak> ptkList = new ArrayList<BangunanPetak>();
//        ptkList = strService.findPetakByIdTingKat(idTingkat);
//        LOG.debug("--ptk--::" + ptk);
//        if (ptkList != null && !ptkList.isEmpty()) {
//            ptk = ptkList.get(0);
//            LOG.debug("-if-ptk--::" + ptk);
//        } else {
//            LOG.debug("-else-ptk--::" + ptk);
//            ptk = null;
//        }
...
//                String temp = aksnama.substring(1, 2) + aksnama.substring(3, aksnama.length());
//                aksnameList.add(temp);
//            }
//        }
//        Collections.sort(aksnameList);
//        for (String str : aksnameList) {
//            sb.append(str + ",");
//        }
//        String aksname1 = sb.toString();
//        aksname1 = aksname1.substring(0, aksname1.length() - 1);
//        LOG.debug("--aksname1--::" + aksname1);
//        InfoAudit infoAudit1 = new InfoAudit();
//        infoAudit1.setDimasukOleh(peng);
//        infoAudit1.setTarikhMasuk(new java.util.Date());
//        bpk.setNama(aksname1);
//        bpk.setInfoAudit(infoAudit1);
//        bpk.setCawangan(peng.getKodCawangan());
//        bpk.setTingkat(bgnTkt);
//        bpk.setBangunan(bgn);
//        bpk.setPetak(ptk);
//        String aksesorino = n6.item(0).getAttributes().getNamedItem("aksesorino").getNodeValue();
//        LOG.debug("--aksesorino--::" + aksesorino);
//        if (aksname1.charAt(0) == 'A') {
//            bpk.setLokasi("Luar Bangunan");
//        } else {
//            bpk.setLokasi("Dalam Bangunan");
//        }
//        String petaktype = n6.item(0).getAttributes().getNamedItem("petaktype").getNodeValue();
//        LOG.debug("--petak type--::" + petaktype);
...
//                    List<BangunanPetakAksesori> pk1 = p2.getSenaraiPetakAksesori();
        ...
        return new RedirectResolution(MaintainBangunanActionBeanPBSN9.class, "showForm4").addParameter("message", msg);
    }
    ...
    public List<BangunanPetak> getBptk() {
        return bptk;
    }

    public void setBp(List<BangunanPetak> bptk) {
        this.bptk = bptk;
    }

    public BangunanPetak getBpetak() {
        return bpetak;
    }

    public void setBpetak(BangunanPetak bpetak) {
        this.bpetak = bpetak;
    }

    public String getBpnama() {
        return bpnama;
    }

    public void setBpnama(String bpnama) {
        this.bpnama = bpnama;
    }

    public BigDecimal getBpluas() {
        return bpluas;
    }

    public void setBpluas(BigDecimal bpluas) {
        this.bpluas = bpluas;
    }

    public Integer getBpsyer() {
        return bpsyer;
    }

    public void setBpsyer(Integer bpsyer) {
        this.bpsyer = bpsyer;
    }
