 ...
 * @author Murali
 */
@UrlBinding("/strata/SemakKertasSiasatan")
public class SenaraiKertasSiasatanActionBean extends BasicTabActionBean {
    ...
    private static final Logger LOG = Logger.getLogger(SenaraiKertasSiasatanActionBean.class);
    ...
    private KandunganFolder catatan1;
    ...
    public Resolution showForm() {
        ...
        return new JSP("strata/kuatkuasa/senarai_semak_kertas_siasataan.jsp").addParameter("tab", "true");
    }
                ...
                if (folderDokumen != null) {
                    if (folderDokumen.getSenaraiKandungan() != null) {
                        for (KandunganFolder kf : folderDokumen.getSenaraiKandungan()) {
                            ...
                            LOG.info("kf.getDokumen.getIdDokumen.getNama----::" + kf.getDokumen().getKodDokumen().getKod());
                            LOG.info("kf.getDokumen.getIdDokumen----::" + kf.getDokumen().getIdDokumen());
                            Dokumen d = dokumenDAO.findById(kf.getDokumen().getIdDokumen());
                            String[] koddoc = {"KRONO", "MINIT", "KRK", "FAKTA", "KPTDH", "SPOKS", "NOTIS", "PCAM","PCTT"};
                            if (ArrayUtils.contains(koddoc, d.getKodDokumen().getKod())) {
                                LOG.info("kf.getDokumen.getIdDokumen.getNama(yg lepas)----::" + kf.getDokumen().getKodDokumen().getKod());
                                kf.setDokumen(d);
                                LOG.info("kf.getDokumen----::" + kf.getDokumen());
                                senaraiKandungan1.add(kf);
                            } else {
                                LOG.info("dokumen baru----::"+d.getBaru()+"--");
                                LOG.info("dokumen baru2----::"+String.valueOf(d.getBaru())+"--");
                                String dBaru = String.valueOf(d.getBaru());
                                if (!dBaru.equals("T")) {
                                    LOG.info("kf.getDokumen.getIdDokumen.getNama(yg 2nd lepas)----::" + kf.getDokumen().getKodDokumen().getKod());
                                    kf.setDokumen(d);
                                    LOG.info("kf.getDokumen 2nd----::" + kf.getDokumen());
                                    senaraiKandungan1.add(kf);
                                }
                            }
    ...
    public Resolution addDocForm() {
        ...
        return new JSP("strata/kuatkuasa/senarai_semak_kertas_tambahan_doc.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution deleteSelected() {
        ...
        rehydrate();
        return new RedirectResolution(SenaraiKertasSiasatanActionBean.class, "showForm");
    }

    public Resolution reload() {
        return new ForwardResolution("/WEB-INF/jsp/strata/kuatkuasa/senarai_semak_kertas_siasataan.jsp");
    }

    public Resolution muatNaikForm1() {

        return new JSP("strata/kuatkuasa/muat_naik_siasatan.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution processUpload() {
        ...
        if (fileToBeUpload != null) {
            try {
                ...
                Logger.getLogger(SenaraiKertasSiasatanActionBean.class).error(ex);
    ...
    public Resolution showRefresh() {
        LOG.info("------Refreshed----");
        return new JSP("strata/kuatkuasa/senarai_semak_kertas_siasataan.jsp").addParameter("popup", "true");
    }

    public Resolution showCatatanEdit() {
        dokumenID = getContext().getRequest().getParameter("dokumenId");
        folderID = getContext().getRequest().getParameter("folderId");
        catatan1 = dokumenService.findCatatan(Long.valueOf(dokumenID), Long.valueOf(folderID));
        catatan = catatan1.getCatatan();
        return new JSP("strata/kuatkuasa/CatatanEdit.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution upDateCatatan() {
        ...
        addSimpleMessage("Maklumat Catatan Berjaya Disimpan");
        return new RedirectResolution(SenaraiKertasSiasatanActionBean.class, "showForm");
        //   return new JSP("strata/kuatkuasa/CatatanEdit.jsp").addParameter("popup", "true");
    }
     ...
     * @return the catatan1
     */
    public KandunganFolder getCatatan1() {
        return catatan1;
    }

    /**
     * @param catatan1 the catatan1 to set
     */
    public void setCatatan1(KandunganFolder catatan1) {
