 ...
 * @author NageswaraRao
 */
@UrlBinding("/pembangunan/melaka/rinagkasan_Permohonan")
public class RingkasanPermohonanActionBean extends AbleActionBean {

    private static final Logger LOG = Logger.getLogger(RingkasanPermohonanActionBean.class);
    @Inject
    ...
    public Resolution showForm() {
        ...
        if (StringUtils.isNotBlank(stageId)) {
            if (stageId.equalsIgnoreCase("perakuanjkbbptg")) {
                modifyPerakuanPengarah();
            }
        } else {
            stageId = getContext().getRequest().getParameter("stageId");
        }

        return new JSP("pembangunan/melaka/ringkasan_permohonan.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate() {
        ...
        FasaPermohonan mohonFasa = devService.findFasaPermohonanByIdAliranAndTKHHNotNULL(idPermohonan, "cetakrencanajkbb");
        if (mohonFasa != null) {
        ...
        if (!(permohonan.getSenaraiKertas().isEmpty())) {
            ...
            if (senaraiKertas.size() > 0) {
                ...
                if (senaraiKertas1.size() > 0) {
                    ...
                    LOG.info("------------JKBB1--------:" + kertasP.getIdKertas());
                }
                if (kertasP != null) {
                    ...
                    for (int j = 0; j < kertasP.getSenaraiKandungan().size(); j++) {
                        ...
                        if (kertasK.getBil() == 1) {
                            ...
                            if (permohonan.getKodUrusan().getKod().equals("PPCS") || permohonan.getKodUrusan().getKod().equals("PPCB")
                                    || permohonan.getKodUrusan().getKod().equals("PYTN") || permohonan.getKodUrusan().getKod().equals("TSPSS")
                                    || permohonan.getKodUrusan().getKod().equals("SBMS") || permohonan.getKodUrusan().getKod().equals("PSBT")) {
                                tujuan1 = tujuanDefaultData();
                            } else {
                                tujuan1 = kertasK.getKandungan();
                            }
                        } else if (kertasK.getBil() == 3) {
                ...
                keputusan = "Mesyuarat Jawatankuasa Pusat Ekspress dan Kebenaran Merancang Negeri Melaka Bil. ..... yang bersidang pada ..... telah meluluskan permohonan tersebut.";
            }
        }
        senaraiRujukanLuar = addPengarahTanah(senaraiRujukanLuar);
        LOG.info("rehydrate finish");
    }

    public void modifyPerakuanPengarah() {

        List<FasaPermohonan> senaraiFp = devService.findFasaPermohonanByIdAliranList(idPermohonan, "perakuanjkbbptg");
        LOG.info("senaraiFp = " + senaraiFp.size());
        FasaPermohonan fp = senaraiFp.get(0);
//        FasaPermohonan fp = devService.findFasaPermohonanByIdAliran(idPermohonan, "perakuanjkbbptg");
    ...
    public List<PermohonanRujukanLuar> addPengarahTanah(List<PermohonanRujukanLuar> senaraiRujukanLuar) {
        stageId = currentStageId();//(String) getContext().getRequest().getSession().getAttribute("taskId"); yus edit
        List<FasaPermohonan> senaraiFp = devService.findFasaPermohonanByIdAliranList(idPermohonan, stageId);
        //senaraiFp = devService.findFasaPermohonanByIdAliranList(idPermohonan, "sediarencanajkbb");
        LOG.info("senaraiFp = " + senaraiFp.size());
        FasaPermohonan fp = senaraiFp.get(0);
//        LOG.info("fp = " + fp.getDimesejOleh().getIdPengguna());
        if (fp != null && fp.getKeputusan() != null) {
...
//        rehydrate();
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new ForwardResolution("/WEB-INF/jsp/pembangunan/melaka/ringkasan_permohonan.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution deleteSingle() {
        ...
        return new RedirectResolution(RingkasanPermohonanActionBean.class, "locate");
    }
