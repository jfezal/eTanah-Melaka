 ...
 * @edited by Afham
 */
@UrlBinding("/pelupusan/sedia_jabatan")
public class JabatanTeknikalActionBean extends AbleActionBean {
    ...
    public Resolution showFormViewOnly() {
        ...
        return new JSP("pelupusan/common/papar_jabatan_teknikal.jsp").addParameter("tab", "true");
    }
...
//        prs = pelupusanService.findSalinan(String.valueOf(permohonanRujukanLuar.getIdRujukan()));
        ...
        if (pprl != null) {
            InfoAudit info = new InfoAudit();
            info = pprl.getInfoAudit();
            info.setTarikhKemaskini(new java.util.Date());
            pprl.setInfoAudit(info);

            pelupusanService.simpanPermohonanRujLuar(pprl);
        }
        ...
        if (permohonan != null) {
            if (permohonan.getCawangan().getKod().equals("02")) {
                if (permohonan.getKodUrusan().getKod().equals("LMCRG") || permohonan.getKodUrusan().getKod().equals("MLCRG")) {
                    ...
                    penggunaList = getSenaraiPengguna02(permohonan.getCawangan());
                }
            } else {
                if (permohonan.getKodUrusan().getKod().equals("LMCRG") || permohonan.getKodUrusan().getKod().equals("MLCRG")) {
                    penggunaList = getSenaraiPenggunaPTG(pguna.getKodCawangan());
...
//code changed by rohan
        if (kod != null) {
            ...
            listKodAgensiAdun = pelupusanService.searchKodAgensiADNActive("%", kodAgensiNama, conf.getProperty("kodNegeri"));
...
//
    ...
    public List<Pengguna> getSenaraiPengguna02(KodCawangan kod) {
        try {
            String query = "Select distinct u from etanah.model.Pengguna u where u.perananUtama in('PTD','PPTD','71','225','PPTT','75','4') and u.kodCawangan.kod = :kod and u.status.kod = 'A' order by u.nama";
            //String query = "Select u from etanah.model.Pengguna u where u.perananUtama in('PTD','PPTD') and u.kodCawangan.kod = :kod order by u.nama"
    ...
    public List<Pengguna> getSenaraiPengguna(KodCawangan kod) {
        try {
            String query = "Select distinct u from etanah.model.Pengguna u where u.perananUtama in('PTD','PPTD','71','225','PPTT') and u.kodCawangan.kod = :kod and u.status.kod = 'A' order by u.nama";
            //String query = "Select u from etanah.model.Pengguna u where u.perananUtama in('PTD','PPTD') and u.kodCawangan.kod = :kod order by u.nama"          
            Session session = sessionProvider.get();
            Query q = session.createQuery(query);
            q.setString("kod", kod.getKod());
