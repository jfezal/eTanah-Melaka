 ...
 * @author abu.mansur
 */
@UrlBinding("/hasil/kodDatoLembaga")
public class KodDatoLembagaActionBean extends AbleActionBean {
    ...
    public Resolution showForm() {
        return new ForwardResolution("/WEB-INF/jsp/hasil/kod_dato_Lembaga.jsp");
    }

    public Resolution createKod() {
        getContext().getRequest().setAttribute("New", Boolean.TRUE);
        senaraiRuju = new SenaraiRujukan();
        return new ForwardResolution("/WEB-INF/jsp/hasil/kod_dato_Lembaga_popup.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution showEditKod() {
        ...
        return new JSP("hasil/kod_dato_Lembaga_popup.jsp").addParameter("popup", "true");
    }

    public Resolution refreshpage() {
        senaraiRujukan = kodSekolahBantuanService.getKodDatoLembaga(searchKod, searchNama, kodNegeri);
    ...
    public Resolution simpanKod() {
        ...
        if (s == null) {
            ...
            sen.setKod("02"); // kodSenarai 02 = Kod Dato' Lembaga
            s.setSenarai(sen);
        }
    ...
    public Resolution doCheckKod() {
        ...
        String kodDato = getContext().getRequest().getParameter("kod");
        LOG.info("kodDato :" + kodDato);
        SenaraiRujukan SR = new SenaraiRujukan();
        if (StringUtils.isNotBlank(kodDato)) {
            SR = senaraiRujukanDAO.findById(kodDato);
            if (SR != null) {
                results = "Ada";
            }
        }
