 ...
 */
@UrlBinding("/lelong/tukarNama")
public class UtilitiTukarNamaActionBean extends AbleActionBean {
    ...
    public Resolution showForm() {
        ...
        return new ForwardResolution("/WEB-INF/jsp/lelong/utilitiTukarNama.jsp");
    }
...
//       List<FasaPermohonan> fasaPermohonanList = lelongService.findFasaPermohonanBayaran(idPermohonan);
        fasaPermohonan = lelongService.findFasaPermohonanBayaran(idPermohonan, "semakanPermohonan");

        if (pengguna.getKodCawangan().getKod().equals(fasaPermohonan.getCawangan().getKod())) {
            ...
            addSimpleError("Sila Masukan Perserahan Daerah" + fasaPermohonan.getCawangan().getName());
        }
...
//        System.out.print("checkPermohonan");
        ...
        if (idPengguna != null) {
            fasaPermohonan = lelongService.findFasaPermohonanBayaran(idPermohonan, "semakanPermohonan");
            if (fasaPermohonan != null) {
                fasaPermohonan.setKeputusanUntuk(idPengguna);
                fasaPermohonanService.saveOrUpdate(fasaPermohonan);
            }
        }
        addSimpleMessage("Kemaskini Berjaya");

//        return new JSP("/WEB-INF/jsp/lelong/utilitiTukarNama.jsp");
        return showForm();
//        return new ForwardResolution("/WEB-INF/jsp/lelong/utilitiTukarNama.jsp");
    }
    ...
    public Resolution getsPembida() {
        ...
        checkIdMohonSebelum = lelongService.checkIdMohonSebelum(idPermohonan);
        LOG.info("ada ke tak id mohon sebelum " + checkIdMohonSebelum.size());

        //ada id mohon sebelum
        if (!checkIdMohonSebelum.isEmpty()) {
            LOG.info("Ada Id Mohon Sebelum");
            idMohonSebelum = checkIdMohonSebelum.get(0).getPermohonanSebelum().getIdPermohonan();
            LOG.info("id mohon sebelum " + idMohonSebelum);
            showIdMohonSebelum = true;
            LOG.info("showIdMohonSebelum " + showIdMohonSebelum);
            ...
            if ("04".equals(conf.getProperty("kodNegeri"))) {
                ...
                if (checkBayaranPerintah.isEmpty()) {
                    ...
                    return new ForwardResolution("/WEB-INF/jsp/lelong/UtilitiSenaraiPermohonanLelongan.jsp");
                }
            }

            List<Task> l;
            try {
                l = WorkFlowService.queryTasksByAdmin(idMohonSebelum);
            ...
            listLel1 = lelongService.getLelonganForFindPembida(idMohonSebelum);//id lelong yg aktif sahaja
            LOG.info("idLelong " + listLel1.get(0).getIdLelong());
...
//            }
            ...
            listLel1 = lelongService.getLelonganForFindPembida(idPermohonan);//id lelong yg aktif sahaja
            LOG.info("size listLel1: " + listLel1.size());
    ...
    public Resolution popupUpload() {
        ...
        caraLelong = getContext().getRequest().getParameter("caraLelong");
        LOG.info(caraLelong);
        return new JSP("lelong/upload_dok_pembida.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution popupScan() {
        ...
        return new JSP("lelong/scan_dok_pembida.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution hapusPembida() {
        String results = "0";

        String idPembida = getContext().getRequest().getParameter("idPembida");

        if (idPembida != null) {
            Pembida hapusPembida = pembidaDAO.findById(Long.parseLong(idPembida));
            Pihak hapusPihak = hapusPembida.getPihak();
            pembidaDAO.delete(hapusPembida);
            pihakDAO.delete(hapusPihak);
        }
    ...
    public Resolution tambahWakil() {
        ...
        String wakilPembidaString = getContext().getRequest().getParameter("wakilPembida");
        getContext().getRequest().getParameter(idPermohonan);
...
//        }
        ...
        view = true;
        return new ForwardResolution("/WEB-INF/jsp/lelong/tambahWakilPembida.jsp").addParameter("popup", "true");
    }
...
//        String idPermohonan = getContext().getRequest().getParameter("idPermohonan");
        ...
        return new StreamingResolution("text/plain", results);
//        return new ForwardResolution("/WEB-INF/jsp/lelong/tambahPembidaBaru.jsp").addParameter("popup", "true");
    }
...
//        String results = "0";
        ...
        view = true;
//        return new StreamingResolution("text/plain", results);
        return new ForwardResolution("/WEB-INF/jsp/lelong/kemaskini_pembida.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution refreshUpload() {

        LOG.info("--------ni untuk REFRESHHHH UPLOAD FILE-------");
    ...
    public Resolution tambahTarikhSenaraiHitam() {
        ...
        return new JSP("lelong/tambahTarikhSenaraiHitamPembida.jsp").addParameter("popup", "true");
    }
