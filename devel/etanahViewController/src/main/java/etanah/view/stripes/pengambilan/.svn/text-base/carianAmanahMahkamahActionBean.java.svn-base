 ...
 * @author nordiyana
 */
@UrlBinding("/pengambilan/carianAmanah")
public class carianAmanahMahkamahActionBean extends AbleActionBean {
private static Logger logger = Logger.getLogger(carianAmanahMahkamahActionBean.class);
    @Inject
...
//    private List<PermohonanPihak>
    ...
    public Resolution showForm() {
        return new JSP("pengambilan/carianAmanahMahkamah.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate() {
        ...
        if (idPermohonan != null) {
            ...
            if (!idHakmilik.isEmpty()) {
                senaraiHakmilikPermohonan = aduanService.getSeneraiPermohonanByidHakmilik(idHakmilik, kod);
...
//        searchHakmilik();
        if(selectedIdHM !=null){
            ...
            if(hp!=null){
                ...
                if(permohonanAduan != null)
                    perihal = permohonanAduan.getPerihal();
//                pSblm=permohonanDAO.findById(permohonan.getIdPermohonan());
//                if(pSblm.getPermohonanSebelum()==null)
//                {
//                addSimpleError("Sila Pilih Permohonan Terdahulu");
...
//                }else{
                    showHP="true";
                    senaraiPihak=aduanService.findAmanahlist(idPermohonan);
...
//        searchHakmilik();
        ...
        if (ids==null ) {
            addSimpleError("Sila Pilih Tuan Tanah yang terlibat");

            }else if(ids != null && ids.length > 0 && getContext().getRequest().getParameter("hakmilikPermohonan.id") != null){
//            hakmilikPermohonan = hakmilikPermohonanDAO.findById(Long.valueOf(selectedIdHM));
            ...
            if(hakmilikPermohonan != null){
                ...
                senaraiPP=aduanService.findPihakByIdMohonList(permohonan.getIdPermohonan());
                System.out.println("list permohonan pihak" +senaraiPP.get(0).getPihak().getNama());
                   if(senaraiPP.size()>0)
                   {
