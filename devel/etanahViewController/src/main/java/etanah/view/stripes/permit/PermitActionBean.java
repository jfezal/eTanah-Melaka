...
@UrlBinding("/permitEntry")
public class PermitActionBean extends AbleActionBean {

    private static final Logger LOG = Logger.getLogger(PermitActionBean.class);
    @Inject
    ...
    public Resolution showForm() {
        return new ForwardResolution("/WEB-INF/jsp/permit/kemasukanPermit.jsp");
    }

    public Resolution semakan() {
        LOG.info("semakan");
        return new ForwardResolution("/WEB-INF/jsp/permit/semakanPermit.jsp");
    }

    public Resolution digiSignForm() {
        LOG.info("digi sign form");
        getContext().getRequest().setAttribute("forDigiSign", Boolean.TRUE);
        return new ForwardResolution("/WEB-INF/jsp/permit/digiSignPermit.jsp");
    }
    ...
    public Resolution simpanKemasukan() throws Exception {
        ...
        if (jenisBorang != null) {
            if (jenisBorang.equalsIgnoreCase("A")) {
                link = "/WEB-INF/jsp/permit/borang4ae.jsp";
            } else if (jenisBorang.equalsIgnoreCase("B")) {
                link = "/WEB-INF/jsp/permit/borang4be.jsp";
            } else if (jenisBorang.equalsIgnoreCase("C")) {
                link = "/WEB-INF/jsp/permit/borang4ce.jsp";
            } else {
                link = "/WEB-INF/jsp/permit/borang4de.jsp";
            }
            addSimpleMessage("Maklumat Permohonan " + noFail + " Telah Berjaya Disimpan");
        } else {
            link = "/WEB-INF/jsp/permit/kemasukanPermit.jsp";
    ...
    public Resolution cariKemasukan() throws Exception {
        pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        noFail = getContext().getRequest().getParameter("noFailRuj");
        noPermit = getContext().getRequest().getParameter("noPermit");
        LOG.info("###### id Pengguna : " + pengguna + " IdPermohonan = " + noFail);
        ...
        Permohonan pExist = pembangunanServ.findPermohonanByIdPermohonan(noFail);
        if (pExist != null) {
            System.out.println("--NO FAIL EXIST--");
            Permit pmt = pembangunanServ.findPerrmitByIdMohonAndNoPermit(noFail, noPermit);
            if (pmt != null) {
                System.out.println("--NO PERMIT EXIST--");

                if (pExist.getCatatan().equalsIgnoreCase("A")) {
                    link = "/WEB-INF/jsp/permit/borang4aeView.jsp";
                } else if (pExist.getCatatan().equalsIgnoreCase("B")) {
                    link = "/WEB-INF/jsp/permit/borang4beView.jsp";
                } else if (pExist.getCatatan().equalsIgnoreCase("C")) {
                    link = "/WEB-INF/jsp/permit/borang4ceView.jsp";
                } else {
                    link = "/WEB-INF/jsp/permit/borang4deView.jsp";
                }

            } else {
                addSimpleMessage("No Permit Tidak Dijumpai !!!");
                link = "/WEB-INF/jsp/permit/digiSignPermit.jsp";
            }
        } else {
            addSimpleMessage("No Fail Dan No Permit Tidak Dijumpai !!!");
            link = "/WEB-INF/jsp/permit/digiSignPermit.jsp";
        }

        addSimpleMessage("Carian No Fail Dan No Permit Dijumpai !!!");
        check();
        return new ForwardResolution(link);
    }
    ...
    public Resolution carianBayaranPopup() {
        ...
        noPermit = getContext().getRequest().getParameter("noPermit");
        return new JSP("permit/carianBayaran.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution carianPihakPopup() {
        ...
        jenisBorang = getContext().getRequest().getParameter("jenisBorang");
        noPermit = getContext().getRequest().getParameter("noPermit");
        return new JSP("permit/carianPihak.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution uploadDoc() {
        LOG.info("###### uploadDoc");
        idPermohonan = getContext().getRequest().getParameter("noFail");
        jenisBorang = getContext().getRequest().getParameter("noFail");
        return new JSP("permit/uploadPelan.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution simpanRekod() throws Exception {
        ...
        check();
        return new ForwardResolution("/WEB-INF/jsp/permit/borang4ae.jsp");
    }
    // END RECORD LPS
