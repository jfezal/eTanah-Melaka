 ...
 * @author asri
 */
@UrlBinding("/pengambilan/noPT_noPU")
public class NoPUNoPTActionBean extends AbleActionBean{
    ...
    public Resolution showForm(){
        System.out.println("MAsuk pupt");
        return new JSP("pengambilan/dev_sedia_PT_PU.jsp").addParameter("tab", "true");
    }
      ...
      if(!hakmilikPermohonanList.isEmpty()){
          for(int i=0;i<hakmilikPermohonanList.size();i++){
            ...
            if(!senaraiKodBpm.contains(kodBPM)){
                ...
                senaraiNoPT[k] = pembangunanService.getNoPTByIdPermohonan(idPermohonan,kodBPM);
                    k++;
         ...
         if(noPTList.isEmpty()){
            ...
            for (int j = 0; j < senaraiPermohonanPlotPelan.size(); j++) {
                ...
                for (int k = 0; k < plotPelan.getBilanganPlot(); k++) {
                       noPT = pembangunanService.findNoPTByIdPlotPelan(plotPelan.getIdPlot());
                    if(noPT!=null){
