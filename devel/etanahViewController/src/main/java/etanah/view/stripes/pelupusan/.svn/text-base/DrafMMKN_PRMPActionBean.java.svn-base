 ...
 * @author Admin
 */
@UrlBinding("/pelupusan/draf_mmkn_prmp")
public class DrafMMKN_PRMPActionBean extends AbleActionBean{
    
     Logger logger = Logger.getLogger(DrafMMKN_PRMPActionBean.class);
    @Inject
...
//        edit = Boolean.FALSE;
        ...
        return new JSP("pelupusan/pertanian/draf_mmkn_mlk.jsp").addParameter("tab", "true");
    }
...
//        ptg = Boolean.TRUE;
    ...
    public void rehydrate() {
        ...
        senaraiHakmilikPihak = pelupusanService.getHakmilikPihakBerkepentinganByStringIdHakmilik(hakmilikPermohonan.getHakmilik().getIdHakmilik());
        if(listprl.size()>0){
...
//        }
//REMOVE
//        stageId = "sedia_draf_rencana_mmkn";
...
//        }
        ...
        BigDecimal luasDipohon = BigDecimal.ZERO;
        String ursan = "";
        String ktanah = "";
        String sbb = "";
//        kULuas = new KodUOM();
...
//            }
 ...
 // rohan added
        ...
        if(hakmilikPermohonan.getHakmilik() != null){
            ...
            luasDipohon = hakmilikPermohonan.getLuasDiluluskan();
            }
            if (hm.getBandarPekanMukim() != null) {
                bpm = hm.getBandarPekanMukim().getNama() ;
            }
...
//                + bpm + ", Daerah " + daerah + " untuk tujuan " + sbb + " .";
        ...
        if (permohonan.getKodUrusan().getKod().equals("PRMP")) {
            tujuanUrusanPlps = "Permit Pertanian";
        ...
        String tujuanPTLot = !noLot.isEmpty()?" ke atas " + lotNama + " " + noLot: " ";
...
//        perihalpermohonan = " Pentadbir Tanah " + permohonan.getCawangan().getDaerah().getNama() + " telah menerima permohonan ini pada " + mtarikmasuk + ".";
        ...
        if((kerja != null) && (pemohon.getPendapatan() != null)){
            logger.info("Kerja :" + kerja);
            if(kerja.equalsIgnoreCase("Tidak Bekerja")){
        ...
        perihaltanah = "Tanah ini ialah tanah " + kodmilik + " yang terletak berhampiran "
...
//        if(stageId.equals("sedia_draf_mmkn")){
            if(permohonan.getKodUrusan().getKod().equals("PRMP")){
                 fasaPermohonan = pelupusanService.findMohonFasaByIdMohonIdPengguna(idPermohonan, "sedia_draf_rencana_mmkn");
            }else
...
//                + bpm + ", DAERAH " + daerah + " UNTUK TUJUAN " + tajukKategoriTanah + ".";
        ...
        String urusanPlps = " ";
        if (permohonan.getKodUrusan().getKod().equals("PRMP")) {
            urusanPlps = "PERMIT TANAH PERTANIAN" ;
        ...
        String stringPermitItem = new String();
         BigDecimal kadarPermitItem = BigDecimal.ZERO;
         if (permohonan.getKodUrusan().getKod().equals("PLPS")||permohonan.getKodUrusan().getKod().equals("PRMP")) {
            permitItem = pelupusanService.findByIdMohonPermitItem(idPermohonan) ;
            tujuanTajukPlps = permitItem.getKodItemPermit().getNama() ;
            kadarPermitItem = !StringUtils.isBlank(permitItem.getKodItemPermit().getRoyaltiTanahKerajaan().toString())?permitItem.getKodItemPermit().getRoyaltiTanahKerajaan():BigDecimal.ZERO;
            stringPermitItem = !StringUtils.isBlank(permitItem.getKodItemPermit().getRoyaltiTanahKerajaan().toString())?permitItem.getKodItemPermit().getRoyaltiTanahKerajaan().toString():" ";
            String uomPermitItem = !StringUtils.isBlank(permitItem.getKodItemPermit().getRoyaltiTanahKerajaanUom().getNama())?permitItem.getKodItemPermit().getRoyaltiTanahKerajaanUom().getNama():" ";
            stringPermitItem = stringPermitItem +" "+ uomPermitItem; 
         }
        ...
        tajukPlps =" PERMOHONAN DARIPADA  " + pihakName + ", NO. K/P : " + noPengenalan + " UNTUK MEMILIKI " + urusanPlps + " DI ATAS " + tajukLot + " " + noLot + ", SELUAS " + luas + " " + koduom1 + " " + bpm + ", DAERAH " + daerah + " UNTUK TUJUAN " + tujuanTajukPlps + ".";
...
//        }
        ...
        String koduom2 = " ";
        if ((hakmilikPermohonan != null) && (hakmilikPermohonan.getKodUnitLuas() != null)) {
            koduom2 = hakmilikPermohonan.getLuasLulusUom().getNama();
        }
        cawangan = pelUtiliti.convertStringtoInitCap(cawangan);
        pihakName = pelUtiliti.convertStringtoInitCap(pihakName);
         BigDecimal kadarBayaran = BigDecimal.ZERO;         
         NumberFormat formatter = NumberFormat.getCurrencyInstance();
         if(luasDipohon!=null){
             kadarBayaran = luasDipohon.multiply(kadarPermitItem);
         }
         String kadarBayaranString = new String();
         if(kadarBayaran!=null){
            kadarBayaranString = formatter.format(kadarBayaran);
            kadarBayaranString = kadarBayaranString.substring(1, kadarBayaranString.length());
         }
        if (permohonan.getKodUrusan().getKod().equals("PLPS")||permohonan.getKodUrusan().getKod().equals("PRMP")) {
            pentabirtanah2 = " Pentadbir Tanah Daerah " + cawangan + " dengan ini memperakukan supaya permohonan daripada " + pihakName
                + " No. KP: " + noPengenalan + " untuk " + tujuanUrusanPlps + " ke atas PT/LOT " + noLot + ", seluas " + luasDipohon + " " + koduom2 + ", Mukim/Kawasan "
                + bpm + ", Daerah " + cawangan + " untuk tujuan " + sbb + " dengan kadar bayaran RM "+ kadarBayaranString +" setahun diluluskan seperti pelan yang dilampirkan dengan dikenakan syarat-syarat seperti berikut:- ";

        }else{
            pentabirtanah2 = " Pentadbir Tanah Daerah " + cawangan + " dengan ini memperakukan supaya permohonan daripada " + pihakName
                + " No. KP: " + noPengenalan + " untuk " + tujuanUrusanPlps + " ke atas " + tajukLot + " " + noLot + ", seluas " + luas + " " + koduom2 + ", Mukim/Kawasan "
...
//                break;
    ...
    public void updateKandungan(int i, String kand) {
        ...
        if (permohonan.getKodUrusan().getKod().equals("PLPS")) {
            ...
            KodDokumen kod = kodDokumenDAO.findById("RMN");
            permohonankertas.setKodDokumen(kod);
            permohonankertas.setNoLaporan(no_laporan);
        } 
...
//                pelupusanService.simpanPermohonanKertasKandungan(permohonanKertasKandungan1);
        ...
        // END : LPS
        return new ForwardResolution("/WEB-INF/jsp/pelupusan/pertanian/draf_mmkn_mlk.jsp").addParameter("tab", "true");
    }
...
//    }
    public Resolution showsyortolaklulus(){
        ...
        if(permohonan.getKodUrusan().getKod().equals("PRMP")){
            idAliran = "sedia_draf_rencana_mmkn";
        }else{
