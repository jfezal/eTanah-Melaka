...
public class PuActionBean extends AbleActionBean {
    ...
    private List<PermohonanPlotPelan> senaraiMPPTerpilih;
    private List<PermohonanPlotPelan> listMPP;
...
//    }
    ...
    public void rehydrate() {
            ...
            if (permohonan.getKodUrusan().getKod().equals("PPCB")) {
                if ("05".equals(conf.getProperty("kodNegeri"))) {
                    ...
                    if (stageId.equals("cetaknotispemilikbersama")) {
                        penggunaList = getSenaraiPenggunaNotis(permohonan.getCawangan());
                    } else {
                        penggunaList = getSenaraiPengguna(permohonan.getCawangan());
                    }
                } else {
                    //penggunaList = getSenaraiPengguna(permohonan.getCawangan()); //29/07/2016 - IWE
                    penggunaList = pembangunanService.findPenggunaForOnlyOneKumpBpel("ptd", permohonan.getCawangan().getKod());
                }
            } else {
                //penggunaList = getSenaraiPengguna(permohonan.getCawangan()); //29/07/2016 - IWE
                penggunaList = pembangunanService.findPenggunaForOnlyOneKumpBpel("ptd", permohonan.getCawangan().getKod());
        ...
        tandatanganDokumen1 = pembangunanService.findPermohonanDokTTByIdPermohonan(idPermohonan, "SN2A");
        ...
        if ("04".equals(conf.getProperty("kodNegeri"))) {
            ...
            hakmilikMukim = pembangunanService.findByIdPermohonan(idPermohonan);
            mppMelaka = pembangunanService.findPermohonanPlotPelanPemilikanByIdPermohonan(idPermohonan);
            System.out.println("mppMelaka : " + mppMelaka.size());

            for (int a = 0; a < mppMelaka.size(); a++) {
                PermohonanPlotPelan pp = (PermohonanPlotPelan) mppMelaka.get(a);
                System.out.println("Jenis-Jenis Kegunaan Tanah : " + pp.getKategoriTanah().getNama());
            }

            noPtList2 = pembangunanService.senaraiNoPTByIdPermohonan(idPermohonan);
            System.out.println("noPtList2 : " + noPtList2.size());

            for (int a = 0; a < noPtList2.size(); a++) {
                NoPt nopt = (NoPt) noPtList2.get(a);
                System.out.println("Luas Di lulus : " + nopt.getLuasDilulus());
            }
        }

        if ("05".equals(conf.getProperty("kodNegeri"))) {
//            if (!permohonan.getKodUrusan().getKod().equals("PPCB")) {
            alamat = pembangunanService.findAlamat(permohonan.getCawangan().getKod());
            listAlamat = pembangunanService.findUniquePemohonByIdPermohonan(idPermohonan);
            if (!listAlamat.isEmpty()) {
                pihak = (Pihak) listAlamat.get(0);
            } else {
            }

            hakmilikMukim = pembangunanService.findByIdPermohonan(idPermohonan);
            if ("05".equals(conf.getProperty("kodNegeri"))) {
                List mppList = new ArrayList();
                mppList = pembangunanService.findluasPlotPelanByIdMohon2(idPermohonan);

                if (!mppList.isEmpty()) {
                    mpp = (PermohonanPlotPelan) mppList.get(0);
                }
            } else {
                mppMelaka = pembangunanService.findPermohonanPlotPelanPemilikanByIdPermohonan(idPermohonan);
                System.out.println("mppMelaka : " + mppMelaka.size());
            ...
            List listPPP = pembangunanService.findPermohonanPlotPelanPemilikanByIdPermohonan(idPermohonan);

            luasKeseluruhan = new BigDecimal(0);
            for (int a = 0; a < listPPP.size(); a++) {
                PermohonanPlotPelan ppp = (PermohonanPlotPelan) listPPP.get(a);
                System.out.println("Luas : " + ppp.getLuas());
                luasKeseluruhan = luasKeseluruhan.add(ppp.getLuas());
                kodKeseluruhan = ppp.getKodUnitLuas().getNama();
            }
        }

        listMPP = devService.findPermohonanPlotPelanGroupByIdPermohonan(idPermohonan);
        senaraiMPPTerpilih = new ArrayList<PermohonanPlotPelan>();

        List<KodKegunaanTanah> listguna = new ArrayList<KodKegunaanTanah>();
        for (PermohonanPlotPelan mpp : listMPP) {
            if (listguna.size() > 0) {
                Boolean found = false;
        ...
        listMPP.clear();
        listMPP.addAll(senaraiMPPTerpilih);
    }
...
//    }
    ...
    public Resolution transferFile() {
        //table integrasi, integrasi dokumen
...
//        return new ForwardResolution("/WEB-INF/jsp/pengambilan/maklumat_hakmilik_pengambilan.jsp").addParameter("tab", "true");
        return new JSP("pembangunan/melaka/pu.jsp").addParameter("tab", "true").addParameter("error", error).addParameter("message", msg);
    }
        ...
        if (permohonan.getKodUrusan().getKod().equals("PPCB")) {
            if ("05".equals(conf.getProperty("kodNegeri"))) {
                if (stageId.equals("cetaknotispemilikbersama")) {
                    ...
                    tandatanganDokumen1 = pembangunanService.findPermohonanDokTTByIdPermohonan(idPermohonan, "SIPU");
                }
            } else {
                tandatanganDokumen1 = pembangunanService.findPermohonanDokTTByIdPermohonan(idPermohonan, "SIPU");
            }
    ...
    public List<PermohonanPlotPelan> getSenaraiMPPTerpilih() {
        return senaraiMPPTerpilih;
    }

    public void setSenaraiMPPTerpilih(List<PermohonanPlotPelan> senaraiMPPTerpilih) {
        this.senaraiMPPTerpilih = senaraiMPPTerpilih;
    }

    public List<PermohonanPlotPelan> getListMPP() {
        return listMPP;
    }

    public void setListMPP(List<PermohonanPlotPelan> listMPP) {
        this.listMPP = listMPP;
    }
