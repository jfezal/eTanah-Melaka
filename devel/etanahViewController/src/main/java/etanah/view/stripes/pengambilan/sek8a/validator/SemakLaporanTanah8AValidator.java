 ...
 * @author mohd.faidzal
 */
public class SemakLaporanTanah8AValidator implements StageListener {

    private static final Logger LOG = Logger.getLogger(JanaBorangCDLaporanTanahValidator.class);
    @Inject
    ...
    public String beforeComplete(StageContext context, String proposedOutcome) {
        ...
        sek8aIntegrationFlowService.completeTask(ref.ABC_STAGE, context.getPermohonan(), context.getPengguna());
    ...
    private void saveBorangPerMohon(Dokumen d, Pengguna pengguna, Permohonan permohonan, KodDokumen kd) {
        ...
        if (kd.getKod().equals("C")) {
            kn = kodNotisDAO.findById("C");
            bp = borangACQService.findBorangA(permohonan.getIdPermohonan(), "C");
        } else {
            kn = kodNotisDAO.findById("D");
            bp = borangACQService.findBorangA(permohonan.getIdPermohonan(), "D");
        }
        if (bp != null) {
        } else {
