 ...
 */
@UrlBinding("/lelong/keputusan_bidaanJLB")
public class KeputusanBidaanJLBActionBean extends AbleActionBean {

    private static final Logger LOG = Logger.getLogger(KeputusanBidaanJLBActionBean.class);
    @Inject
...
//                }
                ...
                if (status == null) {
                    ...
                    if ("04".equals(conf.getProperty("kodNegeri"))) {
                        ...
                        return new JSP("lelong/keputusan_lelonganJLB.jsp").addParameter("tab", "true");
                    } else {
...
//                }
                if (status.equals("AP")) {
                    ...
                    return new JSP("lelong/rekod_bidaanJLB.jsp").addParameter("tab", "true");
                }
                if (status.equals("TB")) {
                    owh = false;
                    error = true;
...
//                                    }
        ...
        if (permohonan.getSenaraiHakmilik().size() == 1 || enkuiri.getCaraLelong().equals("B")) {
            ...
            if (fasa == null) {
                ...
                if (ff.getKeputusan().getKod().equals("AP")) {
                    show = true;
                }
                if (ff.getKeputusan().getKod().equals("TB")) {
                    show = false;
                }
            }
        }
        ...
        if ("05".equals(conf.getProperty("kodNegeri"))) {
            ...
            if ("04".equals(conf.getProperty("kodNegeri"))) {
                ...
                if (idLelong == null || idLelong.isEmpty()) {
                    buttonSimpan = false;
                    return new JSP("lelong/pembida_popupJLB.jsp").addParameter("popup", "true");
                } else {
...
//                    List<AkuanTerimaDeposit> listATD2 = lelongService.findATD(idPermohonan, String.valueOf(lelong.getIdLelong()));
                    ...
                    BigDecimal percentage2 = new BigDecimal("10");
                    hargaBidaan = enkuiri.getHargaBida();
                    BigDecimal hargaRizab = enkuiri.getHargaRizab();
                    depo = hargaRizab.divideToIntegralValue(percentage);
                    depo = depo.multiply(percentage2);
                    if (atd != null) {
                        deposit = depo;
                        baki = enkuiri.getBaki();
//                        deposit = atd.getAmaun();
...
//                }
    ...
    public Resolution simpanKpsn() {
        ...
        senaraifasamohon = lelongService.getPermonanFasaRekodBidaanJLB(idPermohonan);
...
//        p.setJenisPengenalan(jenis);
    ...
    public Resolution saveMLK() {
        ...
        BigDecimal percentage = new BigDecimal("100");
        BigDecimal percentage2 = new BigDecimal("10");
        depo = hargaRizab.divide(percentage);
        depo = depo.multiply(percentage2);
...
//         List<Pihak> senaraiPihak = null ;
        ...
        senaraiLelongan2 = lelongService.listLelonganA(idPermohonan);
        for (Lelongan lelongan : senaraiLelongan2) {
            Pembida pembida1 = enService.getListPembidaByBerjaya(lelongan.getIdLelong(), "BJ");
            if (pembida1 != null) {
                pihak2 = pembida1.getPihak();
            }
        }
...
//            }
        ...
        LOG.info("deposit : " + depo);
        LOG.info("deposit2 : " + deposit2);
        LOG.info("baki : " + baki);
        BigDecimal b = BigDecimal.ZERO;
        if (deposit2 != null) {
            b = depo;
            baki = baki.subtract(depo);
            LOG.info("baki 1: " + baki);
        } else {
            b = deposit;
        }
        LOG.info("b : " + b);
...
//        KodJenisPengenalan jenis = kodJenisPengenalanDAO.findById(kodJenis);
//        p.setJenisPengenalan(jenis);
        pihak2.setNama(getContext().getRequest().getParameter("pembida.pihak.nama"));
        pihak2.setNoPengenalan(getContext().getRequest().getParameter("pembida.pihak.noPengenalan"));
        pihak2.setAlamat1(getContext().getRequest().getParameter("pembida.pihak.alamat1"));
        pihak2.setAlamat2(getContext().getRequest().getParameter("pembida.pihak.alamat2"));
        pihak2.setAlamat3(getContext().getRequest().getParameter("pembida.pihak.alamat3"));
        pihak2.setAlamat4(getContext().getRequest().getParameter("pembida.pihak.alamat4"));
        pihak2.setPoskod(getContext().getRequest().getParameter("pembida.pihak.poskod"));
        pihak2.setNoTelefon1(getContext().getRequest().getParameter("pembida.pihak.noTelefon1"));
        pihak2.setNoTelefon2(getContext().getRequest().getParameter("pembida.pihak.noTelefon2"));
        pihak2.setNoTelefonBimbit(getContext().getRequest().getParameter("pembida.pihak.noTelefonBimbit"));
        KodNegeri kod2 = kodNegeriDAO.findById(kodNegeri);
        pihak2.setNegeri(kod2);
//        LOG.info("p.getNegeri().getNama() = " + p.getNegeri().getNama());
        pihak2.setInfoAudit(info);
        lelongService.saveOrUpdate(pihak2);
        if (!pihak2.getNegeri().getKod().equals(kod2.getKod())) {
            lelongService.saveOrUpdate(pihak2);
        }
        for (Lelongan oo : senaraiLelongan2) {
//            oo.setPembida(p);
...
//            }
    ...
    public Pihak getPihak2() {
        return pihak2;
    }

    public void setPihak2(Pihak pihak2) {
        this.pihak2 = pihak2;
    }
}
