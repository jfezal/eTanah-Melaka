...
@UrlBinding("/pelupusan/surat_kelulusan_pjbtr")
public class KeputusanPermohonanPJBTRActionBean extends AbleActionBean {
...
//    private LupusPermit lupusPermit;
    ...
    private String kodU;
    private static final Logger log = Logger.getLogger(KeputusanPermohonanPJBTRActionBean.class);

    @DefaultHandler
    public Resolution showForm() {
        return new JSP("pelupusan/pjbtr/surat_keputusan_PJBTR.jsp").addParameter("tab", "true");
    }
                ...
                for (int i = 0; i < listtuntutankos.size(); i++) {
                    ...
                    if (permohonantuntutkos.getKodTuntut() != null) {
                        ...
                        if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
                            if (permohonantuntutkos.getKodTuntut().getKod().equals("DISSTR")) {
...
//             permohonanTuntutan = pelupusanService.findPermohonanTuntutanByKodTransName(idPermohonan, "Bayaran Borang 4A (LPS)");
        if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
            permohonanTuntutan = pelupusanService.findPermohonanTuntutanByKodTransKod(idPermohonan, "DISSTR");
        }else {
...
//            permohonanTuntutan.setCawangan(test);
            ...
            if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
                kodTransTuntut = kodTransaksiTuntutDAO.findById("DISSTR");
            }
...
//        kt.setKod("12104");
         ...
         kt = kodTuntutDAO.findById("DISSTR");
        if (!(listtuntutankos.isEmpty())) {
            ...
            log.info("-------permohonantuntutkos-----getKodTuntut() Not Null-------------" + permohonantuntutkos);
            if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
                permohonanTuntutan = pelupusanService.findPermohonanTuntutanByIdMohonAndKodTransTuntut(idPermohonan, "DISSTR");
                if (permohonanTuntutan.getKodTransaksiTuntut().getKod().equals("DISSTR")) {
                    permohonantuntutkos = new PermohonanTuntutanKos();
                    permohonantuntutkos.setAmaunTuntutan(bayaran);
                    permohonantuntutkos.setKodTransaksi(kt.getKodKewTrans());
            ...
            permohonantuntutkos.setCawangan(peng.getKodCawangan());
            if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
                permohonantuntutkos.setItem("Bayaran Pajakan Stratum Tanah Bawah Tanah");
            }           
            pelupusanService.simpanPermohonanTuntutanKos1(permohonantuntutkos);
        }

        if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
            permohonantuntutkos = pelupusanService.findMohonTuntutKosByIdPermohonanAndIdTuntut(idPermohonan, "DISSTR");
        }
        ...
        if(tarikhMulaLesen!=null){
            ...
            if(permit!=null){                
                ...
                if(permohonan.getKodUrusan().getKod().equals("PJBTR")){
                     permit.setKodJenisPermit(kodJenisPermitDAO.findById("I"));
                }
