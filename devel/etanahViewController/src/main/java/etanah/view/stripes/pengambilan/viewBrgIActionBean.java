 ...
 * @author nordiyana
 */
@UrlBinding("/pengambilan/view_BrgI")
public class viewBrgIActionBean extends AbleActionBean{
 @Inject
...
//        getContext().getRequest().setAttribute("semak16H", Boolean.TRUE);
        return new JSP("pengambilan/view_brgI.jsp").addParameter("tab", "true");
    }
...
//        getContext().getRequest().setAttribute("semak16H", Boolean.TRUE);
    ...
    public void rehydrate() {
        ...
        LOGGER.info("----Borang I----");
        if ("04".equals(conf.getProperty("kodNegeri"))) {
            //melaka
            reportName = "ACQDocI_MLK.rdf";
        }
        if ("05".equals(conf.getProperty("kodNegeri"))) {
            //negeri9
            reportName = "ACQDocI_NS.rdf";
        }
        LOGGER.info("reportName : " + reportName);
        List<KandunganFolder> listKandunganFolder = lelongService.getListDokumenI(permohonan.getFolderDokumen().getFolderId());
        if (!listKandunganFolder.isEmpty()) {
            for (KandunganFolder ff : listKandunganFolder) {
                if (ff.getDokumen().getKodDokumen().getKod().equals("I")) {
