 ...
 * @author mohd.faidzal
 */
public class SediaBorangABValidator implements StageListener {

    private static final Logger LOG = Logger.getLogger(SediaBorangABValidator.class);
    @Inject
    ...
    public void onGenerateReports(StageContext context) {
        ...
        namaReport[0] = "ACQDocA_MLK.rdf";
        namaReport[1] = "ACQDocB_MLK.rdf";
        kodReport[0] = "A";
        kodReport[1] = "B";

        if (context.getStageName().equals("semak_borangAB_PTG")) {
            namaReport = new String[2];
            kodReport = new String[2];
            namaReport[0] = "ACQDocA_MLK.rdf";
            namaReport[1] = "ACQKenyataanAmNegeri_MLK.rdf";
            kodReport[0] = "A";
            kodReport[1] = "SKAN";
        }
        if (context.getStageName().equals("cetak_borangB")) {
            namaReport = new String[2];
            kodReport = new String[2];
            namaReport[0] = "ACQDocB_MLK.rdf";
            kodReport[0] = "B";
        }
    ...
    private void saveBorangPerMohon(Dokumen d, Pengguna pengguna, Permohonan permohonan, KodDokumen kd) {
        ...
        if (kd.getKod().equals("A")) {
            kn = kodNotisDAO.findById("NAA");
            bp = borangACQService.findBorangA(permohonan.getIdPermohonan(), "NAA");
        } else {
            kn = kodNotisDAO.findById("NBB");
            bp = borangACQService.findBorangA(permohonan.getIdPermohonan(), "NBB");
        }
        if (bp != null) {
        } else {
