...
@UrlBinding("/pendaftaran/KemasukanSubMC")
public class KemasukanSubMC extends AbleActionBean {
...
//    private List<Pihak> listTuanTanah;
    ...
    private BadanPengurusan bdnUrusMain;
    private WakilPihak bdnUrusSUB;
    private List<WakilPihak> senaraiBdnUrusSUB;
    private String nama;
...
//    private BadanPengurusan bdnUrusSUB;
    @Inject
    StrataPtService strataPtService;
    @Inject
    BadanPengurusanDAO badanPengurusanDAO;
    @Inject
    KodJenisPengenalanDAO jenisPengenalannDAO;
    @Inject
    ...
    public Resolution showForm() {
        ...
        if (idPermohonan != null) {
            ...
            hakmilik = hakmilikPermohonanList.get(0).getHakmilik();
        }
        if (bayaran != null) {
            SimpanBayaran();
            addSimpleMessage("Kemasukan Berjaya Disimpan");
            return new JSP("daftar/kemasukan_bayaran.jsp").addParameter("tab", "true");
        }

        return new JSP("daftar/kemasukan_badan_main.jsp").addParameter("tab", "true");
    }

    public Resolution showFormBayaran() {
        ...
        List<PermohonanTuntutanKos> senaraiMohonTuntutKos = lelongService.listPermohonanTuntutanSTR(idPermohonan);
        for (PermohonanTuntutanKos ptk : senaraiMohonTuntutKos) {
            if (ptk.getKodTuntut().getKod().equals("STR08")) {
                bayaran = String.valueOf(ptk.getAmaunTuntutan());
            }
        }
    ...
    public Resolution SimpanBayaran() {
        ...
        logger.info("bayaran = " + bayaran);

        bayaran = (String) getContext().getRequest().getParameter("bayar");
        List<PermohonanTuntutanKos> senaraiMohonTuntutKos = lelongService.listPermohonanTuntutanSTR(idPermohonan);
        InfoAudit infoAudit = new InfoAudit();
        if (senaraiMohonTuntutKos.size() <= 0) {
            PermohonanTuntutanKos permohonanTuntutanKos = new PermohonanTuntutanKos();

            infoAudit.setDimasukOleh(peng);
            infoAudit.setTarikhMasuk(new java.util.Date());
            KodTuntut kodTuntut = new KodTuntut();
            kodTuntut.setKod("STR08");
            permohonanTuntutanKos.setCawangan(p.getCawangan());
            permohonanTuntutanKos.setPermohonan(p);
            permohonanTuntutanKos.setAmaunTuntutan(new BigDecimal(bayaran));
            permohonanTuntutanKos.setKodTuntut(kodTuntut);
            permohonanTuntutanKos.setItem("Bayaran Pelan Khas");
            permohonanTuntutanKos.setKodTransaksi(p.getKodUrusan().getKodTransaksi());
            permohonanTuntutanKos.setInfoAudit(infoAudit);
            devServ.simpanPermohonanTuntutanKos(permohonanTuntutanKos);
//            addSimpleMessage("Kemasukan Berjaya Disimpan");
        } else {
            for (PermohonanTuntutanKos ptk : senaraiMohonTuntutKos) {
                infoAudit.setDimasukOleh(peng);
                infoAudit.setTarikhMasuk(new java.util.Date());
                ptk.setAmaunTuntutan(new BigDecimal(bayaran));
...
//                addSimpleMessage("Kemasukan Berjaya Disimpan");
        ...
        addSimpleMessage("Kemasukan Berjaya Disimpan");
//        WakilPihak wp = wakilpihakDAO.findById(idUrusan);
//        pengambilanService.hapus(wp);
//        wakilpihakDAO.delete(wp);
        return new RedirectResolution(KemasukanSubMC.class
        ).addParameter("showFormBayaran");
    ...
    public void searchMain(Hakmilik hakmilik, Permohonan permohonan) {

        if (hakmilik != null) {
            List<Hakmilik> senaraiHakmilikSTR = strataPtService.getIdBdnUrus(hakmilik.getIdHakmilik());
            bdnUrusMain = strataPtService.getidPihakByIdBdn(senaraiHakmilikSTR.get(0).getBadanPengurusan().getIdBadan());

//            Long idUrus = senaraiHakmilikSTR.get(0).getBadanPengurusan().getIdBadan();
//            bdnUrusMain = strataPtService.getidPihakByIdBdn(idUrus);
//            strataPtService.geti
        }
    }

    public void searchSub(Hakmilik hakmilik, Permohonan permohonan) {

        if (hakmilik != null) {
            List<Hakmilik> senaraiHakmilikSTR = strataPtService.getIdBdnUrus(hakmilik.getIdHakmilik());
            bdnUrusMain = strataPtService.getidPihakByIdBdn(senaraiHakmilikSTR.get(0).getBadanPengurusan().getIdBadan());
//            String idPihak = String.valueOf(bdnUrusMain.getIdBadan());
            if (bdnUrusMain != null) {
                senaraiBdnUrusSUB = regService.CariSubBadanUrus(permohonan.getIdPermohonan(), String.valueOf(bdnUrusMain.getPihak().getIdPihak()), String.valueOf(senaraiHakmilikSTR.get(0).getBadanPengurusan().getIdBadan()));
            }
        }
    }

    public Resolution simpanSUB() {
        String result = "1";
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        Pengguna peng = (Pengguna) context.getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        ...
        if (idPermohonan != null) {
            p = permohonanDAO.findById(idPermohonan);
            WakilPihak wakilPihak = new WakilPihak();

            int senaraiWP = pengambilanService.findMaxIdWakil();
            senaraiWP = senaraiWP + 1;
//            String id = senaraiWP.get(0).getIdWakil();
            String id = String.valueOf(senaraiWP);

            if (id != null) {
                wakilPihak.setIdWakil(id);
            }
            if (nama != null) {
                wakilPihak.setNama(nama);
            }
            if (suratAlamat1 != null) {
                wakilPihak.setAlamat1(suratAlamat1);
            }
            if (suratAlamat2 != null) {
                wakilPihak.setAlamat2(suratAlamat2);
            }
            if (suratAlamat3 != null) {
                wakilPihak.setAlamat3(suratAlamat3);
            }
            if (suratAlamat4 != null) {
                wakilPihak.setAlamat4(suratAlamat4);
            }
            if (poskod != null) {
                wakilPihak.setPoskod(poskod);
            }
            if (negeri != null) {
                KodNegeri kn = kodNegeriDAO.findById(negeri);
                wakilPihak.setNegeri(kn);
            }
            if (noPengenalan != null) {
                wakilPihak.setNoPengenalan(noPengenalan);
            }
            if (jenispengenalan != null) {
                KodJenisPengenalan kjp = jenisPengenalannDAO.findById(jenispengenalan);
                wakilPihak.setJenisPengenalan(kjp);

            }
            if (bdnUrusMain != null) {
                wakilPihak.setNoRujukan(String.valueOf(bdnUrusMain.getIdBadan()));
            }
            if (p != null) {
                wakilPihak.setPermohonan(p);
            }
            if (bdnUrusMain != null) {
                wakilPihak.setPihak(bdnUrusMain.getPihak());
            }
            ...
            wakilPihak.setInfoAudit(ia);
//            wakilpihakDAO.saveOrUpdate(wakilPihak);
//            wakilpihakDAO.save(wakilPihak);
//            pendaftaranSuratKuasaService.save(wakilPihak);
//            pengambilanService.kemaskiniwakilPihak(wakilPihak);
            pengambilanService.simpanwakilPihak(wakilPihak);
//            wakilpihakDAO.saveOrUpdate(wakilP);
//            wakilpihakDAO.save(wakilP);
//            wakilpihakDAO.update(wakilPihak);

            addSimpleMessage("berjaya");
        }
        rehydrate();
        return new StreamingResolution("text/plain", result);
//        return new RedirectResolution(KemasukanSubMC.class
//        ).addParameter("showForm", "").addParameter("tab", "true").addParameter("idHakmilik", idHakmilik);
    }

    public Resolution kemasukanSubMCBaru() {
...
//        cariFlag = true;
        ...
        return new JSP("daftar/kemasukan_Sub.jsp").addParameter("popup", "true").addParameter("idHakmilik", idHakmilik);
    }
...
//        }
        searchMain(hakmilik, p);
        searchSub(hakmilik, p);
...
//        jenisPelan = "";
//        if(p != null){
//            logger.debug("p.getKodUrusan().getKod():"+p.getKodUrusan().getKod());
//            logger.debug("p.getKodUrusan().getKod():"+p.getKodUrusan().getKod().startsWith("HK"));
//           if(p.getKodUrusan().getKod().startsWith("HK")){
//               jenisPelan = "B1";
//               kodPelan = "1";
//           }else{
//               jenisPelan = "B2";
...
//            kodBPM = kbpm.getbandarPekanMukim();
            ...
            if (StringUtils.isNotBlank(noLot)) {
                //PelanGIS pgis = pelanGISDAO.findById(noLot);
//                PelanGIS pgis = regService.cariPelan(kodNegeri, kodDaerah, kodBPM, noLot, jenisPelan);
...
//                ppService.saveOrUpdate(lpp);
        ...
        return new RedirectResolution(KemasukanPerincianHakmilikActionBean.class
        ).addParameter("tab", "true").addParameter("idHakmilik", idHakmilik);
        ...
        for (String string : id) {
            ...
            if (idMh != null) {
                ...
                if (hl != null) {
                    ...
                    hUService.deleteHakmilikUrusanTrans(hl);
...
//===========================================================================****=======================================================
    ...
    public BadanPengurusan getBdnUrusMain() {
        return bdnUrusMain;
    }

    public void setBdnUrusMain(BadanPengurusan bdnUrusMain) {
        this.bdnUrusMain = bdnUrusMain;
    }

    public WakilPihak getBdnUrusSUB() {
        return bdnUrusSUB;
    }

    public void setBdnUrusSUB(WakilPihak bdnUrusSUB) {
        this.bdnUrusSUB = bdnUrusSUB;
    }

    public List<WakilPihak> getSenaraiBdnUrusSUB() {
        return senaraiBdnUrusSUB;
    }

    public void setSenaraiBdnUrusSUB(List<WakilPihak> senaraiBdnUrusSUB) {
        this.senaraiBdnUrusSUB = senaraiBdnUrusSUB;
    }
    ...
    public BadanPengurusanDAO getBadanPengurusanDAO() {
        return badanPengurusanDAO;
    }

    public void setBadanPengurusanDAO(BadanPengurusanDAO badanPengurusanDAO) {
        this.badanPengurusanDAO = badanPengurusanDAO;
    }

    public KodJenisPengenalanDAO getJenisPengenalannDAO() {
        return jenisPengenalannDAO;
    }

    public void setJenisPengenalannDAO(KodJenisPengenalanDAO jenisPengenalannDAO) {
        this.jenisPengenalannDAO = jenisPengenalannDAO;
    }
