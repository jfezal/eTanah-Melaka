 ...
 * @author w.fairul
 */
@UrlBinding("/pelupusan/jabatan_teknikal12")
public class JabatanTeknikal2ActionBean extends AbleActionBean {
    ...
    private FasaPermohonan kpsnSoKanan;
    ...
    public Resolution showEditJabatanTeknikal() {
        ...
        if (idRujukan != null) {
            ...
            senaraiUlasanJabatanTeknikalKod = pelupusanService.findPermohonanRujLuarByIdPermohonanKod(idPermohonan, kod, kodA);
        }
        return new JSP("pelupusan/surat_ulasan_teknikal.jsp").addParameter("popup", "true");
    }

    public Resolution tutup() throws ParseException {
        System.out.println("------------ tutup ------------");
        rehydrate();
        return new JSP("pelupusan/terima_ulasan_jabatan_teknikal.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution tambahRowjtek() {
        ...
        senaraiUlasanJabatanTeknikalKod = pelupusanService.findPermohonanRujLuarByIdPermohonanKod(idPermohonan, kod, agensi);
        if (!senaraiUlasanJabatanTeknikalKod.isEmpty()) {
            ...
            pl.setKodRujukan(kr);
            pelupusanService.simpanPermohonanRujukanLuar(pl);

            mohonRujLuar = new PermohonanRujukanLuar();
            if (idRujukan != null) {
                mohonRujLuar = pelupusanService.findPermohonanRujLuarByIdPermohonanIdRujKodAgensi(idPermohonan, agensi);
                senaraiUlasanJabatanTeknikalKod = pelupusanService.findPermohonanRujLuarByIdPermohonanKod(idPermohonan, kod, agensi);
            }
        }
        return new JSP("pelupusan/surat_ulasan_teknikal.jsp").addParameter("popup", "true");
    ...
    public Resolution deleteRowjtek() throws ParseException {
        ...
        mohonRujLuar = new PermohonanRujukanLuar();
        if (idRujukan != null) {
            mohonRujLuar = pelupusanService.findPermohonanRujLuarByIdPermohonanIdRujKodAgensi(idPermohonan, kodA);
            senaraiUlasanJabatanTeknikalKod = pelupusanService.findPermohonanRujLuarByIdPermohonanKod(idPermohonan, kod, kodA);
        }
    ...
    public Resolution simpanjtek() {
        ...
        String agensi = getContext().getRequest().getParameter("kod");

        PermohonanRujukanLuar pl = pelupusanService.findPermohonanRujLuarByIdPermohonanIdRuj(idPermohonan, agensi, Long.parseLong(idRujukan), kod);
        InfoAudit info = new InfoAudit();
        ...
        if (pl != null) {
            ...
            mohonRujLuar = new PermohonanRujukanLuar();
            if (idRujukan != null) {
                mohonRujLuar = pelupusanService.findPermohonanRujLuarByIdPermohonanIdRujKodAgensi(idPermohonan, kod);
                senaraiUlasanJabatanTeknikalKod = pelupusanService.findPermohonanRujLuarByIdPermohonanKod(idPermohonan, agensi, kod);
            }
        }

        addSimpleMessage("Maklumat Ulasan Telah Berjaya Di Simpan");
        return new JSP("pelupusan/surat_ulasan_teknikal.jsp").addParameter("popup", "true");
    }

    public Resolution simpanAll() throws ParseException {

        Pengguna pguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        permohonan = permohonanDAO.findById(idPermohonan);
        String kand = getContext().getRequest().getParameter("kandungan5");
        LOG.info("kand - " + kand);
        String id = getContext().getRequest().getParameter("id");
        String agensi = getContext().getRequest().getParameter("agensi");
        String kod = getContext().getRequest().getParameter("kod");

        List<PermohonanRujukanLuar> listMohonRujLuar = pelupusanService.senaraiPermohonanRujLuarByIdPermohonan(idPermohonan);
        LOG.info("listMohonRujLuar - " + listMohonRujLuar.size());
        PermohonanRujukanLuar mrl = pelupusanService.findByIdRujukan(Long.parseLong(id));
        ...
        if (mrl != null) {
            ...
            mrl.setUlasan(kand);
            pelupusanService.simpanPermohonanRujukanLuar(mrl);
        }
        PermohonanRujukanLuar mohonRujLuarTemp = new PermohonanRujukanLuar();

        String trhkkuasa = getContext().getRequest().getParameter("tarikhKuatkuasa");
        String trhterima = getContext().getRequest().getParameter("tarikhTerima");
        String namaPenyedia = getContext().getRequest().getParameter("namaPenyedia");
        String noRujukan = getContext().getRequest().getParameter("noRujukan");

        String d = getContext().getRequest().getParameter("syorJabatan1");
        char disokong = d.charAt(0);

        LOG.info("trhkkuasa -- " + trhkkuasa);
        LOG.info("trhterima -- " + trhterima);
        LOG.info("namaPenyedia -- " + namaPenyedia);
        LOG.info("noRujukan -- " + noRujukan);
        LOG.info("disokong -- " + disokong);
        LOG.info("agensi -- " + agensi);

        mohonRujLuar = pelupusanService.findPermohonanRujLuarByIdPermohonanIdRujKodAgensi(idPermohonan, agensi);

        if (mohonRujLuar != null) {
            LOG.info("-----------simpanJabatanTeknikal----if--------");
            mohonRujLuarTemp = pelupusanService.findPermohonanRujLuarByIdPermohonanIdRujKodAgensi(idPermohonan, agensi);
            LOG.info("-----------simpanJabatanTeknikal----mohonRujLuarTemp--------" + mohonRujLuarTemp);
            mohonRujLuarTemp.setNamaPenyedia(namaPenyedia);
            mohonRujLuarTemp.setNoRujukan(noRujukan);
            mohonRujLuarTemp.setDiSokong(disokong);
//            mohonRujLuarTemp.setUlasan(mohonRujLuar.getUlasan());
            if (!trhterima.isEmpty()) {
//                if (trhterima != "")
                mohonRujLuarTemp.setTarikhTerima(sdf.parse(trhterima));
            }
            if (!trhkkuasa.isEmpty()) {
//                if (trhkkuasa != "")
                mohonRujLuarTemp.setTarikhKuatkuasa(sdf.parse(trhkkuasa));
            }
...
//                mohonRujLuarTemp.setTarikhTerima(curDate);
    ...
    public void simpanJabatanTeknikal() throws ParseException {
        PermohonanRujukanLuar mohonRujLuarTemp = new PermohonanRujukanLuar();

        String trhkkuasa = getContext().getRequest().getParameter("mohonRujLuar.tarikhKuatkuasa");
        String trhterima = getContext().getRequest().getParameter("mohonRujLuar.tarikhTerima");
        String namaPenyedia = getContext().getRequest().getParameter("mohonRujLuar.namaPenyedia");
        String noRujukan = getContext().getRequest().getParameter("mohonRujLuar.noRujukan");

        String d = getContext().getRequest().getParameter("mohonRujLuar.diSokong");
        char disokong = d.charAt(0);
        ...
        mohonRujLuar = permohonanRujukanLuarDAO.findById(idRujukan);
...
//        stageId = "terima_ulasan_teknikal";
        ...
        if (!permohonan.getKodUrusan().getKod().equals("PTGSA")) {
            if (permohonan.getKodUrusan().getKod().equals("PHLP")) {
                permohonanLaporPelan = pelupusanService.findByIdPermohonanPelanNIdHakmilik(idPermohonan, idHakmilik);
            } else if (permohonan.getKodUrusan().getKod().equals("PLPS")) {
                hpList = pelupusanService.findHakmilikPermohonanList(idPermohonan);
                for (HakmilikPermohonan h : hpList) {

                    permohonanLaporPelan = pelupusanService.findByIdPermohonanPelanNIdMH(idPermohonan, h.getId());
                }
            } else {
                permohonanLaporPelan = pelupusanService.findByIdPermohonanPelan(idPermohonan);
            }
        ...
        if (permohonanLaporPelan != null) {
            fasaPermohonan = pelupusanService.findMohonFasaByIdMohonIdPengguna(idPermohonan, "semak_charting");
...
//             }
        ...
        if (kNegeri.equals("04")) {
            kpsnSoKanan = pelupusanService.findMohonFasaByIdMohonIdPengguna(idPermohonan, "semak_charting");
            if (kpsnSoKanan != null) {
                if (kpsnSoKanan.getKeputusan() != null) {
                    keputusan = kpsnSoKanan.getKeputusan().getNama();
                }
            }
        }
    }
    ...
    public Resolution deleteRujukan() {
        ...
        rehydrate();
        return new ForwardResolution("/WEB-INF/jsp/pelupusan/pilih_jabatan_teknikal_2.jsp").addParameter("tab", "true");
    }
...
//                ia.setTarikhKemaskini(new java.util.Date());
    ...
    public FasaPermohonan getKpsnSoKanan() {
        return kpsnSoKanan;
    }

    public void setKpsnSoKanan(FasaPermohonan kpsnSoKanan) {
        this.kpsnSoKanan = kpsnSoKanan;
    }
