...
@UrlBinding("/pengambilan/aduan_kerosakan/semak/pemohon_aduan")
public class pemohonAduanActionBean extends AbleActionBean {

    private static final Logger LOG = Logger.getLogger(pemohonAduanActionBean.class);
    ...
    @Inject
    HakmilikPihakKepentinganService hakmilikPihakServ;
    @Inject
    ...
    private List<Pemohon> pemohonListPemaju;
...
//    private boolean isLayak = true;
    ...
    public Resolution showForm() {
        ...
        return new JSP("pengambilan/aduan_kerosakan/semak/maklumat_pemohon.jsp").addParameter("tab", "true");
        //return new ForwardResolution("/WEB-INF/jsp/pengambilan/aduan_kerosakan/semak/maklumat_pemohon.jsp").addParameter("tab", "true");
    ...
    public Resolution showFormPemaju() {
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        senaraiHakmilikPermohonan = hakmilikPermohonanService.findByIdPermohonanOnly(idmhnLama);
        System.out.println("Start showForm");
        return new JSP("pengambilan/aduan_kerosakan/semak/maklumat_pemaju.jsp").addParameter("tab", "true");
    }

    public Resolution cariPemaju() {
        if (idmhnLama != null) {
            permohonanLama = permohonanService.findById(idmhnLama);
            senaraiHakmilikPermohonan = hakmilikPermohonanService.findByIdPermohonanOnly(idmhnLama);
            senaraiInfoWarta = infoWartaService.findByIdPermohonan(idmhnLama);
            if (senaraiInfoWarta.size() > 0) {
                for (InfoWarta info : senaraiInfoWarta) {
                    if (info.getNotis().getKod().equals("A")) {
                        infoWarta = info;
                        LOG.info("infoWarta" + infoWarta.getNoWarta());
                    }
                }
            }
            if (infoWarta == null) {
                addSimpleError("Carian Tidak Berjaya.Pastikan Id permohonon adalah betul");
            }
            pemohonListPemaju = pemohonServ.findPemohonByIdPermohonanPemaju(idPermohonan);

        }
        return new JSP("pengambilan/aduan_kerosakan/semak/maklumat_pemaju.jsp").addParameter("tab", "true");
    }

    public Resolution searchPemohonPemaju() {
        idmhnLama = getContext().getRequest().getParameter("idmhnLama");
...
//            pemohon = pemohonServ.findPemohonByidmohonAndIdHakmilik(hakmilikPermohonan.getPermohonan(), hakmilikPermohonan.getHakmilik());

            List<Pemohon> listPemohonLama = pemohonServ.findPemohonByIdPermohonan(idmhnLama);
            if (listPemohonLama.size() > 0) {
                pemohon = listPemohonLama.get(0);
...
//        infoAudit = pihakTemp.getInfoAudit();
                ...
                Permohonan pemohonanLama = permohonanService.findById(idmhnLama);

                Pemohon pemohonNew = new Pemohon();
                List<Pemohon> listPemohonNewLama = pemohonServ.findPemohonByIdPermohonanPemaju(idPermohonan);

                if (listPemohonNewLama.size() <= 0) {
                    Alamat alamatNew = new Alamat();
                    alamatNew.setAlamat1(pemohon.getPihak().getAlamat1());
                    alamatNew.setAlamat2(pemohon.getPihak().getAlamat2());
                    alamatNew.setAlamat3(pemohon.getPihak().getAlamat3());
                    alamatNew.setAlamat4(pemohon.getPihak().getAlamat4());
                    alamatNew.setNegeri(pemohon.getPihak().getNegeri());
                    alamatNew.setPoskod(pemohon.getPihak().getPoskod());
                    LOG.info(pemohon.getPihak().getNama());
                    String nama = pemohon.getPihak().getNama();
                    if (nama != null) {
                        pemohonNew.setNama(nama);
                    }

                    pemohonNew.setPihak(pemohon.getPihak());
                    pemohonNew.setAlamat(alamatNew);
                    pemohonNew.setHakmilik(hakmilikPermohonan.getHakmilik());
                    pemohonNew.setCawangan(pemohon.getCawangan());
                    pemohonNew.setPermohonan(permohonan);
                    pemohonNew.setJenisPengenalan(pemohon.getJenisPengenalan());
                    pemohonNew.setPemohonJenis("Pemaju");
                    pemohonNew.setInfoAudit(infoAudit);
                    pemohonServ.saveOrUpdate(pemohonNew);
                }
            }

        }
        return simpanPemajuReload(idmhnLama);
    }

    public Resolution simpanPemajuReload(String idmhnLamaDel) {
        if (idmhnLamaDel != null) {

            permohonanLama = permohonanService.findById(idmhnLamaDel);
            senaraiInfoWarta = infoWartaService.findByIdPermohonan(idmhnLamaDel);
            if (senaraiInfoWarta.size() > 0) {
                for (InfoWarta info : senaraiInfoWarta) {
                    if (info.getNotis().equals("A")) {
                        infoWarta = info;
                    }
                }
            }
...
//                hakmilikPermohonan = hakmilikPermohonanService.findHakmilikPermohonanbyidMH(Long.parseLong(idMohonHakmilik));
                senaraiHakmilikPermohonan = hakmilikPermohonanService.findByIdPermohonanOnly(idmhnLamaDel);
                pemohonListPemaju = pemohonServ.findPemohonByIdPermohonanPemaju(idPermohonan);
    ...
    public Resolution simpanPemaju() {
        if (idPermohonan != null) {
            senaraiHakmilikPermohonan = permohonan.getSenaraiHakmilik();
            senaraiInfoWarta = infoWartaService.findByIdPermohonan(idPermohonan);
            if (senaraiInfoWarta.size() > 0) {
                for (InfoWarta info : senaraiInfoWarta) {
                    if (info.getNotis().equals("A")) {
                        infoWarta = info;
    ...
    public Resolution search() {
        if (idMohonHakmilik != null) {
            hakmilikPermohonan = hakmilikPermohonanService.findHakmilikPermohonanbyidMH(Long.parseLong(idMohonHakmilik));
            pemohonList = pemohonServ.findPemohonByIdPermohonanAndHakmilikBaru(idPermohonan, hakmilikPermohonan.getHakmilik().getIdHakmilik());
...
//        rehydrate();
    ...
    public Resolution simpanTuanTanah() {

        if (idHakmilikPihak == null) {
            idHakmilikPihak = getContext().getRequest().getParameter("idHakmilikPihak");
        }
...
//        infoAudit = pihakTemp.getInfoAudit();
        ...
        HakmilikPihakBerkepentingan hp = hakmilikPihakKepentinganService.findById(idHakmilikPihak);
        Alamat alamatNew = new Alamat();
        alamatNew.setAlamat1(hp.getAlamat1());
        alamatNew.setAlamat2(hp.getAlamat2());
        alamatNew.setAlamat3(hp.getAlamat3());
        alamatNew.setAlamat4(hp.getAlamat4());
        alamatNew.setNegeri(hp.getNegeri());
        alamatNew.setPoskod(hp.getPoskod());

        LOG.info(alamatNew.getAlamat1() + alamatNew.getAlamat2() + alamatNew.getAlamat3() + alamatNew.getAlamat4());
        if (hp != null) {
            LOG.info(hp.getPihak().getIdPihak() + "idPihak");
            pemohonList = pemohonService.senaraiPemohonByIdPihakIdHakmilikIdMohon(hp.getPihak().getIdPihak(), hp.getHakmilik().getIdHakmilik(), idPermohonan);
            if (pemohonList.size() <= 0) {
                Pemohon pemohonNew = new Pemohon();
                pemohonNew.setNama(hp.getNama());
                pemohonNew.setPihak(hp.getPihak());
                pemohonNew.setAlamat(alamatNew);
                pemohonNew.setHakmilik(hp.getHakmilik());
                pemohonNew.setCawangan(hp.getCawangan());
                pemohonNew.setPermohonan(permohonan);
                pemohonNew.setJenisPengenalan(hp.getJenisPengenalan());
                pemohonNew.setPemohonJenis("Pemilik");
                pemohonNew.setInfoAudit(infoAudit);
                pemohonServ.saveOrUpdate(pemohonNew);
            }
        }
...
//        hakmilikPermohonan = hakmilikPermohonanService.findHakmilikPermohonanbyidMH(Long.parseLong(idMohonHakmilik));
        return showForm();
//        return reloadTuanTanah();
    }

    public Resolution deletePengadu() {
//        
        idPemohon = getContext().getRequest().getParameter("idPemohon");
        pemohon = pemohonServ.findById(idPemohon);
        pemohonServ.delete(pemohon);
        if (permohonanLama != null) {
            idmhnLama = permohonanLama.getIdPermohonan();
        }
        addSimpleMessage("Pengadu Telah Berjaya Dihapuskan");
        return new RedirectResolution(pemohonAduanActionBean.class, "cariPemaju");
    }

    public Resolution simpanPengadu() {
        idMohonHakmilik = getContext().getRequest().getParameter("idMohonHakmilik");
        ...
        Alamat alamatNew = new Alamat();
        alamatNew.setAlamat1(penyerahAlamat1);
        alamatNew.setAlamat2(penyerahAlamat2);
        alamatNew.setAlamat3(penyerahAlamat3);
        alamatNew.setAlamat4(penyerahAlamat4);

        Pihak pihak = new Pihak();
        pihak.setPoskod(penyerahPoskod);
        pihak.setNama(penyerahNama);
        pihak.setAlamat1(penyerahAlamat1);
        pihak.setAlamat2(penyerahAlamat2);
        pihak.setAlamat3(penyerahAlamat3);
        pihak.setAlamat4(penyerahAlamat4);
        pihak.setNoPengenalan(pNoPengenalan);
//        pihak.setBangsa(null);
        if (penyerahNegeri != null) {
            pihak.setNegeri(kodNegeriDAO.findById(penyerahNegeri));
        }
        pihak.setPoskod(penyerahPoskod);

//        pihak.setPermohonan(permohonan);
        if (kodJenisPengenalan != null) {
            pihak.setJenisPengenalan(kodJenisPengenalanDAO.findById(kodJenisPengenalan));
        }
        pihak.setInfoAudit(infoAudit);
        pihakService.saveOrUpdatePihak(pihak);

        if (penyerahNegeri != null) {
            alamatNew.setNegeri(kodNegeriDAO.findById(penyerahNegeri));
        }

        alamatNew.setPoskod(penyerahPoskod);
        Pemohon pemohonNew = new Pemohon();
        pemohonNew.setNama(penyerahNama);
        pemohonNew.setPihak(pihak);
        pemohonNew.setAlamat(alamatNew);
        pemohonNew.setHakmilik(hakmilikPermohonan.getHakmilik());
        pemohonNew.setCawangan(hakmilikPermohonan.getPermohonan().getCawangan());
        pemohonNew.setPermohonan(permohonan);
        pemohonNew.setNoPengenalan(pNoPengenalan);
        if (kodJenisPengenalan != null) {
            pemohonNew.setJenisPengenalan(kodJenisPengenalanDAO.findById(kodJenisPengenalan));
        }
        pemohonNew.setPemohonJenis(penyerahEmail);
        pemohonNew.setInfoAudit(infoAudit);
        pemohonServ.saveOrUpdate(pemohonNew);
//            }
...
//        hakmilikPermohonan = hakmilikPermohonanService.findHakmilikPermohonanbyidMH(Long.parseLong(idMohonHakmilik));
//        return showForm();
        return new RedirectResolution(pemohonAduanActionBean.class, "showForm");
//        return reloadTuanTanah();
    }

    public Resolution reloadTuanTanah() {
        if (idMohonHakmilik != null) {
            hakmilikPermohonan = hakmilikPermohonanService.findHakmilikPermohonanbyidMH(Long.parseLong(idMohonHakmilik));
            senaraiHakmilikPihak = hakmilikPihakServ.findHakmilikAllPihakActiveByHakmilik(hakmilikPermohonan.getHakmilik());

        } else {
            idHakmilikPihak = getContext().getRequest().getParameter("idMohonHakmilik");
            hakmilikPermohonan = hakmilikPermohonanService.findHakmilikPermohonanbyidMH(Long.parseLong(idMohonHakmilik));
            senaraiHakmilikPihak = hakmilikPihakServ.findHakmilikAllPihakActiveByHakmilik(hakmilikPermohonan.getHakmilik());
        }
...
//        getContext().getRequest().setAttribute("!edit", Boolean.TRUE);
    ...
    public void rehydrate() throws NamingException {
        ...
        //temporary sett oleh suhaimi..
        //getContext().getRequest().getSession().setAttribute("idPermohonan","");
        // xxxxxxxxxxxxxxxxxxx
...
//            if (hakmilikPermohonanList.size() > 0 && hakmilikPermohonanList.size() <= 1) {
    ...
    public Resolution deleteSelectedPemohon() {
        ...
        if (StringUtils.isNotBlank(idHakmilik)) {
            return new RedirectResolution(pemohonAduanActionBean.class, "doGetPartialPage").addParameter("idHakmilik", idHakmilik);
        } else {
            return new RedirectResolution(pemohonAduanActionBean.class, "getSenaraiHakmilikKepentingan");
        }
    }
...
//        pihakTemp.setNoPengenalan(penyerahNoPengenalan);
    ...
    public Resolution refreshpage() throws NamingException {
        rehydrate();
        return new RedirectResolution(pemohonAduanActionBean.class, "locate");
    }
...
//        getContext().getRequest().setAttribute("pemohon", Boolean.TRUE);
    ...
    public static void setIsDebug(boolean isDebug) {
        pemohonAduanActionBean.isDebug = isDebug;
    }

    public HakmilikService getHakmilikManager() {
        return hakmilikManager;
    }

    public void setHakmilikManager(HakmilikService hakmilikManager) {
        this.hakmilikManager = hakmilikManager;
    }
    ...
    public PermohonanPihakKemaskiniService getMohonPihakKemaskiniService() {
        return mohonPihakKemaskiniService;
    }

    public void setMohonPihakKemaskiniService(PermohonanPihakKemaskiniService mohonPihakKemaskiniService) {
        this.mohonPihakKemaskiniService = mohonPihakKemaskiniService;
    }

    public PermohonanPihakKemaskiniDAO getMohonPihakKemaskiniDAO() {
        return mohonPihakKemaskiniDAO;
    }

    public void setMohonPihakKemaskiniDAO(PermohonanPihakKemaskiniDAO mohonPihakKemaskiniDAO) {
        this.mohonPihakKemaskiniDAO = mohonPihakKemaskiniDAO;
    }

    public PihakCawanganDAO getPihakCawanganDAO() {
        return pihakCawanganDAO;
    }

    public void setPihakCawanganDAO(PihakCawanganDAO pihakCawanganDAO) {
        this.pihakCawanganDAO = pihakCawanganDAO;
    }
    ...
    public PermohonanAtasPihakBerkepentinganService getMapService() {
        return mapService;
    }

    public void setMapService(PermohonanAtasPihakBerkepentinganService mapService) {
        this.mapService = mapService;
    }

    public CawanganService getCawanganService() {
        return cawanganService;
    }

    public void setCawanganService(CawanganService cawanganService) {
        this.cawanganService = cawanganService;
    }
    ...
    public HakmilikPihakKepentinganService getHakmilikPihakServ() {
        return hakmilikPihakServ;
    }

    public void setHakmilikPihakServ(HakmilikPihakKepentinganService hakmilikPihakServ) {
        this.hakmilikPihakServ = hakmilikPihakServ;
    }
    ...
    public PermohonanAtasPerserahanService getMohonAtasSerahService() {
        return mohonAtasSerahService;
    }

    public void setMohonAtasSerahService(PermohonanAtasPerserahanService mohonAtasSerahService) {
        this.mohonAtasSerahService = mohonAtasSerahService;
    }
    ...
    public PemohonService getPemohonServ() {
        return pemohonServ;
    }

    public void setPemohonServ(PemohonService pemohonServ) {
        this.pemohonServ = pemohonServ;
    }
    ...
    public List<InfoWarta> getSenaraiInfoWarta() {
        return senaraiInfoWarta;
    }

    public void setSenaraiInfoWarta(List<InfoWarta> senaraiInfoWarta) {
        this.senaraiInfoWarta = senaraiInfoWarta;
    }
    ...
    public String getIdmhnLama() {
        return idmhnLama;
    }

    public void setIdmhnLama(String idmhnLama) {
        this.idmhnLama = idmhnLama;
    }

    public List<Pemohon> getPemohonListPemaju() {
        return pemohonListPemaju;
    }

    public void setPemohonListPemaju(List<Pemohon> pemohonListPemaju) {
        this.pemohonListPemaju = pemohonListPemaju;
    }
