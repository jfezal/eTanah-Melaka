 ...
 * @author afham
 */
@UrlBinding("/pelupusan/kertas_MMKV2")
public class KertasMMKV2ActionBean extends AbleActionBean {
    ...
    private String kandunganPerakuanPtg;
    ...
    private String kandunganUlasanPenasihat;
    ...
    private List<PermohonanRujukanLuar> senaraiUlasanJabatanTeknikalKod;
    private List<PermohonanRujukanLuar> senaraiUlasanJabatanTeknikalNamaAgensi;
...
//    private List<PermohonanKertasKandungan> senaraiKandunganTajuk;  //Bil - 0
    ...
    private List<PermohonanKertasKandungan> senaraiKandunganPenutup; // Bil - 13
    ...
    private List<PermohonanKertasKandungan> senaraiUlasanPenasihat; // Bil - 11
    ...
    private List<PermohonanKertasKandungan> senaraiSyarat; //Bil - 14
    ...
    private static final Logger LOG = Logger.getLogger(KertasMMKV2ActionBean.class);
    ...
    public Resolution editOnlyKertasMMKPT() {
        ...
        if (kodNegeri.equals("04")) {
            return new JSP(DisPermohonanPage.getKMMK_MAIN_PAGE_MLK()).addParameter("tab", "true");
        } else {
            return new JSP(DisPermohonanPage.getKMMK_MAIN_PAGE_NS()).addParameter("tab", "true");
        }
    }
    ...
    public Resolution editOnlyKertasMMKPTGJMRE() {
        keputusanJktd = Boolean.TRUE;
        HttpSession httpSession = getContext().getRequest().getSession();
        disKertasMMKController = new DisKertasMMKController();
        disKertasMMKController = disKertasMMKController.editKertasMMKPTGJMRE();
        httpSession.setAttribute("disKertasMMKController", disKertasMMKController);
        if (kodNegeri.equals("04")) {
            return new JSP(DisPermohonanPage.getKMMK_MAIN_PAGE_MLK()).addParameter("tab", "true");
        } else {
...
//        keputusanJktd = Boolean.TRUE;
    ...
    public Resolution editDetails() {
        ...
        return new JSP(DisPermohonanPage.getKMMK_SYARAT_LUAS()).addParameter("tab", "true");
    ...
    public void setObjectKertasKandunganPerakuanPtg(String kand, PermohonanKertas permohonanKertas) {
        Pengguna pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        cawangan = new KodCawangan();
        cawangan = (KodCawangan) disLaporanTanahService.getPlpservice().findDAOByPK(pengguna.getKodCawangan(), pengguna.getKodCawangan().getKod());
        PermohonanKertasKandungan pkk = new PermohonanKertasKandungan();
        ...
        pkk.setKandungan(kand);
        disLaporanTanahService.getPelupusanService().simpanPermohonanKertasKandungan(pkk);
        LOG.info("Saving Default Object Kertas Kandungan Syor PTG");
    }
    ...
    public void setObjectKertasKandunganPerihalPemohon(String kand, PermohonanKertas permohonanKertas) {
        ...
        cawangan = (KodCawangan) disLaporanTanahService.getPlpservice().findDAOByPK(pengguna.getKodCawangan(), pengguna.getKodCawangan().getKod());
        PermohonanKertasKandungan pkk = new PermohonanKertasKandungan();
//        kand = getContext().getRequest().getParameter("kandungan");
//        LOG.info("kandungan : "+ kand);
        pkk.setInfoAudit(disLaporanTanahService.findAudit(pkk, "new", pengguna));
        pkk.setBil(10);
        pkk.setSubtajuk("1");
        pkk.setCawangan(cawangan);
    ...
    //untuk kod urusan JMRE sahaja
    public String setDefaultKandunganTajukKertasJMRE(Permohonan p, HakmilikPermohonan hp, Pemohon pmhn, String kodNegeri) {
        Pengguna pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String namaPemohon = pmhn.getPihak() != null ? pmhn.getPihak().getNama() : new String();
        String kandunganTajuk = new String();
        String jadual = new String();
        jadual = p.getSebab() != null ? p.getSebab() : null;

        kandunganTajuk = "PERMOHONAN DARIPADA ";

        if (namaPemohon != null) {
            kandunganTajuk += namaPemohon;
            kandunganTajuk += " UNTUK DISENARAIKAN KE DALAM ";
            kandunganTajuk += jadual.toUpperCase();
        }

        kandunganTajuk += " ENAKMEN RIZAB MELAYU(ERM) BAB 142.";

        return kandunganTajuk;
    }

    //untuk kod urusan JMRE sahaja
    public String setDefaultKandunganTujuanKertasJMRE(Permohonan p, HakmilikPermohonan hp, Pemohon pmhn, String kodNegeri) {
        Pengguna pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String namaPemohon = pmhn.getPihak() != null ? pmhn.getPihak().getNama() : new String();
        ...
        kandunganTujuan = "Kertas ini bertujuan untuk mendapatkan kelulusan daripada Majlis Mesyuarat Kerajaan Negeri ";
        ...
        kandunganTujuan += " untuk menyenaraikan ";

        if (namaPemohon != null) {
            kandunganTujuan += disLaporanTanahService.getPelUtiliti().convertStringtoInitCap(namaPemohon);
            kandunganTujuan += " ke dalam ";
        }

        if (jadual != null) {
            kandunganTujuan += jadual;
        }

        kandunganTujuan += " Enakmen Rizab Melayu (ERM) Bab 142.";

        return kandunganTujuan;
    }

    //untuk kod urusan JMRE sahaja
    public String setDefaultKandunganPerakuanPtgKertasJMRE(Permohonan p, HakmilikPermohonan hp, Pemohon pmhn, String kodNegeri) {
        Pengguna pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String namaPemohon = pmhn.getPihak() != null ? pmhn.getPihak().getNama() : new String();

        String kandunganSyorTnhGali = new String();

        kandunganSyorTnhGali += "Pengarah Tanah dan Galian, Negeri ";

        if (kodNegeri.equals("04")) {
            kandunganSyorTnhGali += "Melaka ";
        } else {
            kandunganSyorTnhGali += "Sembilan Darul Khusus ";
        }
        if ("JMRE".equals(p.getKodUrusan().getKod())) {
            kandunganSyorTnhGali += "mengesyorkan supaya permohonan tanah ini ";
        }
        return kandunganSyorTnhGali;
    }

    public String setDefaultKandunganTajukKertas(Permohonan p, String caw, HakmilikPermohonan hp, Pemohon pmhn, String kodNegeri) {
        Pengguna pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        PermohonanLaporanKawasan laporanKawasan = new PermohonanLaporanKawasan();
        if (!"JMRE".equals(p.getKodUrusan().getKod())) {
            LOG.info("hp.id " + String.valueOf(hp.getId()));
            PermohonanLaporanKawasan plk = pelupusanService.findByidMohonList(idPermohonan);
            if (plk != null) {
                laporanKawasan = (PermohonanLaporanKawasan) disLaporanTanahService.findObject(laporanKawasan, new String[]{String.valueOf(hp.getId()), String.valueOf(plk.getKodRizab().getKod())}, 2);
            }
        }
        String kandunganTajuk = new String();
        String namaPemohon = pmhn.getPihak() != null ? pmhn.getPihak().getNama() : new String();
        ...
        String noWrt = new String();
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        Date trhWrt = new Date();
        ...
        if (!"JMRE".equals(p.getKodUrusan().getKod())) {
            if (hp.getHakmilik() != null) {
                ...
                lot = hp.getHakmilik().getLot() != null ? hp.getHakmilik().getLot().getNama() + " " + hp.getHakmilik().getNoLot().replaceFirst("^0*", "") : null;
                ...
                if (p.getKodUrusan().getKod().equals("BMRE")) {
                    noWrt = laporanKawasan.getNoWarta();
                    trhWrt = laporanKawasan.getTarikhWarta();
                    LOG.info("no warta= " + noWrt + "tarikh warta= " + trhWrt);
                }
            } else {
                if ((p.getKodUrusan().getKod().equals("PBGSA") || p.getKodUrusan().getKod().equals("PLPS")) && p.getCawangan().getKod().equals("05")) {
                    ...
                    //LOG.info("test test : " + hp.getBandarPekanMukimBaru().getNama());
                }
            }
        } else {
            jadual = p.getSebab() != null ? p.getSebab() : null;
        ...
        if (!p.getCawangan().getKod().equals("00")) {
            ...
            int typeNum = p.getKodUrusan().getKod().equals("PPBB") ? 1
                    ...
                    : p.getKodUrusan().getKod().equals("PBRZ") ? 25
                    : p.getKodUrusan().getKod().equals("WMRE") ? 26
                    : p.getKodUrusan().getKod().equals("BMRE") ? 27
                    : p.getKodUrusan().getKod().equals("MMRE") ? 28
                    : p.getKodUrusan().getKod().equals("JMRE") ? 29
                    : p.getKodUrusan().getKod().equals("PBGSA") ? 30
                    : 0;
            switch (typeNum) {
                case 6: //Urusan LPSP
                ...
                case 10: //Urusan PTGSA
                    ...
                    kandunganTajuk += " DI BAWAH AKTA TANAH (KAWASAN-KAWASAN PENEMPATAN BERKELOMPOK) 1960 ";
...
//                    kandunganTajuk += " UNTUK MEMILIKI PERMIT KHAS TANAH PERTANIAN UNTUK TUJUAN BUKAN PERTANIAN";
                        kandunganTajuk += " UNTUK MENGGUNAKAN TANAH PERTANIAN BAGI TUJUAN YANG TIADA BERKAITAN DENGAN PERTANIAN DI BAWAH SEKSYEN 115(4)(F) DAN (G) KANUN "
                                + "TANAH NEGARA 1965 ";
                    }
                    if (lot != null) {
                        kandunganTajuk += " DI ATAS ";
...
//                }
                ...
                case 25: //Urusan PBRZ
                    ...
                    if (namaPemohon != null) {
                        kandunganTajuk += namaPemohon;
                        kandunganTajuk += " UNTUK PEMBATALAN RIZAB DIBAWAH SEKSYEN 64 KANUN TANAH NEGARA 1965";
                    }
...
//                }
                ...
                case 26: //Urusan WMRE
                    ...
                    if (namaPemohon != null) {
                        kandunganTajuk += namaPemohon;
                        kandunganTajuk += " UNTUK MERIZABKAN TANAH KERAJAAN ";
                    }
                    ...
                    if (daerah != null) {
                        kandunganTajuk += ", DAERAH ";
                        kandunganTajuk += daerah.toUpperCase();
                        kandunganTajuk += " NEGERI SEMBILAN DARUL KHUSUS ";

                    }

                    kandunganTajuk += " BAGI MAKSUD AWAM IAITU KAWASAN LAPANG DI BAWAH JAGAAN ";

                    if (namaPemohon != null) {
                        kandunganTajuk += namaPemohon;
                        kandunganTajuk += " SEBAGAI PEGAWAI PENGAWAL MENURUT SEKSYEN 3 ENAKMEN RIZAB MELAYU BAB 142 DAN PERKARA 89(3)(b) PERLEMBAGAAN PERSEKUTUAN.";
                    }
                    kandunganTajuk.toUpperCase();
                ...
                case 27: //Urusan BMRE
                    ...
                    if (namaPemohon != null) {
                        kandunganTajuk += namaPemohon;
                        kandunganTajuk += " UNTUK MEMBATALKAN WARTA KAWASAN LAPANG ";
                    }
                    ...
                    if (noWrt != null) {
                        kandunganTajuk += noWrt.toUpperCase();
                    }

                    if (trhWrt != null) {
                        kandunganTajuk += " BERTARIKH ";
                        kandunganTajuk += trhWrt;
                    }
                    ...
                    kandunganTajuk += " MENGIKUT SEKSYEN 64 KANUN TANAH NEGARA 1965.";
                    break;
...
//                    kandunganTajuk += p.getSebab().toUpperCase();
//                }
                    kandunganTajuk += " MENJADI KAWASAN SIMPANAN MELAYU MENGIKUT SEKSYEN 3 ENAKMEN RIZAB MELAYU BAB 142 DAN PERKARA 89(3)(b) PERLEMBAGAAN PERSEKUTUAN.";
                ...
                case 30: //Urusan PBGSA

                    if (namaPemohon != null) {
                        kandunganTajuk += namaPemohon;
                        kandunganTajuk += " UNTUK MENDAPATKAN PEMBERIMILIKAN TANAH GSA ";
                    }
                    if (tujuan != null) {
                        kandunganTajuk += tujuan.toUpperCase();
                    }
            ...
            String daerah = p.getCawangan() != null ? p.getCawangan().getName() : null;
            String tujuan = p.getSebab() != null ? p.getSebab() : null;
            kandunganTajuk = "PERMOHONAN DARIPADA ";
...
//                }
    ...
    public String setDefaultKandunganTujuanKertas(Permohonan p, String caw, HakmilikPermohonan hp, Pemohon pmhn, String kodNegeri) {
        ...
        if (!"JMRE".equals(p.getKodUrusan().getKod())) {
            if (hp.getHakmilik() != null) {
                ...
                bpm = hp.getHakmilik().getBandarPekanMukim() != null ? hp.getHakmilik().getBandarPekanMukim().getNama() : null;
                kodHM = hp.getHakmilik().getKodHakmilik() != null ? hp.getHakmilik().getKodHakmilik().getKod() + " " + hp.getHakmilik().getNoHakmilik().replaceFirst("^0*", "") : null;
            } else if (p.getKodUrusan().getKod().equals("JMRE")) {
                //JMRE xde id hakmilik
            } else {
                luasTerlibat = hp.getLuasDiluluskan() != null ? hp.getLuasDiluluskan() : hp.getLuasTerlibat();
                if (hp.getKodUnitLuas() != null) {
                    unitLuas = hp.getLuasLulusUom() != null ? hp.getLuasLulusUom().getNama() : hp.getKodUnitLuas().getNama();
        ...
        if (!p.getCawangan().getKod().equals("00")) {
            ...
            if ("MMRE".equals(permohonan.getKodUrusan().getKod()) || "WMRE".equals(permohonan.getKodUrusan().getKod())) {
                kandunganTujuan = "Kertas ini bertujuan untuk mendapatkan pertimbangan Majlis Mesyuarat Kerajaan Negeri ";
            } else if ("JMRE".equals(permohonan.getKodUrusan().getKod())) {
                kandunganTujuan = "Kertas ini bertujuan untuk mendapatkan kelulusan daripada Majlis Mesyuarat Kerajaan Negeri ";
            } else {
            ...
            if ("JMRE".equals(permohonan.getKodUrusan().getKod())) {
                kandunganTujuan += " untuk menyenaraikan ";
            } else {
                kandunganTujuan += " mengenai permohonan daripada ";
            }
            ...
            int typeNum = p.getKodUrusan().getKod().equals("PPBB") ? 1
                    ...
                    : p.getKodUrusan().getKod().equals("PRMMK") ? 24
                    : p.getKodUrusan().getKod().equals("WMRE") ? 26
                    : p.getKodUrusan().getKod().equals("JMRE") ? 27
                    : p.getKodUrusan().getKod().equals("MMRE") ? 28
                    : p.getKodUrusan().getKod().equals("PBGSA") ? 29
                    : 0;
            switch (typeNum) {
                case 6: //Urusan LPSP
                ...
                case 10: //Urusan PTGSA
                    ...
                    kandunganTujuan += " di bawah akta Tanah (Kawasan-Kawasan Penempatan Berkelompok) 1960.";
                ...
                case 12: //Urusan PBMT
                    ...
                    break;
                case 17: //Urusan PBRZ
                    kandunganTujuan += "pembatalan perizaban ";
                    if (tujuan != null) {
                        kandunganTujuan += tujuan.toLowerCase();
                    }
                ...
                case 26: //Urusan WMRE
                    ...
                    if (daerah != null) {
                        kandunganTujuan += "Daerah " + disLaporanTanahService.getPelUtiliti().convertStringtoInitCap(daerah) + " ";
                    }

                    kandunganTujuan += "menjadi Tanah Simpanan Melayu mengikut Seksyen 3 Enakmen Rizab Melayu Bab 142 dan Perkara 89(3)(b) Perlembagaan Persekutuan.";
                ...
                case 28: //Urusan MMRE
                    ...
                    break;
                case 29: //Urusan PBGSA
                    kandunganTujuan += "pemberimilikan tanah GSA bagi tujuan ";
                    if (tujuan != null) {
                        kandunganTujuan += tujuan.toLowerCase();
                    }
        ...
        if (!p.getCawangan().getKod().equals("00")) {
            ...
            switch (typeNum) {
                ...
                case 12: //Urusan PBMT
                    ...
                    break;
                case 17: //Urusan PBRZ
                    kandunganPermohonan += " telah menerima permohonan untuk membatalkan rizab daripada ";
                case 23: //Urusan PTPBP
                ...
                case 28: // Urusan MMRE
                    kandunganPermohonan += " telah menerima permohonan daripada ";
    ...
    public String setDefaultKandunganPerakuanPtgKertas(Permohonan p, String caw, HakmilikPermohonan hp, Pemohon pmhn, String kodNegeri) {
        Pengguna pengguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String kandunganSyorTnhGali = new String();
    ...
    public String setDefaultKandunganPerihalPemohonKertas(Permohonan p, String caw, Pemohon pmhn, String kodNegeri) {
        ...
        List<Pemohon> pmh = pelupusanService.findPemohonByIdPemohonList(permohonan.getIdPermohonan());
        StringBuilder strBuilder = new StringBuilder();
        int bil = 1;
        kandunganPemohon = "Pemohon adalah seperti berikut :" + "\n";

        for (Pemohon pmhn1 : pmh) {
            String namaPemohon = pmhn1.getPihak() != null ? pmhn1.getPihak().getNama() : new String();
            String noPengenalan = pmhn1.getPihak() != null ? pmhn1.getPihak().getNoPengenalan() : new String();
            String alamat = new String();
            if (pmhn1.getPihak() != null) {
                if (StringUtils.isNotBlank(pmhn1.getPihak().getAlamat1())) {
                    alamat += pmhn1.getPihak().getAlamat1();
                }
                if (StringUtils.isNotBlank(pmhn1.getPihak().getAlamat2())) {
                    alamat += ", ";
                    alamat += pmhn1.getPihak().getAlamat2();
                }
                if (StringUtils.isNotBlank(pmhn1.getPihak().getAlamat3())) {
                    alamat += ", ";
                    alamat += pmhn1.getPihak().getAlamat3();
                }
                if (StringUtils.isNotBlank(pmhn1.getPihak().getAlamat4())) {
                    alamat += ", ";
                    alamat += pmhn1.getPihak().getAlamat4();
                }
            }
        ...
        strBuilder.append(kandunganPemohon);
...
//                    kandunganTajuk += " PLOT ";
    ...
    public void setObjectKertasKandunganKeputusanPTG(PermohonanKertas permohonanKertas) {
        ...
        kand += " untuk pertimbangan dan keputusan Majlis Mesyuarat Kerajaan Negeri ";
    ...
    public void rehydrate() {
        ...
        if (idPermohonan != null) {
            permohonan = new Permohonan();
            permohonan = (Permohonan) disLaporanTanahService.findObject(permohonan, new String[]{idPermohonan}, 0);
            if (permohonan != null && !permohonan.getKodUrusan().getKod().equals("JMRE")) {
                if (!permohonan.getCawangan().getKod().equals("00")) {
                    setDefaultKandungan(permohonan.getCawangan().getDaerah().getNama());
                } else {
                    setDefaultKandungan(permohonan.getCawangan().getName());
                }
            }
            if (!kelompok) {
...
//            listPemohonHubungan = new ArrayList();
            ...
            String kod = getContext().getRequest().getParameter("index");
            senaraiUlasanJabatanTeknikalKod = disLaporanTanahService.getPelupusanService().findPermohonanRujLuarByIdPermohonanKod(idPermohonan, "JTK", kod);
            LOG.info("senaraiUlasanJabatanTeknikalKod == " + senaraiUlasanJabatanTeknikalKod.size());
            if (senaraiUlasanJabatanTeknikalKod.size() > 0) {
                for (int i = 0; i < senaraiUlasanJabatanTeknikalKod.size(); i++) {
                    PermohonanRujukanLuar mohonRujuk = new PermohonanRujukanLuar();
                    mohonRujuk = senaraiUlasanJabatanTeknikalKod.get(i);
                    if (mohonRujuk.getUlasan() == null) {
                        mohonRujuk.setUlasan("Tiada Ulasan Diterima");
                    }
                    disLaporanTanahService.getPelupusanService().simpanPermohonanRujLuar(mohonRujuk);
            ...
            senaraiUlasanJabatanTeknikalNamaAgensi = disLaporanTanahService.getPelupusanService().findPermohonanRujLuarByIdPermohonanNamaAgensi(idPermohonan, "JTK", kodNegeri);
            /*
             * Ulasan Adun
             */
            if (kodNegeri.equals("04")) {
...
//                            mohonRujuk.setUlasan(ulasanAgensi);
                        ...
                        if (mohonRujuk.getAgensi() != null) {
                            if (mohonRujuk.getAgensi().getKod().equals("10828")) {
                                LOG.info("kod 10828 xde dlm kod dun");
                            } else {
...
//                senaraiHakmilikPermohonan = disLaporanTanahService.getPlpservice().getHakmilikPermohonanListByCatatanNIdMohon(new String(), idPermohonan);
            ...
            if (StringUtils.isNotBlank(kodDok)) {
                ...
                if (permohonanKertas != null) {
                    ...
                    senaraiSyarat = disLaporanTanahService.getPelupusanService().findByIdLapor(permohonanKertas.getIdKertas(), 14);
                    ...
                    senaraiKandunganPenutup = disLaporanTanahService.getPelupusanService().findByIdLapor(permohonanKertas.getIdKertas(), 13);
                    senaraiUlasanPenasihat = disLaporanTanahService.getPelupusanService().findByIdLapor(permohonanKertas.getIdKertas(), 11);
                    ...
                    if (hakmilikPermohonan != null) {
                        if (kelompok) {
                            ...
                            if (!permohonan.getCawangan().getKod().equals("00")) {
                                ...
                                kandunganTajuk = setDefaultKandunganTajukKertas(permohonan, permohonan.getCawangan().getKod(), hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                            }
                            if (!permohonan.getCawangan().getKod().equals("00")) {
                                ...
                                kandunganTujuan = setDefaultKandunganTujuanKertas(permohonan, permohonan.getCawangan().getKod(), hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                            }
                            if (!permohonan.getCawangan().getKod().equals("00")) {
                                ...
                                kandunganPerihalPermohonan = setDefaultKandunganPerihalPermohonanKertas(permohonan, permohonan.getCawangan().getKod(), hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                            }

                            if ("MMRE".equals(permohonan.getKodUrusan().getKod()) || "WMRE".equals(permohonan.getKodUrusan().getKod()) || "BMRE".equals(permohonan.getKodUrusan().getKod()) || "JMRE".equals(permohonan.getKodUrusan().getKod())) {
                                kandunganPerakuanPtg = setDefaultKandunganPerakuanPtgKertas(permohonan, permohonan.getCawangan().getDaerah().getKod(), hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                            }
                        }
                        if (kodNegeri.equals("04")) {
                            if (permohonan.getCawangan().getKod().equals("00")) {
                                kandunganPerihalPemohon = setDefaultKandunganPerihalPemohonKertas(permohonan, permohonan.getCawangan().getKod(), permohonan.getSenaraiPemohon().get(0), kodNegeri);
                            } else {
...
//
//                        mohonHakmilik.setPermohonan(permohonan);
//                        mohonHakmilik.setInfoAudit(info);
//                        pelupusanService.simpanHakmilikPermohonan2(mohonHakmilik);

                        kandunganTajuk = setDefaultKandunganTajukKertasJMRE(permohonan, hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                        kandunganTujuan = setDefaultKandunganTujuanKertasJMRE(permohonan, hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                        kandunganPerakuanPtg = setDefaultKandunganPerakuanPtgKertasJMRE(permohonan, hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                        kandunganPerihalPermohonan = setDefaultKandunganPerihalPermohonanKertas(permohonan, permohonan.getCawangan().getDaerah().getKod(), hakmilikPermohonan, permohonan.getSenaraiPemohon().get(0), kodNegeri);
                    ...
                    permohonanKertas = disLaporanTanahService.getPelupusanService().findPermohonanKertasByIdPermohonanNKodDokumen(idPermohonan, kodDok);
                    if (permohonanKertas != null) {
                        LOG.info("masuk mohon kertas not null");

                        setObjectKertasKandunganTajuk(kandunganTajuk, permohonanKertas);
                        LOG.info("masuk perihal permohonan222");
                        setObjectKertasKandunganTujuan(kandunganTujuan, permohonanKertas);
                        LOG.info("masuk perihal permohonan");
                        if (!"JMRE".equals(permohonan.getKodUrusan().getKod())) {
                            setObjectKertasKandunganPerihalPermohonan(kandunganPerihalPermohonan, permohonanKertas);
                        }
                        if (StringUtils.isNotBlank(kandunganPerakuanPtg)) {
                            setObjectKertasKandunganPerakuanPtg(kandunganPerakuanPtg, permohonanKertas);
                        }
...
//                disPermohonanBahanBatu.setSyaratBahanBatu(disLaporanTanahService.getPlpservice().findPermohonanBahanBatuanByIdMohon(idPermohonan));
    ...
    public Resolution deleteRowjtek() throws ParseException {

        String idRujukan = getContext().getRequest().getParameter("idRujukan");

        PermohonanRujukanLuar mlr = new PermohonanRujukanLuar();
        mlr = pelupusanService.findByIdRujukan(Long.parseLong(idRujukan));
        if (mlr != null) {
            pelupusanService.deleteMohonRujLuar(Long.parseLong(idRujukan));
        }

        rehydrate();
        addSimpleMessage("Maklumat Telah Berjaya Dihapus");
        return new JSP("pelupusan/kertasMMK/kertasMMKEdit/kertas_jtek.jsp").addParameter("popup", "true");
    }

    public Resolution tambahRowjtek() {

        Pengguna pguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        permohonan = permohonanDAO.findById(idPermohonan);
        String agensi = getContext().getRequest().getParameter("index");

        senaraiUlasanJabatanTeknikalKod = disLaporanTanahService.getPelupusanService().findPermohonanRujLuarByIdPermohonanKod(idPermohonan, "JTK", agensi);
        if (!senaraiUlasanJabatanTeknikalKod.isEmpty()) {
            PermohonanRujukanLuar pl = new PermohonanRujukanLuar();
            InfoAudit info = new InfoAudit();
            info.setDimasukOleh(pguna);
            info.setTarikhMasuk(new java.util.Date());
            pl.setInfoAudit(info);
            pl.setCawangan(permohonan.getCawangan());
            pl.setPermohonan(permohonan);
            KodAgensi ka = new KodAgensi();
            ka = kodAgensiDAO.findById(agensi);
            pl.setAgensi(ka);
            KodRujukan kr = new KodRujukan();
            kr = kodRujukanDAO.findById("FL");
            pl.setKodRujukan(kr);
            disLaporanTanahService.getPelupusanService().simpanPermohonanRujukanLuar(pl);
            rehydrate();
        }
        return new JSP("pelupusan/kertasMMK/kertasMMKEdit/kertas_jtek.jsp").addParameter("popup", "true");
    }

    public Resolution simpanjtek() {

        Pengguna pguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        permohonan = permohonanDAO.findById(idPermohonan);
        String idRujukan = getContext().getRequest().getParameter("idRujukan");
        String ulasan = getContext().getRequest().getParameter("kandungan5");
        String kod = getContext().getRequest().getParameter("index");

        PermohonanRujukanLuar pl = disLaporanTanahService.getPelupusanService().findPermohonanRujLuarByIdPermohonanIdRuj(idPermohonan, "JTK", Long.parseLong(idRujukan), kod);
        InfoAudit info = new InfoAudit();
        ...
        addSimpleMessage("Maklumat Telah Berjaya Di Simpan");
        return new ForwardResolution("pelupusan/kertasMMK/kertasMMKEdit/kertas_jtek.jsp").addParameter("popup", "true");
    }
...
//        permohonanKertas = new PermohonanKertas();
        ...
        int typeNum = type.equals("kTajuk") ? 0
                ...
                : type.equals("kUlasanPenasihat") ? 12
                : type.equals("kPenutup") ? 13
                : 12;
        ...
        switch (typeNum) {
            ...
            case 10:
                ...
                if (kodNegeri.equals("04")) {
                    forwardJSP = DisPermohonanPage.getKMMK_MAIN_PAGE_MLK();
                } else {
            ...
            case 12:
                senaraiUlasanPenasihat = disLaporanTanahService.getPelupusanService().findByIdLapor(permohonanKertas.getIdKertas(), 11);
                forwardJSP = DisPermohonanPage.getKMMK_ULASAN_PENASIHAT();
                break;
            case 13:
                senaraiKandunganPenutup = disLaporanTanahService.getPelupusanService().findByIdLapor(permohonanKertas.getIdKertas(), 13);
                forwardJSP = DisPermohonanPage.getKMMK_PENUTUP();
                break;
        }
        return forwardJSP;
    }

    public Resolution openFramejtek() {
        ctx = (etanahActionBeanContext) getContext();
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        Pengguna pguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String forwardJSP = new String();
        String type = ctx.getRequest().getParameter("type");
        String kod = ctx.getRequest().getParameter("index");
        kodNegeri = disLaporanTanahService.getConf().getProperty("kodNegeri");
        permohonan = new Permohonan();
        permohonan = (Permohonan) disLaporanTanahService.findObject(permohonan, new String[]{idPermohonan}, 0);
        String kodDok = kertasDok(permohonan.getKodUrusan().getKod(), kodNegeri);
...
//                senaraiHakmilikPermohonan = disLaporanTanahService.getPlpservice().getHakmilikPermohonanListByCatatanNIdMohon(new String(), idPermohonan);
        ...
        if (StringUtils.isNotBlank(type)) {
             ...
             * FORWARD TO DIFFERENT POPUP
             */
            forwardJSP = refreshDatajtek(type, kod);
        }
//        rehydrate();
        return new JSP(forwardJSP).addParameter("popup", Boolean.TRUE);
    }

    public String refreshDatajtek(String type, String kod) {
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        permohonan = new Permohonan();
        permohonan = (Permohonan) disLaporanTanahService.findObject(permohonan, new String[]{idPermohonan}, 0);
        kodNegeri = disLaporanTanahService.getConf().getProperty("kodNegeri");
...
//        permohonanKertas = new PermohonanKertas();
        permohonanKertas = disLaporanTanahService.getPelupusanService().findPermohonanKertasByIdPermohonanNKodDokumen(idPermohonan, kodDok);
        String forwardJSP = new String();
        int typeNum = type.equals("jtek") ? 0
                : 12;

        switch (typeNum) {
            case 0:
                String kodA = getContext().getRequest().getParameter("index");
                senaraiUlasanJabatanTeknikalKod = disLaporanTanahService.getPelupusanService().findPermohonanRujLuarByIdPermohonanKod(idPermohonan, "JTK", kodA);
                LOG.info("senaraiUlasanJabatanTeknikalKod == " + senaraiUlasanJabatanTeknikalKod.size());
                if (senaraiUlasanJabatanTeknikalKod.size() > 0) {
                    for (int i = 0; i < senaraiUlasanJabatanTeknikalKod.size(); i++) {
                        PermohonanRujukanLuar mohonRujuk = new PermohonanRujukanLuar();
                ...
                forwardJSP = DisPermohonanPage.getJTEK();
                break;
...
//        LOG.info("CHECKING:..... index :" + index + " kand :" + kand);
        ...
        switch (index) {
            ...
            case 5:
                ...
                updateKandungan(kand, "kPerakuanPtd", permohonanKertas, 14);
                updateMohonFasa();
                senaraiKelompok = disLaporanTanahService.getPelupusanService().findMohonKelompokByIdMohonInduk(idPermohonan);
                if (senaraiKelompok.size() > 0) {
                    kelompok = true;
            ...
            case 11:
                forwardJSP = DisPermohonanPage.getKMMK_ULASAN_PENASIHAT();
                updateKandungan(kand, "kUlasanPenasihat", permohonanKertas, 11);
                break;
            case 13:
                forwardJSP = DisPermohonanPage.getKMMK_PENUTUP();
                updateKandungan(kand, "kPenutup", permohonanKertas, 13);
                break;
            default:
                break;
        }
...
//            }
        ...
        addSimpleMessage("Maklumat Berjaya Disimpan");
        return new JSP(DisPermohonanPage.getKMMK_SYARAT_LUAS()).addParameter("tab", Boolean.TRUE);
    }
...
//            }
        ...
        if (!permohonan.getKodUrusan().getKod().equals("PTGSA") && !permohonan.getKodUrusan().getKod().equals("PBMMK") && !permohonan.getKodUrusan().getKod().equals("PBPTG") && !permohonan.getKodUrusan().getKod().equals("PBPTD") && !permohonan.getKodUrusan().getKod().equals("PBGSA") && !permohonan.getKodUrusan().getKod().equals("PPRU") && !permohonan.getKodUrusan().getKod().equals("PTPBP") && !permohonan.getKodUrusan().getKod().equals("LPSP") && !permohonan.getKodUrusan().getKod().equals("LMCRG") && !permohonan.getKodUrusan().getKod().equals("MLCRG") && !permohonan.getKodUrusan().getKod().equals("PRMMK") && !permohonan.getKodUrusan().getKod().equals("PBRZ")) {
            if (!kelompok) {
                if (!permohonan.getKodUrusan().getKod().equals("PBMT")) {
                    hakmilikPermohonan = new HakmilikPermohonan();
                    ...
                    if (permohonan.getKodUrusan().getKod().equals("PRMP")) {
                        luasDiluluskan = getContext().getRequest().getParameter("luasDiluluskan");
                        BigDecimal b = new BigDecimal(luasDiluluskan);
                        hakmilikPermohonanSave.setLuasDiluluskan(b);
                    } else {
                        if (hakmilikPermohonan.getLuasDiluluskan() != null) {
                            hakmilikPermohonanSave.setLuasDiluluskan(hakmilikPermohonan.getLuasDiluluskan());
                        }
                    ...
                    luasDiluluskan = getContext().getRequest().getParameter("luasDiluluskan");
                    BigDecimal d = new BigDecimal(luasDiluluskan);
                    hakmilikPermohonanSave.setLuasDiluluskan(d);
...
//                        mohonTuntutKosCagarJln.setKuantiti(kuponQty);
            ...
            case 8:
                ...
                if (hakmilikPermohonan.getLuasLulusUom().getKod().equalsIgnoreCase("M")) {
                    String luasLulus1 = getContext().getRequest().getParameter("luasDiluluskan");
    ...
    public Resolution tambahRow() {
        ...
        switch (index) {
            ...
            case 11:
                ...
                disLaporanTanahService.getPelupusanService().simpanPermohonanKertasKandungan(permohonanKertasKandungan);
                rehydrate();
                forwardJSP = refreshData("kUlasanPenasihat");
                break;
            case 14:
                cawangan = new KodCawangan();
                cawangan = disLaporanTanahService.getKodCawanganDAO().findById(pengguna.getKodCawangan().getKod());
            ...
            case 13:
                ...
                disLaporanTanahService.getPelupusanService().simpanPermohonanKertasKandungan(permohonanKertasKandungan);
                rehydrate();
                forwardJSP = refreshData("kPenutup");
                break;
        }
        senaraiKelompok = disLaporanTanahService.getPelupusanService().findMohonKelompokByIdMohonInduk(idPermohonan);
        if (senaraiKelompok.size() > 0) {
...
//                    mohonBahanBatuSave.setTempohSyorUom(disLaporanTanahService.getKodUOMDAO().findById(mohonBahanBatu.getUnitTempohKeluar().getKod()));
    ...
    public String getKandunganPerakuanPtg() {
        return kandunganPerakuanPtg;
    }

    public void setKandunganPerakuanPtg(String kandunganPerakuanPtg) {
        this.kandunganPerakuanPtg = kandunganPerakuanPtg;
    }
    ...
    public List<PermohonanKertasKandungan> getSenaraiUlasanPenasihat() {
        return senaraiUlasanPenasihat;
    }

    public void setSenaraiUlasanPenasihat(List<PermohonanKertasKandungan> senaraiUlasanPenasihat) {
        this.senaraiUlasanPenasihat = senaraiUlasanPenasihat;
    }
    ...
    public List<PermohonanKertasKandungan> getSenaraiKandunganPenutup() {
        return senaraiKandunganPenutup;
    }

    public void setSenaraiKandunganPenutup(List<PermohonanKertasKandungan> senaraiKandunganPenutup) {
        this.senaraiKandunganPenutup = senaraiKandunganPenutup;
    }
    ...
    public List<PermohonanRujukanLuar> getSenaraiUlasanJabatanTeknikalKod() {
        return senaraiUlasanJabatanTeknikalKod;
    }

    public void setSenaraiUlasanJabatanTeknikalKod(List<PermohonanRujukanLuar> senaraiUlasanJabatanTeknikalKod) {
        this.senaraiUlasanJabatanTeknikalKod = senaraiUlasanJabatanTeknikalKod;
    }

    public List<PermohonanRujukanLuar> getSenaraiUlasanJabatanTeknikalNamaAgensi() {
        return senaraiUlasanJabatanTeknikalNamaAgensi;
    }

    public void setSenaraiUlasanJabatanTeknikalNamaAgensi(List<PermohonanRujukanLuar> senaraiUlasanJabatanTeknikalNamaAgensi) {
        this.senaraiUlasanJabatanTeknikalNamaAgensi = senaraiUlasanJabatanTeknikalNamaAgensi;
    }
