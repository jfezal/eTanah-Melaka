...
/**
 *
 * @author muhammad.amin @modified by afham @Modified By Murali @Modified By
 * Shafful
 */
@UrlBinding("/pengambilan/laporan_pelukis_pelan")
public class LaporanPelukisPelanPActionBean extends AbleActionBean {
    ...
    private static final Logger LOG = Logger.getLogger(LaporanPelukisPelanPActionBean.class);
    ...
    public Resolution showForm() {
        ...
        return new JSP("pengambilan/laporan_pelukis_pelan.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution tanahRizabPopup() {
        p = true;
        return new JSP("pengambilan/tanah_rizab_add_1.jsp").addParameter("popup", "true").addParameter("showForm", "false");
    }
    ...
    public Resolution permohonanTerdahuluPopup() {
        ...
        return new JSP("pengambilan/carian_permohonan_terdahulu.jsp").addParameter("popup", "true").addParameter("showForm", "false");
    }

    public Resolution kawasanPopup() {
        getContext().getRequest().setAttribute("status", Boolean.FALSE);
        return new JSP("pengambilan/tambahKawasan.jsp").addParameter("popup", "true");
    }

//    public Resolution editKawasanPopup() {
//        getContext().getRequest().setAttribute("idMohonlaporKws", Boolean.FALSE);
//        return new JSP("pelupusan/popup_edit_kawasan.jsp").addParameter("popup", "true").addParameter("showForm", "false");
//    }
    public Resolution cariSemula() {
        getContext().getRequest().setAttribute("status", Boolean.FALSE);
        return new JSP("pengambilan/carian_permohonan_terdahulu.jsp").addParameter("popup", "true");
    }
...
//         prmhnn = permohonanDAO.findById(id);
        if (idPermohonan != null) {
            ...
            if (permohonan.getKodUrusan().getKod().equals("PHLP") || permohonan.getKodUrusan().getKod().equals("PHLA") || permohonan.getKodUrusan().getKod().equals("PBHL")) {
...
//            senaraiLaporanKawasan = plpservice.find
            ...
            if (hakmilikPermohonanList != null) {
                if (permohonan.getKodUrusan().getKod().equals("PHLP") || permohonan.getKodUrusan().getKod().equals("PTGSA") || permohonan.getKodUrusan().getKod().equals("PHLA") || permohonan.getKodUrusan().getKod().equals("PBHL")) {
                    ...
                    senaraiHakmilikPermohonan = pengambilanService.findByIdPermohonanPP(idPermohonan);
                }
//                hakmilikPermohonan = plpservice.findByIdPermohonan(idPermohonan) ;
                for (HakmilikPermohonan hp : senaraiHakmilikPermohonan) {
                    if ((hp != null) && (hp.getKodMilik() != null)) {
                        kodP = hp.getKodMilik().getKod();
                        kodPString = hp.getKodMilik().getNama();
                    }
                }
            }
            ...
            for (HakmilikPermohonan hp : senaraiHakmilikPermohonan) {
                if ((hp != null) && (hp.getKodDUN() != null)) {
                    kodD = hp.getKodDUN().getKod();
                }
                if ((hp != null) && (hp.getKodKawasanParlimen() != null)) {
                    kodPar = hp.getKodKawasanParlimen().getKod();
                }
            }
...
//            permohonanLaporanPelan= laporanPelukisPelanService.findByidP(idPermohonan);
            for (HakmilikPermohonan hp : senaraiHakmilikPermohonan) {
//                permohonanLaporanPelan = laporanPelukisPelanService.findMohonLaporPelanByidMohon(idPermohonan);
                permohonanLaporanPelan = laporanPelukisPelanService.findMohonLaporPelanByidMohonHackmilikMohan(idPermohonan, hp.getId());
...
//           tanahrizabpermohonan=tanahRizabService.findByidPermohonan(idPermohonan);
                ...
                if (permohonanLaporanPelan != null) {
                    ...
                    PermohonanLaporanPelan plp = new PermohonanLaporanPelan();
                    InfoAudit info = new InfoAudit();
                    info.setDimasukOleh(pguna);
                    info.setTarikhMasuk(new java.util.Date());
                    plp.setInfoAudit(info);
...
//            }
    ...
    public Resolution showEditTanahRizab() {
        ...
        p = true;
        return new JSP("pengambilan/tanah_rizab_edit_1.jsp").addParameter("popup", "true");
    }
...
//        rehydrate();
        ...
        return new RedirectResolution(LaporanPelukisPelanPActionBean.class, "locate");
    }
    ...
    public Resolution openLainlain() {
        ...
        return new ForwardResolution("/WEB-INF/jsp/pengambilan/tambahKawasan.jsp").addParameter("popup", "true");
    }

    public Resolution tutupPermohonan() {
        rehydrate();
        return new ForwardResolution("/WEB-INF/jsp/pengambilan/laporan_pelukis_pelan.jsp").addParameter("tab", "true");
...
//        plpservice.simpanPermohonan(permohonan);
    ...
    public Resolution simpanKawasan() throws ParseException {
        ...
        kws.setHakmilikPermohonan(hakmilikPermohonan);
        System.out.println("-------hakmilik permohonan ialah : " + hakmilikPermohonan);
    ...
    public Resolution editLa() {
        InfoAudit ia = new InfoAudit();
        Pengguna peng = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        Permohonan permohonan = permohonanDAO.findById(idPermohonan);
        String idMohonlaporKws = getContext().getRequest().getParameter("idMohonlaporKws");
        permohonanLaporanKWS = permohonanLaporanKawasanDAO.findById(Long.parseLong(idMohonlaporKws));

        InfoAudit infoAudit = new InfoAudit();
        infoAudit.setDimasukOleh(peng);
        infoAudit.setTarikhMasuk(new java.util.Date());

        permohonanLaporanKWS.setKodRizab(kodRizabDAO.findById(Integer.parseInt(kodRizab)));
        if (kodRizab.equals("99")) {
            permohonanLaporanKWS.setCatatan(catatan);
        }
        permohonanLaporanKWS.setNoWarta(addnoWarta);
        permohonanLaporanKWS.setInfoAudit(infoAudit);
//        if (addtarikhWarta != null && !("").equals(addtarikhWarta)) {
//            permohonanLaporanKWS.setTarikhWarta(sdf.parse(addtarikhWarta));
//        }
        permohonanLaporanKWS.setNoPelanWarta(addnoPelanWarta);
        permohonanLaporanKWS.setPermohonan(permohonan);
        permohonanLaporanKWS.setKodCawangan(permohonan.getCawangan());
        permohonanLaporanKWS.setHakmilikPermohonan(hakmilikPermohonan);
        System.out.println("-------hakmilik permohonan ialah : " + hakmilikPermohonan);
        plpservice.simpanPermohonanLaporKawasan(permohonanLaporanKWS);
        addSimpleMessage("Maklumat Berjaya Disimpan");
        rehydrate();
        edit = Boolean.TRUE;
        return new ForwardResolution("/WEB-INF/jsp/pengambilan/laporan_pelukis_pelan.jsp").addParameter("tab", "true");
    ...
    public Resolution editLaporKawasan() {
        InfoAudit ia = new InfoAudit();
        Pengguna peng = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        String idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        Permohonan permohonan = permohonanDAO.findById(idPermohonan);
        ...
        addnoPelanWarta = permohonanLaporanKWS.getNoPelanWarta();
        if (permohonanLaporanKWS != null) {
            addtarikhWarta = sdf.format(permohonanLaporanKWS.getTarikhWarta()).substring(0, 10);
        }
//        addtarikhWarta = sdf.format(permohonanLaporanKWS.getTarikhWarta().toString());
        System.out.println("-------------------add tarikh warta : " + addtarikhWarta);
        addnoWarta = permohonanLaporanKWS.getNoWarta();
        catatan = permohonanLaporanKWS.getCatatan();
        kodRizab = Integer.toString(permohonanLaporanKWS.getKodRizab().getKod());
//        permohonan = permohonanLaporanKWS.getPermohonan();
        cawangan = permohonanLaporanKWS.getKodCawangan();
        hakmilikPermohonan = permohonanLaporanKWS.getHakmilikPermohonan();
//        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new JSP("pengambilan/popup_edit_kawasan.jsp").addParameter("popup", "true");
    }
...
//        rizab_melayu = getContext().getRequest().getParameter("rizab_melayu");
        ...
        for (HakmilikPermohonan hp : senaraiHakmilikPermohonan) {
            LOG.info("senaraiHakmilikPermohonan : " + senaraiHakmilikPermohonan);
            if (!kodS.equals("0")) {
                Integer a = Integer.parseInt(kodS);
                int kod = a.intValue();
                KodSeksyen kodSeksyen = plpservice.findByKodSeksyen(kod);
                hp.setKodSeksyen(kodSeksyen);
                plpservice.simpanHakmilikPermohonan(hp);
            } else {
                hp.setKodSeksyen(null);
                plpservice.simpanHakmilikPermohonan(hp);
            }
            Permohonan permohonan = permohonanDAO.findById(idPermohonan);
            cawangan = permohonan.getCawangan();
//            permohonanLaporanPelan = plpservice.findByIdPermohonanPelan(idPermohonan);
//            permohonanLaporanPelan = plpservice.findByIdPermohonanPelanNIdHakmilik(idPermohonan, hp.getHakmilik().getIdHakmilik());
            permohonanLaporanPelan = laporanPelukisPelanService.findMohonLaporPelanByidMohonHackmilikMohan(idPermohonan, hp.getId());
            ...
            if (permohonanLaporanPelan == null) {
                ...
                System.out.println("---no litho :" + noLitho);
                System.out.println("---projek kerajaan:" + projekKerajaan);
                System.out.println("---hakmilik permohonan: " + hakmilikPermohonan);
                //hakmilikPermohonanList.add(hakmilikPermohonan);
...
//        hakmilikPermohonan = pelupusanService.findByIdPermohonan(idPermohonan);
            ...
            hp.setKodMilik(kodPemilikan);
            LOG.info("kodmilik in mohon_hakmilik :: " + kodPemilikan);
            pelupusanService.simpanHakmilikPermohonan(hp);
        }
