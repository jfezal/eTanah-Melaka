...
@UrlBinding("/pelupusan/penyediaan_borang5A2")
public class Penyediaan_borang5AActionBean extends AbleActionBean {
...
//    @DefaultHandler
    public Resolution showForm() {
        return new JSP("pelupusan/penyediaan_borang5A2.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate()
    {
        ...
        System.out.println("-------------this is-----------penyediaan_borang5A2");
         String [] bigDecimals = new String[9];
        ...
        while(bigDecimals != null)
        {
            ...
            System.out.println("----------------------jumlah1---------"+bigDecimals[i]);
    ...
    //add by sitifariza.hanim
        public Resolution simpans()
        {
            System.out.println("simpan2()");
            pguna = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
            idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
            String idPemohon = getContext().getRequest().getParameter("pemohon.idPemohon");
            System.out.println("simpan2()1");
            pemohon = pelupusanService.findPemohonByIdPemohon(idPermohonan);
            String[] itemList = {"Cukai Tahun Pertama", "Premium","Upah Ukur/Tanda Sempadan","Kos Sumbangan Infrastruktur","Penyediaan Pelan Suratan Hakmilik Tetap","Pendaftaran Suratan Hakmilik Tetap ","Penyediaan Pelan Suratan Hakmilik Sementara","Pendaftaran Suratan Hakmilik Sementara","Notis"};
            //BigDecimal [] amaunTuntutanList = {cukai, primium, upah, sumbangan, suratanTetap, suratanHakmilikTetap, suratanSementara, pendaftaranTetap, notis};
            jumlah1 =0;
            String [] bigDecimals = new String[9];
            System.out.println("simpan2()2");
            ...
            System.out.println("simpan2()3");
            getContext().getRequest().getSession().setAttribute("values", bigDecimals);
            int i=0;
            etanahActionBeanContext ctx = new etanahActionBeanContext();
            InfoAudit ia = new InfoAudit();
            System.out.println("--------pelupusanService.findPermohonanTuntutanKos------------");
            permohonanTuntutanKoseslist = pelupusanService.findPermohonanTuntutanKos(idPermohonan);
