...
import static etanah.view.stripes.lelong.KeputusanBidaanActionBean.DELIM;
 ...
 */
@UrlBinding("/lelong/keputusan_bidaan")
public class KeputusanBidaanActionBean extends AbleActionBean {

    private static final Logger LOG = Logger.getLogger(KeputusanBidaanActionBean.class);
    @Inject
...
//                }
                if (!senaraifasamohon.isEmpty()) {
                    if (senaraifasamohon.get(0).getKeputusan() == null) {
                        status = null;
                    } else if ("AP".equals(senaraifasamohon.get(0).getKeputusan().getKod())) {
                        status = "AP";
                        LOG.debug("berjaya");
...
//                FasaPermohonan ff = senaraifasamohon.get(0);
    ...
    public Resolution showForm4() {
        ...
        if (enkuiri.getCaraLelong().equals("A")) {
            listATD = lelongService.findDepositH(lelong.getIdLelong());
            listATD2 = lelongService.findDepositT(lelong.getIdLelong());

        } else if (enkuiri.getCaraLelong().equals("B")) {
            listATD = lelongService.findDepositH(enkuiri.getIdEnkuiri());
            listATD2 = lelongService.findDepositT(enkuiri.getIdEnkuiri());
        }
...
//        }
        if ("05".equals(conf.getProperty("kodNegeri"))) {
            ...
            if (idLelong == null || idLelong.isEmpty()) {
                ...
                for (Pembida pl : senaraiPembida) {
                    if (pl != null) {
                        Long idLelong1 = pl.getLelong().getIdLelong();
                        LOG.info("id Lelong dlm table pembida:" + idLelong1);
                        LOG.info("senaraiPembida : " + senaraiPembida.size());
                        Long idPembida = pl.getIdPembida();
                        if ("BJ".equals(pl.getKodStsPembida().getKod())) {
                            pembida = pl;
                            BJ = true;
                        }

//                            if (pl.getKodStsPembida() != null) {
//                                if (pl.getKodStsPembida().getKod().equals("BJ")) {
//                                    pembida = pl;
//                                    LOG.info("id Pembida yg BJ:" + pl.getIdPembida());
//                                    BJ = true;
//                                    LOG.info("hargaRizab:" + pl.getLelong().getHargaRizab());
//                                } else {
//                                    BJ = false;
//                                    LOG.info("id Pembida yg lain dr BJ:" + pl.getIdPembida());
//                                }
////                        break;
//                            }
                    }
...
//            }
            ...
            if (enkuiri.getCaraLelong().equals("A")) {
                ...
                if (!listATD.isEmpty()) {
                    LOG.info("---Masuk listATD not empty---");
                    atd = listATD.get(0);
                    if (atd.getBank() != null) {
                        kodBank = atd.getBank().getKod();
                    }
                ...
                if (!listATD2.isEmpty()) {
                    LOG.info("---Masuk listATD2 not empty---");
                    atd2 = listATD2.get(0);
                    LOG.info("atd : " + atd2.getIdAkuan());
                    if (atd2.getBank() != null) {
                        kodBank2 = atd2.getBank().getKod();
...
//        String idLelong = getContext().getRequest().getParameter("idLelong");
        ...
        if (idPermohonan != null) {
            ...
            if (idLelong != null || idLelong == "") {
                lelong = lelonganDAO.findById(Long.parseLong(idLelong));
            } else {
...
////                        kodJenis = pihak.getJenisPengenalan().getKod();
                    ...
                    long idLelong2 = lelong.getIdLelong();
                    LOG.info("idlelong2= " + idLelong2);
                    BigDecimal baki2 = lelong.getBaki();
...
//            if (enkuiri.getCaraLelong().equals("A")) {
    ...
    public Resolution save() {
        ...
        String idPembida = getContext().getRequest().getParameter("pembida.idPembida");
//        lelong = LelongService.
        ...
        if (!senaraiLelongan2.isEmpty()) {
            LOG.debug("---masuk senarailelongan2 not empty");
...
//            baki = baki.subtract(deposit2);
        ...
        enkuiri.setHargaBida(lelong.getHargaBidaan());
        ...
        if (enkuiri.getCaraLelong().equals("B")) {
            ...
            p.setNegeri(kod2);
            LOG.info("--kod1: " + kod2);
            p.setNama(getContext().getRequest().getParameter("pihak.nama"));
            LOG.info("--kod2: " + kod2);
            p.setNoPengenalan(getContext().getRequest().getParameter("pihak.noPengenalan"));
            LOG.info("--kod3: " + kod2);
            p.setAlamat1(getContext().getRequest().getParameter("pihak.alamat1"));
            LOG.info("--kod4: " + kod2);
            p.setAlamat2(getContext().getRequest().getParameter("pihak.alamat2"));
            LOG.info("--kod5: " + kod2);
            p.setAlamat3(getContext().getRequest().getParameter("pihak.alamat3"));
            LOG.info("--kod6: " + kod2);
            p.setAlamat4(getContext().getRequest().getParameter("pihak.alamat4"));
            LOG.info("--kod7: " + kod2);
            p.setPoskod(getContext().getRequest().getParameter("pihak.poskod"));
            LOG.info("--kod8: " + kod2);
            p.setNoTelefon1(getContext().getRequest().getParameter("pihak.noTelefon1"));
            LOG.info("--kod9: " + kod2);
            p.setNoTelefon2(getContext().getRequest().getParameter("pihak.noTelefon2"));
            LOG.info("--kod10: " + kod2);
            p.setNoTelefonBimbit(getContext().getRequest().getParameter("pihak.noTelefonBimbit"));
            LOG.info("--kod11: " + kod2);
            p.setInfoAudit(info);
            LOG.info("-------NAK SAVE");
            lelongService.saveOrUpdate(p);
...
////        KodJenisPengenalan kodP = kodJenisPengenalanDAO.findById(getContext().getRequest().getParameter("jenis"));
////        p.setJenisPengenalan(kodP);
////        p.setNoPengenalan(pihak.getNoPengenalan());
//        p.setAlamat1(pihak.getAlamat1());
//        LOG.info("alamat: " + pihak.getAlamat1());
...
//        lelongService.saveOrUpdate(p);
        for (Lelongan oo : senaraiLelongan2) {
            ...
            oo.setDeposit(b);
            oo.setBaki(baki);
            oo.setHargaBidaan(lelong.getHargaBidaan());
...
//        enkuiri.setHargaBida(hargaBidaan);
        ...
        enkuiri.setInfoAudit(info);
        lelongService.saveOrUpdate(enkuiri);
        kodNegeri = getContext().getRequest().getParameter("pihak.negeri.kod");
        KodNegeri kod2 = kodNegeriDAO.findById(kodNegeri);
        p.setNegeri(kod2);
//        KodJenisPengenalan jenis = kodJenisPengenalanDAO.findById(kodJenis);
//        p.setJenisPengenalan(jenis);
...
//        }
        ...
        if (bersameke == true) {
            ...
            LOG.info("hargaRizab  : " + hargaRizab);
            ll.setHargaBidaan(lelong.getHargaBidaan());
            ll.setBaki(baki);
        ...
        ia.setDimasukOleh(pengguna);
        ia.setTarikhMasuk(new java.util.Date());
        kodNegeri = getContext().getRequest().getParameter("pembida.pihak.negeri.kod");
        KodNegeri kod2 = kodNegeriDAO.findById(kodNegeri);
        LOG.info("--kod2: " + kod2);
        p.setNegeri(kod2);
//        KodJenisPengenalan jenis = kodJenisPengenalanDAO.findById(pembida.pihak.jenisPengenalan.kod);
//        p.setJenisPengenalan(jenis);
        p.setInfoAudit(ia);
        p.setNama(getContext().getRequest().getParameter("pembida.pihak.nama"));
        p.setNoPengenalan(getContext().getRequest().getParameter("pembida.pihak.noPengenalan"));
...
//        idLelong = String.valueOf(ll.getIdLelong());
    ...
    public Resolution savePembidaMLK() {
        ...
        LOG.info("+++ ID PEMBIDA = " + idPembida);
        LOG.info("+++ pembida = " + pembida.getIdPembida());

        Pembida pem = enService.getPembidaID(pembida.getIdPembida().toString());
        LOG.info("+++ PEM = " + pem.getIdPembida());
...
//            baki = baki.subtract(deposit2);
        ...
        ia.setDimasukOleh(pengguna);
        ia.setTarikhMasuk(new java.util.Date());
        kodNegeri = getContext().getRequest().getParameter("pembida.pihak.suratNegeri.kod");
...
//            pembida = lelongService.getPembida(ll.getIdLelong());
            pembida = pembidaDAO.findById(Long.parseLong(idpem));
            LOG.info("idpem: " + idpem);
            ...
            ll.setPembida(pembida.getPihak());
            ll.setInfoAudit(ia);
...
//            }
    ...
    //created by syazwan
    //for n9, find pembida in table pihak
    public Resolution findPembida() {
        LOG.info("--Masuk findPembida--");
        LOG.debug("idpermohonan=" + idPermohonan);
        String idpem = getContext().getRequest().getParameter("pembida");
        getContext().getRequest().setAttribute("pembida", pembida);
        Long idlel = lelong.getIdLelong();
        LOG.debug("idpem= " + idpem);
        LOG.debug("idLelong= " + idlel);
        Pihak j = null;
        String results = "";

        try {
            j = pihakDAO.findById(Long.parseLong(idpem));
        } catch (Exception ex) {
            LOG.debug(ex);
            return new StreamingResolution("text/plain", results);
        }
        if (j != null) {
            LOG.debug("Pembida found! idJlb=" + j.getIdPihak());
//            LOG.info(j.getAktif());
...
//            String msg = "Jurulelong ini tidak aktif";
            ...
            s.append(j.getNama() != null ? j.getNama() : "").append(DELIM).append(j.getJenisPengenalan() != null ? j.getJenisPengenalan().getKod() : "").append(DELIM).append(j.getNoPengenalan() != null ? j.getNoPengenalan() : "").append(DELIM);
    ...
    //created by syazwan
    //for n9, save pembida, set kod sts to BJ
    //1 hakmilik
    public Resolution savePembidaN9(Long idlel) {
        //Pembida berjaya
        LOG.info("--Masuk savePembidaN9--");
        LOG.debug("idpermohonan=" + idPermohonan);
        LOG.info("---idlel= " + idlel);
        Lelongan lel = lelonganDAO.findById(idlel);
        LOG.info("lel =" + lel);
        Pihak p = pihakDAO.findById(Long.valueOf(getContext().getRequest().getParameter("pembida")));
        LOG.info("idpem =" + p);
        InfoAudit ia = pengguna.getInfoAudit();

        LOG.info("++++ senaraiHM = " + permohonan.getSenaraiHakmilik().size());
//        edited by farizal 14 mei 2015
//        idpihak xsimpan dalam table Lelong jika lebih dr 1 hakmilik (lelongan bersama)
        if (permohonan.getSenaraiHakmilik().size() > 1) {
            if (enkuiri.getCaraLelong().equals("B")) {
                LOG.info("++++ lebih 1 hakmilik ++++");
                List<Lelongan> listLelong = lelongService.listLelonganAK(idPermohonan);

                for (Lelongan listLel : listLelong) {
                    LOG.info("--- LISTLEL = " + listLel.getIdLelong());
                    try {
                        ...
                        listLel.setPembida(p);
                        listLel.setInfoAudit(ia);
                        lelongService.saveOrUpdatee(listLel);
        ...
        LOG.info("++++ lel.idLelong = " + lel.getIdLelong());
        if (permohonan.getSenaraiHakmilik().size() == 1) {
            try {
...
//        Pembida pem = pembidaDAO.findById(Long.valueOf(getContext().getRequest().getParameter("pembida")));
        if (permohonan.getSenaraiHakmilik().size() > 1) {
            if (enkuiri.getCaraLelong().equals("B")) {
                ...
                LOG.info("--2 hm asing--");
//                Pembida pem = pembidaDAO.findById(Long.valueOf(getContext().getRequest().getParameter("pembida")));
                Pembida pem1 = enService.getPembidaID(getContext().getRequest().getParameter("pembida"));
                LOG.info("id Pem laaa :" + pem1.getIdPembida());
//            if (pem1 != null) {
                KodStsPembida ksl = new KodStsPembida();
                ksl.setKod("BJ");
...
//        pembida = lelongService.getPembida(Long.parseLong(idLelong));
//        6 mei 2015
        List<Pembida> senaraiPembida = lelongService.getSenaraiPembidaBerjaya(Long.parseLong(idLelong));
        for (Pembida pembida1 : senaraiPembida) {
            if (pembida1.getKodStsPembida().getKod().equals("BJ")) {
                pembida.equals(pembida1);
            }
        }
        lelong = lelongService.findLelong3(idLelong);
        ...
        if ("04".equals(conf.getProperty("kodNegeri"))) {
            mlk = true;
            List<Pembida> senaraiPembida1 = lelongService.getSenaraiPembidaBerjaya(Long.parseLong(idLelong));
            for (Pembida pembida1 : senaraiPembida1) {
                if (pembida1.getKodStsPembida().getKod().equals("BJ")) {
                    pembida.equals(pembida1);
                }
            }
