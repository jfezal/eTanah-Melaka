...
@UrlBinding("/pengambilan/pengambilan_pemohonTerdahulu")
public class pemohonPermohonanTerdahuluActionBean extends AbleActionBean {

    private static final Logger LOG = Logger.getLogger(pemohonPermohonanTerdahuluActionBean.class);
...
//    private boolean isLayak = true;
    ...
    public Resolution showForm() {
        ...
        return new ForwardResolution("/WEB-INF/jsp/pengambilan/common/maklumat_pemohon_Terdahulu.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate() {
        ...
        if (idSblm != null) {
            ...
            List<HakmilikPermohonan> hakmilikPermohonanList = Sebelum.getSenaraiHakmilik();
            KodNegeri negeriBaru = new KodNegeri();
            KodNegeri negeriLama = new KodNegeri();
            mohonPihakKemaskiniList = Sebelum.getSenaraiPihakKemaskini();
            if (mohonPihakKemaskiniList.size() > 0) {
                mohonPihakKemaskini = Sebelum.getSenaraiPihakKemaskini().get(0);
...
//            if (hakmilikPermohonanList.size() > 0 && hakmilikPermohonanList.size() <= 1) {
            ...
            if (Sebelum.getKodUrusan().getKod().equals("PAADT") || Sebelum.getKodUrusan().getKod().equals("CGADT") || Sebelum.getKodUrusan().getKod().equals("PMADT") || Sebelum.getKodUrusan().getKod().equals("TMADT")) {
                mohonPihakList = permohonanPihakService.getSenaraiPmohonPihakByNotKod(idPermohonan, "WAR");
            } //for urusan consent serentak
            else if (Sebelum.getKodUrusan().getKod().equals("PCPTD") || Sebelum.getKodUrusan().getKod().equals("PGDMB") || Sebelum.getKodUrusan().getKod().equals("PCMMK")) {
                mohonPihakList = permohonanPihakService.getSenaraiPmohonPihakByNotKod(idPermohonan, "PGA");
                penerimaGadaianList = permohonanPihakService.getSenaraiPmohonPihakByKod(idPermohonan, "PGA");
            } else {
                mohonPihakList = Sebelum.getSenaraiPihak();
            }

            pemohonList = Sebelum.getSenaraiPemohon();
    ...
    public Resolution deleteSelectedPemohon() {
        ...
        if (StringUtils.isNotBlank(idHakmilik)) {
            return new RedirectResolution(pemohonPermohonanTerdahuluActionBean.class, "doGetPartialPage").addParameter("idHakmilik", idHakmilik);
        } else {
            return new RedirectResolution(pemohonPermohonanTerdahuluActionBean.class, "getSenaraiHakmilikKepentingan");
        }
    }
...
//                    permohonanPihak.setSyerPenyebut(f.getDenominator());
    ...
    public Resolution refreshpage() {
        rehydrate();
        return new RedirectResolution(pemohonPermohonanTerdahuluActionBean.class, "locate");
    }
