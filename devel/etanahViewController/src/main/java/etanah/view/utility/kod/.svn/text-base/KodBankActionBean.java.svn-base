...
@UrlBinding("/utility_kod/kodBank")
public class KodBankActionBean extends AbleActionBean {
    ...
    private int id_rekod_bank;
    private List<KodBank>listKodBank;
    private KodBank details;
    ...
    public Resolution showForm() {
        return new JSP("/utiliti/utiliti_kod_ruj/kod_bank/listKodBank.jsp");
    }
    
     public Resolution KodList() throws Exception {
         nameTable = getContext().getRequest().getParameter("table_name");
     
        listKodBank =service.allKodBank();
        
         
         if(listKodBank.size()>0){
             setFlag(true);
             addSimpleMessage(listKodBank.size()+" kod ditemui.");
        ...
        return new JSP("/utiliti/utiliti_kod_ruj/kod_akaun/listKodAliran.jsp");
        
    }
   
       public Resolution KodList2() throws Exception {
         
         cawangan = getContext().getRequest().getParameter("cawangan");
         
        
          if(StringUtils.isBlank(cawangan)||cawangan.equals("0")){
             cawangan = null;
         }
            if(StringUtils.isBlank(nama)||nama.equals("0")){
             nama = null;
         }
         listKodBank =service.findKodBank2(cawangan, nama);
   ...
   public Resolution viewAddForm() throws Exception{
        ...
        nameTable = getContext().getRequest().getParameter("table_name");
       id_rekod_bank = service.countKodBank();
        return new JSP("/utiliti/utiliti_kod_ruj/kod_bank/tambahRekodBank.jsp");
   }
    
   public Resolution viewKodBank()throws Exception{
       nameTable = getContext().getRequest().getParameter("table_name");
        kod = getContext().getRequest().getParameter("kod");
       details = service.viewKodBank(kod);
       
       
       return new JSP("/utiliti/utiliti_kod_ruj/kod_bank/kemaskiniKodBank.jsp");
   }
   ...
   public Resolution updateData() throws Exception {
       ...
       // String query="update " + nameTable + " set kod_cawangan= '" +"" + "', id_aliran= '"++"',nama= '"+details.getNama()+"',keterangan_ringkas= '"+details.getKeteranganRingkas()+"',AKTIF ='"+details.isAktif()+"'DIKKINI ='"+pguna.getIdPengguna()+"', TRH_KKINI = TO_DATE('"+ds+"','dd/MM/yyyy HH24:mi:ss'), WHERE KOD = '" + kod + "'";
   ...
   */
    ...
    public KodBank getDetails() {
        return details;
    }

    public void setDetails(KodBank details) {
    ...
    public int getId_rekod_bank() {
        return id_rekod_bank;
    }

    public void setId_rekod_bank(int id_rekod_bank) {
        this.id_rekod_bank = id_rekod_bank;
    }

    public List<KodBank> getListKodBank() {
        return listKodBank;
    }

    public void setListKodBank(List<KodBank> listKodBank) {
        this.listKodBank = listKodBank;
    }
