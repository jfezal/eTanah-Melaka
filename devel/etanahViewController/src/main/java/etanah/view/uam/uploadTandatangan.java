...
@UrlBinding("/uam/uploadTT")
public class uploadTandatangan extends AbleActionBean {
    ...
    public Resolution showForm() {
        ...
        findKodNegeri();
        return new JSP("uam/uploadTT.jsp");
    }
...
//        return new ForwardResolution(NewAndUpdateActionBean.class,"noEdit");
    ...
    public Resolution processUpload() throws Exception {

        if (fileToBeUpload != null) {
            try {
                Blob imej = Hibernate.createBlob(fileToBeUpload.getInputStream());
                Pengguna pguna1 = penggunaDAO.findById(pguna.getIdPengguna());
                pguna1.setImejTT(imej);
                service.savePengguna(pguna1);

            } catch (Exception e) {
                e.printStackTrace();
                addSimpleError("Tanda Tidak Tangan Berjaya Disimpan");
            }

            addSimpleMessage("Tanda Tangan Berjaya Disimpan");
        } else {
            addSimpleError("Sila masukan tandatangan");
        }
    ...
    public Resolution searchUser() throws Exception {
        ...
        if (idParam != null) {
            ...
            return new ForwardResolution("/WEB-INF/jsp/uam/uploadTT.jsp");
        }
...
//         String idParam = getContext().getRequest().getParameter("idParam");
//        viewTT();
        return new ForwardResolution("/WEB-INF/jsp/uam/uploadTT.jsp").addParameter(idParam, pguna.getIdPengguna());
    }
