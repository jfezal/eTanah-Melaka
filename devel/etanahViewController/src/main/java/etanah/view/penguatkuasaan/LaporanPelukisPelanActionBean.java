 ...
 * @author nursyazwani
 */
@UrlBinding("/penguatkuasaan/laporan_pelukisPelan")
public class LaporanPelukisPelanActionBean extends AbleActionBean {
    ...
    private String kodT;
    private String noWartaTanah;

    @DefaultHandler
    public Resolution showForm() {
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new JSP("penguatkuasaan/enf_laporan_pelukis_pelan.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution tanahRizabPopup() {
        return new JSP("penguatkuasaan/tanah_rizab_add.jsp").addParameter("popup", "true").addParameter("showForm", "false");
    }

    public Resolution tanahMilikPopup() {
        return new JSP("penguatkuasaan/tanah_milik_add.jsp").addParameter("popup", "true").addParameter("showForm", "false");
    }

    public Resolution permohonanTerdahuluPopup() {
        return new JSP("penguatkuasaan/permohonan_terdahulu_add.jsp").addParameter("popup", "true").addParameter("showForm", "false");
    }
...
//           tanahrizabpermohonan=tanahRizabService.findByidPermohonan(idPermohonan);
            ...
            if (permohonanLaporanPelan != null) {
                ...
                noWartaTanah = permohonanLaporanPelan.getNoWarta();
            }
...
//            }
    ...
    public Resolution showEditTanahRizab() {
        ...
        return new JSP("penguatkuasaan/tanah_rizab_edit.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution showEditPermohonanTerdahulu() {
        ...
        return new JSP("penguatkuasaan/permohonan_terdahulu_edit.jsp").addParameter("popup", "true");
    }
    ...
    public Resolution simpanTanahMilik() throws ParseException {
        ...
        if (idPermohonan != null && hakmilik != null) {
            tanahRizabList = new ArrayList<TanahRizabPermohonan>();
            hakmilikPermohonan = new HakmilikPermohonan();
            ...
            //Hafifi 10/4/2014 : cari semua maklumat rizab dari urusan IRM dan MAJB
            if (hakmilikPermohonan != null) {
                KodRizab kodRizab = null;

                List<HakmilikUrusan> hakmilikUrusanList = enforcementService.findHakmilikUrusanActiveByIdHakmilikAndUrusan(hakmilikPermohonan.getHakmilik().getIdHakmilik(), "IRM");

                if (hakmilikUrusanList != null || !hakmilikUrusanList.isEmpty()) {
                    kodRizab = kodRizabDAO.findById(3); //Rizab Melayu

                    for (HakmilikUrusan hakmilikUrusan : hakmilikUrusanList) {

                        List<PermohonanRujukanLuar> mohonRujLuarList = enforcementService.findByIDPermohonan(hakmilikUrusan.getIdPerserahan());

                        if (mohonRujLuarList != null || !mohonRujLuarList.isEmpty()) {
                            for (PermohonanRujukanLuar rujukanLuar : mohonRujLuarList) {
                                tanahrizabpermohonan = new TanahRizabPermohonan();
                                tanahrizabpermohonan.setInfoAudit(info);
                                tanahrizabpermohonan.setPermohonan(permohonan);
                                tanahrizabpermohonan.setTarikhWarta(rujukanLuar.getTarikhRujukan());
                                tanahrizabpermohonan.setBandarPekanMukim(hakmilikUrusan.getHakmilik().getBandarPekanMukim());
                                tanahrizabpermohonan.setDaerah(hakmilikUrusan.getHakmilik().getDaerah());
                                tanahrizabpermohonan.setCawangan(hakmilikUrusan.getCawangan());
                                tanahrizabpermohonan.setNoLot(hakmilikUrusan.getHakmilik().getNoLot());
                                tanahrizabpermohonan.setNoWarta(rujukanLuar.getNoRujukan());
                                tanahrizabpermohonan.setRizab(kodRizab);
        ...
        return new ForwardResolution("/WEB-INF/jsp/penguatkuasaan/enf_laporan_pelukis_pelan.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution deleteTanahMilik() {
        ...
        if (hakmilikPermohonan != null) {
            ...
            //hakmilikPermohonan.setHakmilik(hakmilik);
            PermohonanLaporanKawasan laporanKawasan = pelupusanService.findByidMohon(hakmilikPermohonan.getPermohonan().getIdPermohonan());
            if(laporanKawasan != null){
                pelupusanService.deletePermohonanLaporanKwsn(laporanKawasan);
            }
...
//        String kodTanah = mohon.getSenaraiHakmilik().get(0).getHakmilik().getKodTanah().getKod();
        ...
        if (permohonanLaporanPelan != null) {
            ...
            permohonanLaporanPelan.setNoWarta(noWartaTanah);

            if (!kodT.equalsIgnoreCase("0")) {
                KodTanah kodTanah = new KodTanah();
                kodTanah.setKod(kodT);
                permohonanLaporanPelan.setKodTanah(kodTanah);
            } else {
                permohonanLaporanPelan.setKodTanah(null);
            }
...
//        }
    ...
    public String getNoWartaTanah() {
        return noWartaTanah;
    }

    public void setNoWartaTanah(String noWartaTanah) {
        this.noWartaTanah = noWartaTanah;
    }
}
