 ...
 */
public class InitiateN7ABValidator implements StageListener {
    ...
    private static final Logger LOG = Logger.getLogger(InitiateN7ABValidator.class);
            ...
            if ("04".equals(conf.getProperty("kodNegeri"))) {
                if (permohonan.getKodUrusan().getKod().equalsIgnoreCase("127")) {
                    if (keputusan.equalsIgnoreCase("RE")) {
                        initiateN7AB(permohonan, peng);
                        if (!senaraiHakmilik.isEmpty()) {
                            //Need To generate before Noting
                            LOG.info("buat urusan tukar ganti 7AB");
...
//        return null;
    ...
    public void initiateN7AB(Permohonan permohonan, Pengguna pengguna) {
        LOG.info("-------initiateN7AB-------");
        Date now = new Date();
        ...
        KodUrusan kod = kodUrusanDAO.findById("N7AB");
