 ...
 *
 */
public class KemasukanPermohonanSBMSNotification implements StageListener {
...
//        list.add(kodPerananDAO.findById("9"));
    ...
    void initiate(StageContext context, String proposedOutcome) {
        ...
        List<Hakmilik> senaraiHakmilikPNDFTR = new ArrayList<Hakmilik>();
         ...
         List<Hakmilik> senaraiHakmilikLanjutPTD = new ArrayList<Hakmilik>();
        List<Hakmilik> senaraiHakmilikLanjutPNDFTR = new ArrayList<Hakmilik>();
        for (int i = 0; i < senaraiHP.size(); i++) {
            ...
            if (h.getKodHakmilik().getKod().equals("GRN") || h.getKodHakmilik().getKod().equals("PN") || h.getKodHakmilik().getKod().equals("HSD")) {
                ...
                if(hp.getLanjutTempoh().equals("Y")){
                    senaraiHakmilikLanjutPNDFTR.add(h);
                }
            } else {
                comparePTD = true;
                senaraiHakmilikPTD.add(h);
                if(hp.getLanjutTempoh().equals("Y")){
                    senaraiHakmilikLanjutPTD.add(h);
                }
            }
        }
...
//            }
        ...
        if (!senaraiHakmilikLanjutPNDFTR.isEmpty()) {
                            KodUrusan k = kodUrusanDAO.findById("PSPM");

                generateIdPerserahanWorkflow.generateIdPerserahanWorkflow_Integration(k, peng, senaraiHakmilikLanjutPNDFTR, permohonan);
            }
        if (!senaraiHakmilikLanjutPTD.isEmpty()) {
                            KodUrusan k = kodUrusanDAO.findById("PSPM");

                generateIdPerserahanWorkflow.generateIdPerserahanWorkflow_Integration(k, peng, senaraiHakmilikLanjutPTD, permohonan);
            }
