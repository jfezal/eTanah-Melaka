 ...
 * @author mohd.faidzal
 */
public class IntegrasiPendaftaranService {
    ...
    public Permohonan initiate(Permohonan permohonan, KodUrusan ku, String stageName) throws StaleObjectException, Exception {
        Session s = sessionProvider.get();
        Transaction txn = s.beginTransaction();
        Permohonan m = createPermohonanBaru(permohonan, ku, stageName);
        setHakmilikPermohonan(permohonan, m);
        setMohonRujukanLuar(m, permohonan.getIdPermohonan(), "FL");
        txn.commit();
        return m;
    }
...
//        mohonRujukLuar.setNoFail(permohonanBaru.getIdPermohonan());
    ...
    public void hantarNotifikasi(String tajuk, String mesej, KodCawangan caw, String kumpBpel) {
         Session s = sessionProvider.get();
        Transaction txn = s.beginTransaction();
        InfoAudit ia = new InfoAudit();
        ia.setDimasukOleh(pengguna);
        ia.setTarikhMasuk(new Date());
        for (Pengguna p : listPgunaPeranan(kumpBpel, caw.getKod())) {
            Notifikasi notifikasi = new Notifikasi();
            notifikasi.setTajuk(tajuk);
            notifikasi.setMesej(mesej);
            notifikasi.setCawangan(caw);
    ...
    public List<Pengguna> listPgunaPeranan(String kumpBpel, String kodCaw) {
        String query = "SELECT b FROM etanah.model.Pengguna b,etanah.model.PenggunaPeranan p, etanah.model.KodPeranan k "
                + "where b.kodCawangan.kod = :kodCaw "
                + "AND b.idPengguna = p.pengguna.idPengguna "
                + "and p.peranan.kod = k.kod "
                + "and k.kumpBPEL = :kumpBpel";
        Session session = sessionProvider.get();
        Query q = session.createQuery(query);
        q.setString("kumpBpel", kumpBpel);
