 ...
 * @author mohd.faidzal
 */
public class ReportUtilB4K implements Report4KGenerator, ReportUtilMBean {
...
//    }
    @Override
    public Form4k generateReport(String reportName, String[] params, String[] values, String outputPath, Pengguna pengguna, Long idDokumen) {
        Form4k form = new Form4k();
...
//            cmd.append("&").append(params[i]).append("=").append(values[i]).append(" ");
        ...
        try {
            ...
            if (isDebug) {
                LOG.debug("Report " + cmd.toString() + " completed in " + (System.currentTimeMillis() - start + "ms"));
            }
...
//            if (httpConn.getContentType() != null) contentType = httpConn.getContentType().getValue();
            ...
            String DMS_PATH;
            if (pengguna != null) {
                DMS_PATH = dmsUtil.getDMSPath(pengguna);
            
                DMSPATH_WO_DMSBASE = dmsUtil.getFizikalPath(pengguna);
               
            } else {
                throw new RuntimeException("Please pass Pengguna to create a valid DMS directory structure, if you want to stream the result, set outputPath to null");
            }
...
//            f.saveFile(reportName.substring(0, reportName.indexOf(".")), is);
        ...
        form.setDokumenId(idDokumen);
        form.setDokumenPath(getDMSPath());
        return form;
    }
    ...
    public byte[] getReports(String reportName, String params) {
        ...
        if (isDebug) {
            LOG.debug("Report completed in " + (System.currentTimeMillis() - start) + "ms");
        }
        return bytes;
    }
