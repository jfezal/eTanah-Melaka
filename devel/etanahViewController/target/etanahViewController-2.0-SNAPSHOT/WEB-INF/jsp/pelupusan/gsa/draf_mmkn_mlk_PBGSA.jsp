...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        for (var i=0; i<=len; i++){
            $('.hakmilik'+i).click( function() {
                window.open("${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?popupHakmilikDetail&idHakmilik="+$(this).text(), "eTanah",
    ...
    function menyimpan(index,i,f){
        ...
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?simpanKandungan&index='+index+'&kandungan='+kand,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    ...
    {
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?tambahRow&index='+index,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
    function deleteRow(idKandungan,f)
    {
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?deleteRow&idKandungan='+idKandungan,q,
            function(data){
                $('#page_div').html(data);
            }, 'html');
        }
    ...
    function cariPopup(){
        var url = '${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?search';
        window.open(url, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=700 scrollbars=yes");
    }
    function cariPopup2(){
        var url = '${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?showFormCariKodSekatan';
        window.open(url, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=700 scrollbars=yes");
    }
    function semakSyor(f,v){
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmkn_mlk_PBGSA?showsyortolaklulus&syortolaklulus='+v,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
</script>
<s:form beanclass="etanah.view.stripes.pelupusan.DrafMMKN_PBGSAActionBean">
    <s:messages/>
    <s:errors/>
          ...
          <tr>
           ...
           <c:set var="i" value="0" />
            ...
            <c:if test="${actionBean.editPTD}">
                <tr>
                    <td width="1%"><b><c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PBGSA'}"> 5.</c:if>
                                  </b>
                    </td>
    ...
    <c:if test="${actionBean.openPTG}">
        ...
        <table width="90%" border="0">
            <c:if test="${actionBean.editPTG}">
                <tr>
                    <td width="1%"><b><c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PBGSA'}">6.</c:if>
                                  </b>
                    </td>
              ...
              <c:if test="${!actionBean.editPTG}">
                <tr>
                    <td width="1%"><b>
                                      <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PBGSA'}"> 6</c:if>
