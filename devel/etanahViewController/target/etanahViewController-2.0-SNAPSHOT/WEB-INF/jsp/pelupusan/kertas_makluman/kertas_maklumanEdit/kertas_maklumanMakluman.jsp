<%-- 
    Document   : kertas_maklumanMakluman
    Created on : Apr 23, 2013, 4:34:09 PM
    Author     : afham
--%>
...
<script type="text/javascript">
    ...
    {   
        NoPrompt();
        var url = '${pageContext.request.contextPath}/pelupusan/kertas_makluman_mmkV2?tambahRow&index='+index;
        window.open(url, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
    }
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_makluman_mmkV2?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kMakluman',q,
            function(data){
                $('#page_div').html(data);

            }, 'html');
            self.refreshpage2('kMakluman') ;
        }
    }
    ...
    function refreshpage(){
        //        alert('aa');
        NoPrompt();
        opener.refreshV2KertasMaklumanMMK('main');
        self.close();
    }
...
<body>
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.KertasMaklumanMMKActionBean" name="form">
        <s:errors/>
        <s:messages/>    
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>Ulasan Pengarah Tanah dan Galian</legend>
                <table class="tablecloth" border="0" align="center">
                    ...
                    <c:forEach items="${actionBean.senaraiKandunganMakluman}" var="line">
                        <tr>
                            ...
                            <td>
                                <s:textarea  id="kandungan5${i}" name="senaraiKandunganMakluman[${i-1}].kandungan" cols="80"  rows="5" onclick="checkKandungan(${i})" onblur="setKandungan(${i},${line.idKandungan})" class="normal_text"/>
