...
function removeKod(val){
        //alert("==="+val);
        if(confirm('Adakah pasti untuk hapus Kod :"'+val+'" dalam senarai?')) {
            var url = '${pageContext.request.contextPath}/hasil/kodSekolahBantuan?deleteKod&kod=' + val;
            $.get(url,
            function(data){
                $('#page_div').html(data);
                refreshPage3();
     ...
     function editKodSekolah(id){
     //alert("editKodSekolah");
        window.open("${pageContext.request.contextPath}/hasil/kodSekolahBantuan?showEditKod&kod="+id, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
    }
     function create(){
        window.open("${pageContext.request.contextPath}/hasil/kodSekolahBantuan?createKod", "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
    }
...
<s:form name="f1" beanclass="etanah.view.stripes.hasil.KodSekolahBantuanActionBean">
    <s:errors/>
    <s:messages/>
    <div class="subtitle">
              ...
              <display:column title="Kemaskini" style="">
                    <div align="center">
                        <img alt='Klik Untuk Kemaskini' border='0' src='${pageContext.request.contextPath}/pub/images/edit.gif' class='rem'
                             id='rem_${row_rowNum}' onclick="editKodSekolah('${row.kod}');return false;" onmouseover="this.style.cursor='pointer';">
                    </div>
              </display:column>
             <display:column title="Hapus">
                    <div align="center">
                        <img alt='Klik Untuk Hapus' border='0' src='${pageContext.request.contextPath}/images/not_ok.gif' class='rem'
                             id='rem_${row_rowNum}' onclick="removeKod('${row.kod}')">
                    </div>
             </display:column>
            </display:table>
            </fieldset>
