...
<script type="text/javascript">
    ...
    function viewLaporTanah(f, id1, id2, id3){
        var queryString = $(f).formSerialize()
        window.open("${pageContext.request.contextPath}/hasil/laporan_tanah_hasil?showForm2&"+queryString+"&idHakmilik="+id1+"&id="+id2+"&idPermohonan="+id3, "eTanah",
                ...
                <div class="content" align="center">
                    <display:table class="tablecloth" name="${actionBean.senaraiHakmilikPermohonan}"
                        ...
                        <display:column title="Hutang Tahun-Tahun Lepas (RM)" class="${line_rowNum}">
                            ...
                            <div align="center"><c:out value="${minTahun}"/><c:if test="${bilTahun > 0}"> - <c:out value="${maxTahun}"/></c:if><br>------------<br>
                       ...
                       <%--     <display:column title="Penyediaan Laporan Tanah">
                                ...
                                <c:if test="${!actionBean.flag}">
                                    <s:button name=""  onclick="edit('this.form','${line.hakmilik.idHakmilik}', '${line.id}','${actionBean.permohonan.idPermohonan}','${line.idLaporan}');" value="Laporan Tanah" class="longbtn"/>
                                </c:if>
                                <c:if test="${actionBean.flag}">
                                    <s:button name=""  onclick="viewLaporTanah('this.form','${line.hakmilik.idHakmilik}', '${line.id}','${actionBean.permohonan.idPermohonan}');" value="Laporan Tanah1" class="longbtn"/>
                                </c:if>
                        </display:column>--%>
                    <%--disabled="true"--%>
                         <display:column title="Penyediaan Laporan Tanah">
