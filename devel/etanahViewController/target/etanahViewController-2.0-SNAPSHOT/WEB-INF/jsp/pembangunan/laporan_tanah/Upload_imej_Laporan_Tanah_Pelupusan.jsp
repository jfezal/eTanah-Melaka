...
<script type="text/javascript">
    ...
    function refreshlptn(){
        var url = '${pageContext.request.contextPath}/pembangunan/laporanTanahv2?refreshpage';
        $.get(url,
        function(data){
            $('#page_div').html(data);
    ...
    function openFrame(type){        
        ...
        NoPrompt();
        //    alert(idHakmilik);
        window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?openFrame&idHakmilik="+idHakmilik+"&type="+type, "eTanah",
    ...
    <c:choose>
        ...
        function refreshpage2(type,pandanganImej,idLapor, idHakmilik, idlaporTnhSmpdn){
            //        alert(pandanganImej);
            //        alert(type);
            //        alert(idLapor);
            var url = '${pageContext.request.contextPath}/pembangunan/laporanTanahv2?refreshpageUpload&type='+type+'&pandanganImej='+pandanganImej+'&idLapor='+idLapor+'&idHakmilik='+idHakmilik+'&idlaporTnhSmpdn='+idlaporTnhSmpdn;
        ...
        function deleteImage(idImej, idDokumen, type,f) {
            ...
            if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
                var q = $(f).formSerialize();                
                $.post('${pageContext.request.contextPath}/pembangunan/laporanTanahv2?deleteImage',{idImej:idImej,idDokumen:idDokumen,type:type,imageType:imageType,idLapor:idLapor,idlaporTnhSmpdn:idlaporTnhSmpdn},q,
                function(data){
                    $('#page_div').html(data);

                }, 'html');

                self.refreshpage2('imgPopup',imageType,idLapor,idHakmilik,idlaporTnhSmpdn) ;
            }
        }
</script>
