<%-- 
    Document   : bayaranPerihalTsMelaka - Refer BayaranPerihalPembangunanActionBean.java
    Created on : Oct 24, 2013, 11:22:58 AM
...
<script type="text/javascript" language="javascript" >
    ...
    function changeFormat2(objId) {
        ...
        if (isNaN(parseFloat(val)))
            ...
            $(objId).val(parseFloat(val).toFixed(2));

        // update total
        var pelepasan = 0;
        if ($('#pelepasan').val() != "" && !isNaN($('#pelepasan').val()))
            pelepasan = -1 * parseFloat($('#pelepasan').val());
...
<s:messages />
<s:errors />
<s:form beanclass="etanah.view.stripes.pembangunan.bayaranPerihalTsActionBean">
    ...
    <p align="center">        
        ...
        <c:if test="${buttonTS}">
            ...
            <c:if test="${check76hari && !checkSuratPeringatan && !check90hari}">                
                <s:submit name="cetakSuratPeringatan" value="Sedia Surat Peringatan" class="btn" onclick="" />  
            </c:if>

            <c:if test="${check90hari}">       
                
                    <s:submit name="arahanBatal" value="Arahan Pembatalan" class="longbtn" onclick="" />
                    <em>Peringatan</em><em>*</em>Arahan Pembatalan Telah Dikeluarkan Akibat Lewat Tempoh Pembayaran<em>*</em>
                </c:if>
            </c:if>
