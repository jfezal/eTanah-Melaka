...
<script type="text/javascript">
    ...
    function popup(b,p,idT){
        ...
        window.open("${pageContext.request.contextPath}/strata/pbbs/mlk?showForm2&namaBangunan="+b+"&tingkat="+p+"&idTingkat="+idT, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=800,height=250");
    }

    function popupPetakAksesori(b,t,p){

        window.open("${pageContext.request.contextPath}/strata/pbbs/mlk?showForm3&idBangunan="+b+"&idTingkat="+t+"&idPetak="+p, "eTanah",
    ...
    function removePtkAksr(val){
        ...
        if(confirm('Adakah anda pasti?')) {
            var url = '${pageContext.request.contextPath}/strata/pbbs/mlk?deletePtkAksr&idAksesori='+val;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
    ...
    function selectedBangunan(idBangunanVal){
        ...
        var url = '${pageContext.request.contextPath}/strata/pbbs/mlk?selectedBangunan&idBangunan='+idBangunanVal;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function selectedTingkat(idBangunanVal,idTingkatVal){
        ...
        var url = '${pageContext.request.contextPath}/strata/pbbs/mlk?selectedTingkat&idBangunan='+idBangunanVal+'&idTingkat='+idTingkatVal;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function selectedPetak(idBangunanVal,idTingkatVal,idPetakVal){
        ...
        var url = '${pageContext.request.contextPath}/strata/pbbs/mlk?selectedPitak&idBangunan='+idBangunanVal+'&idTingkat='+idTingkatVal+"&idPetak="+idPetakVal;
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
...
<s:form beanclass="etanah.view.strata.MaintainBangunanpbbsActionBean">
    <s:messages/>
    <s:errors/>
    <c:if test="${manual}">
        <c:if test="${actionBean.stageId eq 'kemasukan'}">
            <p><font color="red">Sila klik butang 'Hapus Jadual Petak' di tab Jadual Petak Manual terlebih dahulu.</font></p>
        </c:if>
        <c:if test="${actionBean.stageId ne 'kemasukan'}">
            <p><font color="red">Tab ini hanya berfungsi sekiranya menggunakan fail XML(Jadual Petak).</font></p>
        </c:if>
    </c:if>
    <c:if test="${!manual}">
        <table><tr><td>
                    ...
                    <c:if test="${fn:length(actionBean.pBangunanL) > 0}">
                        ...
                        <div class="subtitle">
                            ...
                            <div class="subtitle">
                                <fieldset class="aras1">
                                    <c:if test="${ekstrak}">
                                        <c:if test="${fn:length(actionBean.pBangunanL) > 0 || fn:length(actionBean.pBangunanProvisionalBlock) > 0}">
