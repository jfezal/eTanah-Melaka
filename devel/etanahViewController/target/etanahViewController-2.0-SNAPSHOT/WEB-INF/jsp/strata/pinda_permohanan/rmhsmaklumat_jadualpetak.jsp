...
<script type="text/javascript">
    ...
    function popup(b,p,idT){
        ...
        window.open("${pageContext.request.contextPath}/strata/RMHSUrusan_rmhs?showForm2&namaBangunan="+b+"&tingkat="+p+"&idTingkat="+idT, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=800,height=250");
    }

    function popupPetakAksesori(b,t,p){

        window.open("${pageContext.request.contextPath}/strata/RMHSUrusan_rmhs?showForm3&idBangunan="+b+"&idTingkat="+t+"&idPetak="+p, "eTanah",
    ...
    --%>   function removePtkAksr(val){
        ...
        if(confirm('Adakah anda pasti?')) {
            var url = '${pageContext.request.contextPath}/strata/RMHSUrusan_rmhs?deletePtkAksr&idAksesori='+val;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<%--        var nama = document.form1.nama.value;
    ...
    function selectedBangunan(idBangunanVal){
        <%--alert(imgObj);
        alert(imgObj.src);--%>
      <%--  imgObj.src = '${pageContext.request.contextPath}/images/kurang.png';--%>

        <%--alert(idBangunanVal);--%>
        document.getElementById("tempIdBangunan").value = idBangunanVal;

        var url = '${pageContext.request.contextPath}/strata/RMHSUrusan_rmhs?selectedBangunan&idBangunan='+idBangunanVal;
...
function selectedTingkat(idBangunanVal,idTingkatVal){
      ...
      alert("idTingkatVal:"+idTingkatVal);--%>
        ...
        var url = '${pageContext.request.contextPath}/strata/RMHSUrusan_rmhs?selectedTingkat&idBangunan='+idBangunanVal+'&idTingkat='+idTingkatVal;
            $.get(url,
            function(data){
            $('#page_div').html(data);
            },'html');
...
function selectedPetak(idBangunanVal,idTingkatVal,idPetakVal){
       <%-- document.getElementById("tempIdTingkat").value = idTingkatVal;--%>
        ...
        var url = '${pageContext.request.contextPath}/strata/RMHSUrusan_rmhs?selectedPitak&idBangunan='+idBangunanVal+'&idTingkat='+idTingkatVal+"&idPetak="+idPetakVal;
            $.get(url,
            function(data){
            $('#page_div').html(data);
            },'html');
        ...
        <table><tr><td>
<s:form  name="form1" beanclass="etanah.view.strata.RMHSMaintainBangunanActionBean">
   <s:messages/>
    <s:errors/>
    <br>
                                ...
                                <c:if test="${fn:length(tgkt.senaraiPetak) > 0}">
                                   ...
                                   <c:if test="${actionBean.tempIdTingkat eq tgkt.idTingkat}">   <%--added for Tingkat--%>
                                    ...
                                    <c:forEach items="${tgkt.senaraiPetak}" var="petak" varStatus="statusP">
                                        ...
                                        <td>${petak.nama}</td>
                                        <td><s:text name="petakLuas[${items}]"  size="10" readonly="readonly" /></td>
                                        <td><s:text name="petakSyer[${items}]" size="12" readonly="readonly" /></td>
...
<c:if test="${actionBean.petakKegunaan[rowNo] eq 'CBU11'}">
                                            ...
                                            <%--<s:select name="petakKegunaan[${items}]" style="width:100px">
                                       ...
                                       <td>
                                        ...
                                        <c:forEach items="${petak.senaraiPetakAksesori}" var="petakAksesori" varStatus="statusPA">
                                          ...
                                          <c:set var="rowNo" value="${items}" />
                                            <%--${actionBean.petakKegunaanAksr[rowNo]}--%>
                                             ...
                                             <c:if test="${actionBean.petakKegunaanAksr[rowNo] eq '01'}">
                                                <c:set var="petakKegunaanAksrVal" value="Tempat Letak Kendera" />
                                             </c:if>
                                             <c:if test="${actionBean.petakKegunaanAksr[rowNo] eq '03'}">
                                                <c:set var="petakKegunaanAksrVal" value="Stor" />
                                             </c:if>

                                          <s:text name="petakKegunaanAksrVal"  value="${petakKegunaanAksrVal}" readonly="readonly" />
                                         ...
                                         ${actionBean.petakKegunaanAksr[rowNo]}
                                            ...
                                            <s:text name="lokasiAksrVal"  value="${actionBean.lokasiAksr[rowNo]}" readonly="readonly" />
                                    ...
                                    <c:if test="${ statusP.last}">
                                       <td>&nbsp;</td> <%-- code added for Bangunan--%>
                                        <td>&nbsp;</td>
                                        <td>
                                         <%--<s:button  onclick="popup('${bgn.nama}','${tgkt.tingkat}','${tgkt.idTingkat}');" value="Tambah Petak" name="tambahPetak" class="btn" />--%></td>
                                        <td>&nbsp;</td><%-- code added for Tingkat--%>
                                        <td>&nbsp;</td>
                                        <td>&nbsp;</td>
                                        <td>&nbsp;</td>
