...
<script type="text/javascript">
  ...
  function viewUrusan(id, id2) {
    window.open("${pageContext.request.contextPath}/daftar/pembetulan/betul?viewUrusanDetail&idUrusan=" + id + "&idMohon=" + id2, "eTanah",
  ...
  function silap() {
    ...
    var url = '${pageContext.request.contextPath}/daftar/pembetulan/betul?saveSilapEndosan' + param;
    $.ajax({
      type: "GET",
      url: url,
      dataType: 'html',
  ...
  function removeChanges(val) {
    ...
    if (answer) {          
      var url = '${pageContext.request.contextPath}/daftar/pembetulan/betul?deleteChanges2&idUrusan=' + val;
      $.get(url,
              function(data) {
                $('#page_div').html(data);
              });
...
<s:form name="form1" beanclass="etanah.view.stripes.nota.pembetulanActionBean">
  ...
  <div class="subtitle">
    <fieldset class="aras1">
      <legend>
        Maklumat Silap Endosan Atas Hakmilik
      </legend>
      ...
      <p>
        <label>&nbsp;</label>
        <s:button name="saveSilapEndosan" value="Pilih" class="btn" onclick="silap();"/>
      </p>
      <br/>

      <c:if test="${fn:length(actionBean.mohonAtasUrusan) > 0}">
        <legend>
          Urusan Pembetulan Atas Hakmilik
        </legend>
        <p style="color:red">
          *Sila Klik Pada Nama Urusan Untuk Pembetulan Id Hakmilik.
        </p>
        <br>
        <div class="content" align="center">          
          <display:table class="tablecloth" name="${actionBean.mohonAtasUrusan}" cellpadding="0" cellspacing="0" id="line">
            ...
            <display:column   title="Nama Urusan" >                     
              <a href="#" onclick="viewUrusan('${line.urusan.idUrusan}', '${line.urusan.idPerserahan}');
    return false;">${line.urusan.kodUrusan.nama}</a>
            </display:column>
