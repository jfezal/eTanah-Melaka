...
<script>
  ...
  function dodacheck(val) {
    ...
    if (v == 'B') {
      ...
      // for consent
      if (lchar== '-'){
          var str = val.substring(0, (strLength) - 1);
          $('#penyerahNoPengenalan').val(str+"-");
      }
    }
  }
...
<c:if test="${empty SSHMA && empty carian}">
    <p class=title>Terima Bayaran: Maklumat Penyerah/Pembayar dan Penyampai</p>
</c:if>
...
<c:if test="${empty SSHMA && empty carian}">
    <span class=instr>Masukkan maklumat penyerah Permohonan/Perserahan dan Penyampai. Jika Penyerah adalah Peguam yang berdaftar,
...
<s:form action="${action}" id="penyerah">
  ...
  <fieldset class="aras1">
    ...
    <p>
      ...
      <s:text name="idPenyerah" size="7" id="idPenyerah" maxlength="7" />
    ...
    <p align="center">
      ...
      <c:if test="${empty SSHMA && empty carian}">
          <fieldset class="aras1">      
              <legend>Maklumat Penyampai</legend>
              <p>
                  <label>Nama</label>
                  <s:text name="namaPenyampai" id="namaPenyampai" size="42" onblur="doUpperCase(this.id)"/>
              </p>
              <p>
                  <label>No. Pengenalan</label>
                  <s:text name="noPengenalanPenyampai" id="noPengenalanPenyampai" size="17"/><em>[cth: 780901-05-7893]</em>
              </p>
              <p>
                  <label>No.Telefon</label>
                  <s:text name="noTelPenyampai" id="noTelPenyampai" size="15"/>
              </p>  
              <br/>
              <p align="center">
                  <s:submit name="CancelAll" value="Batal" class="btn" />
