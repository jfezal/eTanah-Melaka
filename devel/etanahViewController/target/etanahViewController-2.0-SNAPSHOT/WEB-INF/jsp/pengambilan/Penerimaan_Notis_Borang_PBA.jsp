<%--
    Document   : Penerimaan_Notis_Borang_PBA
...
<script type="text/javascript">
    ...
    function scanPemohon(idPihak) {
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?popupScanPemohon&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&isPemohonPihak=true';
    ...
    function addPenghantarNotis(){
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        window.open("${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?popupPenghantarNotis&idHakmilik="+idHakmilik+'&showPP='+showPP+'&showHP='+showHP, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=400");
    }
    ...
    function scan(idPihak) {
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?popupScan&idPihak='+idPihak+'&idHakmilik='+idHakmilik;
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=800,height=700");
    }


    function refreshPagePenerimaanBorang(){
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?refreshpage';
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function notis(h){
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?notisPopup&idPihak='+h;
    ...
    function refreshNotis(idHakmilik,showPP,showHP){
        if(showHP == 'true'){
            if(idHakmilik !=''){
                var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?hakmilikDetails&idHakmilik='+idHakmilik+"&showPP="+showPP;
                $.get(url,
                function(data){
                    $('#page_div').html(data);
                },'html');
                ...
                if(showPP == 'false'){
                    var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?showForm';
                    $.get(url,
                    function(data){
                        $('#page_div').html(data);
                    },'html');
                }else{
                    var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?showFormPP';
                    $.get(url,
                    function(data){
                        $('#page_div').html(data);
                    },'html');
            ...
            var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_borang_PBA?showFormPemohon';
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<s:form beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorangPBAActionBean">
        ...
        <fieldset class="aras1"><br/>
            ...
            <p align="center">
                <display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" pagesize="5" cellpadding="0" cellspacing="0"
                               requestURI="/pengambilan/penerimaan_notis_borang_PBA" id="line">
                    <display:column title="No" sortable="true">${line_rowNum}</display:column>
                    <display:column title="ID Hakmilik">
                        <s:link beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorangPBAActionBean"
            ...
            <br/>
            <br/>
            <c:if test="${actionBean.showPP eq true && fn:length(actionBean.listNotisPemohonASB) > 0}">
                <div align="center" id="pemohonList">
                    <display:table class="tablecloth" name="${actionBean.permohonan.senaraiPemohon}"
                                   cellpadding="0" cellspacing="0" requestURI="/pengambilan/penerimaan_notis_borang_PBA" id="line">
