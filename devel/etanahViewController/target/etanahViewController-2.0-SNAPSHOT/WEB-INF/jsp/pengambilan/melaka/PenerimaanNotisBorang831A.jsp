<%-- 
    Document   : PenerimaanNotisBorang831A
    Created on : 04-Jun-2011, 18:06:37
    Author     : nordiyana
--%>
...
<script type="text/javascript">
    ...
    function muatNaikForm(idPihak) {
        ...
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupUpload&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&showHP='+showHP;
    ...
    function muatNaikFormPemohon(idPihak) {
        ...
        var left = (screen.width/2)-(1000/2);
        var top = (screen.height/2)-(150/2);
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupUpload&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&showHP='+showHP+'&isPemohonPihak=true';
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=200, left=" + left + ",top=" + top);
    }
    ...
    function muatNaikFormMahkamah() {
        var left = (screen.width/2)-(1000/2);
        var top = (screen.height/2)-(150/2);
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupUpload&isMahkamah=true';
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=200, left=" + left + ",top=" + top);
    }
    function scanMahkamah() {
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupScanMahkamah&isMahkamah=true';
    ...
    function scanPemohon(idPihak) {
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupScanPemohon&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&isPemohonPihak=true';
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=800,height=700");
    }
    ...
    function addPenghantarNotis(){
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        window.open("${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupPenghantarNotis&idHakmilik="+idHakmilik+'&showPP='+showPP+'&showHP='+showHP, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=400");
    }

    function addPenghantarNotisM(){
        window.open("${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupPenghantarNotis&isMahkamah=true", "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=400");
    }
    ...
    function scan(idPihak) {
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?popupScan&idPihak='+idPihak+'&idHakmilik='+idHakmilik;
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=800,height=700");
    }


    function refreshPagePenerimaanBorang(){
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?refreshpage';
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function notis(h){
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?notisPopup&idPihak='+h;
    ...
    function refreshNotis(idHakmilik,showPP,showHP,isMahkamah){
        if(showHP == 'true'){
            if(idHakmilik !=''){
                var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?hakmilikDetails&idHakmilik='+idHakmilik+"&showPP="+showPP;
                $.get(url,
                function(data){
                    $('#page_div').html(data);
                },'html');
                ...
                if(showPP == 'true'){
                    var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?showFormPP';
                    $.get(url,
                    function(data){
                        $('#page_div').html(data);
                    },'html');
            ...
            var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?showFormPemohon';
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
        }else if(isMahkamah == 'true'){
            var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831A?showFormMahkamah';
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<s:form beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorang831A">
        ...
        <fieldset class="aras1"><br/>
            ...
            <p align="center">
                <display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" pagesize="5" cellpadding="0" cellspacing="0"
                               requestURI="/pengambilan/penerimaan_notis_831A" id="line">
                    <display:column title="No" sortable="true">${line_rowNum}</display:column>
                    <display:column title="ID Hakmilik">
                        <s:link beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorang831A"
                    ...
                    <display:column title="Luas" style="vertical-align:baseline"><fmt:formatNumber pattern="#,##0.0000" value="${line.hakmilik.luas}"/>&nbsp;${line.hakmilik.kodUnitLuas.nama}
            ...
            <c:if test="${actionBean.showPP eq true && ((fn:length(actionBean.listNotisPemohonASB) > 0) ||(fn:length(actionBean.listNotisPemohonPB) > 0))}">
                <div align="center" id="pemohonList">
                    <display:table class="tablecloth" name="${actionBean.permohonan.senaraiPemohon}"
                                   cellpadding="0" cellspacing="0" requestURI="/pengambilan/penerimaan_notis_831A" id="line">
            ...
            <c:if test="${actionBean.permohonanMahkamah ne null && actionBean.notisMahkamahRPD ne null}">
                <div align="center">
