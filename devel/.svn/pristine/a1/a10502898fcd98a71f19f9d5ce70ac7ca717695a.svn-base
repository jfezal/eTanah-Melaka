<%-- 
    Document   : kertas_pindaan_mmkV2Tujuan
    Created on : Jul 4, 2013, 4:43:20 PM
    Author     : afham
--%>
...
<script type="text/javascript">
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_pindaan_MMKV2?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kTujuan',q,
            function(data){
                $('#page_div').html(data);

            }, 'html');
            self.refreshpage2('kTujuan') ;
        }
    }
            ...
            <fieldset class="aras1">
                ...
                <table class="tablecloth" border="0" align="center">
                    ...
                    <c:forEach items="${actionBean.senaraiKandunganTujuan}" var="line">
                        <tr>
                            <td style="text-align: right"><c:out value="1.${num}"/></td>
                            <td>
                                <s:textarea  id="kandungan5${i}" name="senaraiKandunganTujuan[${i-1}].kandungan" cols="80"  rows="5" onclick="checkKandungan(${i})" onblur="setKandungan(${i},${line.idKandungan})" class="normal_text"/>
                    ...
                    <tr>
                        <td style="text-align:center;" colspan="3">      
                            <s:button value="Tambah" class="btn" name="simpan1"  onclick="addRow('1',this.form)"/>
                            <s:submit name="simpanKandungan" value="Simpan" class="btn" onclick="NoPrompt();"/>
                            <s:hidden name="indexKertas" id="indexKertas" value="1"/>
