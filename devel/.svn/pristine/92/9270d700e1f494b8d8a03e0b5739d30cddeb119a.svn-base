<%-- 
    Document   : muat_naik
    Created on : july 07, 2011, 8:19:28 AM
    Author     : Murali
--%>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {        
        var url = '${pageContext.request.contextPath}/strata/SemakKertasSiasatan??reload&idPermohonan=${idPermohonan}';
        $.ajax({
            type:"GET",
            url : url,
            success : function(data) {
...
<s:form beanclass="etanah.view.strata.SenaraiKertasSiasatanActionBean">
<%--<s:form beanclass="etanah.view.penguatkuasaan.UploadAction1">--%>
    <s:hidden name="folderId" value="${folderId}"/>
    <s:hidden name="kodDokumen" value="${kodDokumen}"/>
    <s:hidden name="urusanDokumenId" value="${urusanDokumenId}"/>
