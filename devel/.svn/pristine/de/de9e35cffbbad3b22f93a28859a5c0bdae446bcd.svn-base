<%--
    ...
    Created on :Sept 9, 2011, 10:23:22 PM
...
<script type="text/javascript">
    ...
    function addMaklumatTanah(i,j,idValue){
        ...
        var url = '${pageContext.request.contextPath}/penguatkuasaan/utiliti_laporan_tanah?addMaklumatTanah'+'&statusDisplay='+i+'&jenisKemasukan='+j+'&idLaporanTanah='+id+'&idValue='+idValue+'&idPermohonan='+idMohon;
        window.open(url, 'etanah', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=1000,scrollbars=yes, left=" + left + ",top=" + top);
    }
    
    function refreshFindInfo(val,idMohon) {
        //        doBlockUI();
        idMohon = $('#idPermohonan').val();
        ...
        //        alert(j);
        var url = '${pageContext.request.contextPath}/penguatkuasaan/utiliti_laporan_tanah?reload&idLaporanTanah=' + val+'&statusOp='+j+'&idPermohonan='+idMohon;
        //        alert(url);
        $.ajax({
            type:"GET",
            url : url,
    ...
    function removeHakmilik(id){
        if(confirm('Adakah anda pasti untuk hapus?')) {
            var url = '${pageContext.request.contextPath}/penguatkuasaan/utiliti_laporan_tanah?deleteJenisHakisan&id='+id;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');}
...
<s:form beanclass="etanah.view.penguatkuasaan.UtilitiLaporanTanahActionBean" name="form">
                    ...
                    <display:table class="tablecloth" name="${actionBean.senaraiLaporanTanahSempadan}" cellpadding="0" cellspacing="0" id="line" >
                        ...
                        <display:column title="Imej Di Atas Tanah ">
                            <c:if test="${edit}">
                                <img src="${pageContext.request.contextPath}/pub/images/icons/upload.png"
                                     onclick="muatNaikForm('${actionBean.permohonan.folderDokumen.folderId}','',
                                         '${actionBean.permohonan.idPermohonan}','IH${line.jenisSempadan}','${actionBean.laporanTanah.idLaporan}','',${line.idLaporTanahSpdn},'Y');return false;" height="30" width="30" alt="Muat Naik"
                                     onmouseover="this.style.cursor='pointer';" title="Muat Naik untuk Dokumen Gambar Hakmilik"/>
                                <br>
                                <c:set value="1" var="count"/>
