...
<script type="text/javascript">
    ...
    function doEdit(d, j, d1){        
        ...
        window.open("${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?showEditPemohonPenerimaForm&jenis_pihak="
    ...
    function selectPemohon(){
        ...
        var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?simpanPemohon'+param + '&idHakmilik=' +  $('#hakmilik').val();
    ...
    function selectTerlibat(){
        ...
        var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?simpanPihakTerlibat'+param + '&idHakmilik=' +  $('#hakmilik').val();
    ...
    function addNew(action){
        ...
        var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?' +action+param+'&jenisPihak=pemohon&idHakmilik=' + idHakmilik;
    ...
    function remove(action, cn, jenis, id){
        ...
        if(param == ''){
            ...
            var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?' +action+param + '&jenisPihak=' + jenis + '&idHakmilik=' + idHakmilik;
    ...
    function removePemohon() {
        ...
        if(param == ''){
            ...
            if(confirm('Adakah anda pasti?')) {
                ...
                var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?deletePemohon'+param;
                $.ajax({
                    type:"GET",
                    url : url,
                    dataType : 'html',
    ...
    function reloadEdit (val) {
        doBlockUI();
        var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?reloadEdit&idHakmilik=' + val;
        $.ajax({
            type:"GET",
            url : url,
            dataType : 'html',
    ...
    function reload (val) {
        doBlockUI();
        var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?reload&idHakmilik=' + val;
        $.ajax({
            type:"GET",
            url : url,
            dataType : 'html',
    ...
    function doOpen(type) {
        var idHakmilik = $('#hakmilik').val();
        var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?showSearchForm&type='+type+'&idHakmilik=' + idHakmilik;
    ...
    <%--    function samaRata(f){
            ...
            function samaRata(f, value){
                var idHakmilik = $('#hakmilik').val();
                var q = $(f).formSerialize();
                $.get('${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?agihSamaRata&jenis='+value+'&hakmilik='+idHakmilik,q,
                function(data){
                    if( data == null || data.length == 0) {
                        alert('Terdapat Masalah');
                        return;
            ...
            function viewPihak(id, jenis){
                window.open("${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?viewPihakDetail&idPihak="+id+"&jenis="+jenis+ '&idHakmilik=' +  $('#hakmilik').val(), "eTanah",
                "status=0,toolbar=0,location=0,menubar=0,width=2000,height=900,scrollbars=yes,fullscreen=yes");
            }

            function editTuanTanah(idPihak){

                window.open("${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?editTuanTanah&idPihak="+idPihak+"&hakmilik="+$('#hakmilik').val(), "eTanah",
                "status=0,toolbar=0,location=0,menubar=0,width=2000,height=900,scrollbars=yes,fullscreen=yes");
            }

            function editPemohon(idPemohon){
                window.open("${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?editPemohon&idPemohon="+idPemohon+ '&hakmilik=' +  $('#hakmilik').val(), "eTanah",
                "status=0,toolbar=0,location=0,menubar=0,width=2000,height=900,scrollbars=yes,fullscreen=yes");
            }

            function editPenerima(idMohonPihak){
                window.open("${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?editPenerima&idMohonPihak="+idMohonPihak+ '&hakmilik=' +  $('#hakmilik').val(), "eTanah",
                "status=0,toolbar=0,location=0,menubar=0,width=2000,height=900,scrollbars=yes,fullscreen=yes");
            }
            ...
            function carianHakmilikPopUp(){
                ...
                //                   alert(statusPage);
                window.open("${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?carianHakmilikPopup&statusPage="+statusPage, "eTanah",
     ...
     function hapusHakmilik() {
        ...
        var x = $('#hakmilik').val();
            if(confirm('Adakah anda pasti?')) {
                ...
                var url = '${pageContext.request.contextPath}/pelupusan/pihak_kepentingan?deleteIdHakmilik='+x;
                $.ajax({
                type:"GET",
                url : url,
                dataType : 'html',
...
<div class="subtitle displaytag">
    <s:messages/>
    <s:errors/>
    <s:form beanclass="etanah.view.stripes.pelupusan.PihakKepentinganActionBean" name="form1">
        <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'MCMCL'}">
        ...
        <fieldset class="aras1">
            ...
            <p>
                <label>Daerah :</label>
                ${actionBean.hakmilikTanah.daerah.nama}
            </p>
            <p>
                <label>Mukim/Pekan/Bandar :</label>
                ${actionBean.hakmilikTanah.bandarPekanMukim.nama}
            </p>
            <p>
                <label>Seksyen :</label>
            ...
            <p>
                <label>Tempat :</label>
                ${actionBean.hakmilikTanah.lokasi}
            </p>
            <p>
                <label>Jenis Hakmilik :</label>
                ${actionBean.hakmilikTanah.kodHakmilik.nama}
            </p>
            <p>
                <label>Kategori Tanah :</label>
                ${actionBean.hakmilikTanah.kategoriTanah.nama}
            </p>
            <p>
                <label>Kod Lot/PT :</label>
                ${actionBean.hakmilikTanah.lot.nama}
            </p>
            <p>
                <label>Nombor Lot/PT :</label>
                ${actionBean.hakmilikTanah.noLot}
            </p>
            <p>
                <label>Keluasan :</label>
                ${actionBean.hakmilikTanah.luas}&nbsp;${actionBean.hakmilikTanah.kodUnitLuas.nama}
            </p>
            <p>
                <label>Syarat Nyata :</label>
                ${actionBean.hakmilikTanah.syaratNyata.syarat}
            </p>
            <p>
                <label>Sekatan Kepentingan :</label>
                ${actionBean.hakmilikTanah.sekatanKepentingan.sekatan}
            </p>
            <p>
            ...
            <p>
                ...
                ${actionBean.hakmilikTanah.noPelan}
            </p>
            ...
            <p>
                ...
                RM <s:format value="${actionBean.hakmilikTanah.cukai}" formatPattern="##0.00"/>
            </p>
            <p>
                <label>Lokasi :</label>
                ${actionBean.hakmilikTanah.lokasi}
        ...
        <fieldset class="aras1">
            ...
            <div class="content" align="center">
                <display:table style="width:90%;" class="tablecloth" name="${actionBean.senaraiPemohon}"
                    ...
                    <display:column title="Alamat Surat Menyurat">
                        ${linePemohon.pihak.alamat1}
                        <c:if test="${linePemohon.pihak.alamat1 ne null && linePemohon.pihak.alamat2 ne null}"> , </c:if>
                        ${linePemohon.pihak.alamat2}
                        <c:if test="${linePemohon.pihak.alamat2 ne null && linePemohon.pihak.alamat3 ne null}"> , </c:if>
                        ${linePemohon.pihak.alamat3}
                        <c:if test="${linePemohon.pihak.alamat3 ne null && linePemohon.pihak.alamat4 ne null}"> , </c:if>
                        ${linePemohon.pihak.alamat4}
                        <c:if test="${linePemohon.pihak.alamat4 ne null && linePemohon.pihak.poskod ne null}">,</c:if>
                        ${linePemohon.pihak.poskod}
                        <c:if test="${linePemohon.pihak.poskod ne null && linePemohon.pihak.negeri.kod ne null}">,</c:if>
                        <font style="text-transform:uppercase;">${linePemohon.pihak.negeri.nama}</font>
                    </display:column>
            ...
            <c:if test="${actionBean.statusBtn eq 'OPEN'}">
                <p>
                    ...
                    <s:textarea name="permohonan.sebab" rows="5" cols="50" class="normal_text"/>
                    
                    </p>
                    <p>
                    <center><s:button name="simpanTujuan" id="save" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div');"/></center>
                    </p>
            </c:if>
            ...
            <c:if test="${actionBean.statusBtn eq 'HIDE'}">
                <p>
                    <label>Tujuan permohonan:</label>
                    <s:textarea name="permohonan.sebab" rows="5" cols="50" class="normal_text" readonly="true"/>
                </p>
            </c:if>

            <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'BMBT' || actionBean.permohonan.kodUrusan.kod eq 'PHLP'}">
                <c:if test="${actionBean.statusBtn eq 'OPEN'}">
                    <p align="center"><label></label>
                        <br>
                        <s:button name="simpanTujuan" id="save" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div');"/>
                    </p>
                </c:if>
                <c:if test="${actionBean.statusBtn eq 'HIDE'}">
                    &nbsp;
