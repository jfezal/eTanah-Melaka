 ...
 * @author nur.aqilah
 */
@UrlBinding("/lelong/mohonTangguh")
public class MohonTangguhActionBean extends AbleActionBean {
...
//                if (fasaPermohonan.getKeputusan().getKod().equals("XN")) {
                ...
                if (fasaPermohonan.getKeputusan().getKod().equals("AG")) {
                    ...
                    error = true;
                    return new JSP("lelong/mohonTangguh.jsp").addParameter("tab", "true");
                }
            }
        }
        return new JSP("lelong/mohonTangguh.jsp").addParameter("tab", "true");
...
//        enkuiri.setUlasanPegawai(fasaPermohonan.getUlasan());
    ...
    public Resolution simpanEnkuiriAwal() {
        ...
        if (StringUtils.isNotEmpty(baruKe) || enkuiri == null) {
            ...
            enk = new Enkuiri();
            if (enkuiriTG.get(0).getBilanganKes() != null) {
                enk.setBilanganKes(enkuiriTG.get(0).getBilanganKes() + 1);
            } else {
                enkuiriTG.get(0).setBilanganKes(1);
                enk.setBilanganKes(enkuiriTG.get(0).getBilanganKes() + 1);
                enService.save(enkuiriTG.get(0));
            }
        } else {
            LOG.info("----tak----");
            enkuiriTG = lelongService.findEnkuiriTG(idPermohonan);
...
//                    gen = "LLGSuratWarta.rdf";
            ...
            if ("05".equals(conf.getProperty("kodNegeri"))) {
                gen = "LLGSuratTangguhSblm_NS.rdf";
                code = "TG";
                lelongService.reportGen(permohonan, gen, code, pengguna);
                //generate surat siasatan at stage janaNotisSiasatan
