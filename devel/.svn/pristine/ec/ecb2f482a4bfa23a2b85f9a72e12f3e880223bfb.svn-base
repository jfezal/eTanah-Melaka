...
<script type="text/javascript">
    ...
    function menyimpan(index,i,f)
    {
        ...
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmk_pcrg?simpanKandungan&index='+index+'&kandungan='+kand,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    ...
    {
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmk_pcrg?tambahRow&index='+index,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
    function deleteRow(idKandungan,f)
    {
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/draf_mmk_pcrg?deleteRow&idKandungan='+idKandungan,q,
            function(data){
                $('#page_div').html(data);
            }, 'html');
        }
    ...
    function semakSyor(f,v){
        var q = $(f).formSerialize();
        $.post('${pageContext.request.contextPath}/pelupusan/draf_mmk_pcrg?showsyortolaklulus&syortolaklulus='+v,q,
        function(data){
            $('#page_div').html(data);
        }, 'html');
    }
    ...
    function refreshPage(v){
        ...
        var url = '${pageContext.request.contextPath}/pelupusan/draf_mmk_pcrg?showsyortolaklulus&syortolaklulus='+syortolaklulusdata;
        $.post(url,
        function(data){
            $('#page_div').html(data);
        },'html');
...
<s:form beanclass="etanah.view.stripes.pelupusan.DrafMMK_PCRGActionBean">
    <s:messages/>
    <s:errors/>
                    ...
                    <fieldset class="aras1">
                        ...
                        <div class="content" align="center">                            
                            <table border="0" width="80%" cellspacing="10%" align="center">
                                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PCRG' || actionBean.permohonan.kodUrusan.kod eq 'MPCRG'}">
                                    <tr><td id="tdLabel" ><b>
