 ...
 */
@UrlBinding("/pelupusan/maklumat_tanah_mlcrg")
public class MaklumatTanahMlcrgActionBean extends AbleActionBean {

    private static Logger logger = Logger.getLogger(MaklumatTanahMlcrgActionBean.class);
    ...
    private List<KodBandarPekanMukim> senaraiKodBPMMLCRGPJLB;
    ...
    private String unitKeluasan;

    @DefaultHandler
    public Resolution showForm() {
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new JSP("pelupusan/maklumat_tanah_dipohon_mlcrg.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution carianIndependent() {
        ...
        if (permohonan.getKodUrusan().getKod().equals("MLCRG") || permohonan.getKodUrusan().getKod().equals("PJLB")) {
    ...
    public void rehydrate() {
        ...
        if (hakmilikPermohonan != null) {
            ...
            logger.debug("jarakDariNama :" + jarakDariNama);

            if (hakmilikPermohonan.getKodUnitLuas() != null) {
                keluasanUOM = hakmilikPermohonan.getKodUnitLuas().getKod();
                unitKeluasan=hakmilikPermohonan.getKodUnitLuas().getNama();
            }
            ...
            logger.debug("Hak milik permohonan is null");
        }

        logger.debug("keluasanUOM :" + keluasanUOM);
        logger.debug("tandaBlok :" + tandaBlok);
        ...
        if (idPermohonan != null) {
            ...
            permohonan = permohonanDAO.findById(idPermohonan);
            permohonanLaporanPelan = laporanPelukisPelanService.findMohonLaporPelanByidMohon(permohonan.getPermohonanSebelum().getIdPermohonan());
            if (permohonanLaporanPelan != null) {

                noLitho = permohonanLaporanPelan.getNoLitho();
            }

            logger.debug("noLitho" + noLitho);
            ...
            if (!permohonan.getKodUrusan().getKod().equals("MLCRG") && !permohonan.getKodUrusan().getKod().equals("MPJLB")) {
                ...
                if (StringUtils.isEmpty(kodDaerah) && StringUtils.isEmpty(kodDaerahSession)) {
                    kodDaerah = hakmilikPermohonan.getBandarPekanMukimBaru() != null ? hakmilikPermohonan.getBandarPekanMukimBaru().getDaerah().getKod() : new String();
                } else {
...
//        String kod2 = getContext().getRequest().getParameter("kodUnitLuas");
        ...
        if (!validateluas) {
            ...
            if (negeri.equals("04") && permohonan.getKodUrusan().getKod().equals("MLCRG")) {
                permohonanLaporanPelan = pelupusanService.findByIdPermohonanPelan(idPermohonan);
                InfoAudit info = new InfoAudit();
                if (permohonanLaporanPelan == null) {
                    info = new InfoAudit();
...
//                     permohonanLaporanPelan.setCatatan(tandaBlok);
    ...
    public Resolution simpanHakmilikPRIZMLK() {
        String idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        Pengguna peng = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        permohonan = permohonanDAO.findById(idPermohonan);
        String val1 = getContext().getRequest().getParameter("bandarPekanMukimBaru.kod");
        ...
        if (permohonan != null) {
            if (permohonan.getKodUrusan().getKod().equals("PRIZ")) { //Tambah else if utk priz
                TanahRizabPermohonan mohonTrizabTemp = new TanahRizabPermohonan();
                mohonTrizabTemp = pelupusanService.findTanahRizabByIdPermohonan(idPermohonan);
                InfoAudit ia = new InfoAudit();
                if (mohonTrizabTemp != null) {
        ...
        if (!StringUtils.isEmpty(lot)) {
            KodLot kodLotPRIZ = new KodLot();
            kodLotPRIZ = kodLotDAO.findById(lot);
            hakmilikPermohonan.setLot(kodLotPRIZ);
        }
        if (!StringUtils.isEmpty(noLot)) {
            hakmilikPermohonan.setNoLot(noLot);
        }
        if (!StringUtils.isEmpty(keluasanUOM)) {
            KodUOM kodUOMPRIZ = new KodUOM();
            kodUOMPRIZ = kodUOMDAO.findById(keluasanUOM);
            hakmilikPermohonan.setKodUnitLuas(kodUOMPRIZ);
        }
        pelupusanService.simpanHakmilikPermohonan(hakmilikPermohonan);
    ...
    public Resolution simpanHakmilikPBRZMLK() {
        ...
        if (permohonan != null) {
            if (permohonan.getKodUrusan().getKod().equals("PBRZ")) { //Tambah else if utk priz
                TanahRizabPermohonan mohonTrizabTemp = new TanahRizabPermohonan();
                mohonTrizabTemp = pelupusanService.findTanahRizabByIdPermohonan(idPermohonan);
                InfoAudit ia = new InfoAudit();
                if (mohonTrizabTemp != null) {
    ...
    public Resolution simpanHakmilikRAYTMLK() {
        String idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        Pengguna peng = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        permohonan = permohonanDAO.findById(idPermohonan);
        String val1 = getContext().getRequest().getParameter("bandarPekanMukimBaru.kod");
        ...
        if (permohonan != null) {
            if (permohonan.getKodUrusan().getKod().equals("RAYT")) { //Tambah else if utk priz
                TanahRizabPermohonan mohonTrizabTemp = new TanahRizabPermohonan();
                mohonTrizabTemp = pelupusanService.findTanahRizabByIdPermohonan(idPermohonan);
                InfoAudit ia = new InfoAudit();
                if (mohonTrizabTemp != null) {
        ...
        if (!StringUtils.isEmpty(lot)) {
            KodLot kodLotRAYT = new KodLot();
            kodLotRAYT = kodLotDAO.findById(lot);
            hakmilikPermohonan.setLot(kodLotRAYT);
        }
        if (!StringUtils.isEmpty(noLot)) {
            hakmilikPermohonan.setNoLot(noLot);
        }
        if (!StringUtils.isEmpty(keluasanUOM)) {
            KodUOM kodUOMRAYT = new KodUOM();
            kodUOMRAYT = kodUOMDAO.findById(keluasanUOM);
            hakmilikPermohonan.setKodUnitLuas(kodUOMRAYT);
        }
        pelupusanService.simpanHakmilikPermohonan(hakmilikPermohonan);
    ...
    public Resolution simpanHakmilik() {
        ...
        if (permohonan != null) {
            ...
            } else if (permohonan.getKodUrusan().getKod().equals("PRIZ")) { //Tambah else if utk priz
                TanahRizabPermohonan mohonTrizabTemp = new TanahRizabPermohonan();
                mohonTrizabTemp = pelupusanService.findTanahRizabByIdPermohonan(idPermohonan);
                InfoAudit ia = new InfoAudit();
                if (mohonTrizabTemp != null) {
    ...
    public Resolution simpanHakmilikPermit() {
        String idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        permohonan = permohonanDAO.findById(idPermohonan);
        ...
        if (senaraiHakMilik != null) {
            ...
            infoAudit.setTarikhMasuk(hakmilikPermohonan.getInfoAudit().getTarikhMasuk());
            hakmilikPermohonan.setInfoAudit(infoAudit);
            String kodSN = getContext().getRequest().getParameter("kod");
            logger.info("Syarat Nyata :" + kodSN);
            kodUL = new KodUOM();
            String kod2 = keluasanUOM;
            if (StringUtils.isNotBlank(kod2)) {
                kodUL.setKod(kod2);
            ...
            if (StringUtils.isNotBlank(kodSN)) {
                hakmilikPermohonan.setSyaratNyataBaru(kodSyaratNyataDAO.findById(kodSN));
            }
            pelupusanService.simpanHakmilikPermohonan(hakmilikPermohonan);
            pelupusanService.simpanPermohonan(permohonan);
        }
...
//        logger.info("THIS IS KODDAERAH->"+kod);
    ...
    public static void setLogger(Logger logger) {
        MaklumatTanahMlcrgActionBean.logger = logger;
    }
...
//        logger.info("THIS IS KODBPM ->"+kodBpm);
    ...
    public List<KodBandarPekanMukim> getSenaraiKodBPMMLCRGPJLB() {
    ...
    public void setSenaraiKodBPMMLCRGPJLB(List<KodBandarPekanMukim> senaraiKodBPMMLCRGPJLB) {
        this.senaraiKodBPMMLCRGPJLB = senaraiKodBPMMLCRGPJLB;
    }
    ...
    public static void setKodDaerahStatic(String kodDaerahStatic) {
        MaklumatTanahMlcrgActionBean.kodDaerahStatic = kodDaerahStatic;
    }
    ...
    public String getUnitKeluasan() {
        return unitKeluasan;
    }

    public void setUnitKeluasan(String unitKeluasan) {
        this.unitKeluasan = unitKeluasan;
    }
