...
<script type="text/javascript">
    ...
    <c:forEach items="${actionBean.listCalender}" var="b"  varStatus="m">
        ...
        name = name + "Penggadai : ${c.nama}<br>";
            </c:if>
            <c:if test="${c.jenis.kod eq 'PG'}" >
                <c:if test="${c.jenis.nama} ne null}" >
        name = name + "${c.jenis.nama} :"
                </c:if>
                <c:if test="${c.nama} ne null}" >
        name = name  ${c.nama} < br >
                </c:if>
...
//            startDate = '${i.tarikh}';
    ...
    <c:forEach items="${actionBean.listCalender2}" var="x"  varStatus="n">
        ...
        name2 = name2 + "Penggadai : ${z.nama}<br>";
            </c:if>
            <c:if test="${z.jenis.kod eq 'PG'}" >
        name2 = name2 + "${z.jenis.nama} : ${z.nama}<br>";
            </c:if>
        </c:forEach>
