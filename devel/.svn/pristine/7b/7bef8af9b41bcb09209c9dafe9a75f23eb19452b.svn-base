        ...
        <c:otherwise>
            ...
            function openLot(checkboxVal) {
                if (checkboxVal.checked) {
                    ...
                    document.getElementById("noLot1").value = '';
                    return true;
...
<body>
    <script>
        ...
        function simpanUnit() {
            var a = $('#jarakDariTanah').val();
            var b = $('#jarakDariTanahUOM').val();

            if (a != '' && b == 0) {
                alert('Sila masukkan unit jarak dari tanah dipohon');
                document.getElementById("jarakDariTanahUOM").focus();
                return false;
            }
            NoPrompt();
        }
    ...
    <s:form beanclass="etanah.view.stripes.common.laporan.tanah.laporantanahNewActionBean" name="form">
        ...
        <s:hidden name="idLaporTanahSempadan" value="${actionBean.disLaporanTanahSempadan.laporanTanahSempadan.idLaporTanahSpdn}"/>
                        ...
                        <table class="tablecloth" border="0">
                            ...
                            <tr id="noLot">
                                ...
                                <td>
                                    ...
                                    <c:if test="${!empty actionBean.disLaporanTanahSempadan.statusSempadan}">
                                        <br>
                                        <s:select name="disLaporanTanahSempadan.kodLot" id="kodLot" value="${actionBean.disLaporanTanahSempadan.kodLot}">
                                            <s:option value="0">Sila Pilih</s:option>
                                            <s:options-collection collection="${listUtil.senaraiLotPelupusan}" label="nama" value="kod" />
                                        </s:select>
                                        <s:text name="disLaporanTanahSempadan.noLot" onkeyup="validateNumber(this,this.value);" id="noLot1"/>&nbsp;
                                        <%--${actionBean.disLaporanTanahSempadan.kodLotUOM.nama}&nbsp;${actionBean.disLaporanTanahSempadan.noLot}
                                        <s:hidden name="disLaporanTanahSempadan.noLot"/><s:hidden name="disLaporanTanahSempadan.kodLot"/>--%>
                                    </c:if>                        
                                </td>
                            </tr>
                            <tr id="idhakmilik">
                            ...
                            <tr>
                                ...
                                <td>
                                  ...
                                  <s:hidden name="idLaporTanahSpdn" id="idLaporTanahSpdn" value="${actionBean.lts.idLaporTanahSpdn}"/>
                            </tr>
                            <tr>
                                <td align="right" colspan="2">
                                    ...
                                    <c:if test="${empty actionBean.disLaporanTanahSempadan.statusSempadan}">
                                        <s:submit name="simpanKandunganSempadan" value="Simpan" class="btn" onclick="return simpanUnit();"/>
                                        <s:button name="showEditSempadan" value="Isi Semula" class="btn" onclick="resetUlasan(this.name, this.form);" />
                                    </c:if>
                                    <c:if test="${!empty actionBean.disLaporanTanahSempadan.statusSempadan}">   
                                       
                                         
                                        <s:submit name="simpanKandunganSempadan" value="Kemaskini" class="btn" onclick="return simpanUnit();"/>
                                    </c:if>
                                    <s:button name="tutup" value="Kembali" class="btn" onclick="kembali('${actionBean.hakmilikPermohonan.id}','${actionBean.laporanTanah.idLaporan}')"/>
