 ...
 * @author nordiyana
 */
@UrlBinding("/pengambilan/mmkns4Aduan")
public class MMKNAduanNSActionBean extends AbleActionBean {
  private static Logger logger = Logger.getLogger(MMKN831BActionBean.class);
    ...
    private String tajuk = "KERTAS PERMOHONAN PENGAMBILAN TANAH SEKSYEN 4 ADUAN";

    @DefaultHandler
    public Resolution showForm() {
        getContext().getRequest().setAttribute("form1", Boolean.TRUE);
        return new JSP("pengambilan/negerisembilan/seksyen4/mmkn_sek4_nsAduan.jsp").addParameter("tab", "true");
    }
...
//                        StringBuilder sb = new StringBuilder();
                    ...
                    tujuan = "Kertas ini bertujuan untuk mendapat pertimbangan Majlis Mesyuarat Kerajaan Negeri Sembilan ke atas rayuan permohonan aduan kerosakan" +
                            " pengambilan tanah Seksyen 4 Akta Pengambilan Tanah 1960, Projek "+ p.getSebab() +" oleh "+p.getSenaraiPemohon().get(0).getPihak().getNama()+" daripada " + namaPengadu + " " +
                            "tuan tanah "+ noLotList +",Daerah " + hakmilik.getDaerah().getNama()+" seluas lebih kurang "+ totalLuas + " " + uom + ". "
...
//
                 String temp = "RAYUAN PERMOHONAN ADUAN KEROSAKAN PENGAMBILAN TANAH SEKSYEN 4 AKTA PENGAMBILAN TANAH 1960, PROJEK "+ p.getSebab() +" OLEH "+ p.getSenaraiPemohon().get(0).getPihak().getNama() +" DARIPADA "+ namaPengadu +" TUAN TANAH "+ noLotList +
                " DAERAH " + hakmilik.getDaerah().getNama() +" SELUAS LEBIH KURANG " + totalLuas + " " + uom + ".";
