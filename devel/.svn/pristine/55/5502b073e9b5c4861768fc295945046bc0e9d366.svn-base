...
<script type="text/javascript">
    ...
    function dopopup(i, kod) {
        ...
        if (kod == 'KHK') {
            ...
            var url = "showEditAmbilKerajaanPopup";
        } else {
    ...
    <%--var d = $('.x'+i).val();--%>
        ...
        function checkPlot() {
            ...
            var kerajaanAmbil = $('#kerajaanAmbil').val();
            var jumcurrent = parseInt(Number(hakmilik)) + parseInt(Number(thakmilik)) + parseInt(Number(kerajaan)) + parseInt(Number(kerajaanAmbil));
    <%--alert(jumcurrent);--%>
    ...
    <%--}--%>
        ...
        function addNewAmbilKerajaan() {
            var total = $('#jumlahPlot').val();
            window.open("${pageContext.request.contextPath}/pembangunan/maklumat_pecahSempadan?showAmbilKerajaanPopup" + "&plot=" + total, "eTanah",
                    "status=0,toolbar=0,location=0,menubar=0,width=950,height=600");
        }
    ...
    <%--alert(url);--%>
        ...
        function validateSaveSBMS(event, f) {
    <%--alert("save");--%>
            if (validateHakmilik() && validateLuasSBMS()) {
    ...
    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PYTN' && actionBean.permohonan.kodUrusan.kod ne 'SBMS'}">
    ...
    <c:if test="${actionBean.kodNegeri eq '05'}">
        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS' || actionBean.permohonan.kodUrusan.kod eq 'TSPSS'}">
                if (document.getElementById("catatan" + i).value == "") {
                    alert('Sila Masukkan Butir-Butir Pembangunan.');
                    $('#catatan' + i).focus();
                    return false;
                }
        </c:if>
        ...
        function validateLuas() {
            ...
            var rowCount = document.getElementById("count").value;
            var sum = 0;
            //        alert('jumlah luas : '+ jumlahLuas);
            for (var i = 0; i < rowCount; i++) {
                var luas = document.getElementById("luas" + i).value;
                var unitLuas = document.getElementById("kodUOM" + i).value;
                var bilPlot = document.getElementById("bilanganPlot" + i).value;
                //            alert(i + '-luas :  ' + luas + " - unit luas : " + unitLuas);
                if (unitLuas !== '') {
    ...
    <c:if test="${actionBean.hpToget0.hakmilik.kodUnitLuas.kod eq 'M'}">
            ...
            sum = (sum + (parseFloat(Number(jumluasTHakmilik))) + (parseFloat(Number(jumluasKerajaan))) + (parseFloat(Number(jumluasKerajaanAmbil))));
    ...
    alert("parseFloat(Number(sum)):"+parseFloat(Number(sum)));--%>
        ...
        function validateLuasSBMS() {
            var koduomAsal = document.getElementById("koduomAsal").value;
            var jumlahLuas = document.getElementById("luas").value;
            var rowCount = document.getElementById("count").value;
            var sum = 0;
            for (var i = 0; i < rowCount; i++) {
    ...
    <c:if test="${actionBean.hpToget0.hakmilik.kodUnitLuas.kod eq 'M'}">
            ...
            var jumluasTHakmilik = $('#jumluasTHakmilik').val();
            if (jumluasTHakmilik > 0) {
                sum = sum + (parseFloat(Number(jumluasTHakmilik)));
            }
            var jumluasKerajaan = $('#jumluasKerajaan').val();
            if (jumluasKerajaan > 0) {
                sum = sum + (parseFloat(Number(jumluasKerajaan)));
            }
            var jumluasKerajaanAmbil = $('#jumluasKerajaanAmbil').val();
            if (jumluasKerajaanAmbil > 0) {
                sum = sum + (parseFloat(Number(jumluasKerajaanAmbil)));
            }
    <%--alert("jumlahLuas:"+jumlahLuas);
    alert("sum:"+sum);   
    alert("parseFloat(Number(jumlahLuas):"+parseFloat(Number(jumlahLuas)));
    ...
    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'TSPSS' || actionBean.permohonan.kodUrusan.kod eq 'SBMS'|| actionBean.permohonan.kodUrusan.kod eq 'SBPS'}">
            ...
            sel0.onchange = function() {
                javascript:validateBilTingkat(i);
            };
    ...
    cell2.appendChild(e2);--%>
            ...
            e2.onkeyup = function() {
                //javascript:validateNumber2(i, i);
            };
            cell2.appendChild(e2);
            ...
            sel4.options[0] = new Option('--Sila Pilih--', '');
            sel4.options[1] = new Option('HEKTAR', 'H');
            sel4.options[2] = new Option('METER PERSEGI', 'M');
    ...
    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
            ...
            e5.setAttribute("name", "listHakmilik[" + i + "].catatan");
            e5.setAttribute("id", "catatan" + i);
...
//         $('.hapusSelect').each(function () {
            var rowIndex = document.getElementById("countDel1").value;

            if (rowIndex > 0) {
                document.getElementById(tableid).deleteRow((parseInt(rowIndex)));
                var testCount = document.getElementById("countDel1").value;
                ...
                try {
                    var url = '${pageContext.request.contextPath}/pembangunan/maklumat_pecahSempadan?deleteSingle&index=' + rowIndex;
                    $.get(url,
                            function(data) {
                                $('#page_div').html(data);
                                $.unblockUI();
...
<s:form beanclass="etanah.view.stripes.pembangunan.MaklumatPecahSempadanActionBean">
    ...
    <s:hidden name="jumluasKerajaanAmbil" id="jumluasKerajaanAmbil" formatPattern="0.0000"/>
    <s:hidden name="koduomAsal" id="koduomAsal"/>
    <div class="subtitle">
        <fieldset class="aras1">
            <legend>Maklumat Plot (Pelan Tatatur Permohonan)</legend>
            <div class="content" align="center">
                <table border="0" width="80%"> 
                    <tr><td><p><label><font color="blue">Jumlah Luas Pelan PA: </font></label>
                                <fmt:formatNumber  pattern="#,##0.0000" value="${actionBean.jumluasPA}"/> ${actionBean.kodUnitLuasPA.nama} &nbsp;
                            </p></td></tr>
                    <tr><td><p><label><font color="blue">Jumlah Luas Hakmilik: </font></label>
                            ...
                            <tr><td><p><label><font color="blue">No Rujukan Pelan Tatatur 1: </font></label>
                    ...
                    <c:if test="${actionBean.kodNegeri eq '05'}"> <%-- condition for NS --%>   
                        <c:if test="${edit}">
                            <tr><td><p><label><font color="blue">No Rujukan Pelan Tatatur : </font></label>
                                        <s:text name="pelanTatatur" id="pelanTatatur" size="30" class="normal_text" maxlength="50"/>&nbsp;(Bil & Tarikh)
                                    </p></td></tr>
                            <tr><td><p><label><font color="blue">No Rujukan Pelan Pra Hitungan : </font></label>
                                                <s:text name="praHitungan" id="praHitungan" size="30" class="normal_text" maxlength="30"/>
                                    </p></td></tr>
                            <tr>                                       
                                <td align="center"> <s:button name="simpanNoRujukan" id="simpan" value="Simpan" class="btn" onclick="saveNoRujukan(this.name, this.form);"/></td>                                                           
                            </tr>
                ...
                <p><b><font color="red">(* Pastikan No Plot Berbeza Untuk Setiap Kemasukan) </font></b></p>
                ...
                <c:if test="${edit}">
                        ...
                        <tr><td>
                                <table class="tablecloth" border="1" width="85%" id="tbl">
                                    <tr>
                                        ...
                                        <th width="17%" align="center"><b>Kegunaan Tanah (Untuk Kiraan Cukai) </b></th>
                                        ...
                                        <th width="10%" align="center"><b>Bilangan Unit</b></th>
                                            <c:if test="${!(actionBean.permohonan.kodUrusan.kod eq 'SBMS' ||actionBean.permohonan.kodUrusan.kod eq 'TSPSS'|| actionBean.permohonan.kodUrusan.kod eq 'PSBT')}">
                                            <th width="16%" align="center"><b>Luas Per-Unit</b></th>
                                            </c:if>                                            
                                            <c:if test = "${actionBean.kodNegeri eq '05'}">
                                                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'||actionBean.permohonan.kodUrusan.kod eq 'TSPSS'}">
                                                <th width="16%" align="center"><b>Keluasan<br></b></th>
                                                    </c:if>
                                                    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PSBT'}">
                                                <th width="16%" align="center"><b>Luas Per-Unit</b></th>
                                                </c:if>
                                            </c:if>
                                            <c:if test = "${actionBean.kodNegeri eq '04'}">
                                                ...
                                                <th width="16%" align="center"><b>Keluasan<br></b></th>
                                                    ...
                                                    <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PSBT'||actionBean.permohonan.kodUrusan.kod eq 'TSPSS'}">
                                                <th width="16%" align="center"><b>Luas Per-Unit</b></th>
                                                </c:if>
                                            </c:if>
                                            <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'SBMS'}">
                                            ...
                                            <c:if test = "${actionBean.kodNegeri eq '04'}">
                                                ...
                                                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
                                                <th width="16%" align="center"><b>Butir-Butir Pembangunan</b></th>
                                                </c:if>
                                                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
                                                <th width="16%" align="center"><b>Pilih</b></th>
                                                </c:if>
                                            </c:if>
                                            <c:if test = "${actionBean.kodNegeri eq '05'}">
                                                <c:if test="${!(actionBean.permohonan.kodUrusan.kod eq 'SBMS' || actionBean.permohonan.kodUrusan.kod eq 'TSPSS')}">
                                ...
                                <td><s:text name="listHakmilik[${i}].noPlot" id="noPlot${i}" size="10" class="normal_text"/></td>
                                ...
                                <td><s:text name="listHakmilik[${i}].luas" id="luas${i}" size="15" class="normal_text" formatPattern="0.0000" onkeyup="validateNumber(this,this.value);"/>
                                    ...
                                    <s:select name="kodUOM${i}" id="kodUOM${i}" value="${plotPelan.kodUnitLuas.kod}" style="">
                                ...
                                <td><s:text name="listHakmilik[${i}].catatan" id="catatan${i}" size="20" class="normal_text"/></td>
                                <td><s:hidden name="countDel1" id="countDel1" value="${i+1}" />
                                    <input type="checkbox" id="countDel${i+1}" name="hapusSelected" value ="${i+1}" class="hapusSelect"/> </td>
                            </tr>
                    ...
                    <tr>
                        <c:if test="${actionBean.kodNegeri eq '04'}">
                            <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
                                <td align="center"> <s:button name="simpanHakmilikPlotSBMS" id="simpanSBMS" value="Simpan" class="btn" onclick="validateSaveSBMS(this.name, this.form);"/></td>          
                            </c:if>
                            <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'SBMS'}">
                                <td align="center"> <s:button name="simpanHakmilikPlot" id="simpanMlk" value="Simpan" class="btn" onclick="validateSave(this.name, this.form);"/></td>          
                            </c:if>
                        </c:if>
                        <c:if test="${actionBean.kodNegeri eq '05'}">
                            <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'||actionBean.permohonan.kodUrusan.kod eq 'TSPSS'}">
                ...
                <c:if test="${!edit}">
                    <display:table class="tablecloth" name="${actionBean.listHakmilik}" cellpadding="0" cellspacing="0" id="line">
                        ...
                        <display:column title="Bil. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'H'}">${line.bilanganPlot}</c:if></display:column>
                        ...
                        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'PSBT'}">
                            <display:column title="Baki Luas" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'H'}"><fmt:formatNumber  pattern="#,##0.0000" value="${line.luas}"/>
                                    ${line.kodUnitLuas.nama}&nbsp;</c:if></display:column>
                        </c:if>
                        <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
                            <display:column title="Keluasan" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'H'}"><fmt:formatNumber  pattern="#,##0.0000" value="${line.luas}"/>
                                    ${line.kodUnitLuas.nama}&nbsp;</c:if></display:column>
                        </c:if>
                        <c:if test="${!(actionBean.permohonan.kodUrusan.kod eq 'PSBT' || actionBean.permohonan.kodUrusan.kod eq 'SBMS')}">
                        ...
                        <c:if test = "${actionBean.kodNegeri eq '05'}">
                            <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS' || actionBean.permohonan.kodUrusan.kod eq 'TSPSS'}">
                                <display:column title="Butir-Butir Pembangunan" style="vertical-align:baseline"><div align="center"> ${line.catatan}</div></display:column>
                            </c:if>
                            <c:if test="${!(actionBean.permohonan.kodUrusan.kod eq 'SBMS' || actionBean.permohonan.kodUrusan.kod eq 'TSPSS')}">
                                <display:column title="Butir-Butir Pembangunan" style="vertical-align:baseline"><div align="center"> ${line.catatan}</div></display:column>
                            </c:if>
                ...
                <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'SBMS' && actionBean.permohonan.kodUrusan.kod ne 'TSPSS'}">
                    <p><b><font color="blue">Maklumat Plot Yang Dirizabkan Kepada Agensi Kerajaan</font></b></p>
                    <p><b><font color="red">(* Pastikan No Plot Berbeza Untuk Setiap Kemasukan) </font></b></p>
                                <display:table class="tablecloth" name="${actionBean.listRizab}" cellpadding="0" cellspacing="0" id="line">

                        <display:column title="Bil" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'R'}">${line_rowNum}
                                <s:hidden name="x" class="x${line_rowNum -1}" value="${line.idPlot}"/></c:if>
                        </display:column>
                        <display:column title="No. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'R'}">${line.noPlot}</c:if></display:column>
                        <display:column title="Bil. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'R'}">${line.bilanganPlot}</c:if></display:column>
                        <display:column title="Kegunaan Tanah" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'R'}">${line.kegunaanTanahLain}</c:if></display:column>
                        <display:column title="Luas Dirizabkan" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'R'}"><fmt:formatNumber  pattern="#,##0.0000" value="${line.luas}"/>
                                ${line.kodUnitLuas.nama}&nbsp;</c:if></display:column>
                        <c:if test="${edit2}">
                            <display:column title="Luas Diluluskan">
                                <s:text name="listNoPT2[${line_rowNum - 1}].luasDilulus" value="0.0000" onkeyup="validateNumber(this,this.value);" formatPattern="#,##0.00" /></display:column>
                        </c:if>
                        <c:if test="${edit}">
                            <display:column title="Kemaskini">
                                <div align="center">
                                    <img alt='Klik Untuk Kemaskini' border='0' src='${pageContext.request.contextPath}/images/edit.gif' class='rem'
                                         id='rem_${line_rowNum}' onclick="dopopup('${line.idPlot}', 'TKHK');
            return false;" onmouseover="this.style.cursor = 'pointer';">
                            ...
                            <display:column title="Hapus">
                                <div align="center">
                                    <img alt='Klik Untuk Hapus' border='0' src='${pageContext.request.contextPath}/images/not_ok.gif' class='rem'
                                         id='rem_${line_rowNum}' onclick="remove('${line.idPlot}')" onmouseover="this.style.cursor = 'pointer';">
                                </div>
                            </display:column>
                        </c:if>
                    </display:table>
                    <c:if test="${edit}">
                        <p>
                            <s:button name="tambahXHakmilik" class="btn" value="Tambah" onclick="addNewRizab();" id="btnR" />
                        </p>
                    </c:if>
                </c:if>
                ...
                <c:if test = "${actionBean.kodNegeri eq '04'}">
                    <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'SBMS'}">
                        <p><b><font color="blue">Maklumat Plot Yang Diserah Kepada Kerajaan</font></b></p>
                                </c:if>
                                <c:if test="${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
                        <p><b><font color="blue">Maklumat Plot Yang Diserah Kepada Kerajaan/Infrastruktur</font></b></p>
                                </c:if>
                            </c:if>
                ...
                <p><b><font color="red">(* Pastikan No Plot Berbeza Untuk Setiap Kemasukan) </font></b></p>
                            <display:table class="tablecloth" name="${actionBean.listKerajaan}" cellpadding="0" cellspacing="0" id="line">
                                <display:column title="Bil" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'K'}">${line_rowNum}
                                        <s:hidden name="x" class="x${line_rowNum -1}" value="${line.idPlot}"/></c:if>
                                </display:column>
                                <display:column title="No. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'K'}">${line.noPlot}</c:if></display:column>
                                <display:column title="Bil. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'K'}">${line.bilanganPlot}</c:if></display:column>
                                <display:column title="Kegunaan Tanah" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'K'}">${line.kegunaanTanahLain}&nbsp;</c:if></display:column>
                    <c:if test ="${actionBean.kodNegeri eq '04'}">
                        <display:column title="Luas Dipohon" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'K'}"><fmt:formatNumber  pattern="#,##0.0000" value="${line.luas}"/>
                                ${line.kodUnitLuas.nama}&nbsp;</c:if>
                        </display:column>
                    </c:if>
                    <c:if test ="${actionBean.kodNegeri eq '05'}">
                        <display:column title="Luas Diserah" style="vertical-align:baseline">
                            <c:if test="${line.pemilikan.kod eq 'K'}"><fmt:formatNumber  pattern="#,##0.0000" value="${line.luas}"/>&nbsp;${line.kodUnitLuas.nama}</c:if>
                        </display:column>
                    </c:if>
                    <c:if test ="${actionBean.kodNegeri eq '04'}">
                        <c:if test = "${actionBean.permohonan.kodUrusan.kod eq 'SBMS'}">
                            <display:column title="Perlu No. PT" style="vertical-align:baseline">
                                <c:if test = "${line.noPt eq 'Y'}"> Ya </c:if>
                                <c:if test = "${line.noPt eq 'T'}"> Tidak </c:if>
                                <c:if test = "${line.noPt eq null}"> - </c:if>
                            </display:column>
                            <display:column title="Penjenisan" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'K'}">
                                    ${line.kategoriTanah.nama}&nbsp;</c:if>
                            </display:column>
                        </c:if>
                    </c:if>

                    <c:if test="${edit2}">
                        <display:column title="Luas Diserah">
                            <s:text name="listNoPT3[${line_rowNum - 1}].luasDilulus" value="0.0000" onkeyup="validateNumber(this,this.value);" formatPattern="#,##0.00" /></display:column>
                    </c:if>
                    <c:if test="${edit}">
                        <display:column title="Kemaskini">
                            <div align="center">
                                <img alt='Klik Untuk Kemaskini' border='0' src='${pageContext.request.contextPath}/images/edit.gif' class='rem'
                                     id='rem_${line_rowNum}' onclick="dopopup('${line.idPlot}', 'AK');
            return false;" onmouseover="this.style.cursor = 'pointer';">
                            </div>
                        </display:column>
                ...
                <c:if test="${edit}">
                    <p>
                    <p><s:button name="tambahKerajaan" class="btn" value="Tambah" onclick="addNewKerajaan();" id="btnK" /></p>
                    </p>
                </c:if>
                <%--</c:if>--%>
                <br>
                ...
                <c:choose>
                    <c:when test="${actionBean.permohonan.kodUrusan.kod eq 'SBPS'}">
                        <p><b><font color="blue">Maklumat Plot Yang Di Ambil Dari Kerajaan</font></b></p>
                        <p><b><font color="red">(* Pastikan No Plot Berbeza Untuk Setiap Kemasukan) </font></b></p>
                                    <display:table class="tablecloth" name="${actionBean.listAmbilKerajaan}" cellpadding="0" cellspacing="0" id="line">
                                        <display:column title="Bil" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'A'}">${line_rowNum}
                                                <s:hidden name="x" class="x${line_rowNum -1}" value="${line.idPlot}"/></c:if>
                                        </display:column>
                                        <display:column title="No. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'A'}">${line.noPlot}</c:if></display:column>
                                        <display:column title="Bil. Plot" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'A'}">${line.bilanganPlot}</c:if></display:column>
                                        <display:column title="Kegunaan Tanah" style="vertical-align:baseline"><c:if test="${line.pemilikan.kod eq 'A'}">${line.kegunaanTanahLain}&nbsp;</c:if></display:column>
                            <display:column title="Luas Diambil" style="vertical-align:baseline">
                                <c:if test="${line.pemilikan.kod eq 'A'}"><fmt:formatNumber  pattern="#,##0.0000" value="${line.luas}"/>&nbsp; ${line.kodUnitLuas.nama}</c:if>
                            </display:column>
                            ...
                            <c:if test="${edit}">
                                <display:column title="Kemaskini">
                                    <div align="center">
                                        <img alt='Klik Untuk Kemaskini' border='0' src='${pageContext.request.contextPath}/images/edit.gif' class='rem'
                                             id='rem_${line_rowNum}' onclick="dopopup('${line.idPlot}', 'AMK');
            return false;" onmouseover="this.style.cursor = 'pointer';">
                                    </div>
                                </display:column>
                        ...
                        <c:if test="${edit}">
                            <p>
                            <p><s:button name="tambahKerajaan" class="btn" value="Tambah" onclick="addNewAmbilKerajaan();" id="btnK" /></p>
                            </p>
                        </c:if>
                    </c:when>
                </c:choose>
                ...
                <c:if test="${edit2}">
                    <p>
                        <s:button name="simpanLuasDilulus" id="save" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div')"/>
                    </p>
                </c:if>
                ...
                <s:hidden name="bilplotKerajaanAmbiltemp" id="kerajaanAmbil"/>
            </div>
        </fieldset>
    </div>
