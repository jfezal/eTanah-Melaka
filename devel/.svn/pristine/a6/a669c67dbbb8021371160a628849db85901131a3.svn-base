 ...
 * @author wan.fairul
 */
@UrlBinding("/strata/mlk/rmhs")
public class MaintainBangunanRMHSActionBean extends BasicTabActionBean {
    ...
    private static final Logger LOG = Logger.getLogger(MaintainBangunanRMHSActionBean.class);
...
//        getContext().getRequest().setAttribute("ekstrak", Boolean.TRUE);
        return new JSP("strata/pbbm/maklumat_jadualpetak_rmhs.jsp").addParameter("tab", "true");
    }
...
//                            listPetakaksr = ptk1.getSenaraiPetakAksesori();
        ...
        if ((permohonan1.getKodUrusan().getKod().equals("RMHS1")) || (permohonan1.getKodUrusan().getKod().equals("RMH1A"))) {
            LOG.info("--RMHS1--");
...
//        stageId = "g_semakkemasukan";
    ...
    public Resolution tambahBangunan() {
        ...
        return new RedirectResolution(MaintainBangunanActionBean.class, "showForm1").addParameter("error", error).addParameter("message", msg);
    ...
    public Resolution tambahPetak() {
        ...
        return new RedirectResolution(MaintainBangunanActionBean.class, "showForm1");
    }
...
//            strService.deleteAll2(d);
        ...
        return new RedirectResolution(MaintainBangunanActionBean.class, "showForm4").addParameter("message", msg);
    }

    @DefaultHandler
    public Resolution readonly() throws ParserConfigurationException, SAXException, IOException {
        LOG.info("--PBBS Read Only calling--:");
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        String msg = getContext().getRequest().getParameter("message");
        String error = getContext().getRequest().getParameter("error");
        if (StringUtils.isNotBlank(msg)) {
...
//                        - Integer.parseInt(namaBngn2Pecahan);
        ...
        Permohonan permohonanRMHS1 = permohonanDAO.findById(idPermohonan);
        LOG.info("---permohonan--RMHS1----" + permohonanRMHS1.getIdPermohonan());
        LOG.info("---permohonan--PBBS----" + permohonanRMHS1.getPermohonanSebelum().getIdPermohonan());
        pBangunanL = strService.findByIDPermohonan(permohonanRMHS1.getPermohonanSebelum().getIdPermohonan());
        LOG.info("---pBangunanL---PBBS---" + pBangunanL);
        pBangunanLandParcel = strService.findByIDPermohonanByLandparcel(permohonanRMHS1.getPermohonanSebelum().getIdPermohonan());
        LOG.info("---pBangunanLandParcel---" + pBangunanLandParcel.size());
        pBangunanProvisionalBlock = strService.findByIDPermohonanByProvisional(permohonanRMHS1.getPermohonanSebelum().getIdPermohonan());
        LOG.info("----pBangunanProvisionalBlock-----" + pBangunanProvisionalBlock.size());
        if (pBangunanL != null) {
            permohonan = permohonanDAO.findById(permohonanRMHS1.getPermohonanSebelum().getIdPermohonan());
...
//                            listPetakaksr = ptk1.getSenaraiPetakAksesori();
        ...
        //to get data from mohon_harta_bersama
        Permohonan permohonan1 = permohonanDAO.findById(idPermohonan);
        LOG.info("--permohonan1-RMHS1-" + permohonan1);
        LOG.info("--ID Sebulam--" + permohonan1.getPermohonanSebelum().getIdPermohonan());
        mc = strService.findBdn(permohonan1.getPermohonanSebelum().getIdPermohonan());
        LOG.info("--mc--" + mc);
        if (!pBangunanL.isEmpty()) {
            if (pBangunanL.get(0) != null && pBangunanL.get(0).getPermohonanSkim() != null) {
                ...
                LOG.info("--mohonSkim--" + mohonSkim);
            }
        }
        
        if (pBangunanL.isEmpty() && !pBangunanLandParcel.isEmpty()) {
            if (pBangunanLandParcel.get(0) != null && pBangunanLandParcel.get(0).getPermohonanSkim() != null) {
                mohonSkim = strService.findSkimById(pBangunanLandParcel.get(0).getPermohonanSkim().getIdSkim());
                LOG.info("--mohonSkim--" + mohonSkim);
            }
        }
