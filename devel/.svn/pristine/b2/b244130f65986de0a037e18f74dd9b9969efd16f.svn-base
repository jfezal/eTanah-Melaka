...
/**
 *
 * @author Rajesh Modified By Murali
 */
@UrlBinding("/pengambilan/mmkizinlalu")
public class MmkIzinLaluActionBean extends AbleActionBean {
    
    private static Logger logger = Logger.getLogger(MmkIzinLaluActionBean.class);
    @Inject
...
//    private PermohonanKertasKandungan kertasBil;
    ...
    private List<HakmilikPermohonan> hakmilikBantahList;
    private String idSblm;
    Hakmilik hakmilik1 = new Hakmilik();
    private String lokasi;
    private String namaTuanTanah;
    String lokasi1;
    String lokasi2;
    ...
    private List<String> tuanTanahList;
    ...
    public Resolution showForm() {
        ...
        return new JSP("pengambilan/negerisembilan/izinlaluaktabekalanletrik/mmk_ns_izinlalu.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate() {
        ...
        if (pemohonList != null && pemohonList.size() > 0 && pemohonList.get(0).getPihak() != null) {
            pem = pemohonList.get(0).getPihak().getNama();
        }
        
        hakmilikPermohonanList = pengambilanService.getHakmilikPermohonanList(idPermohonan);
        hakmilikBantahList = pengambilanService.getListHakmilikPermohonanBantah(idPermohonan);
//        hakmilikBicaraList=pengambilanService.getListHakmilikPermohonanBicara(idPermohonan);
//        hakmilikPerbicaraan = pengambilanService.getHakmilikPerbicaraanByidMH(hakmilikPermohonan.getId());

        perbicaraanKehadiranList = pengambilanService.getBantahanList(permohonan.getIdPermohonan());
...
//        bil2=hakPSebelumList.size();
        ...
        if (!hakmilikPermohonanList.isEmpty()) {
            ...
            for (int w = 0; w < hakmilikBantahList.size(); w++) {
                hp = hakmilikBantahList.get(w);
...
//                    }
                    ...
                    if (hakmilikBantahList.size() > (w + 1)) {
                        hp1 = hakmilikBantahList.get(w + 1);
                    ...
                    if (w > 0) {
                        ...
                        } else if (w == (hakmilikBantahList.size() - 1)) {
                            lokasi1 = hakmilik.getLot().getNama().toUpperCase() + " " + hakmilik.getNoLot() + "," + " " + "Hakmilik " + hakmilik.getKodHakmilik().getKod() + hakmilik.getNoHakmilik() + ", ";
            ...
            String namaTuantanah = "";
            for (int x = 0; x < perbicaraanKehadiranList.size(); x++) {
                
               // if (perbicaraanKehadiranList.get(x).getBantahElektrik().equalsIgnoreCase("1")) {
                    namaTuantanah += perbicaraanKehadiranList.get(x).getNama() + ", ";
               // }
                perbicaraanKehadiran = perbicaraanKehadiranList.get(x);
                logger.info("Kehadiran" + perbicaraanKehadiran.getNama());
//                tuanTanahList.add(perbicaraanKehadiran.getNama());
            }
            logger.debug("namaTuantanah >> " + namaTuantanah);
            tujuan = "Kertas ini disediakan untuk mendapat pertimbangan Majlis Mesyuarat Kerajaan Negeri, Negeri Sembilan Darul Khusus ke atas perintah yang dikeluarkan oleh Pentadbir Tanah " + NamaDaerah + " berkenaan dengan bantahan daripada  "
                    + namaTuantanah + " terhadap kemasukan dan pampasan oleh Tenaga Nasional Berhad ke atas tanah " + lokasi + " bagi projek diatas";
            logger.info("Tuan Tanah " + perbicaraanKehadiran.getNama() + "Bil Tuan Tanah :" + perbicaraanKehadiranList.size());
            
            
            String temp1 = "RAYUAN TERHADAP PERINTAH YANG DIKELUARKAN OLEH PENTADBIR TANAH " + NamaDaerah.toUpperCase() + " DI BAWAH SEKSYEN 11(7) AKTA BEKALAN ELEKTRIK 1990 OLEH ";
            
            String temp2 = "SEBAGAI TUAN TANAH " + lokasi + " TERHADAP KEMASUKAN DAN PAMPASAN OLEH TENAGA NASIONAL BERHAD BAGI " + sbb.toUpperCase();
            heading1 = temp1;
    ...
    public Resolution simpan() {
        ...
        if (getContext().getRequest().getParameter("heading2") != null) {
            ...
            headingObj.setBil(11);
            String kandungan1 = getContext().getRequest().getParameter("heading1");
            String kandungan2 = getContext().getRequest().getParameter("heading2");
            String kandungan3 = getContext().getRequest().getParameter("heading3");
            String kandungan = "";
            kandungan = kandungan1 + " " + kandungan2 + " " + kandungan3;
            if (kandungan == null) {
        ...
        if (getContext().getRequest().getParameter("heading2") != null) {
            ...
            if (headingObj2 == null) {
                ...
                infoAudit = headingObj2.getInfoAudit();
                infoAudit.setDiKemaskiniOleh(peng);
                infoAudit.setTarikhKemaskini(new java.util.Date());
            }
            headingObj2.setKertas(permohonanKertas);
            headingObj2.setBil(10);
            String kandungan = getContext().getRequest().getParameter("heading2");
            if (kandungan == null) {
            ...
            headingObj2.setInfoAudit(infoAudit);
            pendudukanSementaraMMKNService.simpanPermohonanKertasKandungan(headingObj2);
        }
...
//                 permohonanKertasKandungan7.setSubtajuk("7."+ i+"."+str[k-1]);
        ...
        getContext().getRequest().setAttribute("form1", Boolean.TRUE);
        return new RedirectResolution(MmkIzinLaluActionBean.class, "locate");
    }
    ...
    public Resolution simpanPtg() throws ParseException {
        ...
        for (int i = 1; i <= kira9; i++) {
            if (!(senaraiKertasKandungan9.isEmpty()) && i <= senaraiKertasKandungan9.size()) {
                Long id = senaraiKertasKandungan9.get(i - 1).getIdKandungan();
                if (id != null) {
                    permohonanKertasKandungan9 = pendudukanSementaraMMKNService.findkandunganByIdKandungan(id);
                }
...
//       getContext().getRequest().setAttribute("form1", Boolean.TRUE);
    ...
    public List<HakmilikPermohonan> getHakmilikBantahList() {
        return hakmilikBantahList;
    }
    
    public void setHakmilikBantahList(List<HakmilikPermohonan> hakmilikBantahList) {
        this.hakmilikBantahList = hakmilikBantahList;
    }
    
    public String getNamaTuanTanah() {
        return namaTuanTanah;
    }
    
    public void setNamaTuanTanah(String namaTuanTanah) {
        this.namaTuanTanah = namaTuanTanah;
    }
    
    public List<String> getTuanTanahList() {
        return tuanTanahList;
    }
    
    public void setTuanTanahList(List<String> tuanTanahList) {
