 ...
 * @author zipzap
 */
@UrlBinding("/pengambilan/common/RekodBorangIActionBean")
public class RekodBorangIActionBean extends AbleActionBean {
...
//    private List<HakmilikPihakBerkepentingan> pihakKepentinganList;
    ...
    public Resolution showForm() {
        ...
        return new JSP("/pengambilan/APT/kemasukan_borang_i.jsp").addParameter("tab", "true");
...
//        idPermohonan = (String) getContext().getRequest().getParameter("idPermohonan");
        ...
        pb = borangPerPBDAO.findById(Long.parseLong(idBorangPerPb));
        ...
        urusan = p.getKodUrusan().getNama();
        tuntutan = "Y";
        return new JSP("/pengambilan/APT/kemasukan_borang_j.jsp").addParameter("popup", true);
    }
...
//        urlKembali = "aduanKerosakan";
        List<HakmilikPermohonan> senaraiHP = hakmilikpermohonanservice.findByIdPermohonanOnlyHakmilikAmbil(idPermohonan);
        for (HakmilikPermohonan hp : senaraiHP) {
            if (hp.getHakmilik() != null) {
                listHakmilikPermohonan2.add(hp);
                e = borangACQService.findBorangEByIdMohonAndIdHakmilik(idPermohonan, hp.getHakmilik().getIdHakmilik());
                jum = Integer.parseInt(aduanService.findJumlahPihakTuntut(idPermohonan) + "");
            }
        }
        tuntutan = "Y";
...
//        Permohonan p = permohonanDAO.findById(idPermohonan);
    ...
    public Resolution showFormBorangJ() {
...
//        }
    ...
    public void rehydrate() {
        ...
        borangPerMohon = borangACQService.findBorangA(idPermohonan, "NBD");

        infoWarta = infoWartaService.findByIdMohonAndKodNotis1(idPermohonan, "D");
        listHakmilikPermohonan = new ArrayList<PengambilanHakmilikForm>();
...
//                        urusanDokumen = strataService.findUrusanDokbyKOD(kd.getKod(), permohonan.getKodUrusan().getKod());
                    ...
                    if (kf.getDokumen().getPermohonan() != null) {
                        if (kf.getDokumen().getKodDokumen().getKod().equals("I")) {
...
//        return showForm();
        return new JSP("/pengambilan/APT/muat_naik_APT_BRGI.jsp").addParameter("popup", "true");
    }
...
//        senaraiKodMMK = new ArrayList<KodDokumen>();
        KodDokumen kd1 = kodDokumenDao.findById("I");
    ...
    public static void setLOG(Logger LOG) {
        RekodBorangIActionBean.LOG = LOG;
    }
