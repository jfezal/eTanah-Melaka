<%-- 
    Document   : Laporan_Tanah_Sebelum
    Created on : Feb 11, 2011, 11:04:02 AM
    Author     : Rajesh
--%>
...
<script type="text/javascript">
    ...
    function popup(i){
        ...
        var d = $('.x'+i).val();
            window.open("${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?showeditTanahRizab&idTanahRizabPermohonan="+d, "eTanah",
            "status=0,toolbar=0,location=0,menubar=0,width=890,height=600");
}


function refreshRizab(){
var url = '${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?refreshpage';
$.get(url,
function(data){
$('#page_div').html(data);
},'html');
...
var url = '${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?deleteSingle&idTanahRizabPermohonan='
...
function tambahBaru(){
window.open("${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?hakMilikPopup", "eTanah",
...
function addImage(idHakmilik,rowNo) {
        ...
        $.post('${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?addHakmilikImage&idDokumen='+idDokumen+'&idHakmilik='+idHakmilik,
                function(data){
                    $('#page_div').html(data);
                }, 'html');
...
function addImageUtara(){
    var idDokumen = document.getElementById("utaraImej").options[document.getElementById("utaraImej").selectedIndex].value;
        $.post('${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?addLaporanImage&pandanganImej=U&idDokumen='+idDokumen,
                function(data){
                    $('#page_div').html(data);
                }, 'html');
}
...
function addImageSelatan(){
    var idDokumen = document.getElementById("selatanImej").options[document.getElementById("selatanImej").selectedIndex].value;
        $.post('${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?addLaporanImage&pandanganImej=S&idDokumen='+idDokumen,
                function(data){
                    $('#page_div').html(data);
                }, 'html');
}
...
function addImageTimur(){
    var idDokumen = document.getElementById("timurImej").options[document.getElementById("timurImej").selectedIndex].value;
        $.post('${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?addLaporanImage&pandanganImej=T&idDokumen='+idDokumen,
                function(data){
                    $('#page_div').html(data);
                }, 'html');
}
...
function addImageBarat(){
    var idDokumen = document.getElementById("baratImej").options[document.getElementById("baratImej").selectedIndex].value;
        $.post('${pageContext.request.contextPath}/pengambilan/laporan_tanah_sebelum?addLaporanImage&pandanganImej=B&idDokumen='+idDokumen,
                function(data){
                    $('#page_div').html(data);
                }, 'html');
}
...
<s:form beanclass="etanah.view.stripes.pengambilan.LaporanTanahSebelumActionBean">
    <s:messages/>
    <s:hidden name="permohonan.idPermohonan" />
    <div class="subtitle">
        <fieldset class="aras1">
            <legend>Laporan Tanah Sebelum</legend>
                    ...
                    <div>
                         ...
                         <display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" id="line">
                               ...
                               <display:column title="Imej Di Atas Tanah">
                                   ...
                                   <s:button class="btn" value="Add" name="add_${line_rowNum}" id="add_${line_rowNum}" onclick="addImage('${line.hakmilik.idHakmilik}','${line_rowNum}');"/>
                               </display:column>
                            </display:table>
                   </div>
                     <br>
<%--
        ...
        <fieldset class="aras1">
            ...
            <c:if test="${edit}">
                ...
                <display:table class="tablecloth" name="${actionBean.tanahRizabList}" pagesize="20" cellpadding="0" cellspacing="0" id="line"
                               requestURI="${pageContext.request.contextPath}/laporan_tanah_sebelum">
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            ...
            <legend>Ulasan Ketua Penolong Pegawai Tanah</legend>
            <c:if test="${edit}">
                <p>
                    <label>Ulasan :</label>
                    <s:textarea name="fasaPermohonan.ulasan" cols="50"/>
