...
var sdate = new Date(fulldate);
        ...
        if ((sdate.getUTCDate() + 1) < today.getUTCDate()){
            alert("Tarikh yang dimasukkan tidak sesuai.");
            $('#'+id).val("");
        }}--%>
...
<s:form beanclass="etanah.view.stripes.consent.MaklumatMesyuaratActionBean">
    <s:errors/>
    <s:messages/>
    <s:hidden name="permohonanRujukanLuar.idRujukan"/>

    <div class="subtitle">
        <c:if test="${fn:endsWith(actionBean.permohonan.kodUrusan.idWorkflow, 'JKTL_melaka')}">
            <c:if test="${fn:length(actionBean.listMesyuaratTangguh) > 0}">
                <fieldset class="aras1">
                    <legend>
                        Maklumat Mesyuarat Terdahulu
                    </legend>
                    <div class="content" align="center">
                        <display:table class="tablecloth" name="${actionBean.listMesyuaratTangguh}" cellpadding="0" cellspacing="0" id="line"
                            ...
                            <display:column property="noSidang" title="Persidangan" />
                            <display:column title="Tarikh">
                                <fmt:formatDate pattern="dd/MM/yyyy" value="${line.tarikhSidang}"/>
                            </display:column>
                            <display:column title="Masa">
                                <fmt:formatDate pattern="hh:mm aaa" value="${line.tarikhSidang}"/>
                            </display:column>
                            <display:column property="lokasiAgensi" title="Tempat" />
            ...
            <c:if test="${edit}">
                <fieldset class="aras1">
                    ...
                    <p>
                        <label><font color="red">*</font>Persidangan :</label>
                        <s:text name="permohonanRujukanLuar.noSidang" maxlength="31" size="31" id="bilSidang"/>
                    </p>
                    <p>
            ...
            <c:if test="${edit}">
                <fieldset class="aras1">
                    ...
                    <p>
                        ...
                        <!--s:text name="tarikhMesyuarat" id="datepicker" size="31" class="datepicker" onchange="dateValidation(this.id, this.value)"/-->
                        <!--for kes MMK NS, user will enter previous date-->
                        <s:text name="tarikhMesyuarat" id="datepicker" size="31" class="datepicker"/>
                    </p>

                    <p>
                        <label><font color="red">*</font>Nombor Kertas Mesyuarat :</label>
                        <s:text name="permohonanRujukanLuar.noSidang" size="31" id="noSidang"/>
                    </p>
                    <p>
                        <label>&nbsp;</label>
                        <s:button name="simpan" id="save" value="Simpan" class="btn" onclick="doSubmit(this.form, this.name, 'page_div')"/>
