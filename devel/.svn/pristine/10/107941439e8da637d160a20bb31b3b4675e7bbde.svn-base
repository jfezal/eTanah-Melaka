...
@UrlBinding("/utiliti/strata/dokumen")
public class UtilitiDokumenActionBean extends AbleActionBean {

    @Inject
    private PermohonanDAO permohonanDAO; 
    private Permohonan permohonan;
    private List<KandunganFolder> senaraiKandungan = new ArrayList<KandunganFolder>();
    private static final Logger LOG = Logger.getLogger(UtilitiDokumenActionBean.class);

    @DefaultHandler
    public Resolution showForm() {
        return new ForwardResolution("/WEB-INF/jsp/strata/utiliti/utilitiCariDokumen.jsp");
    }
                ...
                if (permohonan != null) {
                    if (!permohonan.getFolderDokumen().getSenaraiKandungan().isEmpty()) {
                        for (KandunganFolder kFolder : permohonan.getFolderDokumen().getSenaraiKandungan()) {
                            if (kFolder.getDokumen().getKodDokumen() != null && kFolder.getDokumen().getNamaFizikal() != null) {
