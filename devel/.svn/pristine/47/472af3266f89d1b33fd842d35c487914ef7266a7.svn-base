 ...
 */
@UrlBinding("/lelong/keputusan_tangguh_batal")
public class KeputusanTangguhBatalActionBean extends AbleActionBean {

    private static final Logger logger = Logger.getLogger(KeputusanTangguhBatalActionBean.class);
    @Inject
    ...
    private HakmilikPermohonan hakmilikPermohonan2;
    ...
    public Resolution showForm() throws WorkflowException {
        if (hakmilik != null) {
            Date dota = mohon.getInfoAudit().getTarikhMasuk();
            logger.info("Dota : " + dota);
            Date date = new Date();
...
//            if (today90.after(date)) {
//                addSimpleError("Hakmilik Ini Telah Membuat Permohonan Tangguh Sebelum Ini Dan Tidak Cukup 90 Hari");
//                logger.info("------after-----");
//            }
//            if (today90.before(date)) {
//                addSimpleMessage("HakMilik Ini Telah Membuat Permohonan Tangguh Sebelum Ini");
//                logger.info("------before-----");
//            }

            logger.info("Hari ke 90       : " + today90);
            logger.info("Sdf Format today : " + sdf.format(date).substring(0, 10));
            logger.info("Sdf Format +90   : " + sdf.format(today90).substring(0, 10));
        }
        return new JSP("lelong/lelong_batal.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution simpan() {
        ...
        if (idMohonla != null) {
            ...
            if (!listEnkuiri.isEmpty()) {
                ...
                //added by nur.aqilah
                //simpan id mohon pptl (id mohon tangguh) dlm table lelong
                List<Lelongan> listLelong = lelongService.listLelonganAK1(idMohonla);
                ...
                if (!listLelong.isEmpty()) {
                    for (Lelongan ll : listLelong) {
                        logger.info("lelong : " + ll.getIdLelong());
                        ll.setPermohonanTangguh(permohonan);
        ...
        PermohonanAsal pasal = null;
        List<PermohonanAsal> list = lelongService.listMohonAsal2(idPermohonan);
        if (list.isEmpty()) {
            pasal = new PermohonanAsal();
        } else {
            pasal = list.get(0);
        }
        pasal.setInfoAudit(ia);
        pasal.setIdPermohonan(permohonan);
        pasal.setIdPermohonanAsal(permohonanSblm);
        pasal.setCawangan(permohonan.getCawangan());
    ...
    public HakmilikPermohonan getHakmilikPermohonan2() {
        return hakmilikPermohonan2;
    }

    public void setHakmilikPermohonan2(HakmilikPermohonan hakmilikPermohonan2) {
        this.hakmilikPermohonan2 = hakmilikPermohonan2;
    }
