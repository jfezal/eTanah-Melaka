 ...
 * @author afham
 */
public class BatuanPTGKhasValidator implements StageListener {
        ...
        for (Task t : l) {
            if (t.getSystemAttributes().getStage() != null) {
                ...
                if (stage.contentEquals("sedia_surat_kelulusan")) {
                    if (p.getKeputusan() != null) {
                        if (p.getKeputusan().getKod().equals("L")) {
                            ...
                            gen1 = "DISSrtKpsnMMKNPBPTGG_MLK.rdf";
                            code1 = "SKM";
                            kd = kodDokumenDAO.findById(code1);
                            LOGGER.info(kd);
                            d = saveOrUpdateDokumen(fd, kd, p.getIdPermohonan());
                    ...
                    if (p.getKeputusan() != null) {
                        if (p.getKeputusan().getKod().equals("L")) {
                            if (!p.getKodUrusan().getKod().equals("LPSP")) {
                                ...
                                gen1 = "DISSrtKpsnMMKNLPSPL_MLK.rdf";
                            }
                            code1 = "SKM";
                            kd = kodDokumenDAO.findById(code1);
                            LOGGER.info(kd);
                            ...
                            //gen1 = "DISSrtKpsnMMKNPBPTGG_MLK.rdf";
                            gen1 = "DISSrtKpsnMMKNLPSPG_MLK.rdf";
                            code1 = "SKM";
                            kd = kodDokumenDAO.findById(code1);
                            LOGGER.info(kd);
                            d = saveOrUpdateDokumen(fd, kd, p.getIdPermohonan());
                ...
                if (stage.contentEquals("sedia_surat_lulustolak")) {
                    ...
                    String statusAliran = new String();
                    if (p.getKodUrusan().getKod().equalsIgnoreCase("PBPTG")) {
                        idAliranMhonFasa = "semak_peraku"; //INTENTIONALLY HARDCODE, SINCE KPSN ONLY AT THIS STAGE
                        mohonFasa = pelupusanService.findMohonFasaByIdMohonIdPengguna(p.getIdPermohonan(), idAliranMhonFasa);

                    } else if (p.getKodUrusan().getKod().equalsIgnoreCase("PPBB")) {
                        idAliranMhonFasa = "perakuan_rencana_jkbb"; //INTENTIONALLY HARDCODE, SINCE KPSN ONLY AT THIS STAGE
                        mohonFasa = pelupusanService.findMohonFasaByIdMohonIdPengguna(p.getIdPermohonan(), idAliranMhonFasa);
                        statusAliran = "JKBB";
                        if (mohonFasa == null) {
                            statusAliran = new String();
                    ...
                    if (!StringUtils.isEmpty(value)) {
                        if (value.equals("L")) {
                            if (statusAliran.equalsIgnoreCase("JKBB")) {
                                gen1 = "DISSKpsnLPPBBJKBB_MLK.rdf";
                            } else if (statusAliran.equalsIgnoreCase("KM")) {
                                gen1 = "DISSKpsnLPPBBKM_MLK.rdf";
                            } else {
                                gen1 = "DISSKpsnLPBPTG_MLK.rdf";
                            }
                            code1 = "SL";
                            kd = kodDokumenDAO.findById(code1);
                            LOGGER.info(kd);
