<%-- 
    Document   : kertasRisalatPerihalTanah
    Created on : Jun 4, 2013, 7:55:31 AM
    Author     : khadijah
--%>
...
<script type="text/javascript">
    ...
    function deleteRow(idKandungan,f,tName)
    {
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/hasil/kertas_risalat?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kTanah',q,
            function(data){
                $('#page_div').html(data);
