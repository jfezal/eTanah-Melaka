...
@UrlBinding("/strata/kertas_MMKNew")
public class KertasMMKNewActionBean extends AbleActionBean {
    ...
    private List<HakmilikPihakBerkepentingan> hakmilikPihak;
    private List<HakmilikUrusan> hakmilikUrusanK = new ArrayList<HakmilikUrusan>();
    private List<HakmilikUrusan> hakmilikUrusanG = new ArrayList<HakmilikUrusan>();
    private List<HakmilikUrusan> hakmilikUrusanP = new ArrayList<HakmilikUrusan>();
    ...
    private List<PermohonanKertasKandungan> senaraiLaporanKandungan7;
    private PermohonanKertasKandungan mohonKertasKandungan;
    private KodDokumen kd = new KodDokumen();
    private String kodNegeri;
    private String tarikhMesyuarat;
    private boolean tarikhMesyuarat2;
    private int bngnTotal;
    private int tingkatTotal;
    private int petakTotal;
    private int tempohPegangan;
    private boolean tempohPegangan1;
    NumberFormat money = new DecimalFormat("##,##,##0.00");
    private static final Logger LOG = Logger.getLogger(KertasMMKNewActionBean.class);

    @DefaultHandler
    public Resolution showForm() {
//        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        getContext().getRequest().setAttribute("display", Boolean.TRUE);
        getContext().getRequest().setAttribute("asas", Boolean.FALSE);
        getContext().getRequest().setAttribute("syor", Boolean.FALSE);
        getContext().getRequest().setAttribute("keputusan", Boolean.FALSE);
        getContext().getRequest().setAttribute("maklumat1", Boolean.TRUE);
        return new JSP("strata/kos_rendah/kertas_mmkNew.jsp").addParameter("tab", "true");
    }

    public Resolution showForm2() {

        getContext().getRequest().setAttribute("asas", Boolean.TRUE);
        getContext().getRequest().setAttribute("syor", Boolean.FALSE);
        getContext().getRequest().setAttribute("keputusan", Boolean.FALSE);
        getContext().getRequest().setAttribute("maklumat2", Boolean.TRUE);
        return new JSP("strata/kos_rendah/kertas_mmkNew.jsp").addParameter("tab", "true");
    }

    public Resolution showForm3() {
        getContext().getRequest().setAttribute("asas", Boolean.TRUE);
        getContext().getRequest().setAttribute("syor", Boolean.TRUE);
        getContext().getRequest().setAttribute("keputusan", Boolean.FALSE);
        getContext().getRequest().setAttribute("maklumat3", Boolean.TRUE);
        return new JSP("strata/kos_rendah/kertas_mmkNew.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution showForm4() {
        ...
        getContext().getRequest().setAttribute("maklumat4", Boolean.TRUE);
        return new JSP("strata/kos_rendah/kertas_mmkNew.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate() {
        ...
        if (mohonHakmilik.getHakmilik() != null) {
            if (mohonHakmilik.getHakmilik().getTempohPegangan() != null) {
                tempohPegangan1 = false;
            } else {
                tempohPegangan1 = true;
            }
        }
        ...
        tarikhMesyuarat2 = false;

        if (pemohon == null || permohonanStrata == null) {
            addSimpleError("Sila masukkan maklumat di tab sebelum.");
        } else if (permohonanKertas != null) {
            ...
            if (pemohon != null) {
                ...
                hakmilikPihak = strService.findPMAktif(mohonHakmilik.getHakmilik().getIdHakmilik());
                List<HakmilikUrusan> huList = strService.findByID(mohonHakmilik.getHakmilik().getIdHakmilik());

                hakmilikUrusanK.clear();
                hakmilikUrusanG.clear();
                hakmilikUrusanP.clear();

                for (HakmilikUrusan hu : huList) {
                    if (hu.getKodUrusan().getKod().startsWith("KV")) {
                        hakmilikUrusanK.add(hu);
                    }
                    if (hu.getKodUrusan().getKod().startsWith("G")) {
                        hakmilikUrusanG.add(hu);
                    }
                    if (hu.getKodUrusan().getKod().contains("PJ")) {
                        hakmilikUrusanP.add(hu);
                    }
                }
...
//            asaspermohon2 = "PERKARA-PERKARA UTAMA YANG PERLU DIURUSKAN BAGI BANGUNAN KOS RENDAH ADALAH SEPERTI BERIKUT:-";
                ...
                senaraiLaporanKandungan5 = strService.findByIdLapor(permohonanKertas.getIdKertas(), 4);
                if (senaraiLaporanKandungan5.isEmpty() || senaraiLaporanKandungan5 == null) {

                    syorpengarah1 = "Adalah disyorkan permohonan supaya diklasifikasikan sebagai bangunan kos rendah ini diperakukan "
                            + "Sijil Kos Rendah bagi " + permohonanStrata.getNamaSkim() + " " + lot + " "
                            + nlot + " " + mohonHakmilik.getHakmilik().getKodHakmilik().getKod() + " "
                            + mohonHakmilik.getHakmilik().getNoHakmilik() + ", " + mohonHakmilik.getHakmilik().getBandarPekanMukim().getNama().toUpperCase()
                            + ", Daerah" + " " + daerah + " "
                            + "berdasarkan asas-asas pertimbangan seperti di para 3";
                }
                ...
                if (senaraiLaporanKandungan6.isEmpty() || senaraiLaporanKandungan6 == null) {
                    ...
                    if (kodNegeri.equals("04")) {
                        ...
                        keputusan1 = "Permohonan supaya diklasifikasi sebagai kos rendah seperti perakuan di para 4 diluluskan/ditolak.";
                    } else {
            ...
            if (pemohon != null) {
                ...
                if ((mohonHakmilik != null) && (permohonanStrata != null)) {
                    tajuk = "KERTAS PERTIMBANGAN BAGI PERMOHONAN SUPAYA DIKELASKAN SEBAGAI BANGUNAN KOS RENDAH  DI BAWAH SEKSYEN 9B(1) AKTA HAKMILIK STRATA 1985 DI ATAS "
                            + mohonHakmilik.getHakmilik().getLot().getNama().toUpperCase() + " " + nLot + ", " + mohonHakmilik.getHakmilik().getKodHakmilik().getKod() + " "
                            + mohonHakmilik.getHakmilik().getNoHakmilik() + ", " + mohonHakmilik.getHakmilik().getBandarPekanMukim().getNama().toUpperCase()
                            + ", DAERAH " + mohonHakmilik.getHakmilik().getDaerah().getNama().toUpperCase();

                    tujuan1 = "Tujuan kertas ini disediakan bagi mempertimbangkan " + permohonanStrata.getPermohonan().getKodUrusan().getNama() + " daripada "
                            + pnama + ".";
...
//            billNo.add("4");
        ...
        PermohonanKertas MohonKertas2 = strService.findKertasByKod(idPermohonan, "MU");
        if (MohonKertas2 != null) {
            senaraiLaporanKandungan7 = strService.findKandunganByIDKertas(MohonKertas2.getIdKertas());
            mohonKertasKandungan = senaraiLaporanKandungan7.get(0);
        }
        if (mohonHakmilik != null) {
            if (mohonHakmilik.getHakmilik().getTarikhLuput() != null) {
                tarikhMesyuarat = sdf.format(mohonHakmilik.getHakmilik().getTarikhLuput());
            }
        }
        if (permohonanStrata != null) {
            latarbelakang1 = sdf.format(permohonanStrata.getCfTarikhKeluar());
        }
    }
    ...
    public Resolution simpanTanggungan() {
        LOG.info("------------Simpan started-----------::");
        String idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        String tujuan = getContext().getRequest().getParameter("tujuan");
        permohonan = permohonanDAO.findById(idPermohonan);
        Pengguna peng = (Pengguna) getContext().getRequest().getSession().getAttribute(etanahActionBeanContext.KEY_USER);
        KodCawangan cawangan = kodCawanganDAO.findById(peng.getKodCawangan().getKod());
        InfoAudit infoAudit = new InfoAudit();
        ...
        if (MohonKertas == null) {
            ...
            mohonKertasNew.setCawangan(cawangan);
        ...
        senaraiLaporanKandungan1 = strService.findKandunganByIDKertas(MohonKertas2.getIdKertas());
        PermohonanKertasKandungan permohonanKertasKand;
...
//        PermohonanKertasKandungan pkk = null;
        if (senaraiLaporanKandungan1.size() == 0) {
            ...
            permohonanKertasKand.setKandungan(tujuan);
            permohonanKertasKand.setCawangan(cawangan);
            permohonanKertasKand.setKertas(MohonKertas2);
            infoAudit = new InfoAudit();
            infoAudit.setDimasukOleh(peng);
            infoAudit.setTarikhMasuk(new java.util.Date());
            permohonanKertasKand.setInfoAudit(infoAudit);
            ...
            for (PermohonanKertasKandungan kertasKand : senaraiLaporanKandungan1) {
                kertasKand.setKandungan(tujuan);
                kertasKand.setInfoAudit(infoAudit);
                kertasKand.setCawangan(cawangan);
                kertasKand.setCawangan(cawangan);
                kertasKand.setKertas(MohonKertas2);
                strService.simpanPermohonanKertasKandungan(kertasKand);
                addSimpleMessage("Maklumat Telah Berjaya Disimpan.");
            }
        }
...
//        ArrayList<String> billNo = new ArrayList<String>();
    ...
    public Resolution simpanSyorPengarah() {
        ...
        if (pkk != null) {
            addSimpleMessage("Maklumat Syor dan Perakuan Timbalan Pengarah Tanah Dan Galian Telah Berjaya Disimpan.");
        } else {
            addSimpleError("Maklumat Syor dan Perakuan Timbalan Pengarah Tanah Dan Galian Gagal Disimpan");
        }
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        getContext().getRequest().setAttribute("display", Boolean.FALSE);
        getContext().getRequest().setAttribute("asas", Boolean.TRUE);
    ...
    public Resolution simpanKeputusan() {
        ...
        senaraiLaporanKandungan6 = strService.findByIdLapor(permohonanKertas.getIdKertas(), 5);
        PermohonanKertasKandungan permohonanKertasKand;
    ...
    public List<HakmilikPihakBerkepentingan> getHakmilikPihak() {
        return hakmilikPihak;
    }

    public void setHakmilikPihak(List<HakmilikPihakBerkepentingan> hakmilikPihak) {
        this.hakmilikPihak = hakmilikPihak;
    }

    public List<HakmilikUrusan> getHakmilikUrusanK() {
        return hakmilikUrusanK;
    }

    public void setHakmilikUrusanK(List<HakmilikUrusan> hakmilikUrusanK) {
        this.hakmilikUrusanK = hakmilikUrusanK;
    }

    public List<HakmilikUrusan> getHakmilikUrusanG() {
        return hakmilikUrusanG;
    }

    public void setHakmilikUrusanG(List<HakmilikUrusan> hakmilikUrusanG) {
        this.hakmilikUrusanG = hakmilikUrusanG;
    }

    public List<HakmilikUrusan> getHakmilikUrusanP() {
        return hakmilikUrusanP;
    }

    public void setHakmilikUrusanP(List<HakmilikUrusan> hakmilikUrusanP) {
        this.hakmilikUrusanP = hakmilikUrusanP;
    }
    ...
    public int getBngnTotal() {
        return bngnTotal;
    }

    public void setBngnTotal(int bngnTotal) {
        this.bngnTotal = bngnTotal;
    }

    public int getTingkatTotal() {
        return tingkatTotal;
    }

    public void setTingkatTotal(int tingkatTotal) {
        this.tingkatTotal = tingkatTotal;
    }

    public int getPetakTotal() {
        return petakTotal;
    }

    public void setPetakTotal(int petakTotal) {
        this.petakTotal = petakTotal;
    }
    ...
    public PermohonanKertasKandungan getMohonKertasKandungan() {
        return mohonKertasKandungan;
    }

    public void setMohonKertasKandungan(PermohonanKertasKandungan mohonKertasKandungan) {
        this.mohonKertasKandungan = mohonKertasKandungan;
    }
    ...
    public boolean isTempohPegangan1() {
        return tempohPegangan1;
    }

    public void setTempohPegangan1(boolean tempohPegangan1) {
        this.tempohPegangan1 = tempohPegangan1;
    }

    public boolean isTarikhMesyuarat2() {
        return tarikhMesyuarat2;
    }

    public void setTarikhMesyuarat2(boolean tarikhMesyuarat2) {
        this.tarikhMesyuarat2 = tarikhMesyuarat2;
    }
