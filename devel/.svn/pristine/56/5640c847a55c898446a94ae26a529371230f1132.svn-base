<%-- 
    Document   : PenerimaanNotisBorang831BC
    Created on : 27-Jul-2011, 17:37:43
    Author     : nordiyana
--%>
...
<script type="text/javascript">
    ...
    function muatNaikForm(idPihak) {
        ...
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?popupUpload&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&showHP='+showHP;
    ...
    function muatNaikFormPemohon(idPihak) {
        ...
        var left = (screen.width/2)-(1000/2);
        var top = (screen.height/2)-(150/2);
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?popupUpload&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&showHP='+showHP+'&isPemohonPihak=true';
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=1000,height=200, left=" + left + ",top=" + top);
    }
    ...
    function scanPemohon(idPihak) {
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?popupScanPemohon&idPihak='+idPihak+'&idHakmilik='+idHakmilik+'&showPP='+showPP+'&isPemohonPihak=true';
    ...
    function addPenghantarNotis(){
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        window.open("${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?popupPenghantarNotis&idHakmilik="+idHakmilik+'&showPP='+showPP+'&showHP='+showHP, "eTanah","status=0,toolbar=0,location=0,menubar=0,width=900,height=400");
    }
    ...
    function scan(idPihak) {
        ...
        var showPP = $('#showPP').val();
        var showHP = $('#showHP').val();
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?popupScan&idPihak='+idPihak+'&idHakmilik='+idHakmilik;
        window.open(url, 'etanah2', "status=0,toolbar=0,location=0,menubar=0,width=800,height=700");
    }


    function refreshPagePenerimaanBorang(){
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?refreshpage';
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function notis(h){
        var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?notisPopup&idPihak='+h;
    ...
    function refreshNotis(idHakmilik,showPP,showHP,isMahkamah){
        if(showHP == 'true'){
            if(idHakmilik !=''){
                var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?hakmilikDetails&idHakmilik='+idHakmilik+"&showPP="+showPP;
                $.get(url,
                function(data){
                    $('#page_div').html(data);
                },'html');
                ...
                if(showPP == 'true'){
                    var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?showFormPP';
                    $.get(url,
                    function(data){
                        $('#page_div').html(data);
                    },'html');
            ...
            var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?showFormPemohon';
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
        }else if(isMahkamah == 'true'){
            var url = '${pageContext.request.contextPath}/pengambilan/penerimaan_notis_831BC?showFormMahkamah';
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
...
<s:form beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorang831BCActionBean">
        ...
        <fieldset class="aras1"><br/>
            ...
            <p align="center">
                <display:table class="tablecloth" name="${actionBean.hakmilikPermohonanList}" pagesize="5" cellpadding="0" cellspacing="0"
                               requestURI="/pengambilan/penerimaan_notis_831BC" id="line">
                    <display:column title="No" sortable="true">${line_rowNum}</display:column>
                    <display:column title="ID Hakmilik">
                        <s:link beanclass="etanah.view.stripes.pengambilan.PenerimaanNotisBorang831BCActionBean"
            ...
            <c:if test="${actionBean.showPP eq true && ((fn:length(actionBean.listNotisPemohonASB) > 0) ||(fn:length(actionBean.listNotisPemohonPB) > 0))}">
                <div align="center" id="pemohonList">
                    <display:table class="tablecloth" name="${actionBean.permohonan.senaraiPemohon}"
                                   cellpadding="0" cellspacing="0" requestURI="/pengambilan/penerimaan_notis_831BC" id="line">
