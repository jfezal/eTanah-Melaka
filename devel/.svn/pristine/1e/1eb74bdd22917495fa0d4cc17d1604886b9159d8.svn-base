...
<script type="text/javascript">
    ...
    function tambahKawasan(){
        ...
        window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?kawasanPopup&idHakmilik="+idHakmilik, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=910,height=800");
    }
    function editLaporKawasan(idMohonlaporKws){
        NoPrompt();
        var idHakmilik = $('#idHakmilik').val();
        window.open("${pageContext.request.contextPath}/pembangunan/laporanTanahv2?kawasanPopupUpdate&idMohonKw="+idMohonlaporKws+"&idHakmilik="+idHakmilik, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=910,height=800");
    }
    ...
    {   NoPrompt();
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pembangunan/laporanTanahv2?deleteRow&idKandungan='+idMohonlaporKws+'&tName='+tName,q,
            function(data){
                $('#page_div').html(data);
    ...
    function refreshpage2(type){
        //        alert(type);
        var idHakmilik = $('#idHakmilik').val();
        var url = '${pageContext.request.contextPath}/pembangunan/laporanTanahv2?refreshpagekawasandalam&type='+type+'&idHakmilik='+idHakmilik;
