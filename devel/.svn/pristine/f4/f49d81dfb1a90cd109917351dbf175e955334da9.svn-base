 ...
 * @author Rajesh
 */
@UrlBinding("/pengambilan/mmknizinlalu_mlk")
public class mmkn_izinlalu_mlkActionBean extends AbleActionBean {

    private static Logger logger = Logger.getLogger(mmkn_izinlalu_mlkActionBean.class);
    @Inject
...
//    private int bil = 0;
    ...
    private List<HakmilikPihakBerkepentingan> tuanTanahList;
    ...
    private String tajuk = "KERTAS PERMOHONAN IZIN LALU ELEKTRIK";
    private String heading1;
    private String heading2;
    private String heading3;
    private String namaDaerah;
    private String bandarPekanMukim;
    private StringBuffer senaraiTuanTanah;
    private StringBuffer senaraiTuanTanahLastRecord;
    private StringBuffer senaraiTuanTanah2;
    private char letter;
    
    @DefaultHandler
    public Resolution showForm() {
        getContext().getRequest().setAttribute("form1", Boolean.TRUE);
        return new JSP("pengambilan/melaka/mmkn_izinlalu_mlk.jsp").addParameter("tab", "true");
    }
    ...
    public void rehydrate() {
        ...
        senaraiTuanTanah = new StringBuffer();
...
//            tajuk=keyboard.nextLine();
//            letter=tajuk.charAt(0);
//            for(int i=0;i<=tajuk.length();i++)
//            {
//                if(tajuk.charAt(i)==letter)
//                {
//                    count++;
//
//
...
//            logger.info(letter+" - "+count);
        ...
        if (idPermohonan != null) {
            ...
            for(HakmilikPermohonan hp1:hakmilikPermohonanList){
                ...
                tuanTanahList=hakmilik.getSenaraiPihakBerkepentingan();
                lastRecordIndex=tuanTanahList.size()-1;
                for(HakmilikPihakBerkepentingan p:tuanTanahList)
                {
                senaraiTuanTanah = senaraiTuanTanah.append(p.getPihak().getNama()).append(", ");
                logger.info("senaraiTuanTanah "+senaraiTuanTanah);
                }

                for(int i=0;i<=tuanTanahList.size();i++){
//                senaraiTuanTanah2 = senaraiTuanTanah2.append(tuanTanahList.get(i).getPihak().getNama()).append(", ");
//                tuanTanahList.lastIndexOf(i);
                logger.debug("last index value in tuan tanah list "+tuanTanahList.lastIndexOf(i));
//                if(i==lastRecordIndex)
//               {
//               senaraiTuanTanahLastRecord = senaraiTuanTanahLastRecord.append(tuanTanahList.get(lastRecordIndex).getPihak().getNama());
//                logger.info("senaraiTuanTanahLastRecord "+senaraiTuanTanahLastRecord);
//                }
            ...
            if(!hakmilikPermohonanList.isEmpty()){
                for (int w = 0; w < hakmilikPermohonanList.size(); w++){
                    ...
                    bandarPekanMukim=WordUtils.capitalizeFully(hakmilik.getBandarPekanMukim().getNama());
                    logger.info("namaDaerah "+namaDaerah);
                    logger.info("bandarPekanMukim "+bandarPekanMukim);
...
//                        StringBuilder sb = new StringBuilder();
                        ...
                        if(w==0){
                            lokasi = hakmilik.getLot().getNama()+" "+hakmilik.getNoLot()+", "+hakmilik.getKodHakmilik().getKod().toUpperCase()+ hakmilik.getLuas()+", " +
                                    bandarPekanMukim+", Daerah " +namaDaerah+", ";
                        }

                        if(w > 0){
                            if (w <= ((hakmilikPermohonanList.size() + 2) - 4)) {
                                lokasi = lokasi +""+hakmilik.getLot().getNama()+""+ hakmilik.getNoLot()+", "+hakmilik.getKodHakmilik().getKod().toUpperCase()+ hakmilik.getLuas()+", " +
                                        bandarPekanMukim+", Daerah " + namaDaerah+", ";
                            } else if (w == (hakmilikPermohonanList.size() - 1)) {
                                lokasi = lokasi +""+hakmilik.getLot().getNama()+" dan " + hakmilik.getNoLot()+", "+hakmilik.getKodHakmilik().getKod().toUpperCase()+ hakmilik.getLuas()+"," +
                                        bandarPekanMukim+", Daerah " + namaDaerah+", ";
                            }
                        }
                 ...
                 tujuan = "Kertas ini disediakan untuk mendapat pertimbangan Majlis Mesyuarat Kerajaan Negeri, Negeri Melaka ke atas perintah yang dikeluarkan oleh Pentadbir Tanah "+namaDaerah+" berkenaan dengan bantahan daripada "
                            +WordUtils.capitalizeFully(senaraiTuanTanah.toString()) +" terhadap kemasukan dan pampasan oleh Tenaga Nasional Berhad ke atas tanah "+lokasi+" bagi projek diatas.";
                  logger.info("namaDaerah "+namaDaerah);
                  logger.info("Tuan Tanah "+senaraiTuanTanah);
                  logger.info("Lokasi "+lokasi);

                  String temp = "RAYUAN TERHADAP PERINTAH YANG DIKELUARKAN OLEH PENTADBIR TANAH "+ namaDaerah.toUpperCase()+
                          " DI BAWAH SEKSYEN 11(7) AKTA BEKALAN ELEKTRIK 1990 OLEH "+ senaraiTuanTanah + 
                " SEBAGAI TUAN TANAH " + lokasi + " TERHADAP KEMASUKAN DAN PAMPASAN OLEH TENAGA NASIONAL BERHAD BAGI "+permohonan.getSebab().toUpperCase();
...
//                senaraiKertasKandungan4 = pendudukanSementaraMMKNService.findByKertasSubTajuk(permohonanKertas.getIdKertas(),4,subtajuk);
                ...
                kandList4 = pendudukanSementaraMMKNService.findByKertasDesc(permohonanKertas.getIdKertas(),4);
                if(kandList4!=null && !kandList4.isEmpty()){
                    ...
                    if(maxKertasKand4!=null){
                            ...
                            for(int k=1;k<=tableCount;k++){
                                ...
                                senaraiPerakuanPengarah[k] = pendudukanSementaraMMKNService.findByKertasSubTajuk(permohonanKertas.getIdKertas(),4,subtajuk1);
                            }
                    }
                }//if
...
//        }//if table count
        ...
        getContext().getRequest().setAttribute("form1", Boolean.TRUE);
        return new RedirectResolution(mmkn_izinlalu_mlkActionBean.class, "locate");
    }
...
//    }
    ...
    public List<HakmilikPihakBerkepentingan> getTuanTanahList() {
        return tuanTanahList;
    }

    public void setTuanTanahList(List<HakmilikPihakBerkepentingan> tuanTanahList) {
        this.tuanTanahList = tuanTanahList;
    }
    
    public StringBuffer getSenaraiTuanTanah() {
        return senaraiTuanTanah;
    }

    public void setSenaraiTuanTanah(StringBuffer senaraiTuanTanah) {
    ...
    public StringBuffer getSenaraiTuanTanahLastRecord() {
        return senaraiTuanTanahLastRecord;
    }

    public void setSenaraiTuanTanahLastRecord(StringBuffer senaraiTuanTanahLastRecord) {
        this.senaraiTuanTanahLastRecord = senaraiTuanTanahLastRecord;
    }
    public StringBuffer getSenaraiTuanTanah2() {
        return senaraiTuanTanah2;
    }

    public void setSenaraiTuanTanah2(StringBuffer senaraiTuanTanah2) {
        this.senaraiTuanTanah2 = senaraiTuanTanah2;
    }
