 ...
 * @author mohd.faidzal
 */
public class SemakBorangJKTandatanganValidator implements StageListener{
     private static final Logger LOG = Logger.getLogger(SemakBorangJKTandatanganValidator.class);
    @Inject
    ...
    public void onGenerateReports(StageContext context) {
        ...
        if (permohonanPengambilan != null) {
          //  if (permohonanPengambilan.getKatPengambilan().equals("1")) {
                namaReport = new String[4];
                kodReport = new String[4];
                namaReport[0] = "ACQSrtIringanBorangK_MLK.rdf";
                kodReport[0] = "SIIK";
                namaReport[1] = "ACQDocJ_MLK.rdf";
                kodReport[1] = "J";
                 namaReport[2] = "ACQDocK_MLK.rdf";
                kodReport[2] = "K";
                namaReport[3] = "ACQSrtIringanBorangJ_MLK.rdf";
                kodReport[3] = "SIIJ";
    ...
    public void janaDokumen(Permohonan permohonan, Pengguna pengguna, String namaReport, String kodReport) {
        try {
            ...
            if (kd.getKod().equals("J") || kd.getKod().equals("K")) {
    ...
    private void saveBorangPerMohon(Dokumen d, Pengguna pengguna, Permohonan permohonan, KodDokumen kd) {
        ...
        if (kd.getKod().equals("J")) {
            kn = kodNotisDAO.findById("NBJ");
            bp = borangACQService.findBorangA(permohonan.getIdPermohonan(), "NBJ");
        } else {
            kn = kodNotisDAO.findById("NBK");
            bp = borangACQService.findBorangA(permohonan.getIdPermohonan(), "NBK");
        }
        if (bp != null) {
        } else {
