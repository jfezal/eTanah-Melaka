...
@UrlBinding("/kaunter/terima_bayaran")
public class TerimaBayaran extends AbleActionBean {
    ...
    private static final Logger LOG = Logger.getLogger(TerimaBayaran.class);
    ...
    public String getHakmlk() {
        return hakmlk;
    }

    public void setHakmlk(String hakmlk) {
        this.hakmlk = hakmlk;
    }
...
//        mhnSblm = m.getPermohonanSebelum();
        ...
        /*End*/
        LOG.info("senaraiUrusan ------------------- :" + senaraiUrusan);
        LOG.info("Bil senaraiUrusan ------------------- :" + senaraiUrusan.size());
        if (senaraiUrusan == null || senaraiUrusan.size() == 0) {
            ...
            LOG.info("Penyerah------------------- :" + penyerahKod);
            LOG.info("Penyerah Nama------------------- :" + penyerahNama);
            LOG.info("ID SEBELUM------------------- :" + idSblm);
            LOG.info("MOHN SEBELUM------------------- :" + mhnSblm.getIdPermohonan());
            ...
            if ("05".equalsIgnoreCase(conf.getProperty("kodNegeri"))) {
                if (idSblm != null) {
                    if (mhnSblm != null) { //added by murali (STRATA)
                        LOG.info("idPenyerah---------" + mhnSblm.getIdPenyerah());
                        LOG.info("kodPenyerah---------" + mhnSblm.getKodPenyerah());
                        LOG.info("penyerahNama---------" + mhnSblm.getPenyerahNama());
                        penyerahKod = mhnSblm.getKodPenyerah();
                        penyerahNama = mhnSblm.getPenyerahNama();
                        idPenyerah = String.valueOf(mhnSblm.getIdPenyerah());
                    }
                }
            } else {
                if (idSblm != null) {
                    if (mhnSblm != null) { //added by murali (STRATA)
                        ...
                        penyerahKod = mhnSblm.getKodPenyerah();
                        penyerahNama = mhnSblm.getPenyerahNama();
                        idPenyerah = (mhnSblm.getIdPenyerah()!=null?mhnSblm.getIdPenyerah().toString():null);
                    }
                }
            }/*End*/
    ...
    public Resolution collectPayment() {
        ...
        return new ForwardResolution("/WEB-INF/jsp/kaunter/bayaran_2.jsp");
    }
        ...
        try {
            ...
            for (int g = 0; g < size; g++) {
                ...
                List<UrusanKaunter> luk = (List<UrusanKaunter>) u.doSave(ctx.getKodNegeri(), pengguna, resit,
                        ...
                        penyerahEmail, penyerahNoTelefon, namaPenyampai, noPengenalanPenyampai,noTelPenyampai, ia);
            ...
            if (senaraiUrusan.get(0).getKodUrusan().equalsIgnoreCase("PHPC")) {
                ...
                if ("05".equals(conf.getProperty("kodNegeri"))) {
                    ...
                    if (bilMohon == 1) {
                        mhn.setBilanganPermohonan(2);
                    } else {
                        List<HakmilikPihakBerkepentingan> hplist = new ArrayList<HakmilikPihakBerkepentingan>();
                        String hakmilik = (String) getContext().getRequest().getSession().getAttribute("hakmlk");
                        Hakmilik hk = new Hakmilik();
                        hk = hakmilikDAO.findById(hakmilik);
                        hplist = hakmilikPihakKepentinganService.findHakmilikPihakByKod(hk, "PM");                       
                        mhn.setBilanganPermohonan(hplist.size());
                    }
                }
...
//                    return new ForwardResolution("/WEB-INF/jsp/kaunter/bayaran_2.jsp");
                ...
                for (TransaksiValue tv : allTransaksi) {
                    ...
                    //add by ida for n9 181013
                    if ("05".equals(conf.getProperty("kodNegeri"))) {
                        if (tv.getKodUrusan().equals("PBS") || tv.getKodUrusan().equals("PBTS")) {
                            LOG.info("START KEW_AKAUN");
                            Transaksi trans = new Transaksi();
                            Akaun akaun = new Akaun();
                            BigDecimal bg = new BigDecimal(0);
                            //akaun.setBaki(bg.subtract(tv.getAmaun())); //set amaun untuk blok prov sahaja
                            akaun.setBaki(bg);
                            ...
                            KodStatusAkaun kodsts = kodStatusAkaunDAO.findById("A");
...
//                        akaun.setPermohonan(senaraiUrusan.get(0).getPermohonan());
                            ...
                            LOG.info("START MOHON TUNTUT KOS");
                            if ("05".equals(conf.getProperty("kodNegeri"))) {
                                PermohonanTuntutanKos mohonTuntutKos = new PermohonanTuntutanKos();
                                //Pengguna pguna = (Pengguna) 

                                InfoAudit infoAudit = new InfoAudit();
                                infoAudit.setDimasukOleh(pengguna);
                                infoAudit.setTarikhMasuk(new java.util.Date());
                                KodTransaksi pk = kodTransaksiDAO.findById("79507");
                                mohonTuntutKos.setInfoAudit(infoAudit);
                                mohonTuntutKos.setCawangan(pengguna.getKodCawangan());

                                mohonTuntutKos.setPermohonan(senaraiUrusan.get(0).getPermohonan());
                                mohonTuntutKos.setItem(pk.getNama());
                                BigDecimal bd = new BigDecimal(500);
                                //BigDecimal prov = new BigDecimal((tv.getAmaun().intValue()) * (bd.intValue()));
                                BigDecimal prov = new BigDecimal(0);
                                mohonTuntutKos.setAmaunTuntutan(prov);

                                KodTuntut kt = kodTuntutDAO.findById("J01");
                                mohonTuntutKos.setKodTuntut(kt);

                                //PermohonanTuntutanKos pk = senaraiMohonTuntutKos1.get(0);
                                LOG.info("pk" + pk.getNama());
                                mohonTuntutKos.setKodTransaksi(pk);
                                mohonTuntutKos.setAmaunSeunit(bd);
                                mohonTuntutKos.setKuantiti(0);
                                strService.simpanBayaran(mohonTuntutKos);
                                LOG.info("bayaran Deposit saved");
...
//                        akaun.setPermohonan(senaraiUrusan.get(0).getPermohonan());
                        ...
                        kodStatusAkaun = pengambilanService.findKodStatusAkaun("A");
        ...
        for (UrusanKaunter u : allUrusan) {
            if (u.hasToInitiateWorkflow()) {
                ...
                String kodCaw = caw.getKod() + (ku.getKePTG() == 'Y' ? ",00" : "");
                    try {
                        WorkFlowService.initiateTask(u.getPermohonan().getIdWorkflow(),
        ...
        //add by azri 25/6/2013: add SB to "wakil_kuasa" table 
        ...
        return new ForwardResolution("/WEB-INF/jsp/kaunter/cetak_resit_2.jsp");
    }
...
//                    p.setSuratNegeri(penyerahNegeri);
    ...
    public Resolution tambahUrusan() {
        return new RedirectResolution(PermohonanKaunter2.class).addParameter("tambahUrusan", "Y");
    }
    ...
    private void saveCaraBayaran(KodCawangan caw, DokumenKewangan dk, InfoAudit ia) throws ParseException {
        ...
        LOG.info("************************************************ baki : "+baki);
        LOG.info("************************************************ dk.getAmaunBayaran() : "+dk.getAmaunBayaran());
        ...
        for (int m = 0; m < senaraiCaraBayaran.size(); m++) {
            ...
            if (cb.getKodCaraBayaran() != null && !cb.getKodCaraBayaran().getKod().equals("0")
                ...
                if ((crByr.getKod().equals("CB")) || (crByr.getKod().equals("CC")) || (crByr.getKod().equals("CT"))
                        || (crByr.getKod().equals("CL")) || (crByr.getKod().equals("IB")) || (crByr.getKod().equals("BC")) || (crByr.getKod().equals("DB"))
                        || (crByr.getKod().equals("CS")) ||(crByr.getKod().equals("E"))) {
                ...
                if(returnBalance){
                    cb.setAmaun(dk.getAmaunBayaran());
                }
                cb.setCawangan(caw);
                cb.setInfoAudit(ia);
                cb.setAktif('Y');
