<%-- 
    ...
    Created on : June 24, 2011, 4:57:39 PM
    Author     : massita
--%>
        ...
        <fieldset class="aras1">
            <legend> </legend>
                <display:table class="tablecloth" name="${actionBean.senaraiAmbilPampasan}" pagesize="5" cellpadding="0" cellspacing="0"
                               requestURI="/pengambilan/RekodAmbilCekPemohon" id="line">
                    <display:column title="No" sortable="true">${line_rowNum}</display:column>
                    <display:column title="Jumlah Nilai Cek(RM)">
                        RM <fmt:formatNumber pattern="#,##0.00" value="${actionBean.sum}"/>
                    </display:column>
            ...
            <%--<c:if test="${actionBean.stageId eq '20_1RekodTarikh'}">
                <display:table class="tablecloth" name="${actionBean.permohonanTuntutanKosList}" pagesize="5" cellpadding="0" cellspacing="0"
                               requestURI="/pengambilan/RekodAmbilCekPemohon" id="line">
                    <display:column title="No" sortable="true">${line_rowNum}</display:column>
                    <display:column title="Jumlah Nilai Cek(RM)">
                        RM <fmt:formatNumber pattern="#,##0.00" value="${line.amaunTuntutan}"/>
                    </display:column>
                    <display:column  title="No. Akaun" >
                        <s:hidden name="idPihak" value="${line.pihak.pihak.idPihak}"/>
                        <s:text name="catatan[${line_rowNum-1}]" id="catatan[${line_rowNum-1}]" />
                    </display:column>
                    <display:column  title="Tarikh Cek Diambil" >
                        <s:text name="tarikhDokDiambil[${line_rowNum-1}]" id="datepicker[${line_rowNum-1}]" class="datepicker" size="12" formatPattern="dd/MM/yyyy"/>
                    </display:column>
