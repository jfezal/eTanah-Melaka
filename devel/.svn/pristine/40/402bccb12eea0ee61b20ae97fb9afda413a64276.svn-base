...
@UrlBinding("/strata/kertasNew")
public class KertasPertimbanganActionBeanNew extends BasicTabActionBean {
    ...
    String syorTP;
    private static final Logger LOG = Logger.getLogger(KertasPertimbanganActionBeanNew.class);
    ...
    List<HakmilikPihakBerkepentingan> hkp;
...
//        permohonanStrata = permohonan.getSenaraiStrata().get(0);
        ...
        if (!Pb.isEmpty()) {
            ...
            if (mohonbng.getPermohonanSkim() != null) {
...
//            LOG.info("-------phb----------::" + phb);
                ...
                PermohonanSkim ms = new PermohonanSkim();
                ms.setCawangan(permohonan.getCawangan());
                int jumSyer = 0;
                for (PermohonanBangunan mb : Pb) {
                    jumSyer = jumSyer + mb.getSyerBlok();
                }
                ms.setJumlahUnitSyer(Long.valueOf(jumSyer));
                ms.setHakmilikPermohonan(hp);
                ms.setInfoAudit(infoAudit);
                strService.saveSkim(ms);
                
                for (PermohonanBangunan mb : Pb) {
                    mb.setPermohonanSkim(ms);
                    strService.saveMohonBangunan(mb);
                }
            }
        }
        
        //store id_skim pada mohon_bngn incase terlepas store masa kemasukan --nad 24/03/2023
           if (hp != null) {
                PermohonanSkim skim = new PermohonanSkim();
                skim = strService.getermohonanSkimList(hp.getId());
                LOG.info("-------list ID MH----------::" + skim);
                Pb = strService.findByIDPermohonan1(permohonan.getIdPermohonan());
                for (PermohonanBangunan mb : Pb) {
                    mb.setPermohonanSkim(skim);
                    strService.saveMohonBangunan(mb);
                }
            }            
        ...
        if (checkUrusanRayuan(permohonan)) {
            ...
            if (!(hakmilik.getSenaraiPihakBerkepentingan().isEmpty())) {
                hkp = hakmilik.getSenaraiPihakBerkepentingan();
            ...
            if (idPermohonan != null) {
                ...
                if (conf.getProperty("kodNegeri").equals("04")) {
                    if (permohonan.getKodUrusan().getKod().equals("PBBD")) {
                        List<PermohonanBangunan> mohonbgnM = strService.findByIDPermohonan(permohonan.getIdPermohonan());
                        if (mohonbgnM.isEmpty()) {
                            List<PermohonanBangunan> mohonbgnL = strService.findByIDMohonByLanded(permohonan.getIdPermohonan());//get Landed only
                            if (!mohonbgnL.isEmpty()) {
                                mohonbngn = mohonbgnL.get(0);
                                PermohonanSkim skim = new PermohonanSkim();
                                skim = strService.findSkimById(mohonbngn.getPermohonanSkim().getIdSkim());
                                jumlah = skim.getJumlahUnitSyer().intValue();
                                LOG.info("---unit syer---:" + jumlah);
                                for (PermohonanBangunan mb : mohonbgnL) {
                                    bil_petak = bil_petak + mb.getBilanganPetak();
                                }
                            }
                        } else {
                            mohonbngn = mohonbgnM.get(0);
                            PermohonanSkim skim = new PermohonanSkim();
                            skim = strService.findSkimById(mohonbngn.getPermohonanSkim().getIdSkim());
                            jumlah = skim.getJumlahUnitSyer().intValue();
                            LOG.info("---unit syer---:" + jumlah);
...
//                }
                if (!(listPemohon.isEmpty())) {
                    if (!(permohonan.getKodUrusan().getKod().equals("PHPP") || permohonan.getKodUrusan().getKod().equals("PHPC"))) {
                        ...
                        pihak = pihakDAO.findById(pemohon.getPihak().getIdPihak());
                        LOG.info("-----------pihak----------" + pihak);
                        if (permohonan.getKodUrusan().getKod().equals("SFUS")) {
                            tujuan = " Tujuan kertas ini disediakan bagi mempertimbangkan untuk kelulusan Permohonan Pengeluaran Sijil "
                                    + "Formula Unit Syer (SIFUS)(Permohonan Pertama) daripada " + pihak.getNama();
                        } else {
                            tujuan = " Tujuan kertas ini disediakan bagi mempertimbangkan untuk kelulusan " + permohonan.getKodUrusan().getNama() + " daripada " + pihak.getNama();
                        }
                    } else {
                        String nama = "";
                        int count = 1;
                ...
                if (!(permohonan.getSenaraiKertas().isEmpty())) {
                    for (int i = 0; i < permohonan.getSenaraiKertas().size(); i++) {
                        PermohonanKertas perKertas = new PermohonanKertas();
                        for (int x = 0; x < permohonan.getSenaraiKertas().size(); x++) {
                            if (permohonan.getSenaraiKertas().get(x).getKodDokumen() == null) {
                                perKertas = permohonan.getSenaraiKertas().get(x);
                            }
                        }
                        for (int j = 0; j < perKertas.getSenaraiKandungan().size(); j++) {
                            kertasKandungan = perKertas.getSenaraiKandungan().get(j);
                        ...
                        PermohonanStrata ms = strService.findIDnoIDMH(idPermohonan);
                        String tarikhCF = sdf.format(ms.getTarikhSiap());
                        String tarikhcpsp = sdf.format(ms.getMcTrhSijil());
                        String tarikhJupem = sdf.format(ms.getMcTrhRuj());
                        String tahunCF = sdf1.format(ms.getTarikhSiap());
                        if (senaraiKandungan1.size() <= 0) {
                            String asas = null;
                            if (ms != null) {
//                                asas = "a) Pemohon telah mendapat kelulusan Sijil Formula Unit Syer (SiFUS) pada " + tarikhCF + " (SF. "
                                asas = "a) Pemohon telah mendapat kelulusan Sijil Formula Unit Syer (SiFUS) pada " + tarikhCF + " ("
//                                        + ms.getNoBuku() + "/" + tahunCF + "). Pemohon juga telah memperolehi Sijil Cadangan Pelan Strata (CPSP) daripada Jabatan Ukur dan Pemetaan Melaka (JUPEM) pada "
                                        + ms.getNoBuku() + "). Pemohon juga telah memperolehi Sijil Cadangan Pelan Strata (CPSP) daripada Jabatan Ukur dan Pemetaan Melaka (JUPEM) pada "
                                        + tarikhcpsp + " (No. Siri: " + ms.getMcNoSijil() + "). Permohonan ini turut disokong oleh Jabatan Ukur dan Pemetaan Melaka seperti suratnya bil. "
                                        + ms.getMcNoRuj() + " bertarikh " + tarikhJupem
                                        + ". " + "\n" + " b) Semakan yang dibuat oleh Pentadbiran ini seterusnya mendapati bahawa permohonan tersebut telah lengkap dan"
                                        + " teratur dan memenuhi syarat - syarat kelulusan sepertimana Seksyen 10 Akta Hakmilik Strata 1985.";
                            } else {
                                asas = "Pemohon telah mendapat kelulusan Sijil Formula Unit Syer (SiFUS) ";
                            }

                            PermohonanKertasKandungan permohonanKertasKandungan1 = new PermohonanKertasKandungan();
                            permohonanKertasKandungan1.setKertas(perKertas);
                            permohonanKertasKandungan1.setBil(4);
                            permohonanKertasKandungan1.setKandungan(asas);
                            permohonanKertasKandungan1.setCawangan(permohonan.getCawangan());
                            permohonanKertasKandungan1.setSubtajuk("4.1");
                        ...
                        if (hakmilik.getSeksyen() != null) {
                            seksyen = hakmilik.getSeksyen().getNama();
                        } else {
                            seksyen = "";
                        }
                        if (senaraiKandungan3.size() <= 0) {
                            String syor = permohonan.getKodUrusan().getNama() + " daripada " + pihak.getNama() + " ke atas " + hakmilik.getLot().getNama() + " "
                                    + hakmilik.getNoLot() + ", " + hakmilik.getKodHakmilik().getNama() + " " + hakmilik.getNoHakmilik() + " "
                                    + hakmilik.getBandarPekanMukim().getNama() + seksyen + ", " + hakmilik.getDaerah().getNama() + " adalah disokong / tidak disokong.";

                            PermohonanKertasKandungan permohonanKertasKandungan1 = new PermohonanKertasKandungan();
                            permohonanKertasKandungan1.setKertas(perKertas);
                            permohonanKertasKandungan1.setBil(5);
                            permohonanKertasKandungan1.setKandungan(syor);
                            permohonanKertasKandungan1.setCawangan(permohonan.getCawangan());
                            permohonanKertasKandungan1.setSubtajuk("5.1");
                            permohonanKertasKandungan1.setInfoAudit(infoAudit);
                            strService.simpanPermohonanKertasKandungan(permohonanKertasKandungan1);
                        }
                    }
        ...
        List<FasaPermohonan> mf = strService.getSenaraiFasaPermohonan(idPermohonan);
        for (FasaPermohonan mfa : mf) {
            if (mfa.getIdAliran().equals("keputusan")) {
                keputusanPTG = mfa.getUlasan();
            }
        }

        senaraiHakmilikPermohonan = permohonan.getSenaraiHakmilik();

        PermohonanKertas mhnKertaMU = strService.findKertasByKod(idPermohonan, "MU");
        if (mhnKertaMU != null) {
            List<PermohonanKertasKandungan> kand = strService.findKandunganByIDKertas(mhnKertaMU.getIdKertas());
            for (PermohonanKertasKandungan mkk : kand) {
                tanggung = mkk.getKandungan();
            }
        }
    ...
    public Resolution showForm() {
        ...
        bil_ptkAksr = 0;
        jumB = 0;
        block = strService.findALLIDBngn(idPermohonan);
        for (PermohonanBangunan mb : block) {
            Long btgkt = strService.findSumPetakAks(mb.getIdBangunan());
            if (btgkt != null) {
                bt.add(btgkt.toString());
            }
            jumB = jumB + mb.getSyerBlok();
        }
        ...
        return new JSP("strata/pbbm/kertas_pertimbanganNew.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution showFormReadOnly() {
        ...
        if (permohonan.getKodUrusan().getKod().equals("PFUS") || permohonan.getKodUrusan().getKod().equals("SFUS")) {
            getContext().getRequest().setAttribute("edit", Boolean.TRUE);
            getContext().getRequest().setAttribute("edit3", Boolean.TRUE);
            getContext().getRequest().setAttribute("edit4", Boolean.FALSE);
            getContext().getRequest().setAttribute("edit5", Boolean.FALSE);
    ...
    public Resolution showForm3() {
        ...
        if ("05".equalsIgnoreCase(kodNegeri)) {
        ...
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new JSP("strata/pbbm/kertas_pertimbanganNS.jsp").addParameter("tab", "true");
    }
    ...
    public Resolution simpan() {
        idPermohonan = (String) getContext().getRequest().getSession().getAttribute("idPermohonan");
        LOG.info("tanggung=" + tanggung);
        if (tanggung == null) {
            tanggung = " ";
        }
//        tanggung = (String) getContext().getRequest().getSession().getAttribute("tanggung");
        permohonan = permohonanDAO.findById(idPermohonan);
        String error = "";
        String msg = "";
        ...
        String skira = getContext().getRequest().getParameter("rowCount2");
        int kira = 0;
        if (skira != null) {
            kira = Integer.parseInt(skira);
        }
        ...
        List<FasaPermohonan> mf = strService.getSenaraiFasaPermohonan(idPermohonan);
        if (keputusanPTG != null) {
            for (FasaPermohonan mfa : mf) {
                if (mfa.getIdAliran().equals("keputusan")) {
                    mfa.setUlasan(keputusanPTG);
                    strService.saveFasaMohon(mfa);
                }
            }
        }

        KodDokumen kd = kodDokumenDAO.findById("MU");

        PermohonanKertas MohonKertas = strService.findKertasByKod(idPermohonan, "MU");
        if (MohonKertas == null) {
            PermohonanKertas mohonKertasNew = new PermohonanKertas();
            mohonKertasNew.setKodDokumen(kd);
            mohonKertasNew.setCawangan(permohonan.getCawangan());
            mohonKertasNew.setPermohonan(permohonan);
            mohonKertasNew.setTajuk("Tanggungan");
            mohonKertasNew.setInfoAudit(infoAudit);
            strService.simpanPermohonanKertas(mohonKertasNew);
            permohonanKertasDAO.saveOrUpdate(mohonKertasNew);

            PermohonanKertasKandungan permohonanKertasKand = new PermohonanKertasKandungan();
            permohonanKertasKand.setKandungan(tanggung);
            permohonanKertasKand.setBil(1);
            permohonanKertasKand.setCawangan(permohonan.getCawangan());
            permohonanKertasKand.setKertas(mohonKertasNew);
            infoAudit = new InfoAudit();
            infoAudit.setDimasukOleh(peng);
            infoAudit.setTarikhMasuk(new java.util.Date());
            permohonanKertasKand.setInfoAudit(infoAudit);
            strService.simpanPermohonanKertasKandungan(permohonanKertasKand);
        } else {
            List<PermohonanKertasKandungan> kand = strService.findKandunganByIDKertas(MohonKertas.getIdKertas());
            if (kand.size() >= 1) {
                for (PermohonanKertasKandungan mkk : kand) {
                    if (mkk.getBil() == 1) {
                        mkk.setKandungan(tanggung);
                        infoAudit = new InfoAudit();
                        infoAudit.setDiKemaskiniOleh(peng);
                        infoAudit.setTarikhKemaskini(tarikhMasuk);
                        mkk.setInfoAudit(infoAudit);
                        strService.simpanPermohonanKertasKandungan(mkk);
                    }
                }
            } else {
                PermohonanKertasKandungan permohonanKertasKand = new PermohonanKertasKandungan();
                ...
                permohonanKertasKand.setBil(1);
                permohonanKertasKand.setCawangan(permohonan.getCawangan());
                permohonanKertasKand.setKertas(MohonKertas);
                infoAudit = new InfoAudit();
                infoAudit.setDimasukOleh(peng);
                infoAudit.setTarikhMasuk(new java.util.Date());
                permohonanKertasKand.setInfoAudit(infoAudit);
        ...
        return new RedirectResolution(KertasPertimbanganActionBeanNew.class, "showForm").addParameter("error", error).addParameter("message", msg);
    }
    ...
    public Resolution simpanSemakKertasptg() {
        ...
        String row3 = getContext().getRequest().getParameter("rowCount3");
        if (row3 != null) {
            kira = Integer.parseInt(row3);
        } else {
            kira = 0;
        }
        ...
        return new RedirectResolution(KertasPertimbanganActionBeanNew.class, "showForm2").addParameter("error", error).addParameter("message", msg);
    }
    ...
    public Resolution deleteSingle() {
        ...
        return new RedirectResolution(KertasPertimbanganActionBeanNew.class, "locate");
    }
    ...
    public Resolution deleteSingleSemakKertasptg() {
        ...
        getContext().getRequest().setAttribute("display", Boolean.TRUE);
        addSimpleMessage("Maklumat telah berjaya dihapus.");
        return new RedirectResolution(KertasPertimbanganActionBeanNew.class, "showForm2");
    }
    ...
    public Resolution simpan1() {
        ...
        return new RedirectResolution(KertasPertimbanganActionBeanNew.class, "showForm3").addParameter("error", error).addParameter("message", msg);
    ...
    public String getNmBngnP() {
        return nmBngnP;
    }

    public void setNmBngnP(String nmBngnP) {
        this.nmBngnP = nmBngnP;
    }
    ...
    public List<HakmilikPihakBerkepentingan> getHkp() {
        return hkp;
    }

    public void setHkp(List<HakmilikPihakBerkepentingan> hkp) {
        this.hkp = hkp;
    }
