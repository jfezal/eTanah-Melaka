...
<s:form beanclass="etanah.view.stripes.pelupusan.JabatanTeknikal2ActionBean">
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            ...
            <legend>Kemaskini <c:if test="${actionBean.kNegeri eq '04'}">Charting</c:if><c:if test="${actionBean.kNegeri eq '05'}">SDP</c:if></legend>


            <c:if test="${actionBean.permohonan.kodUrusan.kod ne 'PLPS' and actionBean.permohonan.kodUrusan.kod ne 'PBMT'}">
                <p>
                    <label>Tolak<c:if test="${actionBean.kNegeri eq '04'}">Ringkas</c:if><c:if test="${actionBean.kNegeri eq '05'}">SDP</c:if> :</label>
