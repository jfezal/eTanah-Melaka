...
<script type="text/javascript">
    ...
    function doSomething(id, val) {
        ...
        var url = '${pageContext.request.contextPath}/hakmilik?carianHakmilikStrata&daerahPilihan='+ kodDaerah + '&type=' + val + '&bpm=' + kodBPM;
        $.ajax({
            url:url,
            success:function(data){                
                $dialog.html(data);
    ...
    function doReload() {
        var url = '${pageContext.request.contextPath}/hakmilik?reloadStrata';
        $.ajax({
            url:url,
            success:function(data){
                $dialog.html(data);
...
<s:form beanclass="etanah.view.stripes.HakmilikActionBean">
    ...
    <table>
        ...
        <tr>
            <td><font color="red">*</font>No Bangunan </td>
            <td>:</td>
            <td><s:text name="hakmilik.noBangunan" id="_noBangunan" onkeyup="this.value = this.value.toUpperCase();"/></td>
        </tr>
        <tr>
            <td><font color="red">*</font>No Tingkat </td>
            <td>:</td>
            <td><s:text name="hakmilik.noTingkat" id="_noTingkat"/></td>
        </tr>
        <tr>
            <td><font color="red">*</font>No Petak </td>
            <td>:</td>
            <td><s:text name="hakmilik.noPetak" id="_noPetak"/></td>
        </tr>
    </table>
    <br/>
    <div align="right">   
        <s:button value="Cari" class="btn" name="carianHakmilikStrataPopup" onclick="if(validate_carian_hakmilik())cari(this.form, this.name);"/>&nbsp;
