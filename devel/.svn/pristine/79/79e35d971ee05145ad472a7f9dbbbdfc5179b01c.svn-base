 ...
 */
@UrlBinding("/penguatkuasaan/laporan_tanah")
public class LaporanTanahActionBean extends AbleActionBean {
    ...
    private static Logger logger = Logger.getLogger(LaporanTanahActionBean.class);
    ...
    public void rehydrate() {
        ...
        if (idPermohonan != null) {
            ...
            if (kodNegeri.equalsIgnoreCase("04")) {
                ...
                } else if (permohonan.getKodUrusan().getKod().equalsIgnoreCase("351") || permohonan.getKodUrusan().getKod().equalsIgnoreCase("352")) {
                    ...
                    if (!hakmilikPermohonanList.isEmpty()) {
                        ...
                        if (hakmilikPermohonan.getHakmilik() != null) {
                            ...
                            if (hakmilikPermohonan.getKodPbt() != null) {
                                kodPBT = hakmilikPermohonan.getKodPbt().getKod();
                            }
                            lokasi = hakmilikPermohonan.getLokaliti();
...
//                    kodDunMohon = KodDUNDAO.findById(kodDunLaporanTanah);
        ...
        getContext().getRequest().setAttribute("edit", Boolean.TRUE);
        return new ForwardResolution("/WEB-INF/jsp/penguatkuasaan/Laporan_Tanah.jsp").addParameter("tab", "true");
    }
        ...
        try {
            ...
            if (!hakmilikPermohonanList.isEmpty()) {
                ...
                if (StringUtils.isNotBlank(kodPBT)) {
                    hakmilikPermohonan.setKodPbt(kodPBTDAO.findById(kodPBT));
                } else {
                    hakmilikPermohonan.setKodPbt(null);
                }

                hakmilikPermohonan.setLokaliti(lokasi);
                enforceService.simpanhakmilikPermohonan(hakmilikPermohonan);
...
//                    hakmilikPermohonan = enforceService.findhakmilikPermohonanByIdpermohonan(idPermohonan);
    ...
    public Resolution simpan() {
        ...
        laporanTanah.setHakmilikPermohonan(hakmilikPermohonanHakisan);
        laporanTanah.setPermohonan(permohonan);
        laporanTanah.setInfoAudit(ia);
        laporanTanah.setJenisLaporan(jenisLaporan);
        laporanTanah = laporanTanahService.simpanLaporanTanah(laporanTanah);
    ...
    public Resolution addHakmilik() {
        ...
        idMH = getContext().getRequest().getParameter("id");
        String statusDisplay = getContext().getRequest().getParameter("statusDisplay");
        if (StringUtils.isNotBlank(idMH)) {
            multipleHakmilikPermohonan = hakmilikPermohonanDAO.findById(Long.parseLong(idMH));
            if (multipleHakmilikPermohonan != null) {
                if (multipleHakmilikPermohonan.getHakmilik() != null) {
                    hakmilik = enforceService.semakIdHakmilik(multipleHakmilikPermohonan.getHakmilik().getIdHakmilik());
                    if (hakmilik != null) {
                        idHakmilik = hakmilik.getIdHakmilik();
                    }
                }

                if (multipleHakmilikPermohonan.getKodPbt() != null) {
                    kodPBT = multipleHakmilikPermohonan.getKodPbt().getKod();
                }

                if (multipleHakmilikPermohonan.getKodDUN() != null) {
                    kodDun = multipleHakmilikPermohonan.getKodDUN().getKod();
                }

                lokasi = multipleHakmilikPermohonan.getLokaliti();
            }

        }

        if (StringUtils.isNotBlank(statusDisplay)) {
            if (statusDisplay.equalsIgnoreCase("edit")) {
                getContext().getRequest().setAttribute("edit", Boolean.TRUE);
            } else {
