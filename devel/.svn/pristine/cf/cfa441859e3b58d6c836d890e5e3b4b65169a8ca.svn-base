...
//------------------------------------------//
@UrlBinding("/pengambilan/maklumat_hakmilikpengambilan")
    ...
    public void rehydrate() {
        ...
        if (idPermohonan != null) {
            ...
            if (flagPBA == "1") {
                ...
                hakmilikPermohonanList = hakmilikpermohonanservice.findByIdPermohonan(idPermohonan); //permohonan.getSenaraiHakmilik();
            }
...
//          hakmilikpermohonanservice.saveSingleHakmilikPermohonan(hmp);
        ...
        KodUOM kUOM = new KodUOM();
        List<HakmilikPermohonan> senaraiMH = p.getSenaraiHakmilik();
//        BigDecimal luasDiambil = new BigDecimal(String.valueOf(luasTerlibat));
        for (int i = 0; i < senaraiMH.size(); i++) {
            HakmilikPermohonan hp = hakmilikPermohonanList.get(i);
            permohonanPengambilan = pengambilanService.findByidMH(String.valueOf(hp.getId()));
            if (permohonanPengambilan != null) {
//                permohonanPengambilan.setLuasAmbil(luasDiambil);
                if (i < luasTerlibat.size()) {
                    if (luasTerlibat.get(i) != null) {
                        permohonanPengambilan.setLuasAmbil(new BigDecimal(luasTerlibat.get(i)));
                    }
                }

                kUOM = kodUOMDAO.findById(hp.getKodUnitLuasBaru().getKod());
                permohonanPengambilan.setKodUnitLuas(kUOM);
                listPermohonanPengambilanNew.add(permohonanPengambilan);
//                listPermohonanPengambilan.addAll(senaraiPPH);
                permohonanPengambilanHakmilikDAO.save(permohonanPengambilan);
            } else {
                PermohonanPengambilanHakmilik pph = new PermohonanPengambilanHakmilik();
                pph.setHakmilikPermohonan(hp);
                pph.setInfoAudit(ia);
//                pph.setLuasAmbil(luasDiambil);
                if (i < luasTerlibat.size()) {
                    if (luasTerlibat.get(i) != null) {
                        BigDecimal luasDiambil = new BigDecimal(String.valueOf(luasTerlibat.get(i)));
                        pph.setLuasAmbil(luasDiambil);
                    }
                }
                if (hp.getKodUnitLuasBaru() != null) {
                    kUOM = kodUOMDAO.findById(hp.getKodUnitLuasBaru().getKod());
                    pph.setKodUnitLuas(kUOM);
                } else if (hp.getKodUnitLuas() != null) {
                    kUOM = kodUOMDAO.findById(hp.getKodUnitLuas().getKod());
                    pph.setKodUnitLuas(kUOM);
                }

                permohonanPengambilanHakmilikDAO.save(pph);
            }
...
//    }
    ...
    public static void setStaticKodBandarPekanMukim(String staticKodBandarPekanMukim) {
        MaklumatHakmilikActionBean.staticKodBandarPekanMukim = staticKodBandarPekanMukim;
    }
    ...
    public static void setLogger(Logger logger) {
        MaklumatHakmilikActionBean.logger = logger;
    }
