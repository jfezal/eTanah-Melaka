 ...
 * @author Rajesh
 */
@UrlBinding("/pengambilan/perintah_bm_2")
public class PerintahBM2ActionBean extends AbleActionBean {
    ...
    private static final Logger logger = Logger.getLogger(PerintahBM2ActionBean.class);
   ...
   //   show = "Edit";
        return new JSP("pengambilan/melaka/bantahanmahkamah/Deraf_Perintah_BM_2.jsp").addParameter("tab", "true");
    }
        ...
        if (idPermohonan != null) {
            ...
            if (senaraiHakmilikPermohonan.size() > 0) {
                ...
                if (hakmilik != null) {
                    ...
                    logger.debug("hakmilik >> "+hakmilik.getIdHakmilik());
                    
                    for (Pemohon pemohon : permohonan.getSenaraiPemohon()) {
                        logger.debug("pemohon.getPihak().getIdPihak() >> "+pemohon.getPihak().getIdPihak());
