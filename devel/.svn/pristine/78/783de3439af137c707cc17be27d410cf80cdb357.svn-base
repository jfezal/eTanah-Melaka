<%-- 
    Document   : popup_tarikh_sebutan : for MELAKA sek 422,423,424,427,428,429
...
<script type="text/javascript">
    ...
    <c:forEach items="${listUtil.senaraiKodPerananRujukanLuar}" var="line">
        ...
        function chooseStatus(val){
            if(val == 'KP'){
                document.getElementById("statusKeputusan").style.visibility = 'visible';
                document.getElementById("statusKeputusan").style.display = '';
            }else{
                document.getElementById("statusKeputusan").style.visibility = 'hidden';
                document.getElementById("statusKeputusan").style.display = 'none';
            ...
            <div class="content">
                ...
                <c:if test="${!actionBean.addInfoByPUU}">
                    <p>
                        ...
                        <s:select name="status" id="status" value="${actionBean.permohonanRujukanLuar.keputusanPendakwaan.kod}">
                            <s:option value="">Sila Pilih</s:option>
                            <s:options-collection collection="${listUtil.senaraiStatusDakwa}" label="nama" value="kod" sort="nama" />
                        </s:select>
                        &nbsp;
                ...
                <c:if test="${actionBean.addInfoByPUU}">
                    <p>
                        <label><em>*</em>Status :</label>
                        <s:select name="status" id="status" value="${actionBean.permohonanRujukanLuar.keputusanPendakwaan.kod}" onchange="chooseStatus(this.value);">
                            <s:option value="">Sila Pilih</s:option>
                            <s:options-collection collection="${listUtil.senaraiStatusDakwa}" label="nama" value="kod" sort="nama" />
                        </s:select>
                        &nbsp;
                    ...
                    <div id="statusKeputusan" style="visibility: hidden; display: none">
                        <p>
            ...
            <div class="content">
                ...
                <c:if test="${!actionBean.addInfoByPUU}">
                    <p align="right">
                        ...
                        <c:if test="${actionBean.idRujukan eq null}">
                            <s:button class="btn" value="Simpan" name="simpanRecordTarikhSebutan" onclick="if(validateForm())save(this.name,this.form);" />
                        </c:if>
                        <c:if test="${actionBean.idRujukan ne null}">
                            <s:button class="btn" value="Simpan" name="editRecordTarikhSebutan" onclick="if(validateForm())save(this.name,this.form);" />
                        </c:if>
                ...
                <div class="content">
                    <fieldset class="aras2">
                        ...
                        <div align="center" >
                            ...
                            <table  width="90%" id="tblKehadiran" class="tablecloth" align="center">
                                <tr>
                                    <th  width="1%" align="center"><b>Pilih</b></th>
                                    <th  width="1%" align="center"><b>Bil</b></th>
                            ...
                            <table width="80%">
                                <tr><td align="right">
                                        <s:button class="btn" value="Tambah" name="add" onclick="addRowKehadiran('tblKehadiran')" />
                                        <s:button class="btn" value="Hapus" name="delete" onclick="deleteRowKehadiran('tblKehadiran')"/>
                            </table>
                        </div>
                        <p>&nbsp;</p>
                    </fieldset>
