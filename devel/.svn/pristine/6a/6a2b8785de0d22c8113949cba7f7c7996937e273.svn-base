...
import etanah.util.HbnCriteriaPagedListImpl;
...
//            if (debug) LOG.debug("took " + (System.currentTimeMillis() - t ) + "ms to find");
    ...
    protected Serializable getPK(T obj) {
        if (meta == null) {
            meta = sessionProvider.get().getSessionFactory().getClassMetadata(persistentClass);
        }
        if (obj != null) {
            return meta.getIdentifier(obj, EntityMode.POJO);
        }
        LOG.error("Unable to get PK for entity!");
        return null;
    }

    private void doTrace(String str, T entity) {
        if (debug) {
            try {
                LOG.debug(String.format("%s %s [%s]", str, persistentClass.getName(), getPK(entity)));
            } catch (Exception e) {
                LOG.warn("Error while fetching entity ID. " + e.getMessage());
            }                
        }
        
        if (entity == null) {
            LOG.warn(String.format("%s %s [null]", str, persistentClass.getName()));
        }
    }
    
    public T saveOrUpdate(T object){
        doTrace("saving/updating", object);
        try {
            Session s = sessionProvider.get();
