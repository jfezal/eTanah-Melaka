...
@UrlBinding("/hasil/bil_cukai_strata")
public class BilCukaiStratactionBean extends AbleActionBean {

    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(BilCukaiStratactionBean.class);
    private static final String DEFAULT_VIEW = "/WEB-INF/jsp/hasil/bil_cukai_strata.jsp";
...
//    public static void setBilCukai(String kodCaw, String tahun, Connection con1, String user) throws SQLException {
        ...
        try {
            con = con1;
            proc = con.prepareCall("{call et_bil_cukai_strata(?, ?, ?, ?, ?, ?)}");
    ...
    public BilCukai searchLatest(){
        ...
        String sql = "SELECT bc from etanah.model.BilCukai bc where bc.jenisCukai = 'S' order by bc.infoAudit.tarikhMasuk DESC";         // FIXME : catatan
        Session s = sessionProvider.get();
        Query q = s.createQuery(sql);
...
private boolean generateTerima01(Date now,  BilCukai bil, Pengguna user, KodDaerah kod) {
        ...
        PrintWriter pw = createFile("bilCukaiStrata.txt", now, user, kod);
...
public void createDir(File f) throws IOException {
    ...
    public PrintWriter createFile(String fileName, Date now, Pengguna user, KodDaerah kod) {
        ...
        try {
            ...
            Logger.getLogger(BilCukaiStratactionBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private String getBilCukaiPath(Pengguna pengguna, String path, KodDaerah kod) {
        if(pengguna != null) {
            ...
            String bilCukaiPath = "BilCukaiStrata" + File.separator + kod.getKod() + File.separator+ dateFolder;
    ...
    public Resolution downloadFile() {
        ...
        Date now = new Date();
        String zipname = "bilCukaiStrata.zip";
     ...
     private boolean createZip(String currentPath) {
        String[] filenames = new String[]{currentPath + "bilCukaiStrata.txt"};
        ...
        try {
            // Create the ZIP file
            String outFilename =currentPath + "bilCukaiStrata.zip";
