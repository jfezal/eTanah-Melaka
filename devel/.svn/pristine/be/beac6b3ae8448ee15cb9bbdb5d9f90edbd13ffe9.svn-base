...
<script type="text/javascript">
    ...
    function doFilterDaerah(kodCaw2){
        ...
        if(kodCaw2 != null){
            var url = '${pageContext.request.contextPath}/lelong/laporanlelong?masuk&kodCawangan=' + kodCaw2+'&reportNama='+report;
            $.get(url,
            function(data){
                $('#display').html(data);
                $('#caw').val(kodCaw2);
    ...
    function doSubmit(f){
        ...
        if(( report == 'LLGLaporanBulananPerintahJual_NS.rdf'
            || report == 'LLGJumlahKomisyenPTD_NS.rdf')&& $('#bulan').val() == ""){
...
<s:form beanclass="etanah.view.stripes.lelong.LaporanLelongActionBean">
    ...
    <div class="subtitle">
        <fieldset class="aras1">
            ...
            <legend>
                <c:if test="${actionBean.reportName eq 'LLGLaporanBulananPerintahJual_NS.rdf'}">
                    Laporan Bulanan Perintah Jual
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLGLaporanLelongan_NS.rdf'}">
                    Laporan Lelongan Awam
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLGJumlahKomisyenPTD_NS.rdf'}">
                    Laporan Komisyen Jualan
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLG_Stat_01_NS.rdf'}">
                    Statistik Permohonan Perintah Jual (16G)
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLG_stat_pembida_NS.rdf'}">
                    Laporan Pembida Berdaftar
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLG_stat_plb_NS.rdf'}">
                    Laporan Senarai Pelelong Berlesen
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLGSuratKpdPelelong_NS.rdf'}">
                    Laporan Senarai Lelongan Untuk Pelelong Berlesen
                </c:if>
                <c:if test="${actionBean.reportName eq 'LLGLaporanPemohon_NS.rdf'}">
            ...
            <c:if test="${
                  reportname eq 'LLGDokSiasatan_NS.rdf'
                      or reportname eq 'LLGSuratBayarBaki_NS.rdf'
                      or reportname eq 'LLGSrtAkuanBersumpahPPT_NS.rdf'
                      or reportname eq 'LLGPhtrNotis_NS.rdf'
                      or reportname eq 'LLGBorangAmPin_NS.rdf'
                  }">
                <p>
            ...
            <c:if test="${reportname eq 'LLGLaporanBulananPerintahJual_NS.rdf'
                          or reportname eq 'LLG_Stat_01_NS.rdf'
                          or reportname eq 'LLGJumlahKomisyenPTD_NS.rdf'
                          or reportname eq 'LLG_stat_pembida_NS.rdf'
                          or reportname eq 'LLGLaporanLelongan_NS.rdf'}">
            ...
            <c:if test="${ reportname eq 'LLG_stat_plb_NS.rdf'}">
                <p>
                    <label>Tarikh Mula :</label>
            ...
            <c:if test="${reportname eq 'LLGLaporanPemohon_NS.rdf'}">
                <p>
                    <label>Tarikh Mula :</label>
                    <s:text id="trh_mula" name="report_p_trh_mula" class="datepicker" formatPattern="dd/MM/yyyy"/>&nbsp;<font size="1" color="red"> (cth : hh / bb / tttt)</font>
                </p>
            ...
            <c:if test="${reportname eq 'LLGLaporanLelongan_NS.rdf'
                          or reportname eq 'LLG_stat_pembida_NS.rdf'}">
                  <p>
            ...
            <c:if test="${ reportname eq 'LLGJumlahKomisyenPTD_NS.rdf'
                           or reportname eq 'LLGLaporanBulananPerintahJual_NS.rdf'}">
                  <p>
            ...
            <c:if test="${reportname eq  'LLG_stat_pembida_NS.rdf' }">
                <p>
                    <label>ID Permohonan :</label>
                    <s:text id="id_mohon" name="report_p_id_mohon" onblur="this.value = this.value.toUpperCase();"/>
                </p>
            ...
            <c:if test="${reportname eq 'LLGLaporanLelongan_NS.rdf'
                          or reportname eq 'LLGLaporanPemohon_NS.rdf'}">
                  <p>
