<%-- 
    Document   : rekod_bidaan
    Created on : Jan 25, 2010, 2:21:22 PM
...
<script type="text/javascript">
    ...
    function refreshPage(){
        var url = '${pageContext.request.contextPath}/lelong/keputusan_bidaan?refreshpage';
        $.get(url,
        function(data){
            $('#page_div').html(data);
        },'html');
    ...
    function deletePembida2(val,val1){
        if(confirm('Adakah anda pasti?')) {
            var url = '${pageContext.request.contextPath}/lelong/keputusan_bidaan?deletePembida2&idPihak='+ val+'&idLelong='+ val1;
            $.get(url,
            function(data){
                $('#page_div').html(data);
            },'html');
    ...
    function popupPembidaRekod(val){
        window.open("${pageContext.request.contextPath}/lelong/keputusan_bidaan?pembidaPopupRekod&idLelong="+val, "eTanah",
...
<s:form beanclass="etanah.view.stripes.lelong.KeputusanBidaanActionBean" id="bida">
    <s:messages/>
    <s:errors/>&nbsp;

    <s:hidden id="idLelong" name="idLelong" value="${actionBean.lelong.idLelong}"/>
    <s:hidden id="idPembida" name="idPembida" value="${actionBean.pembida.idPembida}" /> 
                            ...
                            <c:if test="${actionBean.negeri eq true}">
                                <p>
                                    <label><font color="red">*</font> Nama : </label>
                                        <s:text id="nama" name="pihak.nama" onchange="this.value=this.value.toUpperCase();" style="width:260px;" readonly="true"/>
                                        <img src="${pageContext.request.contextPath}/pub/images/search_icon.gif" height="15" width="15" alt="Senarai Pembida Berdaftar"
                                            onmouseover="this.style.cursor='pointer';" onclick="popupPembida('${actionBean.lelong.idLelong}');return false;" title="Senarai Pembida Berdaftar">
                                            <font color="red" size="2">(Sila Klik Untuk Senarai Pembida)</font>
                                </p>
                                <p>
                                ...
                                <p>
                                    <label> <font color="red">*</font>Negeri : </label>
                                        <s:select id="negeri" name="pihak.suratNegeri.kod" style="width:150px;">
                                            <s:option value="">Sila Pilih</s:option>
                                ...
                                <p>
                                    ...
                                    <s:text name="lelong.hargaBidaan" id="hargaBida" onkeyup="validateNumber(this,this.value);" onblur="validate(this.value)" onchange="calc(this.value);"  formatPattern="###,###,###.00" style="width:150px;"/>&nbsp;
                                </p>
                                <p>
                                ...
                                <c:if test = "${actionBean.BJ eq false}">
                                    ...
                                    <div class="content" align="center">
                                        ...
                                        <c:if test="${fn:length(actionBean.senaraiPembida)>=1}">
                                            ...
                                            <%--<s:button name="simpanPembidaFlow" value="Simpan" class="btn" onclick="simpanBerjaya(this.name, this.form,'page_div')"/>--%>
                                ...
                                <c:if test = "${actionBean.BJ eq true}">
                                    ...
                                    <div class="subtitle">
                                        <fieldset class="aras1" id="">
                                            ...
                                            <p>
                                                <label><font color="red">*</font>Negeri : </label>
                                                    <s:select id="negeri" name="pihak.negeri.kod" style="width:150px;">
                                                        <s:option value="">Sila Pilih</s:option>
                                                    <s:options-collection collection="${listUtil.senaraiKodNegeri}" label="nama" value="kod"/>
                                                </s:select>
                                            </p>
