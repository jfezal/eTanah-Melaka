...
<script type="text/javascript">
    ...
    {   
        NoPrompt();
        var url = '${pageContext.request.contextPath}/pelupusan/kertas_JKTDV2?tambahRow&index='+index;
        window.open(url, "eTanah",
        "status=0,toolbar=0,location=0,menubar=0,width=900,height=600");
    }
    ...
    {   
        ...
        if(confirm('Adakah anda pasti untuk menghapus data ini?')) {
            var q = $(f).formSerialize();
            $.post('${pageContext.request.contextPath}/pelupusan/kertas_JKTDV2?deleteRow&idKandungan='+idKandungan+'&tName='+tName+'&typeName=kHuraianPtd',q,
            function(data){
                $('#page_div').html(data);
    ...
    <s:form beanclass="etanah.view.stripes.pelupusan.KertasJKTDV2ActionBean" name="form">
        ...
        <div class="subtitle">
            <fieldset class="aras1">
                <legend>Huraian <c:choose><c:when test="${actionBean.permohonan.cawangan.kod eq '08'}">Timbalan Pentadbir Tanah Gemas</c:when><c:otherwise>Pentadbir Tanah</c:otherwise></c:choose></legend>
